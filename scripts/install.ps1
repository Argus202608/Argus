# ============================================================================
# Argus Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   iex (irm https://raw.githubusercontent.com/MMArgus-Team/Argus/main/scripts/install.ps1)
#
...
param(
    ...
    [string]$Tag = "",
    [Alias("HermesHome")]
    [string]$ArgusHome = $(if ($env:ARGUS_HOME) { $env:ARGUS_HOME } elseif ($env:HERMES_HOME) { $env:HERMES_HOME } else { "$env:LOCALAPPDATA\argus" }),
    [string]$InstallDir = $(if ($env:ARGUS_HOME) { "$env:ARGUS_HOME\argus" } elseif ($env:HERMES_HOME) { "$env:HERMES_HOME\argus" } else { "$env:LOCALAPPDATA\argus\argus" }),
    ...
    # --- Local source (offline / self-contained desktop app) ---
    # When set to a directory, the repository stage COPIES this local source
    # tree into $InstallDir instead of cloning from GitHub. Used by a locally-
    # built, self-contained desktop app whose commit isn't on GitHub (avoids the
    # 404-on-unpushed-commit failure and works offline for the code fetch). The
    # runtime stages (uv/Python/Node/Electron) still download as usual.
    [string]$LocalSource = "",

    # --- Desktop GUI build (opt-in) ---
    # When set, install.ps1 includes Stage-Desktop in the manifest and
    # builds apps/desktop into a launchable Argus.exe.
    #
    # Why opt-in:
    #   * Argus-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Argus.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Argus.exe
    ...
    #     `argus desktop` already builds on demand.
...
$RepoUrlSsh = "git@github.com:MMArgus-Team/Argus.git"
$RepoUrlHttps = "https://github.com/MMArgus-Team/Argus.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * Argus Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by MMArgus Team.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $ArgusHome)) {
        New-Item -ItemType Directory -Force -Path $ArgusHome | Out-Null
    }
    $envFile = Join-Path $ArgusHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $ArgusHome "node"
...
function Install-Uv {
    # Argus owns its own uv at $ArgusHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (hermes_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `argus update` stay in sync.
    $managedUv = Join-Path $ArgusHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $ArgusHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $ArgusHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $ArgusHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $ArgusHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and ARGUS_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Argus-Setup.exe
...
function Install-Git {
    ...
    which Argus uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\argus\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Argus needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Argus copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\argus\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``HERMES_GIT_BASH_PATH`` (User scope) so Argus can find it in a fresh
    ...
    # Download PortableGit into $ArgusHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $ArgusHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Argus features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Argus needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``HERMES_GIT_BASH_PATH`` (User env scope) so Argus can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $ArgusHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $ArgusHome\git\usr\bin\bash.exe
    $candidates += "$ArgusHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$ArgusHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    Write-Warn "Could not locate bash.exe -- Argus may not find Git Bash."
...
function Test-Node {
    ...
    $managedNode = "$ArgusHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$ArgusHome\node;$env:Path"
        Write-Success "Node.js $version found (Argus-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Argus-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $ArgusHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $ArgusHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\argus-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$ArgusHome\node") { Remove-Item -Recurse -Force "$ArgusHome\node" }
                Move-Item $extractedDir.FullName "$ArgusHome\node"
                ...
                $version = & "$ArgusHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $ArgusHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\argus-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
    ...
    # If -LocalSource points at a readable directory, COPY it into $InstallDir
    # instead of cloning from GitHub. This is the offline/local-build path: the
    # desktop app bundles its own agent source and hands us the path, so we never
    # touch github.com for the code. Runtime stages still run normally afterward.
    if ($LocalSource -and (Test-Path $LocalSource)) {
        $srcResolved = (Resolve-Path $LocalSource).Path
        Write-Info "Using bundled local source at $srcResolved (offline code install)"
        # Fresh copy: clear a stale/broken $InstallDir first so robocopy mirrors
        # cleanly. (An existing valid checkout still gets replaced by the bundled
        # version — the bundled source IS the version this app shipped.)
        if (Test-Path $InstallDir) {
            Remove-Item -Recurse -Force $InstallDir -ErrorAction SilentlyContinue
        }
        New-Item -ItemType Directory -Force -Path $InstallDir -ErrorAction SilentlyContinue | Out-Null
        # robocopy is the robust Windows tree copy (handles long paths, retries).
        # /E all subdirs, /NFL /NDL /NJH /NJS quiet, /R:1 /W:1 minimal retry.
        # Exclude heavy/irrelevant dirs so we don't copy node_modules/.git/venv.
        $rc = Start-Process -FilePath "robocopy.exe" -ArgumentList @(
            "`"$srcResolved`"", "`"$InstallDir`"", "/E",
            "/XD", "node_modules", ".git", ".venv", "__pycache__", "dist", "build",
            "/NFL", "/NDL", "/NJH", "/NJS", "/R:1", "/W:1"
        ) -Wait -PassThru -NoNewWindow
        # robocopy exit codes 0-7 are success (8+ = failure). Anything <8 is OK.
        if ($rc.ExitCode -ge 8) {
            throw "Failed to copy bundled source (robocopy exit $($rc.ExitCode))"
        }
        # Init a git repo so later `argus update` / re-checks don't choke on a
        # non-repo (mirrors the ZIP-fallback path). No origin — a local build
        # updates by re-installing from a newer app, not by pulling.
        Push-Location $InstallDir
        git -c windows.appendAtomically=false init 2>$null | Out-Null
        git -c windows.appendAtomically=false config windows.appendAtomically false 2>$null
        Pop-Location
        Write-Success "Installed from bundled source"
        return
    }
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    $stashName = "argus-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if Argus behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running Argus processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/MMArgus-Team/Argus/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/MMArgus-Team/Argus/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/MMArgus-Team/Argus/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\argus-$zipLabel.zip"
                $extractPath = "$env:TEMP\argus-extract"
    ...
    # next `argus update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Argus-Setup.exe
    ...
    if (Test-Path "venv") {
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running argus processes before recreating venv..."
            # The launcher CLI (argus.exe) plus its child tree.
            & taskkill /F /T /IM argus.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM argus.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m hermes_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not argus.exe.
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install mm-argus package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # users hit and lazy-import errors from `argus dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `argus dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `argus dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up argus command..."

    if ($NoVenv) {
        $argusBin = "$InstallDir"
    } else {
        $argusBin = "$InstallDir\venv\Scripts"
    }

    # Add the venv Scripts dir to user PATH so argus is globally available
    # On Windows, the argus.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")

    if ($currentPath -notlike "*$argusBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$argusBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $argusBin"
    } else {
        Write-Info "PATH already configured"
    }

    # Set ARGUS_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\argus instead
    # of the Unix default ~/.argus
    $currentArgusHome = [Environment]::GetEnvironmentVariable("ARGUS_HOME", "User")
    if (-not $currentArgusHome -or $currentArgusHome -ne $ArgusHome) {
        [Environment]::SetEnvironmentVariable("ARGUS_HOME", $ArgusHome, "User")
        Write-Success "Set ARGUS_HOME=$ArgusHome"
    }
    $env:ARGUS_HOME = $ArgusHome

    # Update current session
    $env:Path = "$argusBin;$env:Path"

    Write-Success "argus command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.argus-bootstrap-complete which tells the Argus
    ...
    $markerPath = Join-Path $InstallDir ".argus-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the ARGUS_HOME directory structure ($ArgusHome, default %LOCALAPPDATA%\argus)
    New-Item -ItemType Directory -Force -Path "$ArgusHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ArgusHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ArgusHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ArgusHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ArgusHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ArgusHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ArgusHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ArgusHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ArgusHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$ArgusHome\.env"
    ...
    # Create config.yaml
    $configPath = "$ArgusHome\config.yaml"
    ...
    # Seed the bundled models.dev snapshot so a brand-new (possibly offline)
    # install has a full model catalog before its first network fetch. The agent
    # refreshes this from https://models.dev/api.json on startup and falls back
    # to this local copy when the network is unreachable. Only seed when absent
    # so we never clobber a cache a running install may have refreshed.
    $modelsCachePath = "$ArgusHome\models_dev_cache.json"
    if (-not (Test-Path $modelsCachePath)) {
        $modelsSeedPath = "$InstallDir\agent\models_dev_api.json"
        if (Test-Path $modelsSeedPath) {
            Copy-Item $modelsSeedPath $modelsCachePath
            Write-Success "Seeded $modelsCachePath from bundled snapshot"
        }
    } else {
        Write-Info "$modelsCachePath already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and Argus's prompt-injection scanner
...
"@
    ...
    Write-Success "Configuration directory ready: $ArgusHome"
    
    # Seed bundled skills into $ArgusHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $ArgusHome\skills ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to $ArgusHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$ArgusHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $ArgusHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Argus-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'argus setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\argus-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\argus-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\argus-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `Argus` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Argus.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_argus_desktop command
    # resolves apps/desktop/release/win-unpacked/Argus.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Argus-managed
    ...
    # The --dir mode produces an unpacked Argus.exe in
    ...
    # are NOT trying to work around). The Argus icon + product name are
    # stamped onto Argus.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\argus-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Argus.exe",
        "$desktopDir\release\win-arm64-unpacked\Argus.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Argus.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Argus icon + identity are stamped onto Argus.exe by the
    ...
    #    Argus.exe. We deliberately do NOT point them at `argus desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Argus.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Argus.lnk')
        )
        ...
        # with the Argus icon, but without this the shortcut can keep drawing
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'argus setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m argus_cli.main setup
    } else {
        python -m argus_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $argusCmd = "$InstallDir\venv\Scripts\argus.exe"
    if (-not (Test-Path $argusCmd)) {
        $argusCmd = "argus"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$ArgusHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'argus whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: argus gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$ArgusHome\logs\gateway.log"
            Start-Process -FilePath $argusCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$ArgusHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: argus gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: argus gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$ArgusHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$ArgusHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ArgusHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ArgusHome\argus\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   argus               " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   argus setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   argus config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   argus config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   argus gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   argus update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Argus repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Argus-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Argus to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/MMArgus-Team/Argus/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
