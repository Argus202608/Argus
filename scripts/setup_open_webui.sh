...
# Bootstrap Open WebUI against Argus's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.argus/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-argus.sh
...
#   ARGUS_API_PORT=8642
#   ARGUS_API_HOST=127.0.0.1
#   ARGUS_API_MODEL_NAME='Argus'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Argus WebUI}"
...
ARGUS_ENV_FILE="${ARGUS_ENV_FILE:-$HOME/.argus/.env}"
ARGUS_API_PORT="${ARGUS_API_PORT:-8642}"
ARGUS_API_HOST="${ARGUS_API_HOST:-127.0.0.1}"
ARGUS_API_CONNECT_HOST="${ARGUS_API_CONNECT_HOST:-127.0.0.1}"
ARGUS_API_MODEL_NAME="${ARGUS_API_MODEL_NAME:-Argus}"
ARGUS_API_BASE_URL="http://${ARGUS_API_CONNECT_HOST}:${ARGUS_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-argus.sh"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$ARGUS_API_BASE_URL")"
...
install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.argus.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.argus</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.argus"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.argus"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-argus.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Argus
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-argus.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.argus/logs/openwebui.log
StandardError=append:%h/.argus/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-argus.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$ARGUS_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring the Argus API server is configured...'
  upsert_env API_SERVER_ENABLED true "$ARGUS_ENV_FILE"
  upsert_env API_SERVER_HOST "$ARGUS_API_HOST" "$ARGUS_ENV_FILE"
  upsert_env API_SERVER_PORT "$ARGUS_API_PORT" "$ARGUS_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$ARGUS_API_MODEL_NAME" "$ARGUS_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$ARGUS_ENV_FILE"
  ensure_env_permissions

  log 'Restarting the Argus gateway so API server settings take effect...'
  argus gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${ARGUS_API_CONNECT_HOST}:${ARGUS_API_PORT}/health" >/dev/null; then
    log 'Argus API server did not answer on the first check. Trying to start the gateway in the background...'
    nohup argus gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${ARGUS_API_CONNECT_HOST}:${ARGUS_API_PORT}/health" >/dev/null
  ...
  log "Argus API endpoint: ${ARGUS_API_BASE_URL}"
