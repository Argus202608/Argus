...
# Python-driven tool session (e.g. Argus terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:MMArgus-Team/Argus.git"
REPO_URL_HTTPS="https://github.com/MMArgus-Team/Argus.git"
# ARGUS_HOME is canonical. Honor HERMES_HOME only as a migration fallback so
# existing unattended installers keep using their original data directory.
ARGUS_HOME="${ARGUS_HOME:-${HERMES_HOME:-$HOME/.argus}}"
...
#   code at /usr/local/lib/argus, command at /usr/local/bin/argus,
#   data still at /root/.argus (ARGUS_HOME).  Matches Claude Code / Codex CLI
...
BRANCH="main"
INSTALL_COMMIT=""
# When set (--local-source <dir>), the repository stage COPIES this local source
# tree into INSTALL_DIR instead of cloning from GitHub. Used by a self-contained
# desktop app that bundles its own agent source (no GitHub fetch; works for a
# locally-built app whose commit was never pushed). Runtime stages still download.
LOCAL_SOURCE=""
ENSURE_DEPS=""
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$ARGUS_HOME/.no-bundled-skills so future"
            echo "                   'argus update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Argus.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.argus/argus"
            echo "                   default (root, Linux): /usr/local/lib/argus"
            echo "  --argus-home PATH   Data directory (default: ~/.argus, or \$ARGUS_HOME)"
            echo "  --hermes-home PATH  Deprecated alias for --argus-home"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Argus installs the code under"
            echo "  /usr/local/lib/argus and links the command into"
            echo "  /usr/local/bin/argus (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$ARGUS_HOME"
            echo "  (default /root/.argus).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Code installs to \$ARGUS_HOME/argus (non-root) or /usr/local/lib/argus (root)."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs argus setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
print_banner() {
    ...
    echo "│             ⚕ Argus Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by MMArgus Team.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# intentional, but they leave the checkout dirty — which forces `argus update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Argus-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Argus","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install argus command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `argus` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $ARGUS_HOME/argus
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $ARGUS_HOME/argus
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux:          INSTALL_DIR = /usr/local/lib/argus
#                             command link in /usr/local/bin
#
# Always no-op when the user set --dir or $ARGUS_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in ARGUS_HOME.
    if is_termux; then
        INSTALL_DIR="$ARGUS_HOME/argus"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        INSTALL_DIR="/usr/local/lib/argus"
        ...
        log_info "  Command: /usr/local/bin/argus"
        log_info "  Data:    $ARGUS_HOME (unchanged)"
        log_info "  uv Python: $UV_PYTHON_INSTALL_DIR (world-readable)"
        return 0
    fi

    # Default: non-root, non-Termux.
    INSTALL_DIR="$ARGUS_HOME/argus"
}
...
# Point a Argus-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $ARGUS_HOME/node/bin — which is NOT on
...
# Argus's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Argus-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$ARGUS_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$ARGUS_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$ARGUS_HOME/node/etc/npmrc"
}

get_argus_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/argus" ]; then
        echo "$link_dir/argus"
    else
        echo "argus"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/MMArgus-Team/Argus/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Argus owns its own uv at $ARGUS_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (hermes_cli/managed_uv.py) looks in the same
    # place, so install.sh and `argus update` stay in sync.
    local _managed_uv="$ARGUS_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $ARGUS_HOME/bin ..."
    mkdir -p "$ARGUS_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/argus-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/argus-uv-installer.$$.sh")"
    ...
    # directly into $ARGUS_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$ARGUS_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# string clears that floor; anything below it is replaced with the Argus-
...
check_node() {
    ...
    # Repair pre-existing Argus-managed installs where `npm install -g` lands
    ...
    # Prefer a Argus-managed Node from a previous run over a too-old system one.
    if [ -x "$ARGUS_HOME/node/bin/node" ] && node_satisfies_build "$("$ARGUS_HOME/node/bin/node" --version)"; then
        export PATH="$ARGUS_HOME/node/bin:$PATH"
        log_success "Node.js $("$ARGUS_HOME/node/bin/node" --version) found (Argus-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Argus-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.argus/node/..."
    ...
    # Place into ~/.argus/node/ and symlink binaries into the same bin dir
    # the Argus command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$ARGUS_HOME/node"
    mkdir -p "$ARGUS_HOME"
    mv "$extracted_dir" "$ARGUS_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$ARGUS_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$ARGUS_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$ARGUS_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$ARGUS_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.argus/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Argus install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Argus itself does not require or retain root access."
...
clone_repo() {
    ...
    # ── Local-source mode (self-contained desktop app) ──────────────────────
    # If --local-source points at a readable directory, COPY it into INSTALL_DIR
    # instead of cloning from GitHub. Offline/local-build path: the app bundles
    # its own agent source and hands us the path, so we never touch github.com
    # for the code. Runtime stages (uv/Python/Node/Electron) still run normally.
    if [ -n "$LOCAL_SOURCE" ] && [ -d "$LOCAL_SOURCE" ]; then
        log_info "Using bundled local source at $LOCAL_SOURCE (offline code install)"
        # Fresh copy: the bundled source IS the version this app shipped, so
        # replace any existing checkout.
        rm -rf "$INSTALL_DIR"
        mkdir -p "$INSTALL_DIR"
        # Prefer rsync (present on macOS/most Linux); fall back to cp -R. Exclude
        # heavy/irrelevant dirs so we don't copy node_modules/.git/venv/caches.
        if command -v rsync >/dev/null 2>&1; then
            rsync -a \
                ...
                || { log_error "Failed to copy bundled source (rsync)"; return 1; }
        else
            # cp -R the contents (note trailing /. to copy dir contents, not the dir).
            cp -R "$LOCAL_SOURCE"/. "$INSTALL_DIR"/ \
                || { log_error "Failed to copy bundled source (cp)"; return 1; }
            # Best-effort prune of runtime junk cp may have copied.
            rm -rf "$INSTALL_DIR/.git" "$INSTALL_DIR/node_modules" \
                   "$INSTALL_DIR/.venv" 2>/dev/null || true
        fi
        # Init a git repo so later `argus update` / repo checks don't choke on a
        # non-repo. No origin — a local build updates by re-installing a newer app.
        ( cd "$INSTALL_DIR" && git init >/dev/null 2>&1 || true )
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `argus update` path
                # (#4735).
                ...
                local stash_name
                stash_name="argus-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Argus behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up argus command..."

    if [ "$USE_VENV" = true ]; then
        ARGUS_BIN="$INSTALL_DIR/venv/bin/argus"
    else
        ARGUS_BIN="$(which argus 2>/dev/null || echo "")"
        if [ -z "$ARGUS_BIN" ]; then
            log_warn "argus not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$ARGUS_BIN" ]; then
        log_warn "argus entry point not found at $ARGUS_BIN"
    ...
    # Create a user-facing shim for the argus command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $ARGUS_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$ARGUS_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/argus"
    cat > "$command_link_dir/argus" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ARGUS_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/argus"
    log_success "Installed argus launcher → $command_link_display_dir/argus"

    # Keep the historical command available for existing automation while
    # making Argus the canonical launcher shown to users.
    rm -f "$command_link_dir/hermes"
    cat > "$command_link_dir/hermes" <<EOF
#!/usr/bin/env bash
exec "$command_link_dir/argus" "\$@"
EOF
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_success "argus command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v argus' \
                >/dev/null 2>&1; then
        ...
        log_info "argus not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Argus — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Argus — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Argus — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so argus works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.argus directory structure (config at top level, code in subdir)
    mkdir -p "$ARGUS_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.argus/.env (top level, easy to find)
    if [ ! -f "$ARGUS_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$ARGUS_HOME/.env"
            log_success "Created ~/.argus/.env from template"
        else
            touch "$ARGUS_HOME/.env"
            log_success "Created ~/.argus/.env"
        fi
    else
        log_info "~/.argus/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$ARGUS_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.argus/config.yaml (top level, easy to find)
    if [ ! -f "$ARGUS_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$ARGUS_HOME/config.yaml"
            log_success "Created ~/.argus/config.yaml from template"
        fi
    else
        log_info "~/.argus/config.yaml already exists, keeping it"
    fi

    # Seed the bundled models.dev snapshot into ~/.argus/models_dev_cache.json
    # so a brand-new (possibly offline) install has a full model catalog before
    # its first successful network fetch. agent/models_dev.py fetches
    # https://models.dev/api.json on startup and refreshes this file in the
    # background; if the network is unreachable it keeps serving this local copy.
    # Only seed when absent — never clobber a cache a running install may have
    # already refreshed to something newer than what this package shipped.
    if [ ! -f "$ARGUS_HOME/models_dev_cache.json" ]; then
        if [ -f "$INSTALL_DIR/agent/models_dev_api.json" ]; then
            cp "$INSTALL_DIR/agent/models_dev_api.json" "$ARGUS_HOME/models_dev_cache.json"
            log_success "Seeded ~/.argus/models_dev_cache.json from bundled snapshot"
        fi
    else
        log_info "~/.argus/models_dev_cache.json already exists, keeping it"
    fi
    ...
    if [ ! -f "$ARGUS_HOME/SOUL.md" ]; then
        cat > "$ARGUS_HOME/SOUL.md" << 'SOUL_EOF'
You are Argus, an intelligent AI assistant created by MMArgus Team. You are helpful, knowledgeable, and direct. You assist users with a wide range of tasks including answering questions, writing and editing code, analyzing information, creative work, and executing actions via your tools. You communicate clearly, admit uncertainty when appropriate, and prioritize being genuinely useful over being verbose unless otherwise directed below. Be targeted and efficient in your exploration and investigations.
SOUL_EOF
        log_success "Created ~/.argus/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.argus/"

    # Seed bundled skills into ~/.argus/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `argus update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'argus update'." \
            > "$ARGUS_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $ARGUS_HOME/.no-bundled-skills"
        log_info "  Future 'argus update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.argus/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.argus/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$ARGUS_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$ARGUS_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.argus/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Argus browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Argus will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Argus browser tools — explicit browser override."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (argus --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `argus update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'argus setup' after install."
        return 0
    fi
    ...
    # Run argus setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m argus_cli.main setup < /dev/tty
    else
        python -m argus_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$ARGUS_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Argus to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$ARGUS_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'argus whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                ARGUS_CMD="$(get_argus_command_path)"
                $ARGUS_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'argus whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'argus gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $ARGUS_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $ARGUS_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: argus gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: argus gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $ARGUS_CMD gateway > "$ARGUS_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.argus/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: argus gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: argus gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $ARGUS_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $ARGUS_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $ARGUS_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}argus${NC}              Start chatting"
    echo -e "   ${GREEN}argus setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}argus config${NC}       View/edit configuration"
    echo -e "   ${GREEN}argus config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}argus gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}argus update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'argus' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'argus' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'argus' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$ARGUS_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$ARGUS_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$ARGUS_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$ARGUS_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$ARGUS_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Argus for pip install"
    ...
    ARGUS_CMD="$(command -v argus 2>/dev/null || echo "")"
    if [ -n "$ARGUS_CMD" ]; then
        log_info "Running argus setup..."
        "$ARGUS_CMD" setup
    else
        log_warn "argus command not found on PATH"
        log_info "Try: python -m argus_cli.main setup"
    fi
}
...
# extract a tree MISSING the electron binary, so the `electron`->`Argus` rename
...
# _purge_electron_build_cache() used by `argus desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Argus-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `argus desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Argus" ]; then
            app="$desktop_dir/release/linux-unpacked/Argus"
        ...
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Argus.app" \
            "$desktop_dir/release/mac/Argus.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("Argus is damaged and can't be opened").
    ...
    # checkout stays clean for the next `argus update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Argus-managed Node
            # provisioned during prerequisites/node-deps (at $ARGUS_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $ARGUS_HOME.
    # $ARGUS_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'argus update' on this host install.
