...
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/Argus202608/argus/main/scripts/install.sh | bash
#
# Or with options:
...
# Configuration
REPO_URL_SSH="git@github.com:Argus202608/argus.git"
REPO_URL_HTTPS="https://github.com/Argus202608/argus.git"
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/Argus202608/argus/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
clone_repo() {
    ...
    if [ -n "$LOCAL_SOURCE" ] && [ -d "$LOCAL_SOURCE" ]; then
        ...
        # Init a git repo so later `hermes update` / repo checks don't choke on a
...
copy_config_templates() {
    ...
    # Seed the bundled models.dev snapshot into ~/.hermes/models_dev_cache.json
    ...
    if [ ! -f "$HERMES_HOME/models_dev_cache.json" ]; then
        if [ -f "$INSTALL_DIR/agent/models_dev_api.json" ]; then
            cp "$INSTALL_DIR/agent/models_dev_api.json" "$HERMES_HOME/models_dev_cache.json"
            log_success "Seeded ~/.hermes/models_dev_cache.json from bundled snapshot"
        fi
    else
        log_info "~/.hermes/models_dev_cache.json already exists, keeping it"
    fi
