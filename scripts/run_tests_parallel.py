...
Environment:
    ARGUS_TEST_WORKERS  Override worker count (default: os.cpu_count())
    ARGUS_TEST_PATHS    Override discovery roots (colon-sep, default: 'tests')
...
#                        ``ARGUS_TEST_IMAGE`` so the fixture skips
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        "--jobs",
        type=int,
        default=int(os.environ.get("ARGUS_TEST_WORKERS") or (os.cpu_count() or 4) * 2),
        help="Parallel worker count (default: $ARGUS_TEST_WORKERS or cpu_count*2)",
    )
    parser.add_argument(
        "--paths",
        default=os.environ.get("ARGUS_TEST_PATHS", ":".join(_DEFAULT_ROOTS)),
    ...
    parser.add_argument(
        ...
        type=float,
        default=float(
            os.environ.get("ARGUS_TEST_FILE_TIMEOUT", _DEFAULT_FILE_TIMEOUT_SECONDS)
        ),
        help=(
            "Per-file wall-clock cap in seconds. On timeout, the pytest "
            "subprocess and its full process tree are SIGKILL'd. "
            f"Default: {_DEFAULT_FILE_TIMEOUT_SECONDS}s ({round(_DEFAULT_FILE_TIMEOUT_SECONDS/60)} min), env: ARGUS_TEST_FILE_TIMEOUT."
        ),
    )
    parser.add_argument(
        "--slice",
        ...
        help=(
            ...
            "Env: ARGUS_TEST_SLICE (format: I/N)."
        ),
    )
    parser.add_argument(
        "--generate-slices",
    ...
    # Parse --slice (or ARGUS_TEST_SLICE) early so we can exit on bad input
    # before doing any expensive discovery.
    slice_raw = args.slice or os.environ.get("ARGUS_TEST_SLICE")
