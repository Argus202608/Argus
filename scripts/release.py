...
Usage:
    ...
    # Override the release date (e.g. for a belated release)
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/MMArgus-Team/Argus",
    ...
    if first_release:
        ...
        lines.append("> for Argus. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Argus Release Tool")
    parser.add_argument("--bump", choices=["major", "minor", "patch"],
                        help="Which semver component to bump")
    parser.add_argument("--publish", action="store_true",
                        help="Actually create the tag and GitHub release (otherwise dry run)")
    parser.add_argument("--date", type=str,
                        help="Override release date stored in metadata (format: YYYY.M.D)")
    ...
    # Determine the human-readable release date stored in package metadata.
    if args.date:
    ...
    # Determine SemVer and use it as the canonical Git/PyPI release tag.
    ...
    if git("tag", "--list", tag_name):
        print(f"Tag {tag_name} already exists. Bump the package version before publishing.")
        return
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Argus v{new_version} ({calver_date})\n\nRelease"
        )
        ...
        # Push
        # Push only this release tag. The local clone may contain upstream tags
        # that must not be copied into the public Argus repository.
        push_result = git_result("push", args.remote, "HEAD", tag_name)
        if push_result.returncode == 0:
            print(f"  ✓ Pushed to {args.remote}")
        else:
            print(f"  ✗ Failed to push to {args.remote}: {push_result.stderr.strip()}")
            print("    Continue manually after fixing access:")
            print(f"    git push {args.remote} HEAD {tag_name}")

        # Build SemVer-named Python artifacts for PyPI and downstream packagers.
        ...
        if gh_bin:
            ...
            if release_exists:
                ...
                gh_cmd = [
                    ...
                    "--title", f"Argus v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(f"  ✓ GitHub release ready: {result.stdout.strip()}")
            ...
            print(
                f"    gh release create {tag_name} --title 'Argus v{new_version} ({calver_date})' "
