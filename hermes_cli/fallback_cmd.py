...
Subcommands:
  ...
  hermes fallback add      Pick provider + model via the same picker as `argus model`,
...
Storage: ``fallback_providers`` in ``~/.argus/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `argus model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  argus fallback add")
        print()
        return
    ...
    print("  Docs: https://mmargus-team.github.io/Argus/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `argus model`, then append the selection to the chain."""
    ...
    print("  `argus model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `argus fallback list` to view, or `argus fallback remove` to delete.")
