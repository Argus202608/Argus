...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"argus send: {file_path} is not a text file. --file reads the "
                ...
                f'  argus send --to telegram "MEDIA:{file_path}"\n'
                f'  argus send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  argus send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"argus send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"argus send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"argus send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"argus send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"argus send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `argus gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.argus/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_hermes_env() -> None:
    """Populate ``os.environ`` from ``~/.argus/.env`` AND bridge top-level
    ...
    1. ``load_dotenv(~/.argus/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.argus/config.yaml`` into
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.argus/.env and ~/.argus/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "argus send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  argus send --to telegram \"hello\"\n"
            "  argus send --to discord:#ops --file report.md\n"
            "  argus send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "argus send: no message provided. Pass text as a positional "
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Argus "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.argus/.env + ~/.argus/config.yaml) — no LLM, "
        ...
        epilog=(
            ...
            "  argus send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `argus send --list telegram`.",
    )
