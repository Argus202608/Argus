...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or argus -c.",
    ...
    "Ctrl+Z suspends Argus to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "argus -c resumes your most recent CLI session. argus -c \"project name\" resumes by title.",
    "argus -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "argus -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "argus chat -t web,terminal enables only specific toolsets for a focused session.",
    "argus chat -s github-pr-workflow preloads a skill at launch.",
    "argus chat -q \"query\" runs a single non-interactive query and exits.",
    "argus chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "argus chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "argus --yolo bypasses all dangerous command approval prompts for the entire session.",
    "argus chat --source telegram tags the session for filtering in argus sessions list.",
    "argus -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "argus doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "argus dump outputs a compact setup summary — great for bug reports.",
    "argus config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "argus config edit opens config.yaml in your default editor.",
    "argus config check scans for missing or stale configuration options.",
    "argus sessions browse opens an interactive session picker with search.",
    "argus sessions stats shows session counts by platform and database size.",
    "argus sessions prune --older-than 30 cleans up old sessions.",
    "argus skills search react --source skills-sh searches the skills.sh public directory.",
    "argus skills check scans installed hub skills for upstream updates.",
    "argus skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "argus skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "argus mcp add github --command npx adds MCP servers from the command line.",
    "argus mcp serve runs Argus itself as an MCP server for other agents.",
    "argus auth add lets you add multiple API keys for credential pool rotation.",
    "argus completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "argus logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "argus backup creates a zip backup of your entire Argus home directory.",
    "argus profile create coder creates an isolated profile that becomes its own command.",
    "argus profile create work --clone copies your current config and keys to a new profile.",
    "argus update syncs new bundled skills to ALL profiles automatically.",
    "argus gateway install sets up Argus as a system service (systemd/launchd).",
    "argus memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "argus webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: argus tools disables unused tools, argus skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "argus models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as argus -w).",
    ...
    "ARGUS_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Argus tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'argus profile create coder' creates the 'coder' command.",
    "argus profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Argus prints a resume command with session ID and stats.",
    "argus sessions export backup.jsonl exports all sessions for backup or analysis.",
    "argus -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "argus skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .argus/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: argus cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Argus runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "argus gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Argus via a local macOS server.",
    ...
    "Context files (.argus.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Argus loads project context from .argus.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "argus mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "argus mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "argus mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — argus tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.argus/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "argus chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "argus plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "argus acp runs Argus as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "ARGUS_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "argus auth add nous or argus auth add openai-codex sets up OAuth-based providers.",
    ...
    "argus status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.argus/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Argus your own.",
    ...
    "Before a gateway session reset, Argus auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.argus/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Profiles scope Argus state via HERMES_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "ARGUS_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    ...
    "Every interrupt during an agent run is logged to ~/.argus/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'argus profile create ls' would be rejected.",
    "argus profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "argus claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "ARGUS_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via argus tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.argus/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.argus/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Argus auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Argus config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Argus is working.',
    ...
    '/reload reloads ~/.argus/.env into the running session — pick up new API keys without restarting.',
    ...
    "ARGUS_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.argus/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.argus/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'argus curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "argus curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'argus curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'argus auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'ARGUS_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "ARGUS_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'argus dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'argus dashboard embeds the full Argus TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.argus/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.argus/dashboard-plugins/ — no npm build required.',
    ...
    'ARGUS_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'ARGUS_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'ARGUS_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'ARGUS_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.argus/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.argus/skills/ so drop-in skills appear without restarting the session.',
    ...
    'argus -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'argus chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'argus chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'argus chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "argus chat --source tool tags programmatic chats so they don't clutter argus sessions list.",
    'argus dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'argus sessions rename <ID> "new title" renames any past session; argus sessions delete <ID> removes one.',
    'argus import restores a session export or profile archive produced by sessions export or profile export.',
    'argus fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'argus pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'argus setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'argus status --deep runs the full health sweep across every component; plain argus status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'ARGUS_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'ARGUS_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.argus/plugins/ — trust-gated by design.',
    "ARGUS_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'ARGUS_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'ARGUS_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'ARGUS_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'ARGUS_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'ARGUS_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'ARGUS_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'ARGUS_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'ARGUS_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'ARGUS_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'ARGUS_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Argus block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from argus sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.argus/dashboard-plugins/.',
]
