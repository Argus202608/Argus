...
def reconcile_profile_gateways(
    *,
    ...
    is what ``argus gateway start`` (no ``-p``) targets. Without it,
    bare ``argus gateway start`` inside the container would land on
    ...
    # ``argus gateway start`` (no ``-p``) has somewhere to land;
    ...
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            ...
            # is not — that comes later via `argus setup`). Use it as the
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    if os.environ.get("ARGUS_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    ...
    * **s6-overlay v3:** PID 1 is ``s6-svscan`` and the command lives on the
    ...
    ``argus`` invocations carry no wrapper, so fall back to peeling a bare
    ``init`` prefix. The wrapper re-execs ``argus <subcommand>``, so an
    explicit leading ``argus`` is peeled too. Shared by the legacy-gateway
    ...
    # The wrapper re-execs `argus <subcommand>`; peel an explicit argus.
    if args and Path(args[0]).name in {"argus", "hermes"}:
        args = args[1:]
    return args
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``argus dashboard ...``) never spawns or
