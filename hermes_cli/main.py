...
Usage:
    argus                     # Interactive chat (default)
    argus chat                # Interactive chat
    argus gateway             # Run gateway in foreground
    argus gateway start       # Start gateway as service
    argus gateway stop        # Stop gateway service
    argus gateway status      # Show gateway status
    argus gateway install     # Install gateway service
    argus gateway uninstall   # Uninstall gateway service
    ...
    argus cron list           # List cron jobs
    argus cron status         # Check if cron scheduler is running
    argus doctor              # Check configuration and dependencies
    argus honcho setup                    # Configure Honcho AI memory integration
    argus honcho status                   # Show Honcho config and connection status
    argus honcho sessions                 # List directory → session name mappings
    argus honcho map <name>               # Map current directory to a session name
    argus honcho peer                     # Show peer names and dialectic settings
    argus honcho peer --user NAME         # Set user peer name
    argus honcho peer --ai NAME           # Set AI peer name
    argus honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    argus honcho mode                     # Show current memory mode
    argus honcho mode [hybrid|honcho|local]  # Set memory mode
    argus honcho tokens                   # Show token budget settings
    argus honcho tokens --context N       # Set session.context() token cap
    argus honcho tokens --dialectic N     # Set dialectic result char cap
    argus honcho identity                 # Show AI peer identity representation
    argus honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    argus honcho migrate                  # Step-by-step migration guide: OpenClaw native → Argus + Honcho
    argus version             Show version
    argus update              Update to latest version
    argus uninstall           Uninstall Argus
    argus acp                 Run as an ACP server for editor integration
    argus sessions browse     Interactive session picker with search

    argus claw migrate --dry-run  # Preview migration without changes
"""
...
# When the user upgrades code via ``git pull`` (or ``argus update``
...
# ``argus update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
def _set_process_title() -> None:
    """Set the process title to 'argus' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``argus tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``argus.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("argus")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"argus", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"argus")
        # Windows: the .exe name is already ``argus.exe`` — nothing to do.
    except Exception:
        pass
...
def _config_default_interface_early() -> str:
    ...
    try:
        ...
        if home:
            ...
            cfg_path = os.path.join(os.path.expanduser("~"), ".argus", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``ARGUS_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
        return False
    if os.environ.get("ARGUS_TUI") == "1" or "--tui" in argv:
        return True
...
# `entry.tsx`; this is just the earlier cousin. ``ARGUS_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("ARGUS_TUI_NO_EARLY_DISABLE") == "1":
        return
...
def _try_termux_ultrafast_version() -> bool:
    ...
    if os.environ.get("ARGUS_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
from hermes_cli.subcommands.mm import build_mm_parser
from hermes_cli.subcommands.security import build_security_parser
from hermes_cli.subcommands.dump import build_dump_parser
from hermes_cli.subcommands.mm_memory_eval import build_mm_memory_eval_parser
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (argus tools, argus setup, argus model) use
...
# every subsequent ``os.getenv("ARGUS_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.argus/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `argus mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Argus' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        ...
        candidate = home / ".argus" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # parent directory name (e.g. ~/.argus/profiles/coder or
    # /opt/data/profiles/coder).  If HERMES_HOME points to the hermes root
    # instead (e.g. systemd hardcodes HERMES_HOME=/root/.argus), we must
    # still read active_profile — the user may have switched profiles via
    # `hermes profile use` and the gateway should honour that choice.
    # See issue #22502.
    hermes_home_env = os.environ.get("ARGUS_HOME", "")
    ...
    # run-script as ARGUS_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``argus gateway run`` to mean
    ...
    if profile_name is None and not os.environ.get("ARGUS_S6_SUPERVISED_CHILD"):
        try:
...
# Load .env from ~/.argus/.env first, then project root as dev fallback.
...
# Bridge security.redact_secrets from config.yaml → ARGUS_REDACT_SECRETS env
...
try:
    ...
    if _cfg_path.exists():
        ...
        try:
            ...
            pass
        if "ARGUS_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["ARGUS_REDACT_SECRETS"] = str(_early_redact).lower()
...
def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("ARGUS_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("ARGUS_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    # Determine whether Argus itself has been explicitly configured (model
    ...
    # Only count these if Argus has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Argus to use their tokens.
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo argus {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo argus {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  argus --tui --resume {target}")
    if title:
        print(f'  argus --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    # Workspace installs are intentionally scoped to ``ui-tui`` (see
    # _make_tui_argv below). The root lock also describes desktop, web, and
    # every sibling workspace, so comparing all of it makes a successful
    # ``npm install --workspace ui-tui`` look incomplete forever. Walk only the
    # selected workspace's dependency closure; standalone layouts keep the
    # original whole-lock comparison below.
    if ws_root != root:
        try:
        ...
        def resolve_dependency(parent_key: str, dep_name: str) -> str | None:
            # Node resolution prefers a package-local node_modules before the
            # hoisted workspace root. Lockfile keys use POSIX separators.
            parent = parent_key.strip("/")
            candidates: list[str] = []
            while parent:
                candidates.append(f"{parent}/node_modules/{dep_name}")
            ...
            for candidate in candidates:
                if candidate in wanted:
                    return candidate
            # File/workspace dependencies also have a real package entry whose
            # ``name`` matches (the node_modules entry itself is a link).
            for candidate, meta in wanted.items():
                if (isinstance(meta, dict)
                        and meta.get("name") == dep_name
                        and "/node_modules/" not in candidate
                        and not candidate.startswith("node_modules/")):
                    return candidate
            return None

        if workspace_key and workspace_key in wanted:
            pending: list[tuple[str, bool]] = [(workspace_key, True)]
            required: dict[str, bool] = {}
            while pending:
                package_key, is_required = pending.pop()
                if package_key in required and (required[package_key] or not is_required):
                    continue
                required[package_key] = is_required or required.get(package_key, False)
                meta = wanted.get(package_key)
                if not isinstance(meta, dict):
                    continue

                # npm link entries point at the local workspace containing the
                # actual dependency metadata; inspect both sides.
                if meta.get("link") and isinstance(meta.get("resolved"), str):
                    target = str(meta["resolved"]).strip("/")
                    if target in wanted:
                        pending.append((target, is_required))

                deps: dict[str, bool] = {}
                for dep_name in (meta.get("dependencies") or {}):
                    deps[str(dep_name)] = True
                # Build tooling belongs to local workspace packages, not to
                # installed third-party packages' devDependencies.
                if (package_key == workspace_key
                        or package_key.startswith(f"{workspace_key}/packages/")):
                    for dep_name in (meta.get("devDependencies") or {}):
                        deps[str(dep_name)] = True
                for dep_name in (meta.get("optionalDependencies") or {}):
                    deps.setdefault(str(dep_name), False)
                for dep_name, dep_required in deps.items():
                    dep_key = resolve_dependency(package_key, dep_name)
                    if dep_key is None:
                        if dep_required:
                            return True
                        continue
                    # A required child of an optional, platform-skipped parent
                    # is still optional for this installation.
                    pending.append((dep_key, is_required and dep_required))
            ...
            for name, is_required in required.items():
                ...
                if name not in installed:
                    if not is_required or pkg.get("optional") or pkg.get("peer"):
                        continue
                    return True
                if (isinstance(installed[name], dict)
                        and comparable(pkg) != comparable(installed[name])):
                    return True
            return False
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``ARGUS_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("ARGUS_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``ARGUS_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("ARGUS_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    hermes_home = os.environ.get("ARGUS_HOME") or str(Path.home() / ".argus")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "ARGUS_HOME": hermes_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``argus update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("ARGUS_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this Argus checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `argus update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Argus checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `argus --tui`\n"
        "If the checkout is still inconsistent, run `argus update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (ARGUS_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("ARGUS_NODE")
    ...
    ext_dir = os.environ.get("ARGUS_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with ARGUS_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset ARGUS_TUI_DIR (e.g. `unset ARGUS_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
...
def _launch_tui(
    ...
    env["ARGUS_PYTHON_SRC_ROOT"] = os.environ.get(
        "ARGUS_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("ARGUS_PYTHON", sys.executable)
    env.setdefault("ARGUS_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["ARGUS_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["ARGUS_MODEL"] = model
        env["ARGUS_INFERENCE_MODEL"] = model
    if provider:
        env["ARGUS_TUI_PROVIDER"] = provider
        env["ARGUS_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["ARGUS_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["ARGUS_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["ARGUS_TUI_SKILLS"] = value
    if query:
        env["ARGUS_TUI_QUERY"] = query
    if image:
        env["ARGUS_TUI_IMAGE"] = image
    if checkpoints:
        env["ARGUS_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["ARGUS_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["ARGUS_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["ARGUS_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["ARGUS_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["ARGUS_ACCEPT_HOOKS"] = "1"
    ...
    # ARGUS_TUI_RESUME is an internal hand-off from the Python wrapper to the
    ...
    # still set ARGUS_TUI_RESUME themselves.
    env.pop("ARGUS_TUI_RESUME", None)
    if resume_session_id:
        env["ARGUS_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `argus update` so
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``ARGUS_KANBAN_BOARD`` for the chat session.
    ...
    """
    if os.environ.get("ARGUS_KANBAN_BOARD"):
        return
    try:
        from hermes_cli.kanban_db import get_current_board

        os.environ["ARGUS_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.argus/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Argus — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Argus still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``ARGUS_TUI=1`` → always TUI
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("ARGUS_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'argus sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'argus doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Argus isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  argus setup")
        print()
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'argus setup' at any time to configure.")
        sys.exit(1)
    ...
    # setup (config, rules files, plugins, MCP servers) or from Argus itself.
    # Implemented as a superset of --ignore-user-config + --ignore-rules plus
    # plugin/MCP discovery suppression (ARGUS_SAFE_MODE is checked by
    # hermes_cli/plugins.py and tools/mcp_tool.py).
    if getattr(args, "safe_mode", False):
        os.environ["ARGUS_SAFE_MODE"] = "1"
        os.environ["ARGUS_IGNORE_USER_CONFIG"] = "1"
        os.environ["ARGUS_IGNORE_RULES"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.argus/config.yaml and return built-in defaults. Set BEFORE
    ...
    if getattr(args, "source", None):
        os.environ["ARGUS_SESSION_SOURCE"] = args.source
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Argus?")
        print()
    ...
    # has no creds.json.  Every subsequent `argus gateway` then paid a 30s
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: argus gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `argus gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  argus gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Argus'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  argus gateway")
        ...
        print()
        print("  Or install as a service: argus gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'argus whatsapp' to try again.")
...
def cmd_postinstall(args):
    ...
    print("⚕ Argus post-install bootstrap")
    print()
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``argus model``) and the setup wizard
    ...
    effective_provider = (
        config_provider or os.getenv("ARGUS_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'argus model' for "
                "available providers, or run 'argus doctor' to diagnose config "
                "issues."
            )
    ...
    # a leftover OPENAI_BASE_URL in ~/.argus/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.argus/.env if the active provider is not 'custom'.
...
#
# Argus uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `argus model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `argus model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Argus only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``argus model`` flow, then route aux tasks to them here.
    """
    from hermes_cli.config import load_config
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Argus URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``argus setup`` / ``argus model``.
    ...
    recover from a malformed paste without editing ``~/.argus/.env`` by hand.
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `argus setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(
                f"    Argus will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Argus CLI with a provider."""
...
def cmd_slack(args):
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: argus slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `argus slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1
...
def cmd_mm(args):
    """Dispatch `hermes mm <action>` (multimodal utilities)."""
    action = getattr(args, "mm_action", None)
    if action == "doctor":
        from hermes_cli.mm_doctor import run_mm_doctor
        raise SystemExit(run_mm_doctor(args))
    # No/unknown action → show help for the mm command.
    import sys
    print("usage: argus mm <action>\n\nactions:\n  doctor    "
          "Check multimodal readiness (voice/search/memory/tracking)",
          file=sys.stderr)
    raise SystemExit(2)
...
def cmd_mm_memory_eval(args):
    """离线视频 → 构多模态记忆 → 跑 QA 召回评测, 写回 answer_predict。"""
    # This command builds real multimodal memory, so a missing REQUIRED
    # capability (e.g. rapidocr → memory backend refuses to start) would make it
    # silently produce empty results. Warn up front (non-blocking).
    _warn_if_multimodal_not_ready()

    from hermes_cli.mm_memory_eval import run_mm_memory_eval

    return run_mm_memory_eval(
        video=args.video,
        json_path=args.json,
        mode=getattr(args, "mode", "tool"),
        source=getattr(args, "source", "camera"),
        answer_timing=getattr(args, "answer_timing", "after"),
        query_types=getattr(args, "query_types", None),
        asr_vtt=getattr(args, "asr_vtt", "auto"),
        scene_probe=getattr(args, "scene_probe", True),
...
def cmd_backup(args):
    """Back up Argus home directory to a zip file."""
...
def cmd_uninstall(args):
    """Uninstall Argus (or just the Chat GUI with --gui)."""
...
# even run ``argus update`` again to roll forward. The post-pull syntax
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``argus update --gateway`` so interactive prompts (stash restore,
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``argus update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (argus web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``argus update`` can unconditionally call ``argus desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``argus desktop`` (interactive launch) skips the build when the
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``argus update`` that pulled new source but hasn't built yet).
    """
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Argus'`` desktop build failure: a corrupt zip in the
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Argus.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Argus.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Argus.exe: Access is
    ...
    an unrelated "Argus" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Argus is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Argus.app/Contents/MacOS/Argus  ->  app bundle = .../Argus.app
    app = exe.parents[2]
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``argus desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Argus Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Argus Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def cmd_gui(args: argparse.Namespace):
    ...
    if getattr(args, "fake_boot", False):
        env["ARGUS_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["ARGUS_DESKTOP_IGNORE_EXISTING"] = "1"
    root_override = getattr(args, "argus_root", None) or getattr(args, "hermes_root", None)
    if root_override:
        env["ARGUS_DESKTOP_ARGUS_ROOT"] = str(Path(root_override).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["ARGUS_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  argus gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # holds Argus.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Argus.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Argus.exe, close any")
                    print("  running Argus desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> argus desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Argus is
    ...
    if source_mode:
        print("→ Launching Argus Desktop from source build...")
    ...
    print(f"→ Launching packaged Argus Desktop: {' '.join(launch_command)}")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``argus dashboard`` processes other than ourselves.

    ``argus dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``argus update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Argus Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``argus dashboard`` as
    ...
    environment variable ``ARGUS_DESKTOP_CHILD_PID`` on the spawned
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `argus update`.
    ...
    print("  Preview now:  argus curator run --dry-run")
    print("  Pause it:     argus curator pause")
    print(
        "  Docs:         https://mmargus-team.github.io/Argus/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``argus update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``argus update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: argus curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``argus dashboard`` processes.

    Called at the end of ``argus update`` (default ``reason``) and also
    from ``argus dashboard --stop`` (which overrides ``reason``).  The
    ...
    """
    # When the Argus Desktop Electron app spawns this dashboard as a
    # backend child, it sets ARGUS_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("ARGUS_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    if killed:
        ...
        print("    argus dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    """
    staging = f"{dst}.argus-update-staging"
    backup = f"{dst}.argus-update-old"
...
def _update_via_zip(args):
    """Update Argus by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `argus update "
            f"--branch {branch}`, or update against main with `argus update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/MMArgus-Team/Argus/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="argus-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"argus-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for Argus itself
        ...
        # GitHub ZIPs extract to Argus-<branch>/.
        extracted = os.path.join(tmp_dir, f"Argus-{branch}")
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: argus skills list-modified  "
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "argus-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Argus behaves unexpectedly.")
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Argus couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Argus couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Argus behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Argus couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Argus couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `argus update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/MMArgus-Team/Argus.git",
    "git@github.com:MMArgus-Team/Argus.git",
    "https://github.com/MMArgus-Team/Argus",
    "git@github.com:MMArgus-Team/Argus",
}
OFFICIAL_REPO_URL = "https://github.com/MMArgus-Team/Argus.git"
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Argus repository.")
        print("  This means you may miss updates from MMArgus-Team/Argus.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/MMArgus-Team/Argus.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/MMArgus-Team/Argus.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``argus update``, every profile is now current.
    """
    homes = []
...
# Install-scoped breadcrumb dropped right before ``argus update`` mutates the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``argus update`` left half-done.
    ...
    try:
        ...
        print(
            "⚠ A previous `argus update` was interrupted mid-install — "
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``argus update`` is still progressing even if pip/uv itself is silent.
    """
...
def _detect_concurrent_hermes_instances(
    ...
    The Argus Desktop Electron app spawns ``hermes.EXE`` as a backend child,
    so during ``argus update`` the user-invoked process and the desktop's
    ...
    chain are excluded so the running ``argus update`` invocation never
    ...
    # file lock) is the parent. Without excluding it, every ``argus update``
    ...
    #      hermes.exe sitting *under* a non-Argus parent (e.g. a Argus
...
def _format_concurrent_instances_message(
    ...
    lines.append("")
    lines.append("  Close Argus Desktop, exit any open `hermes` REPLs, and")
    lines.append("  stop the gateway (`argus gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `argus update --force` if you've already")
...
def _quarantine_running_hermes_exe(
    ...
    when ``argus update`` runs, ``hermes.exe`` IS the live process, and uv
    ...
    handles (recovers in <1s), or the Argus Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Argus Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            "    Close Argus Desktop, exit other `hermes` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `argus update`."
        )
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``argus update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `argus update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    # hours later inside ``argus update``'s desktop-rebuild or skill-sync
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``argus update``, not the venv we just wrote
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `argus update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `argus update --force` after closing other hermes processes."
        )
        return
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `argus update`, so we install root-only first
    ...
    # hung. The chatty npm-deprecation noise during `argus update` comes from
...
class _UpdateOutputStream:
    """Stream wrapper used during ``argus update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.argus/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``argus update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``argus update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.argus/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``argus update --gateway``) the update is already
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.argus/logs/update.log`` only, never the terminal.

    During ``argus update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``argus update --check``: fetch and report without installing.
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``argus update`` (apply
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``argus update`` without requiring a reinstall.  The
    ...
    path_comment = (
        "# Argus — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    ...
    HERMES_HOME directories.  The ``--backup`` flag on ``argus update``
    ...
    # Render path using display_hermes_home so the user sees ~/.argus/...
    try:
    ...
    print(f"  Restore:  argus import {out_path}")
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    print("→ Stopping Windows gateway process(es) before updating Argus...")
    try:
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: argus gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``argus gateway start`` uses (``gateway_windows._spawn_detached``).
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``argus update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update Argus to the latest version.
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `argus update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Argus via pip (for PyPI installs)."""
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # every user who ran ``argus update`` for the 7 minutes between
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``argus update`` again later once a fix lands.")
                    else:
        ...
        # build.  ``argus desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``argus desktop`` shouldn't be forced into a full
        # Electron build by ``argus update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        ...
        if (desktop_dir / "package.json").exists() and find_node_executable("npm") and has_desktop_app:
            ...
            if build_result.returncode != 0:
                build_result = _run_logged_subprocess(_desktop_build_cmd, cwd=PROJECT_ROOT)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `argus desktop` to retry)")
        ...
        # current — so copy it straight over ~/.argus/cache/model_catalog.json
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'argus config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: argus config migrate")
            else:
                print()
                print("Skipped. Run 'argus config migrate' later to configure.")
        else:
        ...
        # don't have it. Tying the refresh to ``argus update`` gives users a
        ...
        # When running as ``argus update --gateway`` (spawned by the gateway's
        ...
        try:
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            if _manage_cmd is None:
                                print(
                                    ...
                                    f"    To let `argus update` restart it automatically, allow\n"
                        ...
                        print(
                            ...
                            f"Check the gateway with: argus gateway status"
                        )
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: argus gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: argus -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Argus gateway unit files are still installed.
        # When both hermes.service (from a pre-rename install) and the
        # current hermes-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `argus update` surfaces the issue until the user migrates.
        try:
            from hermes_cli.gateway import (
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                print()
                print("⚠ Legacy Argus gateway unit(s) detected:")
                ...
                print()
                print("    argus gateway migrate-legacy")
                print()
        ...
        print("  argus model              # Select provider and model")
...
def cmd_profile(args):
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → argus -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.argus)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  argus profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     argus -p {name} chat")
                else:
        ...
        if dist_name:
            ...
            print(f"  (run `argus profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → argus -p {name}  ({wrapper})")
        print()
        ...
        try:
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  argus -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      argus -p {plan.manifest.name} chat")
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `argus profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"argus -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        if data.get("hermes_requires"):
            print(f"Requires:     Argus {data['hermes_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    if mf.hermes_requires:
        print(f"  Requires: Argus {mf.hermes_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``argus dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``argus dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No argus dashboard processes running.")
        return 0

    print(f"{len(pids)} argus dashboard process(es) running:")
    for pid in pids:
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``argus dashboard register`` for OAuth.
    ...
    print("    [2] OAuth via Nous Portal (run `argus dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    if choice == "2":
        ...
        print(
            ...
            "the dashboard again:\n"
            "    argus dashboard register\n"
            "  It provisions a Nous Portal OAuth client and writes "
            "ARGUS_DASHBOARD_OAUTH_CLIENT_ID into ~/.argus/.env for you.\n"
            "  Docs: https://mmargus-team.github.io/Argus/docs/"
...
def _warn_if_multimodal_not_ready() -> None:
    """Print a non-blocking advisory if required multimodal capabilities are
    missing, pointing the user at `argus setup multimodal`. Best-effort — any
    error here must never stop the dashboard from starting."""
    try:
        ...
        missing_required = [
            ...
            if c.get("required") and c.get("status") != OK
        ]
        ...
        try:
            ...
            warn = cyan = lambda s: s  # noqa: E731
        print(warn("⚠ 多模态未完全就绪 — 以下必需能力缺失,相关功能将无法工作:"))
        for c in missing_required:
            print(warn(f"    ✗ {c['label']}: {c.get('reason', '')}"))
            if c.get("fix"):
                print(cyan(f"      修复: {c['fix']}"))
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No argus dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `argus update`.
    ...
    # ── Project config → HERMES_HOME sync ─────────────────────────────
    # The project-root config.yaml is the source of truth (git-tracked, travels
    # across machines). Copy it over <HERMES_HOME>/config.yaml BEFORE any
    # load_config / start_server / gateway spawn, so both this process and the
    # spawned gateway (which inherits HERMES_HOME) read the latest project
    ...
    try:
        ...
        # HERMES_HOME-relative model paths (Qwen2.5-0.5B-Instruct, RapidOCR) resolve to
    ...
    # capability is missing (e.g. rapidocr → memory backend refuses to start),
    # the user would otherwise land in a chat where features silently don't
    # work. Warn up front and point at the fix — but don't block startup, since
    # the dashboard also serves non-multimodal pages.
    _warn_if_multimodal_not_ready()
    ...
    if (
        ...
        and os.environ.get("ARGUS_DESKTOP") != "1"
    ):
        ...
        # HERMES_HOME falls back to $HOME/.argus = /opt/data/.argus — an
        ...
        # returns the root for both layouts: ~/.argus for a standard install
    ...
    # the same logs directory as every other Argus surface.
    try:
        from hermes_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if "ARGUS_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["ARGUS_WEB_DIST"])
            if "ARGUS_WEB_DIST" in os.environ
...
def cmd_logs(args):
    """View and filter Argus log files."""
...
_BUILTIN_SUBCOMMANDS = frozenset(
    {
        ...
        "dump", "mm-memory-eval", "fallback", "gateway", "hooks", "import", "insights",
        "gui", "desktop", "kanban", "login", "logout", "logs", "lsp", "mcp", "memory", "migrate", "moa",
        "mm", "model", "onboarding", "pairing", "pets", "plugins", "portal", "postinstall", "profile",
        "project", "proxy",
        "prompt-size",
        "send", "sessions", "setup",
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("ARGUS_TUI") == "1")
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["ARGUS_DEFER_AGENT_STARTUP"] = "1"
            os.environ["ARGUS_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["ARGUS_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def main():
    ...
    # ``argus update`` runs on Windows. Silent no-op on non-Windows or when
    ...
    # Self-heal a venv left half-built by an interrupted ``argus update``
    ...
    # after _apply_profile_override() (module import, above) so HERMES_HOME —
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://mmargus-team.github.io/Argus/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `argus model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.argus/.env.  Currently supports "
            "Bitwarden Secrets Manager.  See: "
            "https://mmargus-team.github.io/Argus/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `argus whatsapp` which sets up "
    ...
    build_mm_parser(subparsers, cmd_mm=cmd_mm)
    ...
    # mm-memory-eval command (parser in hermes_cli/subcommands/mm_memory_eval.py)
    # =========================================================================
    build_mm_memory_eval_parser(subparsers, cmd_mm_memory_eval=cmd_mm_memory_eval)
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.argus/checkpoints/",
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Argus shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `argus computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `argus tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `argus computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Argus app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            # Honor ARGUS_CUA_DRIVER_CMD for local-build testing — same
            ...
            if path:
                ...
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: argus computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: argus computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: argus computer-use install --upgrade")
                return
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: argus computer-use permissions grant")
    ...
    # for one release. The Argus-Setup.exe success screen tells users to
    # run `argus desktop` from a terminal, so the canonical name needs
