"""
Unified self-relaunch for Argus CLI.
...
def resolve_hermes_bin() -> Optional[str]:
    """Find the argus entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("argus")`` on PATH.
    ...
    ``argus.exe`` on PATH or the ``sys.executable -m hermes_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with argus.
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            print(
                f"\nArgus relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run argus.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
