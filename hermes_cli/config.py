...
Config files are stored in ~/.argus/ for easy access:
- ~/.argus/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.argus/.env         - API keys and secrets

This module provides:
- argus config          - Show current configuration
- argus config edit     - Open config in editor
- argus config set      - Set a specific value
- argus config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``argus config set`` (which rewrites
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.argus/config.yaml`` causes ``load_config()``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  argus config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Argus makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Argus itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``argus update``, the TUI build.
...
#   on every plugin install / ``argus update``.
...
#
# IMPORTANT: ``ARGUS_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (ARGUS_LANGFUSE_PUBLIC_KEY,
# ARGUS_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Argus runtime location — never via dashboard env writer.
    # NOT aARGUS_* blanket: integration credentials (ARGUS_GEMINI_*,
    # ARGUS_LANGFUSE_*, ARGUS_SPOTIFY_*, ...) ARE allowed.
    "ARGUS_HOME", "ARGUS_PROFILE", "ARGUS_CONFIG", "ARGUS_ENV",
})
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "ARGUS_TOOL_PROGRESS", "ARGUS_TOOL_PROGRESS_MODE",
    ...
    # observability/langfuse` or `argus tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "ARGUS_LANGFUSE_ENV",
    "ARGUS_LANGFUSE_RELEASE",
    "ARGUS_LANGFUSE_SAMPLE_RATE",
    "ARGUS_LANGFUSE_MAX_CHARS",
    "ARGUS_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ARGUS_MANAGED", "").strip()
    if raw:
...
def is_managed() -> bool:
    """Check if Argus is running in package-manager-managed mode.

    Two signals: the ARGUS_MANAGED env var (set by the systemd service),
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Argus was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.
    ...
    3. ARGUS_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.argus``)
    --------------------------------------------------------------------------
    ...
    bind-mount it (``~/.argus:/opt/data``) so config/sessions/memory persist
    ...
    ``docker`` and ``argus update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``mmargus/argus`` image bakes a ``docker``
    ...
    #    container, and honouring it wrongly blocks ``argus update``.
    try:
        method = (
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Argus lives in a ``uv tool`` layout.
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade mm-argus"
    if method == "docker":
        return "docker pull mmargus/argus:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade mm-argus"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade mm-argus"
        return "pip install --upgrade mm-argus"
    return "argus update"
...
# Long-form text for ``argus update`` / ``--check`` when running inside the
...
#     — that script installs a *new* host-side Argus, it doesn't update
...
✗ ``argus update`` doesn't apply inside the Docker container.

Argus runs as a published image (mmargus/argus), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull mmargus/argus:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate hermes-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm mmargus/argus:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/mmargus/argus/tags
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``argus update`` inside Docker.
...
def format_managed_message(action: str = "modify this Argus installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("ARGUS_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Argus installation is managed by NixOS "
            f"(ARGUS_MANAGED={env_hint}).\n"
            "Edit services.argus.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Argus installation is managed by Homebrew "
            f"(ARGUS_MANAGED={env_hint}).\n"
            "Use:\n"
    ...
    return (
        f"Cannot {action}: this Argus installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Argus."
    )
...
def _resolve_hermes_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the ARGUS_UID / ARGUS_GID env vars set by Docker deployments.

    Docker containers running Argus commonly set these to map the in-container
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("ARGUS_UID", "").strip()
    gid_str = os.environ.get("ARGUS_GID", "").strip()
    try:
...
def _chown_to_hermes_uid(path) -> None:
    """Chown ``path`` to ``ARGUS_UID:ARGUS_GID`` if those env vars are set.
...
def _secure_dir(path):
    ...
    The mode can be overridden via the ARGUS_HOME_MODE environment variable
    (e.g. ARGUS_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse HERMES_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``ARGUS_UID``/``ARGUS_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ARGUS_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Argus runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("ARGUS_CONTAINER") or os.environ.get("ARGUS_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set ARGUS_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def ensure_hermes_home():
    """Ensure ~/.argus directory structure exists with secure permissions.
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Argus-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Argus
        ...
        # identity slot (SOUL.md). Empty by default. The ARGUS_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Argus adds a coding operating brief
    ...
    "terminal": {
        ...
        # skipped. When empty, Argus auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Argus sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Argus
        ...
        #  "/home/user/.argus/cache/documents:/output"]
        ...
        # bundled Argus image, which drops to the `hermes` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Argus sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Argus should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.argus/checkpoints/`` size (MB).  When
    ...
    # AGENTS.md, CLAUDE.md, .argus.md, or .cursorrules before Argus applies
    ...
    # single read_file page exceeds these limits, Argus truncates the
    ...
    # kanban dashboard (see ``argus dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # model; override via `argus model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # users aren't surprised.  ARGUS_TUI_RESUME=<id> always wins.
        ...
        # CLI, TUI, and desktop app. Manage with `argus pets`. Disabled until
    ...
    "dashboard": {
        ...
        # ``ARGUS_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``ARGUS_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``ARGUS_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``ARGUS_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``ARGUS_DASHBOARD_PUBLIC_URL``).
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Argus appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.argus/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.argus/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # ------------------------------------------------------------------ #
    # Multimodal streaming subsystem (always-on video/audio agent).
    #
    # Numeric hyperparameters for the 6-role multimodal agent ported from
    # streaming_demo (FrontWorker / MemoryWriter / RouterWorker / SearchWorker
    # / RecallWorker / MemoryReviewer). These tune worker cadence, frame
    # sampling, memory aggregation and review behavior. They are consumed by
    # agent.multimodal via the Argus config glue (agent/multimodal/hermes_glue.py).
    #
    # NOTE: model / endpoint / API-key selection is intentionally NOT here —
    # the multimodal workers reuse Argus' own provider/model resolution
    # (`argus model`, providers, credential pool). Switching the Argus model
    # automatically switches the multimodal model. ASR/TTS endpoints likewise
    # come from the pluggable SpeechFactory (cloud APIs slot in there), not config.
    # ------------------------------------------------------------------ #
    # ═══════════════════════════════════════════════════════════════════════
    # SETTINGS — the multimodal subsystem's behavior knobs (renamed from the old
    # flat ``multimodal:`` block). Per-role LLM endpoints moved to
    # ``model.{monitor,watcher,memory[.recall]}``; the audio interface moved to
    # the ``audio:`` section. agent/multimodal/hermes_glue.flatten_mm_config()
    # merges all three homes back onto the legacy flat field names the Config
    # dataclass reads, with a fallback to the pre-v32 flat ``multimodal:`` block.
    # ═══════════════════════════════════════════════════════════════════════
    "settings": {
        "enabled": True,
        # Phase-2 main-agent vision turns: when the browser streams camera/screen
        # frames, the main chat agent switches to this vision/MLLM provider for
        # the turn (then restores). Empty → reuse the top-level `auxiliary.vision`
        # provider/model. Format mirrors a `/model` switch ("model --provider X").
        #   vision_model    — e.g. "qwen-vl-max" or "gpt-4o" ("" = auxiliary.vision)
        #   vision_provider — e.g. "custom" / "openrouter" ("" = auxiliary.vision)
        # Frames considered "active" only if newer than vision_stream_max_age_sec.
        "vision_model": "",
        "vision_provider": "",
        # NOTE: the deep-research worker endpoint moved to model.watcher.*, the
        ...
        # was removed); a monitor only exists once the user opts in via set_monitor
        # and each monitor gates on its own per-monitor enabled flag.
        # MonitorAgent daemon behavior knobs (non-secret; overridable in config.yaml).
        # New frames wake MonitorWorker immediately (0.8s debounce); this is the
        # fallback heartbeat for static scenes / aggregation flushes.
        "monitor_tick_sec": 1.0,
        "monitor_debounce_sec": 0.8,    # coalesce bursty new-frame wakeups
        "monitor_overload_period": 8,   # after N ticks pegged at 64-frame cap, double stride
        "monitor_fail_disable_after": 5,  # consecutive LLM failures → auto-disable that monitor
        # NOTE: the MonitorAgent model endpoint moved to model.monitor.*.
        # At each report period the window's SPEAKs are always merged into one
        # timestamped digest (fmt_agg, deterministic local concat — no LLM, no
        # token limit). The old monitor_merge_enabled/max_tokens knobs were removed
        # (v33: they were never consumed).
        # Independent layered-memory backend (MemoryWriter + MemoryReviewer):
        # builds episodic/entity memory from the SAME live frames the main agent
        # streams in. Runs its own loops + SQLite; reads the shared FrameBuffer.
        # ON by default. Note: each session with a live video stream spins up
        # timed LLM calls (writer ~every 5s, reviewer ~every 60s) on the memory
        # backend model, but the backend stays idle (no LLM calls) until frames
        # actually arrive, so text-only chat is unaffected. v33: always on (the
        # settings.memory_enabled gate was removed).
        # NOTE: the MemoryWriter/Reviewer LLM backend moved to model.memory.*
        # (provider "gemini"/"openai"/"" + base_url + api_key + model), and the
        # recall sub-agent endpoint to model.memory.recall.*. The ASR/TTS
        # speech_provider moved to the audio: section.
        # (删: FrontWorker 1s 自驱死配置 — FrontWorker 已删, 这些 key 无消费者:
        ...
        "cont_now_frames": 2,
        "cont_recent_history_turns": 16,
        # Per-session QueryWorker admission control: two jobs may execute at
        # once and at most eight additional jobs may wait for a slot.
        ...
        "writer_image_jpeg_quality": 85,
        "writer_asr_window_sec": 15.0,
        # E9: inject known entities into the Writer prompt in 3 tiers (tier-1
        # carries representative-frame thumbnails). Guards against "ghost
        # entities" polluting tier-1 via a macro-hit ranking signal.
        "writer_entity_enabled": True,
        "writer_entity_min_seen": 2,
        "writer_entity_macro_lookback": 8,
        "writer_entity_tier1_n": 5,
        "writer_entity_tier2_n": 10,
        "writer_entity_tier3_n": 15,
        "writer_entity_tier1_thumb_side": 0,      # 0 = original frame (no re-encode)
        "writer_entity_tier1_thumb_quality": 80,
        "writer_entity_tier1_visual_types": "",   # "" = all types get a thumbnail
        "writer_reused_id_enabled": True,         # LLM may pin reused_entity_id (fast path)
        # E10: Writer history uses structured macro+micro events instead of raw
        # [scene observation] lines (only the current pending segment stays raw).
        "writer_event_timeline_enabled": True,
        ...
        "recall_max_rounds": 4,
        "recall_topk_micro": 5,
        "recall_topk_entity": 5,
        "recall_distill_max_tokens": 512,
        ...
        "conv_min_turns": 1,
        "conv_max_bg_obs": 200,
        "conv_max_audio_obs": 300,
        # FrameBuffer (in-memory rolling raw frames)
        "buffer_seconds": 1800.0,
        "buffer_capture_fps": 2.0,
        # Session-scoped external-search evidence cache. MemoryWriter never
        # writes these facts; fresh text-search results populate them.
        "search_facts_max": 64,
        "search_fact_ttl_sec": 3600.0,
        "search_fact_value_max_chars": 4000,
        # MemoryStore aggregation triggers
        "mem_l2_macro_min_micro": 5,
        "mem_l2_macro_max_duration": 180.0,
        ...
        "agg_l2_frames": 50,
        "agg_l3_frames": 128,
        "mem_micro_max_duration": 30.0,
        "mem_micro_max_ticks": 6,
        # ⑥ MemoryReviewer — periodic / macro-triggered memory revision
        "reviewer_enabled": True,
        "reviewer_wake_interval": 60.0,
        "reviewer_total_frames": 128,
        ...
        # FrameStore (key-frame persistence + dedup)
        "frame_store_max": 4000,
        "frame_store_dedup_scan_n": 8,
        "frame_store_ts_exact_eps": 0.3,
        # (frame_store_dhash_lookback/threshold removed in v33: layer-2 dHash
        #  dedup was deleted; frame-level dedup moved to FrameBuffer entry.)
        "cont_recall_frames_max": 4,
        "ui_event_thumb_max_side": 480,
        "ui_event_thumb_jpeg_quality": 70,
        "inflight_brief_ttl": 30.0,
        # Failure backstops
        "memory_max_consecutive_failures": 10,
        "reviewer_max_consecutive_failures": 10,
    },

    # ═══════════════════════════════════════════════════════════════════════
    # AUDIO — the multimodal subsystem's speech interface (ASR/TTS/env-audio),
    # split out of the old ``multimodal:`` block. flatten_mm_config() merges
    # these keys back onto the flat Config dataclass fields (fallback: legacy
    # flat ``multimodal.<key>``). NOTE: this is the /multimodal live-stream
    # audio path — the main chat's ``tts:``/``stt:`` sections are separate.
    # ═══════════════════════════════════════════════════════════════════════
    "audio": {
        # ASR/TTS backend for the multimodal agent:
        #   "hermes" — reuse Argus' built-in ASR/TTS providers (top-level
        #              `tts:`/`stt:`). "local" — bundled HTTP placeholder
        #              backend. "cloud" — user-implemented direct integration.
        "speech_provider": "local",
        # Environment-audio gating (video-embedded speech → ASR)
        "env_audio_enabled": True,
        "env_audio_window_sec": 5.0,
        "env_audio_min_rms": 0.005,
        "env_audio_min_text_chars": 2,
        # ── Audio backends (port from streaming_demo; HTTP services) ──
        # User-speech ASR (mic → text → asked as a question). Qwen3-ASR-style:
        # POST <asr_url> multipart `file`, returns {"text": ...}. For a cloud
        # OpenAI-compatible ASR set asr_api_key (Bearer) + asr_model (form field).
        "asr_url": "",
        "asr_timeout": 30.0,
        "asr_run_vllm": False,
        "asr_api_key": "",
        "asr_model": "",
        # Environment-audio ASR: own endpoint/key/model triple (env_url/
        # env_api_key/env_asr_model); all empty → fall back to the asr_* service.
        # Backend: "dashscope"/"qwen" (single segment) or "whisperx"
        # (multi-speaker diarize). DashScope uses compatible-mode input_audio.
        ...
        # DashScope (Qwen) realtime speech: streaming mic ASR + streaming TTS.
        # dashscope_api_key is a per-account Bailian key (not bundled); blank →
        # realtime ASR/TTS silently disabled. ASR + TTS share this one key.
        "dashscope_api_key": "",
        "realtime_asr_enabled": True,
        "realtime_asr_model": "qwen3-asr-flash-realtime",
        "realtime_asr_vad_threshold": 0.5,
        "realtime_asr_vad_silence_ms": 1200,
        "realtime_tts_enabled": True,
        "realtime_tts_model": "qwen3-tts-flash-realtime",
        "realtime_tts_voice": "Cherry",
        # TTS playback speed. 1.0 sounds slow/robotic; 1.3 ≈ natural pace.
        "realtime_tts_speech_rate": 1.3,
        "whisperx_timeout": 60.0,
        "whisperx_language": "zh",
        "whisperx_diarize": True,
        "whisperx_min_speakers": 0,   # 0 = unbounded
        "whisperx_max_speakers": 0,   # 0 = unbounded
        "conv_max_audio_obs": 300,    # cap audio_observation turns in memory log
    },
    ...
    # response. If not, Argus feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Argus auto-pauses the goal and
    ...
    # always goes to ~/.argus/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${ARGUS_SKILL_DIR} and ${ARGUS_SESSION_ID} in SKILL.md
    ...
    "curator": {
        ...
        # normally restored on every `argus update`, so pruning them only
        ...
        # skipped), snapshot ~/.argus/skills/ into
        # ~/.argus/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `argus curator rollback`.
        "backup": {
            "enabled": True,
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Argus*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (ARGUS_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Argus' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.argus/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or ARGUS_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `argus doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``hermes_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Argus to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via ARGUS_CRON_MAX_PARALLEL env var.
    ...
    "kanban": {
        ...
        # decomposition is manual via `argus kanban decompose <id>` or
    ...
    #
    # Core Argus tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.argus/logs/.
    ...
    "model_catalog": {
        "enabled": True,
        "url": "https://mmargus-team.github.io/Argus/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `argus model` invocation; network failures
    ...
    "gateway": {
        ...
        # the ARGUS_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.argus/.env,
        ...
        # behavior: files must live under the Argus cache, under
        ...
        # user. Bridged to ARGUS_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Argus
        # cache (~/.argus/cache/{documents,images,audio,video,screenshots})
        ...
        # to ARGUS_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # ARGUS_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to ARGUS_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.argus/state.db.
    ...
    "sessions": {
        ...
        # rewrites ``~/.argus/sessions/session_{sid}.json`` on every turn
    ...
    # ``argus update`` behaviour.
    "updates": {
        # Run a full ``hermes backup``-style zip of HERMES_HOME before every
        # ``argus update``.  Backups land in ``<HERMES_HOME>/backups/`` and
        # can be restored with ``hermes import <path>``.  Off by default:
        # zipping a large HERMES_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``argus update
        ...
        "backup_keep": 5,
        # What `argus update` does with uncommitted local changes to the
    ...
    # `argus tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.argus/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.argus/.env (or your shell) and never in config.yaml.
            ...
            # ~/.argus/bin/ on first use.  When False you must install
    ...
    "computer_use": {
        ...
        # by default upstream. Argus disables it for our users unless they
        # explicitly opt in here. When false (default), Argus sets
    ...
    # v32: model.{monitor,watcher,memory[.recall]} endpoints + multimodal→settings
    #      (audio interface split into a new `audio:` section).
    "_config_version": 32,
}
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'argus model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Argus auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Argus, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ARGUS_BOT)",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'argus' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Argus API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Argus API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Argus API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # ARGUS_TOOL_PROGRESS and ARGUS_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # should never break `argus update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Argus versions) write ``models`` as
...
def normalize_bare_custom_endpoint(config: Dict[str, Any]) -> Optional[str]:
    """Give a bare ``model.provider: custom`` endpoint a durable
    ``custom_providers`` row. Mutates *config* in place; returns the added
    entry's name, or None when there was nothing to do.

    Why this exists — the ``custom`` provider has TWO on-disk shapes, and only
    one of them survives a provider switch:

    1. ``custom_providers:`` list — a real registry row. This is what the CLI
       custom-endpoint flow writes (``main._save_custom_provider``), and it gets
       a stable ``custom:<host>`` provider slug.
    2. bare ``model: {provider: custom, base_url: ...}`` — what a hand-written
       config.yaml naturally looks like. There is NO registry row; the picker
       fabricates one on the fly from ``model.base_url``
       (``model_switch._bare_custom_provider_def``, which returns None the
       moment that URL is empty).

    Shape 2 was a one-way door: switching the main model to any other provider
    correctly clears the now-stale ``model.base_url`` (it belonged to the old
    provider), which also destroys the only record of the endpoint — so its
    picker row dropped to zero models and the user could never switch back.
    Callers had to remember to archive it first, and only the GUI's
    ``/api/model/set`` did (TUI ``config.set`` and the CLI paths did not).

    Normalizing at startup collapses the two shapes into shape 1 *before*
    anything can switch away, so no switch path needs to know about this at
    all. Idempotent: dedups by base_url (trailing-slash insensitive), so
    re-running on an already-normalized config is a no-op. That matters because
    ``sync_project_config`` re-copies the project config.yaml — which carries
    shape 2 — on every startup.
    """
    model_cfg = config.get("model")
    if not isinstance(model_cfg, dict):
        return None
    if str(model_cfg.get("provider") or "").strip().lower() not in {"custom", "local"}:
        return None
    ...
    for entry in providers:
        if (
            ...
            and str(entry.get("base_url") or "").strip().rstrip("/") == target
        ):
            return None  # already registered

    entry: Dict[str, Any] = {
        "name": _auto_custom_endpoint_name(base_url),
        "base_url": base_url,
    }
...
def _auto_custom_endpoint_name(base_url: str) -> str:
    """Display label for a normalized custom endpoint.

    Mirrors ``main._auto_provider_name`` so a normalized row is labelled exactly
    like one the CLI custom-endpoint flow creates (the provider slug is derived
    from the URL host, not this label, so the two stay interchangeable). Kept
    local rather than imported: ``main`` is a heavy import and this runs during
    early startup.
    """
    import re
    ...
    return name.capitalize() or "Custom endpoint"
...
_KNOWN_ROOT_KEYS = {
    ...
    # Multimodal subsystem: settings (behavior knobs) + audio (speech interface).
    # ``multimodal`` retained for pre-v32 flat configs (flatten_mm_config falls
    # back to it). ``skills`` is a valid section too.
    "settings", "audio", "multimodal", "skills",
}
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'argus setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Argus won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # ── Vision-capability checks for vision-REQUIRED roles ───────────────
    issues.extend(_validate_vision_capability(config))

    return issues


# Roles that MUST run on a vision-capable model (they process video frames /
# images). Auxiliary roles (compression, text memory extraction, title, …) are
# intentionally EXCLUDED — they never see images.
_VISION_REQUIRED_HINT = (
    "Switch this role to a vision-capable model, or confirm the model really "
    "accepts image input. If it doesn't, the system won't send images and the "
    "result is undefined."
)


def _validate_vision_capability(config: Dict[str, Any]) -> List["ConfigIssue"]:
    """Warn when a vision-REQUIRED role points at a known text-only model, or
    when ``model.supports_vision: true`` contradicts a known text-only model.

    Only WARN, never block — the user asked for a prompt without forced
    intervention. Unknown models (classify_vision → None) are left silent to
    avoid false alarms on every model switch. Roles whose dedicated
    ``*_base_url`` is empty follow the main agent and are not re-checked here.
    """
    try:
    ...
    main_base_url = str(model_cfg.get("base_url") or "").strip()
    declared = _coerce_bool(model_cfg.get("supports_vision"))
    if main_model:
        cap = classify_vision(main_provider, main_model, main_base_url)
        if cap is False:
            if declared is True:
                out.append(ConfigIssue(
                    "warning",
                    f"model.supports_vision is true but '{main_model}' is a known "
                    f"text-only model — this looks reversed. The main agent needs a "
                    f"vision model for multimodal features.",
                    _VISION_REQUIRED_HINT,
                ))
            else:
                out.append(ConfigIssue(
                    "warning",
                    f"Main agent model '{main_model}' is a known text-only model; "
                    f"multimodal features (live frames, image questions) won't work.",
                    _VISION_REQUIRED_HINT,
                ))

    # ── Multimodal roles (watcher / monitor / memory) ───────────────────
    # Read via the shared flatten so the new nested layout
    # (model.watcher/monitor/memory) and the legacy flat multimodal.* both
    # resolve to the same flat provider/base_url/model keys checked below.
    try:
        from agent.multimodal.hermes_glue import flatten_mm_config
        mm = flatten_mm_config(config or {})
    except Exception:
        mm = config.get("multimodal")
        mm = mm if isinstance(mm, dict) else {}
    # (label, provider_key, model_key, base_url_key). memory uses memory_model
    # and provider "gemini" points at a Google generateContent endpoint whose
    # gemini-* models are all vision-capable. watcher maps to the watcher_* keys
    # (v33 rename from worker_*; watcher_model holds the deep-research model pin).
    mm_roles = (
        ("multimodal deep-research watcher", "watcher_provider", "watcher_model", "watcher_base_url"),
        ("multimodal monitor", "monitor_provider", "monitor_model", "monitor_base_url"),
        ("multimodal memory writer/reviewer", "memory_provider", "memory_model", "memory_base_url"),
    )
    for label, pkey, mkey, ukey in mm_roles:
        base_url = str(mm.get(ukey) or "").strip()
        provider = str(mm.get(pkey) or "").strip()
        model = str(mm.get(mkey) or "").strip()
        # Empty base_url → this role follows the main agent (checked above).
        # Exception: memory_provider=gemini uses memory_base_url; if that's
        # empty it can't run as gemini anyway (a separate error path handles
        # that), so only check when we have a concrete model to classify.
        if not base_url:
            continue
        if not model:
            continue
        cap = classify_vision(provider, model, base_url)
        if cap is False:
            out.append(ConfigIssue(
                "warning",
                f"{label} model '{model}' is a known text-only model; this role "
                f"requires a vision-capable model.",
                _VISION_REQUIRED_HINT,
            ))
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'argus doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("ARGUS_HOME", "~/.argus")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("ARGUS_TOOL_PROGRESS")
            old_mode = get_env_value("ARGUS_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from ARGUS_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from ARGUS_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("ARGUS_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from ARGUS_TIMEZONE)")
            else:
                config["timezone"] = ""
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `argus plugins enable <name>` to activate."
                    )
    ...
    #   3. Creates `~/.argus/logs/curator/` if missing (belt-and-suspenders
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: argus config set <key> <value>")
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``argus config set``
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.argus/config.yaml as-is, without merging defaults or migrating.
...
#   openai-codex (OAuth — argus auth) — OpenAI Codex
#   nous         (OAuth — argus auth) — Nous Portal
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Argus writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.argus/.env.
    ...
    interactive menu render (`argus tools`, `argus setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `argus tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Argus env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.argus/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.argus/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.argus/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.argus/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Argus — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Argus vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.argus/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Argus Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Warn on stale ARGUS_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("ARGUS_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale ARGUS_MAX_ITERATIONS={_env_ghost} "
                f"(run 'argus doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  argus config edit     # Edit config file", Colors.DIM))
    print(color("  argus config set <key> <value>", Colors.DIM))
    print(color("  argus setup           # Run setup wizard", Colors.DIM))
    print()
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `argus config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `argus config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: argus config set <key> <value>")
            print()
            print("Examples:")
            print("  argus config set model anthropic/claude-sonnet-4")
            print("  argus config set terminal.backend docker")
            print("  argus config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'argus config migrate' to add them")
        ...
        print("  argus config           Show current configuration")
        print("  argus config edit      Open config in editor")
        print("  argus config set <key> <value>   Set a config value")
        print("  argus config check     Check for missing/outdated config")
        print("  argus config migrate   Update config with new options")
        print("  argus config path      Show config file path")
        print("  argus config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``argus config`` UI so users
