"""``hermes logs`` — view and filter Argus log files.
...
def tail_log(
    ...
    *,
    ...
    if not log_path.exists():
        ...
        print(f"(Logs are created when Argus runs — try 'argus chat' first)")
        sys.exit(1)
...
def list_logs() -> None:
    ...
    if not found:
        print("  (no log files yet — run 'argus chat' to generate logs)")
