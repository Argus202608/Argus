...
``argus gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``argus gateway start/stop/restart``).
    """
    ...
    # `argus gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    detection always fails. Since every Argus runtime call inside the
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    hermes_home = Path(os.environ.get("ARGUS_HOME", "/opt/data"))
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    (the ``hermes`` user — UID 10000 — running every Argus runtime
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        try:
            ...
            # on one code path.
            pass
        # chown may clear setuid/setgid bits (notably on macOS, and on some
        # Linux filesystems), so apply the final mode after ownership.
        path.chmod(mode)
...
class GatewayNotRegisteredError(S6Error):
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`argus profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        Special case: ``profile == "default"`` emits ``argus gateway
        ...
        # gateway/run.py) so s6 always wins. The ARGUS_S6_SUPERVISED_CHILD
        ...
        if profile == "default":
            gateway_cmd = "argus gateway run --replace"
        else:
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``argus gateway
