...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Argus. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Argus deployment — users can tweak them in the Slack UI after
    pasting.
    ...
    if include_assistant:
        features["assistant_view"] = {
            "assistant_description": "Chat with Argus in threads and DMs.",
        }
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your Argus agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``hermes_cli/main.py``):
      ...
      --name NAME     Override the bot display name (default: "Argus")
    ...
    """
    name = getattr(args, "name", None) or "Argus"
    description = getattr(args, "description", None) or "Your Argus agent on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("ARGUS_HOME") or str(Path.home() / ".argus")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Argus app\n"
