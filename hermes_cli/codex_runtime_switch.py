...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Argus'
...
def apply(
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Argus' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Argus for
        ...
        try:
            ...
            # Permissions + Argus tool callback are always-on production
            ...
            if "hermes-tools" in mig_report.migrated:
                msg_lines.append(
                    "Argus tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default Argus runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "Argus tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default Argus runtime.")
