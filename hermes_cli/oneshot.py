...
def _run_agent(
    ...
    if image_paths:
        try:
            ...
            content, skipped = build_native_content_parts(prompt, image_paths)
            if skipped:
                logging.debug("oneshot image attachments skipped: %s", skipped)
            result = agent.run_conversation(
                content, persist_user_message=prompt)
            return result.get("final_response") or ""
        except Exception as exc:
            logging.debug("oneshot image attachment path failed: %s", exc)
