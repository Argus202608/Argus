"""Slash command definitions and autocomplete for the Argus CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Argus config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Argus works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Argus is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.argus/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Argus to the latest version", "Info"),
    CommandDef("version", "Show Argus version", "Info", aliases=("v",)),
...
# Telegram allows up to 100 BotCommands. Argus ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Argus
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    entries.append(("hermes", "Talk to Argus or run a subcommand", "[subcommand] [args]"))
    seen.add("hermes")
