"""
Baked-in build metadata for Argus.
...
To make ``argus dump`` and the startup banner identify the exact commit the
image was built from, the Docker build writes the build-time ``$ARGUS_GIT_SHA``
arg into ``<project_root>/.argus_build_sha``.  This module is the single
...
- Returns ``None`` when the file is absent.  Source installs and dev images
  built without the ``ARGUS_GIT_SHA`` build-arg fall through to live-git
...
def get_build_sha(short: int = 8) -> Optional[str]:
    ...
    Reads ``<project_root>/.argus_build_sha`` if present.  The file is
    written by the Dockerfile's ``ARGUS_GIT_SHA`` build-arg and contains
