"""
MCP Server Management CLI — ``argus mcp`` subcommand.

Implements ``argus mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.argus/config.yaml under the ``mcp_servers`` key.
"""
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.argus/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``argus mcp login`` to distinguish a genuine authentication
    ...
    try:
        ...
        return ArgusTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  argus mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  argus mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  argus mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: argus mcp test " + name)
        return
...
def cmd_mcp_remove(args):
    ...
    # earlier `argus mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  argus mcp add <name> --url <endpoint>')
        _info('  argus mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    landed before reporting success. Shared by ``argus mcp login`` and
    ``argus mcp reauth`` so both behave identically for a single server.
    """
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `argus mcp remove` + `argus mcp add` to reconfigure auth.")
        return False
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `argus mcp login " + name + "`.")
            return False
        if tools:
...
def cmd_mcp_reauth(args):
    ...
    ``argus mcp reauth <name>`` re-auths a single server (same as ``login``).
    ``argus mcp reauth --all`` discovers every ``auth: oauth`` server in
    ...
    if not name:
        ...
        _info("Usage: argus mcp reauth <name>   |   argus mcp reauth --all")
        return
    if name not in servers:
        _error(f"Server '{name}' not found in config.")
        if servers:
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'argus mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``argus mcp`` subcommands."""
    ...
    if handler:
        ...
        _info("argus mcp                                    Open the catalog picker (default)")
        _info("argus mcp catalog                            List Nous-approved MCPs")
        _info("argus mcp install <name>                     Install a catalog MCP")
        _info("argus mcp serve                              Run as MCP server")
        _info("argus mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("argus mcp add <name> --command <cmd>         Add a stdio server")
        _info("argus mcp add <name> --preset <preset>       Add from a known preset")
        _info("argus mcp remove <name>                      Remove a server")
        _info("argus mcp list                               List configured servers")
        _info("argus mcp test <name>                        Test connection")
        _info("argus mcp configure <name>                   Toggle tools")
        _info("argus mcp login <name>                       Re-authenticate OAuth")
        _info("argus mcp reauth <name> | --all              Re-auth one or all OAuth servers")
        print()
