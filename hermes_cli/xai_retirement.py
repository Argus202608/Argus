...
Pure logic: walks a Argus config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `argus doctor` and a future `hermes migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Argus config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Argus config and return retirement issues.
