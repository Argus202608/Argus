...
try:
    ...
    # running `argus dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["ARGUS_WEB_DIST"]) if "ARGUS_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``argus gateway run`` — but the
    desktop app spawns a ``argus dashboard`` backend, not a gateway, so a cron
...
async def _lifespan(app: "FastAPI"):
    ...
    # loop is not blocked and ARGUS_DASHBOARD_READY fires without delay.
    ...
    # Desktop-spawned backends (ARGUS_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `hermes
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("ARGUS_DESKTOP") == "1":
...
app = FastAPI(title="Argus", version=__version__, lifespan=_lifespan)
...
# NOTE: The legacy multimodal subsystem (/api/multimodal/* + DualAgent WS) was
# removed in phase 9. The mainline /multimodal page now routes through the main
# Argus agent (RouterEngine + MemoryBackend, registered via tools/delegate_tool
# and started inside _make_agent in tui_gateway/server.py).

# ---------------------------------------------------------------------------
# Session token for protecting sensitive endpoints (reveal).
# The desktop shell mints the token and injects it via
# ARGUS_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("ARGUS_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
_SESSION_HEADER_NAME = "X-Argus-Session-Token"
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Argus-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Argus (no terminal prompt), "
...
class ModelAssignment(BaseModel):
    ...
    # ``argus model`` custom flow collects. Honored only on the main slot for
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Argus provider slug — fine.
    ...
    1. Vendor-name → Argus-provider mapping: when the provider string is
       not a known Argus provider/alias (e.g. ``moonshotai``, ``x-ai`` is
...
_MANAGED_FILES_ROOT_ENV = "ARGUS_DASHBOARD_FILES_ROOT"
...
def _default_hermes_root_is_opt_data() -> bool:
    raw = os.environ.get("ARGUS_HOME", "").strip()
    if not raw:
        return False
    try:
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``argus update``.
    ...
    a container — e.g. the hermes-webui image sharing the Argus source tree),
    the dashboard's ``argus update`` button is the correct update path and
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Argus root is actually /opt/data
    # (the container/hosted layout) or when ARGUS_DASHBOARD_FILES_ROOT is set.
    if _default_hermes_root_is_opt_data():
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        # and which providers are registered so ``argus status`` and the
...
# streamed to a per-action log file under ``~/.argus/logs/<action>.log`` so
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "ARGUS_NONINTERACTIVE": "1"},
    }
    if sys.platform == "win32":
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``argus gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``argus gateway restart`` children race each other on the
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``argus gateway restart`` in the background."""
    try:
...
async def gateway_drain(request: Request):
    ...
    active (no ``ARGUS_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
...
async def update_hermes():
    """Kick off ``argus update`` in the background."""
    if _dashboard_local_update_managed_externally():
        message = (
            "Argus updates are managed outside this dashboard in "
            "containerized environments. The built-in local updater is "
            "disabled here."
        )
        _record_completed_action("argus-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "argus-update",
    ...
    try:
        proc = _spawn_hermes_action(["update"], "argus-update")
    except Exception as exc:
        _log.exception("Failed to spawn argus update")
...
async def check_hermes_update(force: bool = False):
    """Report whether a Argus update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/hermes/update`` actually runs ``argus update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed Argus version string
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.argus/config.yaml`` under ``tts.``.
    """
...
def get_profiles_sessions(
    ...
    NOTE: `def` (not `async def`) — the SQLite calls below are synchronous and
    would block the entire event loop if declared async. FastAPI runs sync
    endpoints in a threadpool, so a slow query here no longer starves other
    REST handlers (was the cause of cascading /api/... 15s timeouts).
    """
    if archived not in ("exclude", "only", "include"):
...
async def search_sessions(q: str = "", limit: int = 20, profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Argus surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Argus supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
async def get_config(profile: Optional[str] = None):
    # Config-only scope (contextvar, no global RLock): this handler only reads
    # config, never the skills module globals, so it must not contend on
    # _SKILLS_PROFILE_LOCK — that lock is held across whole handler bodies
    # elsewhere and would serialize this hot endpoint under the desktop app's
    # concurrent load. For the default/current profile it is a pure no-op.
...
def get_model_info(profile: Optional[str] = None):
    ...
    """
    try:
        # Config-only scope (no global RLock) — see /api/config rationale.
        with _config_profile_scope(profile):
            cfg = load_config()
...
def get_model_options(profile: Optional[str] = None, refresh: bool = False):
    ...
    try:
        ...
        # onboarding picker) sees the SAME full provider universe `argus model`
        ...
        # Config/model inventory resolves paths through get_hermes_home() at
        # call time and never touches the import-time skills directory globals.
        # Avoid holding _SKILLS_PROFILE_LOCK across live catalog discovery: a
        # slow provider must not serialize unrelated skills/profile requests.
        with _config_profile_scope(profile):
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `argus model` does so GUI onboarding lands on a
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.argus/config.yaml`` — applies to **new** sessions only.
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # endpoint, mirroring the ``argus model`` custom flow
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``argus model`` universe). This is what lets the desktop Keys tab
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `argus model` provider — would be
...
async def get_env_vars(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict) -> dict:
        ...
        return {
            ...
            # CLI `argus model` picker uses (not desktop-only prefix guesses).
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Argus from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Argus to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Argus from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Argus to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Argus in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Argus through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Argus via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Argus through an IMAP/SMTP mailbox.",
        "docs_url": "https://mmargus-team.github.io/Argus/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Argus to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Argus inside Feishu / Lark.",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect Argus to Google Chat via Cloud Pub/Sub.",
        "docs_url": "https://mmargus-team.github.io/Argus/docs/user-guide/messaging/google_chat",
    },
    "wecom": {
    ...
    "weixin": {
        ...
        "docs_url": "https://mmargus-team.github.io/Argus/docs/user-guide/messaging/weixin/",
        "env_vars": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN", "WEIXIN_BASE_URL"),
        "required_env": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN"),
    },
    "bluebubbles": {
        "name": "BlueBubbles (iMessage)",
        "description": "Use Argus through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Argus to a QQ Bot from the QQ Open Platform.",
    ...
    "teams": {
        "docs_url": "https://mmargus-team.github.io/Argus/docs/user-guide/messaging/teams",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Argus to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Argus as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://mmargus-team.github.io/Argus/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://mmargus-team.github.io/Argus/docs/user-guide/messaging/webhooks/",
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in argus gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in argus gateway setup",
...
_TELEGRAM_ONBOARDING_USER_AGENT = f"ArgusDashboard/{__version__}"
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Argus").strip() or "Argus"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Argus appears
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.argus/.anthropic_oauth.json`` — Argus-managed PKCE flow (what
    ...
    if hermes_creds and hermes_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "hermes_pkce",
            "source_label": f"Argus PKCE ({_HERMES_OAUTH_FILE})",
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Argus even
    when they also have a separate Argus-managed PKCE login.
    """
    try:
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    Argus never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "argus auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "argus auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "argus auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "argus auth add minimax-oauth",
    ...
    },
    {
        ...
        "flow": "loopback",
        "cli_command": "argus auth add xai-oauth",
        "docs_url": "https://mmargus-team.github.io/Argus/docs/guides/xai-grok-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "argus auth add anthropic",
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Argus, so the disconnect
    ...
    user sees exactly what executes, and Argus then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Argus — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``argus model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `argus model` universe (zero-edit for new plugins).
    try:
        from hermes_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"argus auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    sync with the `argus model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        # Anthropic clears only the Argus-managed PKCE file and auth-store entry.
...
#          → persists to ~/.argus/.anthropic_oauth.json AND credential pool
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Argus file AND credential pool.
    ...
    for _endpoint in _ANTHROPIC_OAUTH_TOKEN_URLS:
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "argus-dashboard/1.0",
            },
            method="POST",
        )
        try:
...
async def _start_device_code_flow(
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("ARGUS_PORTAL_BASE_URL")
...
def _xai_loopback_worker(session_id: str) -> None:
    ...
    try:
        ...
        base_url = hauth._xai_validate_inference_base_url(
            os.getenv("ARGUS_XAI_BASE_URL", "").strip().rstrip("/")
...
# ---------------------------------------------------------------------------
# Multimodal memory debug endpoints
# ---------------------------------------------------------------------------


_MM_DEBUG_TABLES = [
    "micro_events", "macro_events", "super_events", "entities", "entity_event",
    "entity_frame", "edges", "entity_states", "revision_log", "entity_quotes",
    "entity_rep_frames", "frame_embeddings", "memory_frames", "audio_observations",
    "screen_texts", "screen_tables",
    "task_states",
]


def _mm_debug_root() -> Path:
    return get_hermes_home() / "memories" / "multimodal"


def _mm_debug_db_files() -> List[Path]:
    root = _mm_debug_root()
    if not root.exists():
        return []
    return sorted(
        [p for p in root.glob("*.sqlite") if p.is_file()],
        key=lambda p: p.stat().st_mtime,
        reverse=True,
    )


def _mm_debug_resolve_db(db_name: str) -> Path:
    name = Path(str(db_name or "")).name
    if not name.endswith(".sqlite"):
        name += ".sqlite"
    root = _mm_debug_root().resolve()
    path = (root / name).resolve()
    if root not in path.parents or not path.exists() or path.suffix != ".sqlite":
        raise HTTPException(status_code=404, detail="Memory DB not found")
    return path


def _mm_debug_connect(path: Path) -> sqlite3.Connection:
    # Inspector endpoints are strictly read-only. URI mode avoids SQLite trying
    # to create journal files beside an active memory DB and makes that contract
    # explicit (also works for profile homes mounted read-only).
    c = sqlite3.connect(
...
def _mm_debug_json(raw: Any, default: Any) -> Any:
    if raw is None:
        return default
...
def _mm_debug_table_exists(c: sqlite3.Connection, table: str) -> bool:
    row = c.execute(
...
def _mm_debug_column_exists(
    c: sqlite3.Connection, table: str, column: str,
) -> bool:
    if not _mm_debug_table_exists(c, table):
        return False
    try:
        return any(
            str(r["name"]) == column
...
def _mm_debug_count(c: sqlite3.Connection, table: str) -> int:
    if not _mm_debug_table_exists(c, table):
        return 0
    try:
        return int(c.execute(f"SELECT COUNT(*) FROM {table}").fetchone()[0] or 0)
    except Exception:
        return 0
...
def _mm_debug_rows(
    ...
    *,
    ...
    """Read a safe projection from both current and legacy memory schemas."""
    if not _mm_debug_table_exists(c, table):
        return []
    try:
        present = {
        ...
        rows = c.execute(
            f"SELECT {','.join(selected)} FROM {table}{order} LIMIT ?",
...
def _mm_debug_meta(c: sqlite3.Connection) -> Dict[str, str]:
    if not _mm_debug_table_exists(c, "meta"):
        return {}
    try:
...
def _mm_debug_frame_b64(db_path: Path, frame_id: str, *, thumb: bool = True) -> str:
    fid = str(frame_id or "").strip()
    if not re.fullmatch(r"f_[0-9a-f]{10}", fid):
        return ""
    frames_dir = _mm_debug_root() / "frames" / db_path.stem
    names = [f"{fid}_thumb.jpg", f"{fid}.jpg"] if thumb else [f"{fid}.jpg", f"{fid}_thumb.jpg"]
    for name in names:
        p = frames_dir / name
        try:
...
def _mm_debug_preview(text: Any, limit: int = 260) -> str:
    s = re.sub(r"\s+", " ", str(text or "")).strip()
    return s[:limit] + ("..." if len(s) > limit else "")


def _mm_debug_terms(query: str) -> List[str]:
    q = str(query or "").strip().lower()
    if not q:
        return []
    terms = [t for t in re.findall(r"[\w\u4e00-\u9fff.#%+-]+", q) if len(t) >= 2]
    refs = re.findall(r"(?:table|figure|fig)\s*\.?\s*\d+|[表图]\s*\d+", q, re.I)
    for ref in refs:
        ref_l = re.sub(r"\s+", " ", ref.lower()).strip()
        if ref_l and ref_l not in terms:
            terms.append(ref_l)
        compact = re.sub(r"[^a-z0-9\u4e00-\u9fff]+", "", ref_l)
        if compact and compact not in terms:
            terms.append(compact)
    return terms[:24]


def _mm_debug_score(query: str, haystack: str) -> int:
    terms = _mm_debug_terms(query)
    if not terms:
        return 1
    hay = str(haystack or "").lower()
    hay_norm = re.sub(r"[^a-z0-9\u4e00-\u9fff]+", "", hay)
    score = 0
    for term in terms:
        norm = re.sub(r"[^a-z0-9\u4e00-\u9fff]+", "", term.lower())
...
def _mm_debug_snippet(text: str, query: str, limit: int = 700) -> str:
    raw = str(text or "").strip()
    if not raw:
        return ""
    low = raw.lower()
    best = -1
    for term in _mm_debug_terms(query):
        idx = low.find(term.lower())
        if idx >= 0 and (best < 0 or idx < best):
            best = idx
        norm = re.sub(r"[^a-z0-9\u4e00-\u9fff]+", "", term.lower())
        if best < 0 and norm:
            compact = re.sub(r"[^a-z0-9\u4e00-\u9fff]+", "", low)
            nidx = compact.find(norm)
            if nidx >= 0:
                best = max(0, min(len(raw) - 1, nidx))
    if best < 0:
        return raw[:limit] + ("..." if len(raw) > limit else "")
    half = max(80, limit // 2)
    start = max(0, best - half)
    end = min(len(raw), best + half)
    while start > 0 and raw[start] not in "\n。.!?":
        start -= 1
    while end < len(raw) and raw[end - 1] not in "\n。.!?":
        end += 1
        if end - start > limit + 120:
            break
    snippet = raw[start:end].strip()
    return ("..." if start > 0 else "") + snippet + ("..." if end < len(raw) else "")


def _mm_debug_table_payload(r: sqlite3.Row, *, query: str = "") -> Dict[str, Any]:
    rows = _mm_debug_json(r["rows_json"], [])
    columns = _mm_debug_json(r["columns_json"], [])
    row_hits = []
    for idx, item in enumerate(rows if isinstance(rows, list) else []):
        hay = json.dumps(item, ensure_ascii=False) if not isinstance(item, str) else item
        if not query or _mm_debug_score(query, hay) > 0:
            row_hits.append({"index": idx, "row": item})
        if len(row_hits) >= 30:
            break
    if not row_hits and isinstance(rows, list):
        row_hits = [{"index": idx, "row": item} for idx, item in enumerate(rows[:12])]
    return {
        ...
        "snippet": _mm_debug_snippet(r["raw_text"] or json.dumps(rows, ensure_ascii=False), query),
        "source": r["source"] or "",
        "confidence": float(r["confidence"] or 0.0),
    }


def _mm_debug_db_summary(path: Path) -> Dict[str, Any]:
    st = path.stat()
    out: Dict[str, Any] = {
        "name": path.name,
        "stem": path.stem,
    ...
    try:
        with _mm_debug_connect(path) as c:
            out["meta"] = _mm_debug_meta(c)
            out["counts"] = {t: _mm_debug_count(c, t) for t in _MM_DEBUG_TABLES}
            if _mm_debug_table_exists(c, "memory_frames"):
                r = c.execute("SELECT MAX(t_observed) AS t FROM memory_frames").fetchone()
                out["latest_observed_ts"] = float(r["t"] or 0.0) if r else 0.0
            elif _mm_debug_table_exists(c, "screen_texts"):
                r = c.execute("SELECT MAX(t_observed) AS t FROM screen_texts").fetchone()
                out["latest_observed_ts"] = float(r["t"] or 0.0) if r else 0.0
    except Exception as e:
        out["error"] = str(e)
    return out
...
def _mm_debug_log_lines(*, session_id: str = "", db_stem: str = "", limit: int = 200) -> List[str]:
    log_path = get_hermes_home() / "logs" / "agent.log"
    if not log_path.exists():
        return []
    try:
        from hermes_cli.logs import _read_tail
        lines = _read_tail(log_path, 3000)
    except Exception:
        try:
            lines = log_path.read_text(errors="replace").splitlines()[-3000:]
        except Exception:
            return []
    owner_tokens = [session_id.lower().strip(), db_stem.lower().strip()]
    owner_tokens = [token for token in owner_tokens if token]
    diagnostic_needles = [
        "recall", "mm-recall-gate", "search_screen_text", "screen_table",
        "screen_tables", "writer", "reviewer", "ocr", "json 解析", "json parse",
        "timeout", "too large",
    ]

    def _has_exact_token(line: str, token: str) -> bool:
        # Session ids commonly share prefixes (root vs continuation/sibling).
        # Treat letters, digits, underscore, and hyphen as token characters so
        # asking for ``turn-A`` cannot match ``turn-A-sibling``.
        return re.search(
    ...
    if owner_tokens:
        # With an explicit session/DB scope, ownership is mandatory.  Generic
        # words such as "writer" or "ocr" occur in every multimodal session and
        # must never pull another conversation's logs into this inspector.
        out = [
            line for line in lines
            if any(_has_exact_token(line, token) for token in owner_tokens)
        ]
    else:
        out = [
            line for line in lines
            if any(needle in line.lower() for needle in diagnostic_needles)
        ]
    return out[-max(20, min(limit, 500)):]


@app.get("/api/multimodal/memory/debug/sessions")
def mm_debug_sessions(limit: int = 50, profile: Optional[str] = None):
    # Keep every filesystem/SQLite read under the request's context-local
    # profile scope.  This handler is intentionally synchronous so FastAPI
    # dispatches the potentially large inspector scan to its worker pool
    # instead of blocking the dashboard event loop.
    with _config_profile_scope(profile):
        files = _mm_debug_db_files()[:max(1, min(limit, 200))]
        return {
            "root": str(_mm_debug_root()),
            "sessions": [_mm_debug_db_summary(p) for p in files],
        }


def _mm_debug_session_payload(
    db_name: str, session_id: Optional[str] = None, limit: int = 250,
):
    path = _mm_debug_resolve_db(db_name)
    ...
    evolution: Dict[str, List[Dict[str, Any]]] = {
        "entity_states": [], "revisions": [],
    }
    health: Dict[str, Any] = {}
    try:
        with _mm_debug_connect(path) as c:
            counts = {t: _mm_debug_count(c, t) for t in _MM_DEBUG_TABLES}
            health["counts"] = counts

            # The Memory landing view is intentionally semantic rather than a
            # dump of worker ticks: what entities/events exist, and how they
            # changed. Projections tolerate older DBs missing evolve columns.
            entity_rows = _mm_debug_rows(
                c, "entities",
                [
                    "id", "name", "type", "attributes", "aliases",
                    "first_seen", "last_seen", "seen_count",
                    "representative_frame_id", "merged_into", "revised_at",
                    "revision_count",
                ],
                order_by="last_seen DESC", limit=200,
            )
            entity_frame_rows = _mm_debug_rows(
                c, "entity_frame",
                ["entity_id", "frame_id", "micro_id", "t_observed"],
                order_by="t_observed ASC", limit=1000,
            )
            frames_by_entity: Dict[str, List[str]] = {}
            for row in entity_frame_rows:
                eid = str(row.get("entity_id") or "")
                fid = str(row.get("frame_id") or "")
                bucket = frames_by_entity.setdefault(eid, [])
                if fid and fid not in bucket and len(bucket) < 24:
                    bucket.append(fid)
            ...
            for row in entity_rows:
                ...
                if str(row.get("merged_into") or "").strip():
                    # Loser/pruned rows stay visible through evolution/revision
                    # logs, but must not masquerade as current memory entities.
                    continue
                ...
                entities.append({
                    ...
                    "attributes": _mm_debug_json(row.get("attributes"), {}),
                    "aliases": _mm_debug_json(row.get("aliases"), []),
                    ...
                    "representative_frame_id": str(
                        row.get("representative_frame_id") or ""),
                    "frame_ids": frames_by_entity.get(eid, []),
                    "merged_into": str(row.get("merged_into") or ""),
                    "revised_at": float(row.get("revised_at") or 0.0),
                    "revision_count": int(row.get("revision_count") or 0),
                })

            entity_event_rows = _mm_debug_rows(
                c, "entity_event", ["entity_id", "micro_id", "t_observed"],
                order_by="t_observed ASC", limit=2000,
            )
            entities_by_micro: Dict[str, List[str]] = {}
            for row in entity_event_rows:
                mid = str(row.get("micro_id") or "")
                row_eid = str(row.get("entity_id") or "")
                if row_eid not in active_entity_ids:
                    continue
                name = entity_names.get(
                    row_eid,
                    row_eid,
                )
                bucket = entities_by_micro.setdefault(mid, [])
                if name and name not in bucket:
                    bucket.append(name)

            for row in _mm_debug_rows(
                c, "micro_events",
                [
                    "id", "t_start", "t_end", "description", "subject",
                    "object", "action", "macro_id", "facts_keys",
                    "frame_ids", "superseded_by", "revised_at",
                    "revision_count",
                ],
                order_by="t_start ASC", limit=400,
            ):
                ...
                events["micro"].append({
                    ...
                    "facts_keys": _mm_debug_json(row.get("facts_keys"), []),
                    "frame_ids": _mm_debug_json(row.get("frame_ids"), []),
                    "entity_names": entities_by_micro.get(mid, []),
            ...
            for row in _mm_debug_rows(
                c, "macro_events",
                [
                    "id", "t_start", "t_end", "label", "summary",
                    "super_id", "key_entities", "narrative_arc",
                    "entity_arcs", "superseded_by", "revised_at",
                    "revision_count",
                ],
                order_by="t_start ASC", limit=160,
            ):
                events["macro"].append({
                    ...
                    "super_id": str(row.get("super_id") or ""),
                    "key_entities": _mm_debug_json(row.get("key_entities"), []),
                    "narrative_arc": _mm_debug_json(row.get("narrative_arc"), []),
                    "entity_arcs": _mm_debug_json(row.get("entity_arcs"), {}),
                    "superseded_by": str(row.get("superseded_by") or ""),
                    "revised_at": float(row.get("revised_at") or 0.0),
                    "revision_count": int(row.get("revision_count") or 0),
                })
            ...
            for row in _mm_debug_rows(
                c, "super_events",
                [
                    "id", "t_start", "t_end", "label", "description",
                    "macro_ids", "is_root", "narrative_arc",
                    "superseded_by", "revised_at", "revision_count",
                ],
                order_by="t_start ASC", limit=80,
            ):
                events["super"].append({
                    "id": str(row.get("id") or ""),
                    "t_start": float(row.get("t_start") or 0.0),
                    ...
                    "label": str(row.get("label") or ""),
                    "description": str(row.get("description") or ""),
                    "macro_ids": _mm_debug_json(row.get("macro_ids"), []),
                    "is_root": bool(row.get("is_root") or False),
                    "narrative_arc": _mm_debug_json(row.get("narrative_arc"), []),
                    "superseded_by": str(row.get("superseded_by") or ""),
                    "revised_at": float(row.get("revised_at") or 0.0),
                    "revision_count": int(row.get("revision_count") or 0),
            ...
            for row in _mm_debug_rows(
                c, "entity_states",
                [
                    "id", "entity_id", "t_observed", "state_label",
                    "attributes_delta", "new_aliases", "confidence",
                    "evidence_frame_ids", "micro_id", "source", "note",
                ],
                order_by="t_observed ASC", limit=500,
            ):
                eid = str(row.get("entity_id") or "")
                evolution["entity_states"].append({
                    "id": int(row.get("id") or 0),
                    "entity_id": eid,
                    "entity_name": entity_names.get(eid, eid),
                    "t_observed": float(row.get("t_observed") or 0.0),
                    "state_label": str(row.get("state_label") or "updated"),
                    "attributes_delta": _mm_debug_json(
                        row.get("attributes_delta"), {}),
                    "new_aliases": _mm_debug_json(row.get("new_aliases"), []),
                    "confidence": float(row.get("confidence") or 0.0),
                    "evidence_frame_ids": _mm_debug_json(
                        row.get("evidence_frame_ids"), []),
                    "micro_id": str(row.get("micro_id") or ""),
            ...
            for row in _mm_debug_rows(
                c, "revision_log",
                [
                    "id", "t_applied", "reviewer_round", "op", "target_ids",
                    "new_ids", "payload", "reason", "success", "error",
                    "actor",
                ],
                order_by="t_applied ASC", limit=300,
            ):
                evolution["revisions"].append({
                    "id": int(row.get("id") or 0),
                    "t_applied": float(row.get("t_applied") or 0.0),
                    "reviewer_round": int(row.get("reviewer_round") or 0),
                    "op": str(row.get("op") or ""),
                    "target_ids": _mm_debug_json(row.get("target_ids"), []),
                    "new_ids": _mm_debug_json(row.get("new_ids"), []),
                    "payload": _mm_debug_json(row.get("payload"), {}),
                    "reason": str(row.get("reason") or ""),
                    "success": bool(row.get("success", True)),
                    "error": str(row.get("error") or ""),
                    "actor": str(row.get("actor") or "reviewer"),
                })

            table_counts: Dict[str, int] = {}
            if _mm_debug_table_exists(c, "screen_tables"):
                for r in c.execute("SELECT frame_id, COUNT(*) AS n FROM screen_tables GROUP BY frame_id"):
                    table_counts[str(r["frame_id"])] = int(r["n"] or 0)
            if _mm_debug_table_exists(c, "memory_frames"):
                has_screen = _mm_debug_table_exists(c, "screen_texts")
                has_micro = _mm_debug_table_exists(c, "micro_events")
                screen_cols = (
                    "st.app AS app,st.window_title AS window_title,"
                    "st.ocr_blocks AS ocr_blocks,st.raw_text AS raw_text,"
                    "st.source AS ocr_source"
                    if has_screen else
                    "'' AS app,'' AS window_title,'[]' AS ocr_blocks,"
                    "'' AS raw_text,'' AS ocr_source"
                )
                micro_cols = (
                    "me.description AS observation_text"
                    if has_micro else "'' AS observation_text"
                )
                joins = ""
                if has_screen:
                    joins += " LEFT JOIN screen_texts st ON st.frame_id=mf.frame_id"
                if has_micro:
                    joins += " LEFT JOIN micro_events me ON me.id=mf.micro_id"
                rows = c.execute(
                    f"SELECT mf.frame_id,mf.t_observed,mf.wall_ts,mf.micro_id,"
                    f"mf.source_type,mf.note,{screen_cols},{micro_cols} "
                    f"FROM memory_frames mf{joins} "
                    "ORDER BY mf.t_observed DESC LIMIT ?",
                    (max(1, min(limit, 500)),),
                ).fetchall()
                for r in reversed(rows):
                    blocks = _mm_debug_json(r["ocr_blocks"], [])
                    raw = r["raw_text"] or ""
                    observation = r["observation_text"] or ""
                    timeline.append({
                        "kind": "memory_frame",
                        "frame_id": r["frame_id"],
                        "t_observed": float(r["t_observed"] or 0.0),
                        "wall_ts": float(r["wall_ts"] or 0.0),
                        "micro_id": r["micro_id"] or "",
                        "app": r["app"] or "",
                        "window_title": r["window_title"] or "",
                        "source": r["source_type"] or "unknown",
                        "ocr_source": r["ocr_source"] or "",
                        "note": r["note"] or "",
                        "raw_preview": _mm_debug_preview(raw or observation, 360),
                        "observation_preview": _mm_debug_preview(observation, 600),
                        "raw_len": len(raw),
                        "block_count": len(blocks) if isinstance(blocks, list) else 0,
                        "table_count": table_counts.get(str(r["frame_id"]), 0),
                        "thumb_b64": _mm_debug_frame_b64(path, r["frame_id"], thumb=True),
                    })
            elif _mm_debug_table_exists(c, "screen_texts"):
                # Legacy DBs predate the all-scene frame index. Preserve their
                # OCR-only timeline rather than rendering an empty inspector.
                rows = c.execute(
                    "SELECT frame_id,t_observed,app,window_title,ocr_blocks,raw_text,source "
                    "FROM screen_texts ORDER BY t_observed DESC LIMIT ?",
                    (max(1, min(limit, 500)),),
                ).fetchall()
                for r in reversed(rows):
                    blocks = _mm_debug_json(r["ocr_blocks"], [])
                    timeline.append({
                        "kind": "screen_text", "frame_id": r["frame_id"],
                        "t_observed": float(r["t_observed"] or 0.0),
                        "app": r["app"] or "", "window_title": r["window_title"] or "",
                        "source": r["source"] or "", "raw_preview": _mm_debug_preview(r["raw_text"], 360),
                        "raw_len": len(r["raw_text"] or ""),
                        "block_count": len(blocks) if isinstance(blocks, list) else 0,
                        "table_count": table_counts.get(str(r["frame_id"]), 0),
                        "thumb_b64": _mm_debug_frame_b64(path, r["frame_id"], thumb=True),
                    })
            if _mm_debug_table_exists(c, "screen_tables"):
                for r in c.execute(
                    "SELECT * FROM screen_tables ORDER BY t_observed DESC LIMIT 80"
                ).fetchall():
                    tables.append(_mm_debug_table_payload(r))
            if _mm_debug_table_exists(c, "micro_events"):
                empty_frames = c.execute(
                    "SELECT COUNT(*) FROM micro_events WHERE frame_ids IS NULL OR frame_ids='' OR frame_ids='[]'"
                ).fetchone()[0]
                health["micro_events_without_frames"] = int(empty_frames or 0)
            if _mm_debug_table_exists(c, "screen_texts"):
                empty_ocr = c.execute(
                    "SELECT COUNT(*) FROM screen_texts WHERE raw_text IS NULL OR TRIM(raw_text)=''"
                ).fetchone()[0]
                health["screen_texts_without_raw_text"] = int(empty_ocr or 0)
            if _mm_debug_table_exists(c, "memory_frames"):
                by_source = {}
                for r in c.execute(
                    "SELECT COALESCE(NULLIF(source_type,''),'unknown') AS source, "
                    "COUNT(*) AS n FROM memory_frames GROUP BY source"
                ).fetchall():
                    by_source[str(r["source"])] = int(r["n"] or 0)
                health["memory_frames_by_source"] = by_source
                health["memory_frames_without_micro"] = int(c.execute(
                    "SELECT COUNT(*) FROM memory_frames "
                    "WHERE micro_id IS NULL OR TRIM(micro_id)=''"
                ).fetchone()[0] or 0)
            frames_dir = _mm_debug_root() / "frames" / path.stem
            health["frame_files"] = len(list(frames_dir.glob("*.jpg"))) if frames_dir.exists() else 0
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
    logs = _mm_debug_log_lines(session_id=session_id or "", db_stem=path.stem, limit=220)
    health["recent_log_warnings"] = [ln for ln in logs if "warning" in ln.lower() or "error" in ln.lower()][-80:]
    return {
        "session": _mm_debug_db_summary(path),
        "timeline": timeline,
        "memory": {
            "entities": entities,
            "events": events,
...
@app.get("/api/multimodal/memory/debug/session/{db_name}")
def mm_debug_session(
    db_name: str,
    ...
    with _config_profile_scope(profile):
        return _mm_debug_session_payload(db_name, session_id=session_id, limit=limit)


def _mm_debug_frame_payload(db_name: str, frame_id: str):
    path = _mm_debug_resolve_db(db_name)
    fid = str(frame_id or "").strip()
    if not re.fullmatch(r"f_[0-9a-f]{10}", fid):
    ...
    detail: Dict[str, Any] = {
        "frame_id": fid,
        "image_b64": _mm_debug_frame_b64(path, fid, thumb=False),
        "thumb_b64": _mm_debug_frame_b64(path, fid, thumb=True),
        "memory_frame": None,
        "screen_text": None,
        "memory_frame": None,
        "tables": [],
    ...
    try:
        with _mm_debug_connect(path) as c:
            if _mm_debug_table_exists(c, "memory_frames"):
                r = c.execute(
                    "SELECT * FROM memory_frames WHERE frame_id=?", (fid,)
                ).fetchone()
                if r:
                    detail["memory_frame"] = {
                        "frame_id": r["frame_id"],
            ...
            if _mm_debug_table_exists(c, "screen_texts"):
                r = c.execute("SELECT * FROM screen_texts WHERE frame_id=?", (fid,)).fetchone()
                if r:
                    detail["screen_text"] = {
                        "frame_id": r["frame_id"],
                        "t_observed": float(r["t_observed"] or 0.0),
                        "app": r["app"] or "",
                        "window_title": r["window_title"] or "",
                        "ocr_blocks": _mm_debug_json(r["ocr_blocks"], []),
                        "raw_text": r["raw_text"] or "",
                        "source": r["source"] or "",
                        "created_at": float(r["created_at"] or 0.0),
                    }
            if _mm_debug_table_exists(c, "screen_tables"):
                detail["tables"] = [
                    _mm_debug_table_payload(r)
                    for r in c.execute("SELECT * FROM screen_tables WHERE frame_id=? ORDER BY id", (fid,)).fetchall()
                ]
            if _mm_debug_table_exists(c, "micro_events"):
                active_micro_sql = (
                    "AND (superseded_by IS NULL OR superseded_by='') "
                    if _mm_debug_column_exists(
                        c, "micro_events", "superseded_by") else ""
                )
                detail["micro_events"] = [
                    {
                        "id": r["id"], "t_start": float(r["t_start"] or 0.0),
                        "t_end": float(r["t_end"] or 0.0),
                        "description": r["description"] or "",
                        "frame_ids": _mm_debug_json(r["frame_ids"], []),
                    }
                    for r in c.execute(
                        "SELECT id,t_start,t_end,description,frame_ids FROM micro_events "
                        f"WHERE frame_ids LIKE ? {active_micro_sql}"
                        "ORDER BY t_end",
                        (f"%{fid}%",),
                    ).fetchall()
                ]
            if _mm_debug_table_exists(c, "entity_frame") and _mm_debug_table_exists(c, "entities"):
                active_entity_sql = (
                    "AND (e.merged_into IS NULL OR e.merged_into='') "
                    if _mm_debug_column_exists(
                        c, "entities", "merged_into") else ""
                )
                detail["entities"] = [
                    {
                        "entity_id": r["id"], "name": r["name"], "type": r["type"],
                        "attributes": _mm_debug_json(r["attributes"], {}),
                        "t_observed": float(r["t_observed"] or 0.0),
                    }
                    for r in c.execute(
                        "SELECT e.id,e.name,e.type,e.attributes,ef.t_observed "
                        "FROM entity_frame ef JOIN entities e ON e.id=ef.entity_id "
                        f"WHERE ef.frame_id=? {active_entity_sql}"
                        "ORDER BY ef.t_observed",
                        (fid,),
                    ).fetchall()
                ]
    except Exception as e:
...
@app.get("/api/multimodal/memory/debug/session/{db_name}/frame/{frame_id}")
def mm_debug_frame(
    db_name: str, frame_id: str, profile: Optional[str] = None,
):
    with _config_profile_scope(profile):
        return _mm_debug_frame_payload(db_name, frame_id)


def _mm_debug_search_payload(
    query: str,
    scope: str = "all",
    ...
    # A one-character query produces no terms in _mm_debug_terms(), which
    # otherwise scores every inspected row as a match.  Refuse that accidental
    # full-index dump before opening up to 100 SQLite files.
    if len(re.sub(r"\s+", "", q)) < 2:
        return {"query": q, "scope": scope, "results": []}
    max_results = max(1, min(limit, 100))
    files = _mm_debug_db_files()
    if session:
        files = [_mm_debug_resolve_db(session)]
    elif scope == "latest":
        files = files[:1]
    elif scope == "today":
        today = datetime.now().strftime("%Y%m%d")
        files = [p for p in files if p.stem.startswith(today)]
    ranked_results: List[Tuple[int, float, int, Dict[str, Any]]] = []
    result_order = 0

    def _consider(item: Dict[str, Any]) -> None:
        """Retain only the best ``max_results`` lightweight candidates.

        Thumbnails and parsed table rows are deliberately materialized after
        this bounded heap is final.  A broad search can inspect hundreds of
        thousands of rows, but it must never base64-encode a frame or retain a
        full table payload for every transient match.
        """
        nonlocal result_order
        result_order += 1
        ranked = (
            int(item.get("score") or 0),
            float(item.get("t_observed") or 0.0),
            -result_order,
            item,
        )
        if len(ranked_results) < max_results:
            heapq.heappush(ranked_results, ranked)
        elif ranked[:3] > ranked_results[0][:3]:
            heapq.heapreplace(ranked_results, ranked)

    for path in files[:100]:
        try:
            with _mm_debug_connect(path) as c:
                if _mm_debug_table_exists(c, "screen_tables"):
                    for r in c.execute("SELECT * FROM screen_tables ORDER BY t_observed DESC LIMIT 400").fetchall():
                        hay = " ".join([
                            r["table_id"] or "", r["title"] or "", r["raw_text"] or "",
                            r["columns_json"] or "", r["rows_json"] or "",
                        ])
                        score = _mm_debug_score(q, hay)
                        if score <= 0:
                            continue
                        _consider({
                            "session": path.name, "kind": "screen_table",
                            "score": score, "frame_id": r["frame_id"],
                            "t_observed": float(r["t_observed"] or 0.0),
                            "title": r["title"] or r["table_id"] or "table",
                            "snippet": _mm_debug_snippet(r["raw_text"] or "", q),
                            "_table_row": dict(r),
                            "_thumb_path": path,
                        })
                if _mm_debug_table_exists(c, "screen_texts"):
                    for r in c.execute(
                        "SELECT frame_id,t_observed,app,window_title,raw_text,source "
                        "FROM screen_texts ORDER BY t_observed DESC LIMIT 800"
                    ).fetchall():
                        hay = " ".join([r["app"] or "", r["window_title"] or "", r["raw_text"] or ""])
                        score = _mm_debug_score(q, hay)
                        if score <= 0:
                            continue
                        _consider({
                            "session": path.name, "kind": "screen_text",
                            "score": score, "frame_id": r["frame_id"],
                            "t_observed": float(r["t_observed"] or 0.0),
                            "title": r["window_title"] or r["app"] or "OCR text",
                            "snippet": _mm_debug_snippet(r["raw_text"] or "", q),
                            "source": r["source"] or "",
                            "_thumb_path": path,
                        })
                if _mm_debug_table_exists(c, "audio_observations"):
                    for r in c.execute(
                        "SELECT id,t_observed,speaker,text,source "
                        "FROM audio_observations ORDER BY COALESCE(t_observed,0) DESC LIMIT 1000"
                    ).fetchall():
                        score = _mm_debug_score(q, r["text"] or "")
                        if score <= 0:
                            continue
                        _consider({
                            "session": path.name, "kind": "audio_observation",
                            "score": score, "frame_id": "",
                            "t_observed": float(r["t_observed"] or 0.0),
                            "title": (r["speaker"] or "ASR transcript"),
                            "snippet": _mm_debug_snippet(r["text"] or "", q),
                            "source": r["source"] or "asr",
                            "thumb_b64": "",
                        })
                if _mm_debug_table_exists(c, "micro_events"):
                    active_micro_where = (
                    ...
                    for r in c.execute(
                        "SELECT id,t_start,t_end,description,frame_ids FROM micro_events "
                        f"{active_micro_where}"
                        "ORDER BY t_end DESC LIMIT 500"
                    ).fetchall():
                        score = _mm_debug_score(q, r["description"] or "")
                        if score <= 0:
                            continue
                        fids = _mm_debug_json(r["frame_ids"], [])
                        fid0 = fids[0] if isinstance(fids, list) and fids else ""
                        _consider({
                            "session": path.name, "kind": "micro_event",
                            "score": score, "frame_id": fid0,
                            "t_observed": float(r["t_end"] or 0.0),
                            "title": r["id"],
                            "snippet": _mm_debug_snippet(r["description"] or "", q),
                            "frame_ids": fids if isinstance(fids, list) else [],
                            "_thumb_path": path if fid0 else None,
                        })
        except Exception as e:
            _consider({
                "session": path.name, "kind": "error", "score": -1,
                "title": "read failed", "snippet": str(e),
            })

    results = [
        ranked[3]
        for ranked in sorted(ranked_results, key=lambda item: item[:3], reverse=True)
    ]
    for item in results:
        table_row = item.pop("_table_row", None)
        if table_row is not None:
            payload = _mm_debug_table_payload(table_row, query=q)
            item["snippet"] = payload["snippet"]
            item["table"] = payload
        thumb_path = item.pop("_thumb_path", None)
        frame_id = str(item.get("frame_id") or "")
        item["thumb_b64"] = (
            _mm_debug_frame_b64(thumb_path, frame_id, thumb=True)
            if isinstance(thumb_path, Path) and frame_id
            else ""
        )
    return {"query": q, "scope": scope, "results": results}


@app.get("/api/multimodal/memory/debug/search")
def mm_debug_search(
    query: str,
    scope: str = "all",
    session: Optional[str] = None,
    ...
    with _config_profile_scope(profile):
        return _mm_debug_search_payload(
            query, scope=scope, session=session, limit=limit,
        )


def _mm_debug_trace_payload(
    session_id: Optional[str] = None,
    ...
    db_stem = ""
    if db:
        db_stem = _mm_debug_resolve_db(db).stem
    messages: List[Dict[str, Any]] = []
    if session_id:
        state_db = get_hermes_home() / "state.db"
        try:
            with sqlite3.connect(str(state_db), timeout=1.0) as c:
                c.row_factory = sqlite3.Row
                rows = c.execute(
                    "SELECT role,content,tool_name,tool_calls,timestamp,finish_reason "
                    "FROM messages WHERE session_id = ? AND "
                    "(tool_name LIKE '%recall%' OR tool_name LIKE '%memory%' "
                    " OR tool_name = 'query_multimodal' "
                    " OR tool_calls LIKE '%recall%' OR tool_calls LIKE '%query_multimodal%' "
                    " OR content LIKE '%recall_multimodal_memory%' "
                    " OR content LIKE '%search_screen_text%') "
                    "ORDER BY timestamp DESC LIMIT ?",
                    (session_id, max(1, min(limit, 300))),
                ).fetchall()
                messages = [
                    {
                        "role": r["role"], "tool_name": r["tool_name"] or "",
                        "timestamp": float(r["timestamp"] or 0.0),
                        "finish_reason": r["finish_reason"] or "",
                        "content": r["content"] or "",
                        "tool_calls": _mm_debug_json(r["tool_calls"], []),
                    }
                    for r in reversed(rows)
                ]
        except Exception:
    ...
    return {
        ...
        "db": db or "",
        "messages": messages,
        "logs": _mm_debug_log_lines(session_id=session_id or "", db_stem=db_stem, limit=limit),
    }


@app.get("/api/multimodal/memory/debug/trace")
def mm_debug_trace(
    session_id: Optional[str] = None,
    db: Optional[str] = None,
    limit: int = 120,
    ...
    with _config_profile_scope(profile):
        return _mm_debug_trace_payload(session_id=session_id, db=db, limit=limit)
...
# servers managed here show up under `argus mcp list` and vice versa.  Secrets
...
async def list_mcp_catalog(profile: Optional[str] = None):
    ...
    `argus mcp catalog` / `argus mcp install` read.  ``profile`` scopes
...
# spawn the real `argus gateway <verb>` so behaviour matches the CLI exactly.
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `argus memory setup` to configure a new one.",
            )
...
def _profile_setup_command(name: str) -> str:
    ...
    return "argus setup" if name == "default" else f"{name} setup"
...
def _profile_scope(profile: Optional[str]):
    ...
    # NB: the attribute is ARGUS_HOME on both modules. It was renamed from
    # HERMES_HOME in the Argus rename; this reader was missed, so every one of the
    # ~43 endpoints wrapped in _profile_scope raised AttributeError at __enter__
    # and returned a generic 500 (the handlers catch broad `Exception`, which hid
    # the real cause — e.g. GET /api/model/moa reporting "Failed to read MoA
    # config" when the config was fine). Keep these names in sync with
    # tools/skills_tool.py and tools/skill_manager_tool.py.
    with _SKILLS_PROFILE_LOCK:
        old_home = _skills_tool.ARGUS_HOME
        old_skills_dir = _skills_tool.SKILLS_DIR
        old_mgr_home = _skill_mgr.ARGUS_HOME
        old_mgr_skills_dir = _skill_mgr.SKILLS_DIR
        _skills_tool.ARGUS_HOME = profile_dir
        _skills_tool.SKILLS_DIR = profile_dir / "skills"
        _skill_mgr.ARGUS_HOME = profile_dir
        _skill_mgr.SKILLS_DIR = profile_dir / "skills"
        try:
            yield profile_dir if token is not None else None
        finally:
            _skills_tool.ARGUS_HOME = old_home
            _skills_tool.SKILLS_DIR = old_skills_dir
            _skill_mgr.ARGUS_HOME = old_mgr_home
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``argus tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``argus tools`` picker shows
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``argus tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.argus/.env`` via ``save_env_value`` —
    the same store ``argus tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``argus tools post-setup <key>`` and the frontend tails the log via
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Argus,
...
def _resolve_chat_argv(
    ...
    Session resume is propagated via the ``ARGUS_TUI_RESUME`` env var —
    matching what ``hermes_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``ARGUS_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``ARGUS_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``ARGUS_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``ARGUS_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    env.setdefault("ARGUS_TUI_DISABLE_MOUSE", "1")
    env.setdefault("ARGUS_TUI_INLINE", "1")
    env["ARGUS_TUI_DASHBOARD"] = "1"

    if profile_dir is not None:
        env["ARGUS_HOME"] = str(profile_dir)

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["ARGUS_TUI_RESUME"] = resume

    if sidecar_url:
        env["ARGUS_TUI_SIDECAR_URL"] = sidecar_url

    if active_session_file:
        env["ARGUS_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    ...
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["ARGUS_TUI_GATEWAY_URL"] = gateway_ws_url
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Argus inside WSL2 to use the dashboard's /chat "
...
# NOTE: The legacy /api/multimodal/ws WebSocket endpoint (DualAgent protocol)
# was removed in phase 9. Multimodal sessions now use the standard /api/ws
# gateway with the set_live_watcher tool.
...
# ARGUS_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def mount_spa(application: FastAPI):
    ...
    def _serve_index(prefix: str = ""):
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__ARGUS_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
            ...
            bootstrap_script = (
                f'<script>window.__ARGUS_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__ARGUS_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__HERMES_BASE_PATH__="{prefix}";'
                f"window.__HERMES_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # (the MC Pages app), not the Argus backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Argus Teal",         "description": "Classic dark teal — the canonical Argus look"},
    {"name": "default-large", "label": "Argus Teal (Large)", "description": "Argus Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.argus/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.argus/dashboard-themes/*.yaml for user-created themes.
...
async def get_dashboard_themes():
    ...
    from `~/.argus/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    1. User plugins:    ~/.argus/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.argus/plugins/  (only if ARGUS_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()
    ...
    if env_var_enabled("ARGUS_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".argus" / "plugins", "project"))
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"argus auth {name}"
                        break
            except Exception:
                pass
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.argus/plugins/``) ship with the CWD and are
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``ARGUS_DESKTOP_READY_FILE`` and waits for this JSON.
    Normal CLI/dashboard launches still use the stdout READY line below.
    """
    target = os.environ.get("ARGUS_DESKTOP_READY_FILE")
    if not target:
        return
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing ARGUS_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Argus exposes a
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `argus dashboard register` (Nous Portal) or "
    ...
    # ARGUS_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            print(f"ARGUS_DASHBOARD_READY port={actual_port}", flush=True)
            print(f"  Argus Web UI → http://{host}:{actual_port}")
