...
_ARGUS_MODEL_WARNING = (
    ...
    "for use with Argus. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Hermes 3 / Argus 4 chat families.
...
def list_authenticated_providers(
    ...
    *,
    ...
    # https://mmargus-team.github.io/Argus/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Argus release. Falls back to the in-repo
    ...
    # --- 1. Check Argus-mapped providers ---
    from hermes_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from hermes_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # --- 2. Check Argus-only providers (nous, openai-codex, copilot, opencode-go) ---
    from hermes_cli.providers import ARGUS_OVERLAYS
    from hermes_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Argus provider ID.
    # ARGUS_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Argus IDs ("copilot").
    ...
    for pid, overlay in ARGUS_OVERLAYS.items():
        ...
        # Resolve Argus slug — e.g. "github-copilot" → "copilot"
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Argus's canonical write key (matches
            ...
            # Argus writes ``models:`` as a dict keyed by model id
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Argus's own writer uses; a hyphen variant is accepted
            ...
            # active model. Argus's own writer (main.py::_save_custom_provider)
