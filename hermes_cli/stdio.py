...
on Japanese locales, etc.).  Argus's banners, tool output feed, and slash
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Argus is running
...
def configure_windows_stdio() -> bool:
    ...
    # Augment PATH with the Argus-managed Git install directories so
    ...
    # install without Argus-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Argus's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Argus's
    ...
    Set this before launching Argus (User env var in Windows Settings, or
    export in a PowerShell profile) and Argus picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Argus-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\argus\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    ...
    Any subprocess Argus spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Argus-managed tool directories to our
    ...
    candidate_dirs = [
        ...
        # Argus venv Scripts directory — host of the argus.exe shim itself,
        ...
        os.path.join(local_appdata, "argus", "argus", "venv", "Scripts"),
