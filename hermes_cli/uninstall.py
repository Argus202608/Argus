...
- Keep data: Remove code but keep ~/.argus/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Argus PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing Argus or legacy Hermes PATH entries.
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Argus" comment and following line
                if '# Argus' in line or '# Hermes' in line or '# hermes-agent' in line:
                    skip_next = True
                    continue
                if skip_next and (('argus' in line.lower() or 'hermes' in line.lower()) and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing the current or legacy install.
                if ('argus' in line.lower() or 'hermes' in line.lower()) and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the canonical Argus and legacy Hermes wrapper scripts."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "argus",
        Path.home() / ".local" / "bin" / "hermes",
        Path("/usr/local/bin/argus"),
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it is one of our wrappers.
                content = wrapper.read_text()
                if 'argus_cli' in content or 'hermes_cli' in content or 'hermes-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(hermes_home: Path) -> list:
    ...
    the FHS fix).  Only symlinks that resolve into this Argus home's ``node``
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``argus gateway run`` processes
...
#
#   1. Sets User-scope env vars ``HERMES_HOME`` and ``ARGUS_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\argus\git\cmd``, ``%LOCALAPPDATA%\argus\git\bin``,
#      ``%LOCALAPPDATA%\argus\git\usr\bin``, ``%LOCALAPPDATA%\argus\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\argus\git\`` and Node to
#      ``%LOCALAPPDATA%\argus\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``argus dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\argus\gateway-service\`` and sometimes
...
def _hermes_path_markers(hermes_home: Path) -> list[str]:
    """Path-entry substrings that identify Argus-owned User-PATH entries."""
    root = str(hermes_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # Sweep the canonical install plus historical layouts used by upgrades.
    markers = [
        root + "\\argus",
        root + "\\hermes-agent",
        root + "\\git",
...
def remove_path_from_windows_registry(hermes_home: Path) -> list[str]:
    """Strip Argus-owned entries from User-scope PATH in the registry.
...
def remove_hermes_env_vars_windows() -> list[str]:
    """Delete Argus and legacy Hermes User-scope environment variables."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("ARGUS_HOME", "ARGUS_GIT_BASH_PATH", "HERMES_HOME", "HERMES_GIT_BASH_PATH"):
                try:
...
def remove_portable_tooling_windows(hermes_home: Path) -> list[Path]:
    ...
    ``%LOCALAPPDATA%\\argus\\``.  Only called on full uninstall; they're
...
def run_gui_uninstall(args):
    ...
    Mirrors ``argus uninstall --gui``. Removes the desktop app's built
    ...
    print(color("│         ⚕ Argus Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Argus Chat GUI installation was found.")
        print(f"  Checked: {hermes_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Argus agent stays installed.", Colors.CYAN))
    print()
    ...
    if agent_is_installed(hermes_home):
        ...
        print(f"  • The Argus agent at {hermes_home / 'argus'}")
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Argus agent is still installed. Run 'hermes' to use the CLI,")
    print("or 'argus uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.argus/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.argus/ for future reinstall
    """
    ...
    # full wipe (code + ~/.argus data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Argus Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Argus data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Argus code but keep your configuration and data.")
    ...
    *,
    ...
    if _is_windows():
        ...
        if removed_path_entries:
            ...
            log_info("No Argus-owned PATH entries in User environment")

        log_info("Removing Argus User environment variables...")
        removed_env = remove_hermes_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Argus-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing Argus command and legacy aliases...")
    ...
    #     (only when they still point into this Argus home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Argus-managed node/npm/npx symlinks...")
    ...
    if removed_node_links:
        ...
        log_info("No Argus-managed node/npm/npx symlinks found")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.argus/, remove only the code checkout.
    ...
    # 5. Optionally remove ~/.argus/ data directory (and named profiles)
    if full_uninstall:
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/MMArgus-Team/Argus/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/MMArgus-Team/Argus/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Argus! ⚕")
    print()
