...
class CLICommandsMixin:
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: argus --checkpoints")
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Argus config/state.
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run argus chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `argus gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                ...
                return
            _cprint("  Tip:   Use /history or `argus sessions list` to find sessions.")
            return
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `argus sessions list` to see available sessions.")
            return
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: argus bundles create "
        ...
        _cprint(
            ...
            f"Manage with `argus bundles`.{_RST}"
        )
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a Argus-managed isolated debug "
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Argus keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Argus is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Argus is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while Argus is busy."
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update Argus to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``argus update`` so the user sees update output directly and gets
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update Argus')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Argus"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="⚕  Update Argus",
            detail="This will exit the current session and run `argus update`.",
            choices=choices,
        )
        if raw is None:
