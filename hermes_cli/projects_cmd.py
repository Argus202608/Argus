...
def build_parser(
    ...
    p_create.add_argument("name", help="Human name, e.g. 'Argus'")
...
def projects_command(args: argparse.Namespace) -> int:
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: argus project <action> [options]\n"
                "Run 'argus project --help' for the full list.",
                file=sys.stderr,
            )
        return 0
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    if not projs:
        print("No projects yet. Create one with `argus project create <name>`.")
        return 0
    for p in projs:
...
def _cmd_set_primary(args, conn, proj) -> int:
    if not pdb.set_primary(conn, proj.id, args.path):
        print(
            ...
            f"add it first with `argus project add-folder`.",
            file=sys.stderr,
        )
        return 1
