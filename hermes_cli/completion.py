"""Shell completion script generation for argus CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""# Argus bash completion
# Add to ~/.bashrc:
...
complete -F _hermes_completion argus hermes
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef argus hermes
# Argus zsh completion
# Add to ~/.zshrc:
...
{top_cmds_str}
            ...
            _describe 'Argus command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c argus -f -s p -l profile"
    ...
    lines.append("")
    lines.append("# Legacy command inherits the canonical Argus completions")
    lines.append("complete -c hermes -w argus")
    lines.append("")
    return "\n".join(lines)
