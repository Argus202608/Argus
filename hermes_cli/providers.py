"""
Single source of truth for provider identity in Argus.
...
2. **Argus overlays** — transport type, auth patterns, aggregator flags,
...
# -- Argus overlay ----------------------------------------------------------
# Argus-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class ArgusOverlay:
    """Argus-specific provider metadata layered on top of models.dev."""
...
ARGUS_OVERLAYS: Dict[str, ArgusOverlay] = {
    "moa": ArgusOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": ArgusOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "openai-codex": ArgusOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": ArgusOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": ArgusOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": ArgusOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="ARGUS_QWEN_BASE_URL",
    ),
    "lmstudio": ArgusOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": ArgusOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": ArgusOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": ArgusOverlay(
    ...
    "kimi-for-coding": ArgusOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": ArgusOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": ArgusOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": ArgusOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": ArgusOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": ArgusOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": ArgusOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": ArgusOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "opencode": ArgusOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": ArgusOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "azure-foundry": ArgusOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": ArgusOverlay(
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Argus overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Argus overlay
    ...
    overlay = ARGUS_OVERLAYS.get(canonical)
    ...
    if overlay is not None:
        # Argus-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in ARGUS_OVERLAYS
    if provider == "bedrock":
