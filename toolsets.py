...
_HERMES_CORE_TOOLS = [
    ...
    "terminal", "process",
    # Read the desktop GUI's embedded terminal pane (gated on ARGUS_DESKTOP
    ...
    # spawned as a kanban worker (ARGUS_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `argus tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (ARGUS_KANBAN_TASK env "
    ...
    "hermes-cron": {
        ...
        # core tools users see interactively — then `argus tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as hermes-cli; gated by `argus tools`",
        "tools": _HERMES_CORE_TOOLS,
        "includes": []
    },
