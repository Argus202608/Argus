...
class GatewayKanbanWatchersMixin:
    ...
    async def _kanban_notifier_watcher(self, interval: float = 5.0) -> None:
        ...
        **Multi-board:** iterates every board discovered on disk per
        ...
        try:
            ...
            return
        env_override = os.environ.get("ARGUS_KANBAN_DISPATCH_IN_GATEWAY", "").strip().lower()
        if env_override in {"0", "false", "no", "off"}:
            logger.info("kanban notifier: disabled via ARGUS_KANBAN_DISPATCH_IN_GATEWAY env")
            return
        try:
            cfg = _load_config()
        ...
        while self._running:
            try:
                def _collect():
                    ...
                    # ARGUS_KANBAN_DB pins the board path; without this guard
    ...
    async def _kanban_dispatcher_watcher(self) -> None:
        ...
        no separate `argus kanban daemon` process needed. When false, the
        ...
        # watcher here. Honours ARGUS_KANBAN_DISPATCH_IN_GATEWAY env var
        ...
        env_override = os.environ.get("ARGUS_KANBAN_DISPATCH_IN_GATEWAY", "").strip().lower()
        if env_override in {"0", "false", "no", "off"}:
            logger.info("kanban dispatcher: disabled via ARGUS_KANBAN_DISPATCH_IN_GATEWAY env")
            return
            ...
            try:
                ...
                if _is_corrupt_board_db_error(exc):
                    ...
                    logger.error(
                        ...
                        "then run `argus kanban init` if you need a fresh board.",
                        slug,
                        fingerprint[0],
                    )
                    return None
        ...
        def _auto_decompose_tick(auto_decompose_per_tick: int) -> int:
            ...
            for b in boards:
                ...
                prev_env = os.environ.get("ARGUS_KANBAN_BOARD")
                try:
                    os.environ["ARGUS_KANBAN_BOARD"] = slug
                    try:
                    ...
                    if prev_env is None:
                        os.environ.pop("ARGUS_KANBAN_BOARD", None)
                    else:
                        os.environ["ARGUS_KANBAN_BOARD"] = prev_env
            return successes
                ...
                if bad_ticks >= HEALTH_WINDOW:
                    ...
                    if now - last_warn_at >= 300:
                        logger.warning(
                            ...
                            "`argus kanban list --status ready`.",
                            bad_ticks,
                        )
                        last_warn_at = now
