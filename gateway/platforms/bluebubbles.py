...
# group mention gating without custom aliases, use conservative Argus wake
# words so `require_mention: true` is a one-line enablement path. Historical
# Hermes wake words remain accepted for existing group chats.
...
class BlueBubblesAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> List[re.Pattern]:
        ...
        JSON list, or comma/newline-separated), or None (use Argus defaults).
        """
        if raw is None:
