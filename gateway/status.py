...
``~/.argus`` but can be overridden via the environment variable.  This means
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("ARGUS_GATEWAY_LOCK_DIR")
    if override:
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Argus gateway lifecycle subcommand from a command line.
    ...
    has_gateway_entry = (
        ...
        or any(
            t.rsplit("/", 1)[-1]
            in ("argus", "argus.exe", "hermes", "hermes.exe")
            for t in tokens
        )
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `argus gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Argus gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    ...
    ``profiles``).  The root/default home (``~/.argus`` or ``$HERMES_HOME``)
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `argus status` show per-profile
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # services share the same default ~/.argus (HERMES_HOME not set
    ...
    # ``argus gateway stop`` on Windows would be misclassified as an
