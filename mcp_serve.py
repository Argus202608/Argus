"""
Argus MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Argus-specific extra)

Usage:
    argus mcp serve
    argus mcp serve --verbose
...
def _get_sessions_dir() -> Path:
    ...
    try:
        ...
        return Path(os.environ.get("ARGUS_HOME", Path.home() / ".argus")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        ...
        directory_file = Path(
            os.environ.get("ARGUS_HOME", Path.home() / ".argus")
...
class EventBridge:
    ...
    def _poll_once(self, db):
        ...
        try:
            ...
            db_file = Path(os.environ.get("ARGUS_HOME", Path.home() / ".argus")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Argus MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "argus",
        instructions=(
            "Argus messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Argus MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
