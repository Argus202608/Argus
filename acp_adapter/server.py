"""ACP agent server — exposes Argus via the Agent Client Protocol."""
...
class HermesACPAgent(acp.Agent):
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Argus version",
        },
    )
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["argus-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="argus", version=HERMES_VERSION),
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        # not the executor's. Also set ARGUS_INTERACTIVE so approval.py
        # takes the CLI-interactive path (which calls the registered
        # callback via prompt_dangerous_approval) instead of the
        # non-interactive auto-approve branch (GHSA-96vc-wcxf-jjff).
        # ACP's conn.request_permission maps cleanly to the interactive
        # callback shape — not the gateway-queue ARGUS_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind ARGUS_SESSION_KEY for this session so per-session caches
            ...
            previous_interactive = os.environ.get("ARGUS_INTERACTIVE")
            ...
            previous_session_id = os.environ.get("ARGUS_SESSION_ID")
            ...
            try:
                ...
                # Restore ARGUS_INTERACTIVE.
                if previous_interactive is None:
                    os.environ.pop("ARGUS_INTERACTIVE", None)
                else:
                    os.environ["ARGUS_INTERACTIVE"] = previous_interactive
                # Restore ARGUS_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("ARGUS_SESSION_ID", None)
                else:
                    os.environ["ARGUS_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            ...
            # stomp on each other's ContextVar writes (ARGUS_SESSION_KEY in
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Argus v{HERMES_VERSION}"
