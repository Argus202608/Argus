"""CLI entry point for the Argus ACP adapter.

Loads environment variables from ``~/.argus/.env``, configures logging
...
try:
    ...
    # shadowing Argus's own modules — ``argus acp`` can be started from any
...
def _load_env() -> None:
    """Load .env from HERMES_HOME (default ``~/.argus``)."""
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog=os.path.basename(sys.argv[0]) or "argus-acp",
        description="Run Argus as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Argus version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Argus provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.argus/node/ "
...
def _run_setup() -> None:
    ...
    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "argus", "model"]
        hermes_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting Argus ACP adapter")
