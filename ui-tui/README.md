...
React + Ink terminal UI for Argus. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `ARGUS_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `argus setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.argus/.argus_history` or under `ARGUS_HOME` (legacy history is migrated automatically).
...
### Setup (`setup.ts`)
`/setup` — launches external `argus setup` wizard, suspends Ink while it runs
...
ui-tui/
  ...
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external argus setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.argus/logs/tui-parent.log
...
tui_gateway/
  ...
  slash_worker.py        persistent Argus CLI subprocess for slash commands
```
