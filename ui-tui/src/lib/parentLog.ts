...
const logDir = join(process.env.ARGUS_HOME?.trim() || join(homedir(), '.argus'), 'logs')
...
// a real ~/.argus (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('argus-tui: parent lifecycle log unavailable\n')
    }
  }
}
