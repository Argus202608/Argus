...
describe('createGatewayEventHandler', () => {
  ...
  it('renders a monitor SPEAK as a standalone bubble (display-only, not the main turn)', () => {
    const appended: Msg[] = []
    const onEvent = createGatewayEventHandler(buildCtx(appended))

    onEvent({ payload: { source: 'monitor', monitor_id: 'mon_1', monitor_label: '进球提醒' }, type: 'message.start' } as any)
    onEvent({ payload: { source: 'monitor', text: '姆巴佩进球了' }, type: 'message.delta' } as any)
    onEvent({ payload: { source: 'monitor', monitor_label: '进球提醒', text: '姆巴佩进球了' }, type: 'message.complete' } as any)

    const bubble = appended.find(m => m.role === 'system' && m.text.includes('姆巴佩进球了'))
    expect(bubble).toBeDefined()
    expect(bubble!.text).toContain('👁 进球提醒')
    // Must NOT have produced a main-agent assistant bubble.
    expect(appended.some(m => m.role === 'assistant')).toBe(false)
  })

  it('keeps two concurrent monitors separate (keys on monitor_id, not request_id)', () => {
    const appended: Msg[] = []
    const onEvent = createGatewayEventHandler(buildCtx(appended))

    // Two different monitors interleave — neither carries request_id.
    onEvent({ payload: { source: 'monitor', monitor_id: 'mon_a', monitor_label: 'A' }, type: 'message.start' } as any)
    onEvent({ payload: { source: 'monitor', monitor_id: 'mon_b', monitor_label: 'B' }, type: 'message.start' } as any)
    onEvent({ payload: { source: 'monitor', monitor_id: 'mon_a', text: 'alert-A' }, type: 'message.delta' } as any)
    onEvent({ payload: { source: 'monitor', monitor_id: 'mon_b', text: 'alert-B' }, type: 'message.delta' } as any)
    onEvent({ payload: { source: 'monitor', monitor_id: 'mon_a', monitor_label: 'A', text: 'alert-A' }, type: 'message.complete' } as any)
    onEvent({ payload: { source: 'monitor', monitor_id: 'mon_b', monitor_label: 'B', text: 'alert-B' }, type: 'message.complete' } as any)

    expect(appended.some(m => m.role === 'system' && m.text.includes('👁 A') && m.text.includes('alert-A'))).toBe(true)
    expect(appended.some(m => m.role === 'system' && m.text.includes('👁 B') && m.text.includes('alert-B'))).toBe(true)
  })

  it('renders a watcher answer as a standalone bubble', () => {
    const appended: Msg[] = []
    const onEvent = createGatewayEventHandler(buildCtx(appended))

    onEvent({ payload: { source: 'watcher', request_id: 'req_abc', brief: '分析论文' }, type: 'message.start' } as any)
    onEvent({ payload: { source: 'watcher', request_id: 'req_abc', text: '论文核心贡献是…' }, type: 'message.delta' } as any)
    onEvent({ payload: { source: 'watcher', request_id: 'req_abc', text: '论文核心贡献是…' }, type: 'message.complete' } as any)
  ...
  it('ignores the watcher_running placeholder (only the final answer bubbles)', () => {
    ...
    onEvent({ payload: { source: 'watcher_running', request_id: 'req_x' }, type: 'message.start' } as any)
    onEvent({ payload: { source: 'watcher_running', request_id: 'req_x', text: '🔬 深度分析进行中…', status: 'running' }, type: 'message.complete' } as any)

    expect(appended.length).toBe(0)
  })

  it('does not let a monitor SPEAK hijack the in-flight main-agent turn', () => {
    const appended: Msg[] = []
    const onEvent = createGatewayEventHandler(buildCtx(appended))

    // Main turn in flight.
    onEvent({ payload: {}, type: 'message.start' } as any)
    onEvent({ payload: { text: 'The user asked about ' }, type: 'message.delta' } as any)
    // Monitor SPEAK arrives mid-turn — must not disturb the main stream.
    onEvent({ payload: { source: 'monitor', monitor_label: 'X' }, type: 'message.start' } as any)
    onEvent({ payload: { source: 'monitor', text: 'alert' }, type: 'message.delta' } as any)
    onEvent({ payload: { source: 'monitor', monitor_label: 'X', text: 'alert' }, type: 'message.complete' } as any)
    // Main turn resumes and completes.
    onEvent({ payload: { text: 'the weather.' }, type: 'message.delta' } as any)
    onEvent({ payload: { text: 'The user asked about the weather.' }, type: 'message.complete' } as any)

    const mainBubble = appended.find(m => m.role === 'assistant')
    expect(mainBubble).toBeDefined()
    expect(mainBubble!.text).toBe('The user asked about the weather.')
    const monitorBubble = appended.find(m => m.role === 'system' && m.text.includes('alert'))
    expect(monitorBubble).toBeDefined()
  })
