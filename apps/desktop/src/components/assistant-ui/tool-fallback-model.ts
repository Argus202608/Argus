...
// Streaming-stable "does this message have visible answer text yet". Returns
// on the first non-whitespace text part without concatenating the whole
// message, so as a useAuiState selector its boolean output only flips
// false→true once per turn (not on every token flush). Mirrors thread.tsx's
// contentHasVisibleText; kept here so the tool pile can gate on the same
// signal without importing from thread.tsx (which imports this file).
export const selectMessageHasVisibleText = (state: { message: { content?: unknown } }): boolean => {
  ...
  for (const part of content) {
    const row = part as { text?: unknown; type?: unknown } | null
