...
export interface Translations {
  ...
  assistant: {
    ...
    tool: {
      ...
      outputAlt: string
      rawResponse: string
      /** Section label above the classified call-args panel. */
      argsLabel: string
      /** A payload arg: only its length is known. */
      argsChars: (count: number) => string
      /** A credential arg: neither value nor length is sent. */
      argsWithheld: string
      /** An array/object arg: only its element count is known. */
      argsItems: (count: number) => string
      /** Fields dropped past the backend's per-call cap. */
      argsElided: (count: number) => string
      copyActivity: string
      recoveredOne: string
