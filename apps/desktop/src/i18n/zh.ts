...
export const zh: Translations = {
  ...
  boot: {
    ...
    steps: {
      ...
      loadingSettings: '正在加载 Argus 设置',
      loadingSessions: '正在加载最近会话',
      startingDesktopConnection: '正在启动桌面连接',
      startingHermesDesktop: '正在启动 Argus 桌面版…'
    },
    errors: {
      backgroundExited: 'Argus 后台进程已退出。',
      backgroundExitedDuringStartup: 'Argus 后台进程在启动期间退出。',
  ...
  notifications: {
    ...
    backendOutOfDateMessage: '你的 Argus 后端早于当前桌面构建，可能无法正常工作。请更新以保持一致。',
    updateHermes: '更新 Argus',
    ...
    errors: {
      ...
      methodNotAllowed: '桌面后端拒绝了该请求 (405 Method Not Allowed)。请尝试重启 Argus Desktop。',
    ...
    native: {
      ...
      rejectAction: '拒绝',
      inputTitle: '需要输入',
      inputBody: 'Argus 正在等待你的回应。',
      turnDoneTitle: 'Argus 已完成',
      turnDoneBody: '回复已就绪。',
      turnErrorTitle: '本轮失败',
  ...
  settings: {
    ...
    resetConfirm: '将所有设置恢复为 Argus 默认值？',
    exportFailed: '导出失败',
    resetFailed: '重置失败',
    nav: {
      providers: '提供方',
    ...
    notifications: {
      ...
      focusedHint: '完成提醒仅在 Argus 处于后台时触发。',
      kinds: {
        approval: {
          label: '需要批准',
        ...
        input: {
          label: '需要输入',
          description: 'Argus 提出了问题，或需要密码或密钥。'
        },
        turnDone: {
          label: '回复就绪',
          description: 'Argus 在后台时完成了一轮对话。'
        },
        turnError: {
          label: '本轮失败',
          description: '本轮以错误结束。'
    ...
    appearance: {
      ...
      colorMode: '颜色模式',
      colorModeDesc: '选择固定模式，或让 Argus 跟随系统设置。',
      ...
      pet: {
        title: '宠物',
        intro:
          '领养一只悬浮在应用上的 petdex 动画宠物，它会根据 Argus 的状态做出反应——工具执行时奔跑、成功时欢呼、出错时沮丧。',
        restartHint: '宠物功能需要重启——当前运行的应用在此功能加入前启动。请退出并重新打开 Argus，然后回到此处。',
        scaleTitle: '大小',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone: '当 Argus 需要本地时间上下文时使用。留空则使用系统时区。',
      agent: {
        imageInputMode: '控制图片附件如何发送给模型。',
        maxTurns: 'Argus 停止一次运行前工具调用轮次的上限。'
      },
      terminal: {
        cwd: '工具与终端操作的默认项目目录。',
      ...
      fileReadMaxChars: 'Argus 单次文件读取可读取的最大字符数。',
      approvals: {
        mode: 'Argus 如何处理需要显式审批的命令。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'Argus 从应用内更新时（无终端提示），保留本地源码修改（暂存）或丢弃（放弃）。通过终端更新时始终会询问。'
      }
    }),
    about: {
      ...
      automaticUpdatesDesc: 'Argus 会在后台自动检查更新，并在有可用更新时通知你。',
    ...
    gateway: {
      ...
      envOverride: '环境变量覆盖',
      intro:
        'Argus Desktop 默认会启动自己的本地网关。当你希望此应用控制另一台机器上或可信代理后的现有 Argus 后端时，可以使用远程网关。下面可按 profile 指定各自的远程主机。',
      appliesTo: '应用于',
      ...
      envOverrideDesc: '取消设置 ARGUS_DESKTOP_REMOTE_URL 和 ARGUS_DESKTOP_REMOTE_TOKEN 后才会使用下面保存的设置。',
      localTitle: '本地网关',
      localDesc: '在 localhost 启动私有 Argus 后端。这是默认方式，并且可离线工作。',
      remoteTitle: '远程网关',
      remoteDesc:
        '将此桌面外壳连接到远程 Argus 后端。托管网关使用 OAuth 或用户名密码；自托管网关也可能使用会话 token。',
      ...
      savedTitle: '网关设置已保存',
      restartingMessage: 'Argus Desktop 将使用已保存设置重新连接。',
      savedMessage: '已保存，下一次重启生效。',
      connectedTo: (baseUrl, version) => `已连接到 ${baseUrl}${version ? ` · Argus ${version}` : ''}`,
    ...
    providers: {
      ...
      intro: '使用订阅登录，无需复制 API 密钥。Argus 会在应用中为你完成浏览器登录。',
      connected: '已连接',
      collapse: '收起',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: '请重启 Argus 以使用宠物功能——当前后端版本过旧。',
      empty: '没有匹配的宠物。',
      turnOff: '关闭',
      turnOn: '开启',
      installed: '已安装',
    ...
    generatePet: {
      ...
      staleBackend: '请更新 Argus 以生成宠物。',
      backgroundHint: '你可以关闭此窗口——完成后 Argus 会通知你。',
    ...
    nav: {
      ...
      settings: { title: '设置', detail: '配置 Argus 桌面端' },
    ...
    hermesActiveSessions: (version, count) => `Argus ${version} · 活跃会话 ${count}`,
    restartGateway: '重启网关',
  ...
  messaging: {
    ...
    platformIntro: {
      ...
      signal: '在可访问的位置运行 signal-cli REST 桥接，然后把 Argus 指向该 URL 和已注册的电话号码。',
      whatsapp: '启动 Argus 自带的 WhatsApp 桥接，首次运行时扫描二维码，然后启用该平台。',
      bluebubbles:
        '在装有 iMessage 的 Mac 上运行 BlueBubbles Server，暴露其 API，然后用服务器密码把 Argus 指向该 URL。',
      ...
      weixin:
        '运行 `argus gateway setup`，选择 Weixin，然后使用个人微信账号扫描并确认二维码。Argus 会通过腾讯 iLink Bot API 连接并保存凭据。',
      qqbot: '在 QQ 开放平台 (q.qq.com) 注册一个应用，复制 App ID 和 Client Secret。',
      api_server:
        '把 Argus 暴露为兼容 OpenAI 的 API。设置一个鉴权密钥，然后把 Open WebUI / LobeChat 等指向 host:port。',
  ...
  profiles: {
    ...
    deleting: '删除中…',
    createDesc: '配置档案是相互独立的 Argus 环境：各自拥有独立的配置、技能和 SOUL.md。',
    nameLabel: '名称',
    cloneFrom: '克隆来源',
    cloneFromNone: '无（空白）',
  ...
  cron: {
    ...
    newCron: '新建定时任务',
    emptyDescNew: '按 cron 表达式排程一个提示词。Argus 会运行它，并把结果发送到你选择的目的地。',
  ...
  sidebar: {
    ...
    projects: {
      ...
      createFailed: '无法创建项目',
      deleteConfirm: '这会从 Argus 中移除已保存的项目。文件、git 仓库和工作树保持不变。',
      startWork: '新建工作树',
  ...
  composer: {
    ...
    placeholderStarting: '正在启动 Argus…',
    placeholderReconnecting: '正在重新连接 Argus…',
    ...
    attachUrlDesc: 'Argus 将抓取该页面并作为本回合的上下文。',
  ...
  updates: {
    ...
    unsupportedMessage: '此版本的 Argus 无法在应用内自行更新。',
    connectionRetry: '请检查网络连接后重试。',
    latestBody: '你正在运行最新版本。',
    latestBodyBackend: '后端正在运行最新版本。',
    allSetTitle: '已是最新',
    availableTitle: '有可用更新',
    availableBody: '新版 Argus 已可安装。',
    availableTitleBackend: '后端有可用更新',
    availableBodyBackend: '已连接的 Argus 后端有新版本可安装。',
    availableBodyNoChangelog: '已有新版本可用。此安装方式无法显示更新日志。',
    updateNow: '立即更新',
    maybeLater: '稍后再说',
    moreChanges: count => `另有 ${count} 项更改。`,
    manualTitle: '从终端更新',
    manualBody: '你是从命令行安装的 Argus，因此更新也需要在那里运行。请将此命令粘贴到终端：',
    manualPickedUp: '下次启动 Argus 时会使用新版本。',
    guiSkewTitle: '请更新桌面应用',
    guiSkewBody:
      '后端已更新，但此桌面应用包未更改。请更新或重新安装 Argus 桌面应用（你的 AppImage / .deb / .rpm）以保持一致。',
    copy: '复制',
    copied: '已复制',
    done: '完成',
    applyingBody:
      'Argus 更新器会在自己的窗口中接管，并在完成后自动重新打开 Argus。更新期间请不要自行重新打开 Argus。',
    applyingBodyBackend: '远程后端正在应用更新并将重启。恢复后 Argus 会自动重新连接。',
    applyingClose: '此窗口会在更新期间关闭，随后 Argus 会自动重新打开。',
    errorTitle: '更新未完成',
  ...
  install: {
    ...
    failedTitle: '安装失败',
    settingUpTitle: '正在设置 Argus Agent',
    finishingTitle: '正在收尾',
    failedDesc:
      '某个安装步骤失败。在 Windows 上，如果另一个 Argus CLI 或桌面实例正在运行，可能会出现这种情况。请停止正在运行的 Argus 实例后重试。可查看下面的详情或 desktop 日志中的完整记录。',
    activeDesc: '这是一次性设置。Argus 安装器正在下载依赖并配置你的机器。之后启动会跳过此步骤。',
  ...
  onboarding: {
    headerTitle: '开始设置 Argus Agent',
    headerDesc: '连接模型提供方即可开始对话。大多数选项只需一次点击。',
    preparingInstall: 'Argus 正在完成安装。首次运行通常不到一分钟。',
    starting: '正在启动 Argus…',
    ...
    recommended: '推荐',
    connected: '已连接',
    featuredPitch: '一个订阅，300+ 前沿模型 — 运行 Argus 的推荐方式',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: '自托管',
        description: '将 Argus 指向本地或自托管的 OpenAI 兼容端点 (vLLM、llama.cpp、Ollama 等)。'
      }
    },
    backToSignIn: '返回登录',
    getKey: '获取密钥',
    ...
    flowSubtitles: {
      ...
      device_code: '在浏览器中打开验证页面 — Argus 会自动连接',
      loopback: '打开浏览器登录 — Argus 会自动连接',
    ...
    authorizeThere: '请在那里授权 Argus。',
    copyAuthCode: '复制授权码并粘贴到下面。',
    pasteAuthCode: '粘贴授权码',
    reopenAuthPage: '重新打开授权页面',
    autoBrowser: provider => `已在浏览器中打开 ${provider}。请在那里授权 Argus，连接会自动完成，无需复制或粘贴。`,
  ...
  preview: {
    ...
    largeTitle: '此文件较大',
    largeBody: (label, size) => `${label} 大小为 ${size}。Argus 只会显示前 512 KB。`,
    previewAnyway: '仍然预览',
    ...
    web: {
      ...
      failedToLoad: '预览加载失败',
      tryAgain: '重试',
      restarting: 'Argus 正在重启...',
      askRestart: '让 Argus 重启服务器',
      lookingRestart: taskId => `Argus 正在查找要重启的预览服务器 (${taskId})`,
      restartingTitle: '正在重启预览服务器',
      restartingMessage: 'Argus 正在后台工作。可在预览控制台查看进度。',
      ...
      finishedRestarting: message => `Argus 已完成预览服务器重启${message ? `: ${message}` : ''}`,
      failedRestarting: message => `服务器重启失败：${message}`,
      unknownError: '未知错误',
      restartedTitle: '预览服务器已重启',
      reloadingNow: '正在重新加载预览。',
      restartFailedTitle: '预览重启失败',
      restartFailedMessage: 'Argus 无法重启服务器。',
      stillWorking: 'Argus 仍在工作，但还没有收到重启结果。服务器命令可能正在前台运行。',
  ...
  assistant: {
    thread: {
      ...
      showEarlier: '显示更早的消息',
      loadingResponse: 'Argus 正在加载回复',
      resumeWhenBackgroundDone: count =>
        count === 1 ? '后台任务完成后将自动继续' : `${count} 个后台任务完成后将自动继续`,
      thinking: '思考中',
      preparingTool: name => `正在准备 ${name}`,
      toolHistory: count => `${count} 次工具调用`,
      // 一批工具"到底做了什么", 按工具族归类。裸的"N 次工具调用"没有信息量,
      // 而关掉思考模式时它是完成态唯一的标签。与 web 的 summarizeStep 对齐。
      stepRead: (n: number) => `读了 ${n} 个文件`,
      stepEdit: (n: number) => `改了 ${n} 个文件`,
      stepRun: (n: number) => `执行了 ${n} 条命令`,
      stepSearch: (n: number) => `搜索了 ${n} 次`,
      stepBrowse: (n: number) => `浏览了 ${n} 个页面`,
      stepLook: (n: number) => `看了 ${n} 帧画面`,
      stepCall: (n: number) => `${n} 次工具调用`,
      stepFailed: (n: number) => `${n} 个失败`,
      expandAllTools: '全部展开',
    ...
    approval: {
      gatewayDisconnected: 'Argus 网关未连接',
      sendFailed: '无法发送审批响应',
      run: '运行',
      command: '命令',
      moreOptions: '更多审批选项',
      ...
      alwaysDescription: pattern =>
        `这会将“${pattern}”模式加入永久允许列表 (~/.argus/config.yaml)。Argus 对类似命令将不再询问，包括当前会话和未来会话。`,
      alwaysAllow: '始终允许'
    },
    clarify: {
      notReady: '澄清请求尚未就绪',
    ...
    tool: {
      ...
      rawResponse: '原始响应',
      argsLabel: '入参',
      argsChars: count => `${count} 字符 (未显示)`,
      argsWithheld: '已隐去 (凭证)',
      argsItems: count => `${count} 项`,
      argsElided: count => `还有 ${count} 个字段 (未显示)`,
      copyActivity: '复制活动',
  ...
  prompts: {
    ...
    sudoTitle: '管理员密码',
    sudoDesc: 'Argus 需要你的 sudo 密码来运行特权命令。它只会发送给你的本地 agent。',
    sudoPlaceholder: 'sudo 密码',
    secretTitle: '需要密钥',
    secretDesc: 'Argus 需要一个凭据才能继续。',
  ...
  desktop: {
    ...
    modelSwitchConfirmAction: '仍然切换',
    modelSwitchConfirmMessage: '该模型的已知价格异常偏高。',
    modelSwitchConfirmTitle: '确认切换模型',
    modelSwitchWarningTitle: '模型已切换，但有提示',
    sessionExported: '会话已导出',
    sessionExportFailed: '无法导出会话',
    imageSaved: '图片已保存',
    downloadStarted: '下载已开始',
    restartToUseSaveImage: '重启 Argus 桌面版后可使用保存图片。',
    restartToSaveImages: '重启 Argus 桌面版以保存图片',
    ...
    handoff: {
      ...
      timedOut: '等待网关超时。`argus gateway` 是否正在运行？'
    }
  },
  ...
  multimodal: {
    ...
    welcome: {
      title: '开始一段多模态对话',
      body: '直接提问，或开启摄像头 / 屏幕共享——画面会随问题一起发送给具备视觉能力的智能体。'
    },
    composer: {
      listening: '正在聆听…',
      micError: (err: string) => `麦克风启动失败：${err}`,
      micStopRecording: '点击结束录音',
      micReady: '麦克风已就绪，等待语音…点击取消',
      micLabel: '语音（流式识别）',
      ...
      micLabelEndSend: '结束录音并发送',
      micLabelFinalizing: '正在完成识别并发送',
      micLabelConnecting: '正在准备麦克风，点击取消',
      micLabelStart: '开始单次语音输入',
      asrFailed: (msg: string) => `语音识别失败：${msg}`,
      micFailedTitle: (msg: string) => `麦克风失败：${msg}`,
      ttsAriaOn: '关闭语音播报',
      ttsAriaOff: '开启语音播报',
      dialogNeedFinishMic: '请先结束当前单次录音，再开启语音对话',
      askPlaceholder: '提问，或开启摄像头 / 屏幕共享',
      askWithVideoPlaceholder: '提问（开启画面后，画面会随问题一起发送）…',
      stopGenerating: '停止生成',
      stopShort: '停止',
      send: '发送',
      recordingHint: '录音中…点击麦克风结束并发送',
      thinkingProcess: '思考过程',
      needConfirm: '需要你确认',
      answerPlaceholder: '输入你的回答',
      answerPlaceholderEnter: '输入你的回答，回车提交…',
      submit: '提交',
      readAloud: '朗读',
      noMoreInfo: '(无更多信息)',
      voiceInput: '语音输入',
      monitorTag: '监控提醒',
      selectedLabel: (answer: string) => `已选择：${answer}`,
      emptyChoice: '（空）'
    },
    video: {
      camera: '摄像头',
      screenShare: '屏幕',
      notStarted: '未开启画面',
      startingRecord: '正在启动记录',
      connectionPaused: '连接中断，记录已暂停',
      recLabel: (source: string, frames: number) => `REC · ${source === 'camera' ? '摄像头' : '屏幕'} · ${frames} 帧`,
      recDropped: (n: number) => `（丢 ${n}）`,
      previewLabel: (source: string, phase: string) => `预览 · ${source === 'camera' ? '摄像头' : '屏幕'} · ${phase}`,
      startCamera: '摄像头',
      startScreen: '屏幕共享',
      stopCapture: (source: string) => `停止${source === 'camera' ? '摄像头' : '屏幕共享'}`,
      cameraError: (msg: string) => `摄像头开启失败：${msg}`,
      screenError: (msg: string) => `屏幕共享开启失败：${msg}（macOS 需在"系统设置 → 隐私与安全 → 屏幕录制"授权）`,
      resuming: '正在恢复当前会话，完成后可开启画面记录。',
      captureDiag: (detail: string) => `采集诊断：${detail}`
    },
    screenPicker: {
      title: '选择要共享的窗口或屏幕',
      loading: '正在读取窗口列表…',
      noSources: '未发现可共享的源。',
      macPermission: 'macOS 上首次使用需在【系统设置 → 隐私与安全 → 屏幕录制】给 Argus 授权后重启 app。',
      tabScreen: '整个屏幕',
      tabWindow: '窗口',
      shareAudioEnabled: '开启后随共享同时捕获系统音频（Loopback）',
      shareAudioDisabled: '当前系统版本没有可用的屏幕共享系统音频通道',
      shareAudioLabel: '同时分享音频',
      shareAudioNote: '（需 Windows 或 macOS 13+）',
      cancel: '取消',
      startShare: '开始共享',
      noPreview: '无预览',
      noTitle: '(无标题)'
    ...
    observations: {
      injectedFrames: '注入帧',
      injectedFramesHint: '（提问时若有画面流，这里显示模型本回合看到的帧）',
      videoObs: '画面观察',
      audioObs: '音频观察',
      audioObsHint: '（共享屏幕并分享音频后才有）',
      searchFacts: '搜索事实',
      empty: '（空）',
      noneYet: '（暂无）',
      clickToEnlarge: '点击放大'
    ...
    deepAnalysis: {
      title: '深度分析',
      titleWithLabel: (label: string) => `🔬 深度分析 · ${label}`,
      completed: '已完成',
      inProgress: '进行中',
      stopping: '正在停止',
      interrupted: '已中断',
      ...
      thinking: '思考过程',
      thinkingInProgress: '思考过程…',
      toolCall: (name: string, arg?: string) => `🔧 调用 ${name}${arg ? `(${arg})` : ''}`,
      toolFailed: (name: string, error: string) => `⚠️ ${name} 调用失败：${error}`,
      searchLookup: (query: string) => `🔎 搜索「${query}」`,
      memoryLookup: (query: string) => `🧩 记忆「${query}」`,
      lookupLine: (kind: string, query: string, result?: string) => `${kind === 'search' ? '🔎 搜索' : '🧩 记忆'}「${query}」${result ? ` → ${result}` : '…'}`,
      noInterpretation: '📝 本段无独立解读，内容已并入下方报告',
      waitingFrames: '等待新画面…',
      waitingFramesSeg: (seg: number) => `第${seg}段 等待新画面…`,
      accumulatingSeg: (seg: number, have: number, need: number) => `第${seg}段 攒帧中… ${have}/${need}`,
      accumulating: (have: number, need: number) => `攒帧中… ${have}/${need}`,
      ttlRemaining: (sec: number) => `· ⏱ 余 ${sec}s`,
      completedHint: '深度研究已完成；如需再次分析，请新建任务',
      previewWaiting: (seg?: number) => `⏳ ${seg !== undefined ? `第${seg}段 ` : ''}等待新画面…`,
      previewAccumulating: (have: number, need: number, seg?: number) => `⏳ ${seg !== undefined ? `第${seg}段 ` : ''}正在积累视频帧… (${have}/${need})`,
      previewSaw: (desc: string) => `👁 ${desc}`,
      previewSegAnalyzing: (seg: number) => `🎬 第 ${seg} 段分析中…`
    },
    monitor: {
      title: '监控',
      count: (n: number) => `${n}`,
    ...
    watcher: {
      ...
      fallbackLabel: '深度研究',
      completedHint: '深度研究已完成；如需再次分析，请新建任务',
      stoppingHint: '正在等待当前轮次收尾',
      inProgress: '进行中',
      completed: '已完成',
      stopping: '正在停止',
      interrupted: '已中断',
      switchLabel: (label: string, state: string) => `${label}：${state}`
    },
    trajectory: {
      title: 'QueryWorker 完整轨迹',
      working: '工作中',
      completed: '已完成',
      waitingFirstEntry: '等待 QueryWorker 第一条结构化轨迹…',
      subtitle: '结构化执行记录：冻结画面、OCR、计划/实际工具、证据与结果；不包含模型隐藏思维链。',
      actualCall: '实际调用',
      plannedCall: '计划调用',
      toolReturned: '工具返回',
      chars: (n: number) => `${n} 字`,
      frames: (n: number) => `${n} 帧`,
      frozenInputTitle: '提问时刻冻结输入帧（QueryWorker 实际看到）',
      recallEvidenceTitle: 'Recall 证据帧',
      inputFrame: (n: number) => `输入帧 ${n}`,
      clickToEnlarge: '点击放大查看',
      evidenceFrame: '证据帧',
      ocrHelperTitle: (n: number) => `OCR 辅助文字 · ${n}`,
      ocrSkipped: '已跳过 OCR。',
      ocrSkippedReason: (reason: string) => `已跳过 OCR：${reason}。`,
      ocrTimeout: 'OCR 超时；QueryWorker 已继续使用原始画面。',
      ocrError: 'OCR 提取失败；QueryWorker 已继续使用原始画面。',
      ocrErrorReason: (reason: string) => `OCR 提取失败：${reason}；QueryWorker 已继续使用原始画面。`,
      ocrNoText: 'OCR 已完成，但没有识别到可用文字。',
      ocrTimeUnknown: '时间未知',
      ocrNoTextInFrame: '（该帧未识别到文字）',
      evidenceLabel: (kind: string, range?: string) => `${kind}${range ? ` ${range}` : ''}`,
      evidenceMemoryKind: '记忆',
      framesCompact: (n: number) => `${n}帧`,
      rawEvent: (event: string, phase: string) => `原始事件 · ${event} / ${phase}`
    },
    ocr: {
      helperAvailable: (n: number) => `OCR 辅助文字 · ${n} 条`,
      helperSkipped: 'OCR 辅助文字 · 已跳过',
      helperTimeout: 'OCR 辅助文字 · 超时',
      helperError: 'OCR 辅助文字 · 提取失败',
      helperEmpty: 'OCR 辅助文字 · 未识别到文字'
    },
    recall: {
      traceTitle: (steps: number, tools: number) => `Recall 执行轨迹 · ${steps} 步${tools ? ` · ${tools} 个内部工具` : ''}`,
      traceSubtitle: '结构化决策与证据摘要，不包含模型隐藏思维链。',
      decisionSummary: (summary: string) => `决策摘要：${summary}`,
      started: (n: number) => `接手问题并锁定提问时刻 · 冻结输入帧 ${n}`,
      analysisStart: '开始分析问题，准备决定 Recall / Search',
      recallCall: (toolName: string, round?: number) => `Recall 调用 · ${toolName}${round ? ` · 外层第 ${round} 轮` : ''}`,
      startRecall: '开始召回多模态记忆',
      roundRead: (round: number | string, count: number) => `Recall 第${round}轮读取记忆工具 · ${count} 项结果`,
      roundDistill: (round: number | string) => `Recall 第${round}轮提炼出有效线索`,
      roundDecisionContinue: (round: number) => `Recall 第${round}轮决策 · 继续`,
      roundDecisionStop: (round: number) => `Recall 第${round}轮决策 · 停止`,
      roundSkipDuplicate: (round: number | string) => `Recall 第${round}轮跳过重复记忆读取`,
      visualReview: (kept: number, total: number) => `Recall 视觉复核 · 保留 ${kept}/${total} 帧`,
      quickTool: (toolName: string, len: number) => `Recall 快速工具返回 · ${toolName} · ${len} 字证据`,
      failed: (stage?: string) => `Recall 请求失败${stage ? ` · ${stage}` : ''}`,
      completeFound: (n: number) => `Recall 完成 · ${n} 条线索`,
      completeNotFound: 'Recall 完成 · 未找到可靠线索',
      returnFound: (n: number, frames: number) => `Recall 返回 · ${n} 条线索${frames ? ` · ${frames} 帧` : ''}`,
      returnNotFound: (frames: number) => `Recall 返回 · 未找到可靠线索${frames ? ` · ${frames} 帧` : ''}`,
      searchReturn: (len: number) => `Search 返回 · ${len} 字证据`,
      composingAnswer: '证据已齐，正在组织最终答案',
      answerComplete: '回答已完成并回填原问题',
      answerCompleteBackfill: '回答已完成并回填原问题',
      subtaskFailed: (channel: string, target: string) => `${channel} 子任务失败 · ${target}`,
      phase: (phase: string, round?: number) => `Recall ${phase}${round ? ` · 第${round}轮` : ''}`,
      planRound: (recallCount: number, searchCount: number) => `完成一轮规划${recallCount ? ` · Recall ${recallCount}` : ''}${searchCount ? ` · Search ${searchCount}` : ''}`,
      planRoundNoTools: '完成一轮规划 · 本轮未调用 Recall / Search',
      retryLimitStop: '停止重试 Recall · 相同任务已连续失败 2 次',
      skipDuplicate: '跳过重复 Recall · 已用本次分析的召回结果',
      roundDecision: (round: number | string, verdict: string) => `Recall 第${round}轮决策 · ${verdict}`,
      decisionEnough: '证据已足够',
      decisionContinueTools: (n: number) => `继续检索 ${n} 个工具`,
      decisionNoTools: '无后续工具',
      evidenceSummary: (text: string) => `证据摘要：${text}`,
      noVisualConflict: '未发现需要纠正的画面冲突',
      searchCall: (toolName: string) => `Search 调用 · ${toolName}`,
      searchStart: '开始检索',
      searchPhase: (phase: string) => `Search ${phase}`,
      taskCancelled: '任务已取消',
      taskFailed: '任务执行失败',
      outerRound: (round: number) => `外层第 ${round} 轮`,
      innerRounds: (n: number) => `内层轮数 ${n}`,
      evidenceChars: (n: number) => `${n} 字证据`,
      parallelRead: (seconds: string) => `并行读取 ${seconds}s`,
      newEvidenceFrames: (n: number) => `新证据帧 ${n}`,
      cluesSoFar: (n: number) => `已有线索 ${n}`
    },
    readiness: {
      notReady: '多模态未完全就绪',
      details: '详情',
      ...
      close: '关闭',
      expand: '展开',
      capsMissing: '以下能力缺失，相关功能将无法工作。运行',
      runSetup: 'setup.sh',
      toFix: '补齐。'
    },
    capture: {
      notStarted: '采集未启动',
      waitingSession: '等待会话',
      initBackend: '正在初始化多模态后端',
      staleRejected: '多模态后端拒绝了过期的采集代次',
      sessionChanged: '画面目标会话已发生变化',
      gatewayUnavailable: 'Gateway 不可用',
      gatewayState: (state: string) => `WebSocket 状态为 ${state}`,
      firstFrameTimeout: '视频已开启，但首帧在 2 秒内尚未就绪',
      streamStopped: '画面流已停止',
      streamReplaced: '画面流已被替换',
      authorized: '画面已授权，正在等待后端会话就绪',
      creatingSession: '画面已授权，正在创建后端会话并启动记录',
      cancelled: '画面已授权，但后端会话创建已取消',
      noSessionId: '当前聊天还没有可用的后端会话 ID',
      videoNotReady: '预览已开启，但 videoWidth/videoHeight 仍为 0',
      encodeFailed: 'Canvas/JPEG 编码返回空结果',
      notifyRejected: '首帧未写入后端 FrameBuffer',
      creatorNotReady: '画面已授权，但主会话创建器尚未就绪'
    },
    voiceErrors: {
      streamingNotEnabled: '流式语音未启用（需在配置里填 dashscope_api_key）',
      sharedAudioSliceFailed: (reason: string) => `共享音频切片失败: ${reason}`,
      sharedAudioNotReceived: (reason: string) => `共享音频 ASR 未接收: ${reason}`,
      sharedAudioAsrFailed: (reason: string) => `共享音频 ASR 请求失败: ${reason}`,
      sharedAudioNoSamples: '共享音频没有收到有效采样，请检查 macOS"屏幕与系统音频录制"权限，然后停止并重新共享屏幕。',
      sharedAudioStartFailed: (detail: string) => `共享音频录制启动失败: ${detail}`,
      micPermissionDenied: '麦克风权限已被拒绝',
      turnMismatch: '语音轮次标识与后端不一致',
      sessionCreateFailed: '无法创建语音会话，请重试',
      connectionUnavailable: '语音连接不可用，请重试',
      asrEmpty: '未识别到语音，请重试或检查麦克风输入设备',
      asrFinishTimeout: '语音识别完成超时，请重试',
      asrDispatchFailed: '语音已识别，但发送失败，请重试',
      asrServiceUnavailable: '语音识别服务暂时不可用，请重试',
      asrTurnStale: '录音轮次已失效，请重新录音',
      submitFailedWithReason: (reason: string) => `语音未能提交（${reason}）`,
      submitFailedGeneric: '语音未能提交，请重试',
      recordingFinalizeFailed: '录音收尾失败，请重试',
      audioUploadInterrupted: '音频上传中断，请重试'
    },
    chat: {
      showVideo: '显示视频流',
      hideVideo: '隐藏视频流',
    ...
    misc: {
      ...
      callFailed: '调用失败',
      foundChars: (chars: number, clues: string) => `找到 ${chars} 字${clues}`,
      clues: (n: number) => ` · ${n} 条线索`,
      cannotEnableMonitor: '无法开启监控',
      cannotEnableResearch: '无法开启深度研究',
      cannotPauseResearch: '无法暂停深度研究'
    },
    memoryDebug: {
      panelTitle: '多模态 Memory Debug',
      panelSubtitle: '证据帧 · OCR · 实体 · 事件 · 演化 · Worker trajectory',
      refreshLabel: '刷新 Memory Debug',
      dbSelectLabel: 'Memory 数据库',
      noDbOption: '当前会话暂无 Memory 数据库',
      tabMemory: '本次记忆',
      tabFrame: '帧详情',
      tabSearch: '搜索',
      tabDebug: '高级 Debug',
      otherSessionNotice: '当前正在手动查看其他会话的 Memory；不会写入或影响当前对话。',
      noMemoryYet: '本会话尚未写入 Memory。开启摄像头或屏幕共享并产生观察后再刷新；也可以在上方手动选择历史数据库。',
      statFrames: '记忆帧',
      statEntities: '实体',
      statEvents: '事件',
      statEvolution: '演化记录',
      inDbTotal: (total: number | string) => `库内共 ${total}`,
      section1Title: '1. 本次留下了哪些帧',
      section1Hint: '仅显示真正进入 memory_frames 的证据帧。',
      viewAllAndOcr: '查看全部与 OCR',
      noFramesYet: '本次还没有写入记忆帧。',
      section2Title: '2. 本次有哪些实体',
      section2Hint: '人物、物体、地点及其属性、别名和证据帧。',
      noEntities: '尚未抽取出实体。',
      seenCount: (n: number) => `出现 ${n} 次`,
      aliases: (list: string) => `别名：${list}`,
      section3Title: '3. 本次发生了哪些事件',
      section3Hint: 'micro 是局部观察，macro 是阶段总结，super 是跨阶段叙事。',
      noEvents: '尚未形成事件。',
      section4Title: '4. 实体和事件如何演化',
      section4Hint: '按时间展示状态变化和 Reviewer 修订。',
      noEvolution: '尚无演化或修订记录。',
      searchLabel: '搜索 Memory',
      searchPlaceholder: '搜索 OCR、表格、事件和语音',
      scopeAll: '全部会话',
      scopeToday: '今天',
      scopeLatest: '所选数据库',
      searchButton: '搜索',
      searchEmpty: '输入关键词并点击搜索；搜索不会自动轮询。',
      trajectoryGroupTitle: 'Worker trajectory · 按问题分组',
      trajectoryGroupHint: '只按明确的 parent/client/task 映射归属问题；没有可靠关联的持续 Memory/OCR/Reviewer 事件不会按时间硬挂到最近问题。',
      trajectoryOtherSession: '历史 Memory 数据库没有持久化 live trajectory；为避免混淆，这里不会叠加当前会话的 Worker 轨迹。切回当前会话数据库即可查看实时 Debug。',
      trajectoryEmpty: '暂无 trajectory。开始采集、提问、Recall 或 Monitor 后会实时出现。',
      noRecallMessages: '没有持久化的 Recall 消息。',
      backgroundTrajectory: '持续后台 / 未关联事件',
      showEarlier: '收起较早事件',
      showAllEvents: (n: number) => `显示全部 ${n} 条`,
      noFrameImage: '没有帧图像',
      noDescription: '(暂无描述)',
      noExplanation: '(无说明)'
    }
