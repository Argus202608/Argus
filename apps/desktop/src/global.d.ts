...
declare global {
  interface Window {
    argusDesktop: {
      ...
      // Screen-share source picker (Part 2). renderer 主动拉 sources → 弹自己
      // 的选源 modal → set 一下, 后续 getDisplayMedia 就用这个源。
      multimodalSourcePicker: {
        listSources: () => Promise<{
          ok: boolean
          error?: string
          sources: Array<{
            id: string           // 'window:12345:0' / 'screen:0:0'
            name: string
            displayId: string
        ...
        setSelectedSource: (source: { id: string; name: string } | null) => Promise<{ ok: boolean; cleared?: boolean }>
        getSelectedSource: () => Promise<{ id: string; name: string } | null>
      }
...
export interface DesktopUpdateApplyResult {
   ...
   *  `argus update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  hermesRoot?: string
