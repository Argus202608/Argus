...
describe('fresh profile-session ownership', () => {
  it('invalidates pending capture synchronously before publishing the fresh-session request', () => {
    const captureWasInvalidated: boolean[] = []

    const unlisten = $freshSessionRequest.listen(() => {
      captureWasInvalidated.push(stopCaptureAndNotify.mock.calls.length > 0)
    })
