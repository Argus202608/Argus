...
function persistSessionPreviewRegistry(registry: SessionPreviewRegistry) {
  ...
  try {
    ...
    // An empty registry means "no previews registered" — localStorage-key-absent
    // already communicates that, so writing '{}' would be both wasteful and
    // semantically wrong (loadSessionPreviewRegistry treats missing-key as empty).
