...
const SKIN_KEY = 'argus-desktop-theme-v2'
const MODE_KEY = 'argus-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'argus-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'argus-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'argus-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  window.argusDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('argus-boot-background', chromeBg)
    window.localStorage.setItem('argus-boot-color-scheme', rendered)
  } catch {
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.argusDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
/** Sync the desktop skin with the active Argus backend theme on connect. */
