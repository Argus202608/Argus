...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { argusDesktop: { readDir: typeof readDir } }).argusDesktop = { readDir }
})
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { argusDesktop: unknown }).argusDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { argusDesktop: unknown }).argusDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.argusDesktop is missing', async () => {
    delete (window as unknown as { argusDesktop?: unknown }).argusDesktop
