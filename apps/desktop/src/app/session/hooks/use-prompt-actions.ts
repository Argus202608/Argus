...
import { ensureCaptureBoundToSession } from '@/store/multimodal-capture'
...
/** Every path that starts a model turn must cross the same visual-input
 * barrier. It is a no-op without an armed stream; with one, it waits until the
 * session owns the capture and its first frame has been acknowledged. */
async function ensurePromptCaptureBarrier(sessionId: string): Promise<void> {
  await ensureCaptureBoundToSession(sessionId)
}
...
export function usePromptActions({
  ...
  const submitPromptText = useCallback(
    async (rawText: string, options?: SubmitTextOptions) => {
      ...
      // A slash/skill dispatch can create the runtime and then call this
      // callback before React re-renders. The prop is stale in that same async
      // closure, while the session owner updates the ref synchronously.
      const liveSessionId = activeSessionIdRef.current || activeSessionId

      // One submit in flight per session — drop any concurrent re-fire so a
      // stalled turn can't stack the same prompt into multiple real turns.
      const submitLockKey = selectedStoredSessionIdRef.current || liveSessionId || '__pending_new__'
      ...
      let sessionId: null | string = liveSessionId
      ...
      try {
        // A fresh draft may already have an authorized camera/screen stream but
        // no backend session. Once the normal main-chat session exists, wait
        // for on-demand multimodal activation and enqueue its first frame before
        // prompt.submit. This guarantees that the first visual question and a
        // first-turn set_monitor both see a ready FrameBuffer/MonitorEngine.
        await ensurePromptCaptureBarrier(sessionId)
        ...
        // Multimodal per-turn deep-think flag (the 深度思考 toggle in the ChatBar).
        // Only send it when ON so a plain text turn's payload is unchanged.
        const deepThinking = $mmDeepThinking.get()

        const submitPayload = (sid: string) =>
          deepThinking
            ? { session_id: sid, text, deep_thinking: true, client_request_id: optimisticId }
            : { session_id: sid, text, client_request_id: optimisticId }
        ...
        try {
          ...
          if (isSessionNotFoundError(firstErr) && selectedStoredSessionIdRef.current) {
            ...
            if (recoveredId) {
              ...
              clearCaptureSessionTransferClaims()
              await ensurePromptCaptureBarrier(recoveredId)
  ...
  const executeSlashCommand = useCallback(
    async (rawCommand: string, options?: { sessionId?: string; recordInput?: boolean }) => {
      const ensureSessionId = async (sessionHint?: string): Promise<string | null> => {
        const existing = sessionHint || activeSessionIdRef.current

        if (existing) {
          return existing
        }

        // Slash/skill/quick-command execution can be the first action on a
        // fresh draft too. Give its newly-created runtime the same armed
        // camera/screen handoff that normal text submit receives.
        claimCaptureForNextMainSession()

        try {
          const created = await createBackendSessionForSend()
      ...
      if (isSessionNotFoundError(err) && selectedStoredSessionIdRef.current) {
        try {
          ...
          if (recoveredId) {
            ...
            setActiveSessionId(recoveredId)
            clearCaptureSessionTransferClaims()
            await ensureCaptureBoundToSession(recoveredId)
  ...
  const reloadFromMessage = useCallback(
    async (parentId: string | null) => {
      ...
      try {
        await ensurePromptCaptureBarrier(activeSessionId)
