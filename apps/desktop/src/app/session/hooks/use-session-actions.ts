...
import { deleteSession, getSession, getSessionMessages, setSessionArchived } from '@/argus'
...
import { stopCaptureAndNotify } from '@/store/multimodal-capture'
import { hasMicCaptureIntent, stopMic } from '@/store/multimodal-voice'
...
export function useSessionActions({
  ...
  const freshDraftEpochRef = useRef(0)
  const pendingSessionCreateCountRef = useRef(0)
  const createSessionInFlightRef = useRef<Promise<string | null> | null>(null)

  const startFreshSessionDraft = useCallback(
    (replaceRoute = false) => {
      freshDraftEpochRef.current += 1
      // A new draft may begin while the previous draft's backend create is
      // still pending. Detach its single-flight pointer; its epoch guard will
      // close the late runtime, while this draft can create independently.
      createSessionInFlightRef.current = null
      clearCaptureSessionTransferClaims()

      // This is also the cancellation token for an OS camera permission or
      // screen-source picker that has not resolved yet. At that point there is
      // no published source for isCapturing() to see, but a late grant must not
      // attach itself to the new draft.
  ...
  const createBackendSessionForSend = useCallback(
    async (preview: string | null = null): Promise<string | null> => {
      ...
      const pending = (async (): Promise<string | null> => {
        ...
        const startingFreshDraftEpoch = freshDraftEpochRef.current

        pendingSessionCreateCountRef.current += 1
        ...
        try {
          ...
          if (
            freshDraftEpochRef.current !== startingFreshDraftEpoch ||
          ...
          pendingSessionCreateCountRef.current = Math.max(
            0,
            pendingSessionCreateCountRef.current - 1
          )
          window.setTimeout(() => {
            creatingSessionRef.current = pendingSessionCreateCountRef.current > 0
          }, 0)
        }
      })()

      createSessionInFlightRef.current = pending

      try {
        return await pending
      } finally {
        if (createSessionInFlightRef.current === pending) {
          createSessionInFlightRef.current = null
        }
      }
    },
  ...
  const resumeSession = useCallback(
    async (storedSessionId: string, replaceRoute = false) => {
      ...
      const preservingSameConversation = previousStoredSessionId === storedSessionId

      if (!preservingSameConversation) {
        clearCaptureSessionTransferClaims()
        // A user-driven conversation switch is an ownership boundary even if
        // a reconnect has temporarily cleared the live runtime id. Release the
        // old media grant immediately; never keep a camera lit while a slow
        // profile swap/resume decides which different chat will own it.

        // Invalidate pending permission/picker work as well as an already-live
        // stream. Otherwise an old conversation's delayed OS grant can create
        // or bind capture after the user has selected this different chat.
        stopCaptureAndNotify()
        ...
        // Do not leave runtime A advertised while stored conversation B is
        // resolving or its profile gateway is swapping. Media controls bind to
        // the live id, so that overlap would let a fresh permission grant send
        // frames into A after the user had already selected B. Warm cached B is
        // published again below once its owning gateway is ready.
        setActiveSessionId(null)
        activeSessionIdRef.current = null
      }
