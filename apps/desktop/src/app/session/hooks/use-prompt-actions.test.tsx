...
const {
  ...
  return {
    cancelCaptureForNextMainSessionClaim: vi.fn(),
    claimCaptureForNextMainSession: vi.fn(),
    ensureCaptureBoundToSession: vi.fn<() => Promise<void>>(async () => undefined),
    mmDeepThinking: atom(false),
    stopCaptureAndNotify: vi.fn()
  }
})

vi.mock('@/store/multimodal', () => ({
  $mmDeepThinking: mmDeepThinking,
  cancelCaptureForNextMainSessionClaim,
  claimCaptureForNextMainSession,
  clearCaptureSessionTransferClaims: vi.fn(),
  preserveCaptureForNextRuntimeRebind: vi.fn()
}))
...
beforeEach(() => {
  cancelCaptureForNextMainSessionClaim.mockClear()
  claimCaptureForNextMainSession.mockClear()
  ensureCaptureBoundToSession.mockClear()
  ensureCaptureBoundToSession.mockResolvedValue(undefined)
  stopCaptureAndNotify.mockClear()
  mmDeepThinking.set(false)
})
...
function Harness({
  ...
  const actions = usePromptActions({
    ...
    createBackendSessionForSend: async preview => {
      const created = await (createBackendSessionForSend ?? (async () => RUNTIME_SESSION_ID))(preview)

      // The production session creator synchronously updates this live ref
      // before returning. Mirror that contract so same-closure slash dispatch
      // tests can distinguish R1 reuse from an accidental R2 creation.
      if (created) {
        activeSessionIdRef.current = created
      }
...
describe('usePromptActions first visual prompt ordering', () => {
  afterEach(() => {
    cleanup()
    ensureCaptureBoundToSession.mockReset()
    ensureCaptureBoundToSession.mockResolvedValue(undefined)
    $busy.set(false)
    vi.restoreAllMocks()
  })

  it('waits for capture activation and the acknowledged first frame before prompt.submit', async () => {
    ...
    ensureCaptureBoundToSession.mockReturnValueOnce(binding)
    ...
    const submit = handle!.submitText('画面里有什么？')
    await Promise.resolve()
    await Promise.resolve()

    expect(ensureCaptureBoundToSession).toHaveBeenCalledWith(RUNTIME_SESSION_ID)
    expect(requestGateway).not.toHaveBeenCalledWith('prompt.submit', expect.anything())

    finishBinding()
    await submit

    expect(order).toEqual(['prompt.submit'])
    expect(requestGateway).toHaveBeenCalledWith('prompt.submit', {
      client_request_id: expect.stringMatching(/^user-/),
      session_id: RUNTIME_SESSION_ID,
      text: '画面里有什么？'
    })
  })
...
describe('usePromptActions fresh slash capture claim', () => {
  afterEach(() => {
    cleanup()
    vi.restoreAllMocks()
  })

  it('claims an armed fresh draft before a quick command creates its runtime', async () => {
    const order: string[] = []

    claimCaptureForNextMainSession.mockImplementation(() => {
      order.push('claim')
    })
    ...
    const requestGateway = vi.fn(async (method: string) => {
      if (method === 'slash.exec') {
        return { output: 'quick command complete' } as never
      }
    ...
    await handle!.submitText('/my-quick-command')

    expect(order).toEqual(['claim', 'create'])
    expect(cancelCaptureForNextMainSessionClaim).not.toHaveBeenCalled()
    expect(requestGateway).toHaveBeenCalledWith('slash.exec', {
      command: 'my-quick-command',
      session_id: RUNTIME_SESSION_ID
    })
  })

  it('cancels the fresh-draft capture claim when slash session creation returns no runtime', async () => {
    ...
    expect(claimCaptureForNextMainSession).toHaveBeenCalledTimes(1)
    expect(cancelCaptureForNextMainSessionClaim).toHaveBeenCalledTimes(1)
    expect(requestGateway).not.toHaveBeenCalledWith('slash.exec', expect.anything())
  })

  it('cancels the fresh-draft capture claim when slash session creation throws', async () => {
    const createBackendSessionForSend = vi.fn(async () => {
    ...
    await expect(handle!.submitText('/my-quick-command')).resolves.toBe(true)
    expect(claimCaptureForNextMainSession).toHaveBeenCalledTimes(1)
    expect(cancelCaptureForNextMainSessionClaim).toHaveBeenCalledTimes(1)
  })

  it('reuses one fresh runtime when a skill dispatch submits its message', async () => {
    const runtimes = ['runtime-skill-R1', 'runtime-skill-R2']

    const createBackendSessionForSend = vi.fn(async () => (
      runtimes[createBackendSessionForSend.mock.calls.length - 1] ?? null
    ))
    ...
    const requestGateway = vi.fn(async (method: string) => {
      if (method === 'slash.exec') {
        return {
          type: 'skill',
          name: 'vision-helper',
          message: 'inspect the armed camera frame'
        } as never
      }
    ...
    await handle!.submitText('/vision-helper')

    expect(createBackendSessionForSend).toHaveBeenCalledTimes(1)
    expect(claimCaptureForNextMainSession).toHaveBeenCalledTimes(1)
    expect(cancelCaptureForNextMainSessionClaim).not.toHaveBeenCalled()
    expect(requestGateway).toHaveBeenCalledWith('slash.exec', {
      command: 'vision-helper',
      session_id: 'runtime-skill-R1'
    })
    expect(ensureCaptureBoundToSession).toHaveBeenCalledWith('runtime-skill-R1')
    expect(requestGateway).toHaveBeenCalledWith('prompt.submit', {
      client_request_id: expect.stringMatching(/^user-/),
      session_id: 'runtime-skill-R1',
      text: 'inspect the armed camera frame'
    })
    expect(stopCaptureAndNotify).not.toHaveBeenCalled()
  })
})
...
describe('usePromptActions visual barriers on prompt-submit siblings', () => {
  beforeEach(() => {
    $busy.set(false)
    $messages.set([
  ...
  it('waits for capture readiness before reloadFromMessage regenerates', async () => {
    let finishBinding!: () => void
    ...
    const reloading = handle!.reloadFromMessage('a1')
    await waitFor(() => expect(ensureCaptureBoundToSession).toHaveBeenCalledWith(RUNTIME_SESSION_ID))
    expect(requestGateway).not.toHaveBeenCalledWith('prompt.submit', expect.anything())
  ...
  it('waits for capture readiness before a restore rewind submits', async () => {
    let finishBinding!: () => void
    ...
    const restoring = handle!.restoreToMessage('u1')
    await waitFor(() => expect(ensureCaptureBoundToSession).toHaveBeenCalledWith(RUNTIME_SESSION_ID))
    expect(requestGateway).not.toHaveBeenCalledWith('prompt.submit', expect.anything())
  ...
  it('uploads file bytes via file.attach on a remote gateway and submits the rewritten ref', async () => {
    ...
    const requestGateway = vi.fn(async (method: string, params?: Record<string, unknown>) => {
      ...
      if (method === 'file.attach') {
        return {
          attached: true,
          path: '/remote/work/.argus/desktop-attachments/report.txt',
...
describe('usePromptActions eager attachment upload (drop-time)', () => {
  ...
  it('uploads a dropped file the moment it lands (active session) and rewrites the chip with the gateway ref', async () => {
    ...
    $connection.set({ mode: 'remote' } as never)
    const readFileDataUrl = vi.fn(async () => 'data:application/pdf;base64,JVBERi0=')
    Object.defineProperty(window, 'argusDesktop', { configurable: true, value: { readFileDataUrl } })
