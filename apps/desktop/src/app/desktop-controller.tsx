...
import { DesktopMmReadinessCard } from '@/components/desktop-mm-readiness-card'
import { DesktopOnboardingOverlay } from '@/components/desktop-onboarding-overlay'
import { GatewayConnectingOverlay } from '@/components/gateway-connecting-overlay'
import { ScreenSourcePickerHost } from '@/components/multimodal/screen-source-picker'
...
import { getCronJobs, getSessionMessages, listAllProfileSessions, type SessionInfo, triggerCronJob } from '../argus'
...
import { configureDraftCaptureSessionEnsurer } from '../store/multimodal-capture'
import { configureDraftMicSessionEnsurer } from '../store/multimodal-voice'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.argusDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.argusDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    const unsubscribe = window.argusDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.argusDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // argus:// deep links (plus the legacy hermes:// alias) from docs and integrations.
  ...
  useEffect(() => {
    const unsubscribe = window.argusDesktop?.onDeepLink?.(payload => {
    ...
    void window.argusDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.argusDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  const { refreshHermesConfig, sttEnabled } = useHermesConfig({
    activeSessionIdRef,
  ...
  useEffect(() => {
    const ensureDraftMainSession = async (): Promise<string | null> => {
      ...
      // A selected durable conversation with no live id is being resumed; it
      // is not a fresh draft. Never let a media button create an unrelated
      // runtime that can win the resume race and replace the selected chat.
    ...
    configureDraftCaptureSessionEnsurer(ensureDraftMainSession)
    configureDraftMicSessionEnsurer(ensureDraftMainSession)
  ...
  const overlays = (
    <>
      ...
      {!isSecondaryWindow() && (
        <DesktopMmReadinessCard
          active={gatewayState === 'open'}
          requestGateway={requestGateway}
        />
      )}
      ...
      <PaneMain>
        <Routes>
          ...
          <Route
            ...
            path="artifacts"
          />
          {/* Multimodal is merged into the main chat (ChatView right rail); the
              standalone page is retired. Redirect any lingering /multimodal
              deep-links to the main chat. */}
          <Route element={<Navigate replace to={NEW_CHAT_ROUTE} />} path="multimodal" />
