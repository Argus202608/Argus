...
import { HermesGateway } from '@/argus'
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.argusDesktop>['getConnection']>> | null
  ) => void
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    // composer stayed disabled forever on "Starting Argus...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Argus…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Argus gateway". resolveGatewayWsUrl
