...
// Argus' reasoning levels (VALID_REASONING_EFFORTS); `none` = thinking off.
// Empty config = Argus default (medium), shown as Medium.
...
// backend now surfaces the full `argus model` universe (every canonical
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `argus model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Argus runs the flow for you.`}
          </p>
        )}
