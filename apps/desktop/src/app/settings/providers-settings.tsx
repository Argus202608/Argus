...
import { disconnectOAuthProvider, listOAuthProviders } from '@/argus'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.argusDesktop?.terminal)
...
//      `argus model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Argus can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Argus can't clear via the API, but ships a
...
export function ProvidersSettings({ onClose, onViewChange, view }: ProvidersSettingsProps) {
  ...
  // Argus never deletes creds another tool owns behind a silent API call.
