...
import type {
  ...
  MmMemoryDebugSearchResponse,
  MmMemoryDebugSessionResponse,
  MmMemoryDebugSessionsResponse,
...
const MEMORY_DEBUG_REQUEST_TIMEOUT_MS = 60_000
...
export type {
  ...
  MessagingPlatformUpdate,
  MmMemoryDebugEntity,
  MmMemoryDebugEntityState,
  MmMemoryDebugEvent,
  MmMemoryDebugFrameResponse,
  MmMemoryDebugRevision,
  MmMemoryDebugSearchResponse,
  MmMemoryDebugSearchResult,
  MmMemoryDebugSessionResponse,
  MmMemoryDebugSessionsResponse,
  MmMemoryDebugSessionSummary,
  MmMemoryDebugTablePayload,
  MmMemoryDebugTimelineItem,
  MmMemoryDebugTraceMessage,
  MmMemoryDebugTraceResponse,
  MoaConfigResponse,
  MoaModelSlot,
  ModelAssignmentRequest,
...
export class HermesGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Argus gateway connection closed',
      connectErrorMessage: 'Could not connect to Argus gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Argus gateway is not connected',
...
export function getMultimodalMemoryDebugSession(
  db: string,
  params: { limit?: number; session_id?: string } = {}
): Promise<MmMemoryDebugSessionResponse> {
  const query = new URLSearchParams()
...
export function searchMultimodalMemoryDebug(
  queryText: string,
  params: { limit?: number; scope?: 'all' | 'latest' | 'today'; session?: string } = {}
): Promise<MmMemoryDebugSearchResponse> {
...
export function getMultimodalMemoryDebugTrace(
  params: { db?: string; limit?: number; session_id?: string } = {}
): Promise<MmMemoryDebugTraceResponse> {
  const query = new URLSearchParams()
