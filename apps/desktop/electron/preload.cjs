...
// Keep this computation inside the preload. Every app window is sandboxed, so
// Electron's preload `require` polyfill cannot load local CommonJS helpers.
// The sandboxed `process` global does expose platform + getSystemVersion().
function supportsDisplayMediaSystemAudio() {
  if (process.platform === 'win32') return true
  if (process.platform !== 'darwin') return false

  const match = String(process.getSystemVersion?.() || '').match(/^(\d+)/)
...
const argusDesktop = {
  ...
  screenShareSystemAudio: supportsDisplayMediaSystemAudio(),
  ...
  // ── Multimodal screen-share source picker (Part 2) ──────────────────────
  // 屏幕共享的"用户选窗口/屏幕"链路。renderer 先 listSources 拿列表, 用户在
  // modal 里挑一个, setSelectedSource 存到主进程; 之后调 getDisplayMedia 时,
  // 主进程的 setDisplayMediaRequestHandler 会用这个选择去回调。
  multimodalSourcePicker: {
    listSources: () => ipcRenderer.invoke('hermes:multimodal:listSources'),
    setSelectedSource: source => ipcRenderer.invoke('hermes:multimodal:setSelectedSource', source),
    getSelectedSource: () => ipcRenderer.invoke('hermes:multimodal:getSelectedSource'),
  },
