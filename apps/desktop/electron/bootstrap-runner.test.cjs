...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'argus', 'scripts')
...
test('resolveInstallScript prefers the bundled source (self-contained app) over everything', async () => {
  ...
  try {
    ...
    fs.writeFileSync(bundledScript, '#!/bin/sh\necho bundled\n')

    let downloadCalled = false
    const logs = []
    const result = await resolveInstallScript({
      installStamp: { commit: 'a'.repeat(40) }, // even with a stamp, bundled wins
      sourceRepoRoot: null,
      bundledSourceRoot: bundledRoot,
      hermesHome: home,
      emit: ev => logs.push(ev),
      _download: async () => {
        downloadCalled = true
        throw new Error('should not download when bundled source exists')
      }
    })

    assert.equal(result.source, 'bundled')
    assert.equal(result.path, bundledScript)
    assert.equal(result.bundledSource, bundledRoot)
    assert.equal(downloadCalled, false, 'must not touch the network')
  } finally {
    fs.rmSync(home, { recursive: true, force: true })
  }
})
