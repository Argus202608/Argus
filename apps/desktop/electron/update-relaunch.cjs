 ...
 * Background
 * ----------
 * After `argus update` + `argus desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything ARGUS_DESKTOP_* is preserved
// plus ARGUS_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['ARGUS_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['ARGUS_DESKTOP_']
