...
var require_display_media_streams = __commonJS({
  "electron/display-media-streams.cjs"(exports2, module2) {
    "use strict";
    var os = require("node:os");
    var MIN_MACOS_SYSTEM_AUDIO_MAJOR = 13;
    function numericMajor(version) {
      const match = String(version || "").trim().match(/^(\d+)/);
      return match ? Number.parseInt(match[1], 10) : null;
    }
    function resolveHostSystemVersion(platform = process.platform, electronProcess = process, kernelRelease = os.release()) {
      try {
    ...
    function supportsDisplayMediaSystemAudio(platform = process.platform, systemVersion = resolveHostSystemVersion(platform)) {
      if (platform === "win32") return true;
      if (platform !== "darwin") return false;
      const macOSMajor = numericMajor(systemVersion);
      return macOSMajor !== null && macOSMajor >= MIN_MACOS_SYSTEM_AUDIO_MAJOR;
    }
    function buildDisplayMediaStreams2(video, audioRequested, platform = process.platform, systemVersion = resolveHostSystemVersion(platform)) {
    ...
    module2.exports = {
      buildDisplayMediaStreams: buildDisplayMediaStreams2,
      resolveHostSystemVersion,
      supportsDisplayMediaSystemAudio
    };
  }
});
var require_api_request_diagnostics = __commonJS({
  "electron/api-request-diagnostics.cjs"(exports2, module2) {
    "use strict";
    var API_PENDING_LOG_MS2 = 5e3;
    var API_SLOW_LOG_MS2 = API_PENDING_LOG_MS2;
    function safeApiMethod2(method) {
      const normalized = String(method || "GET").trim().toUpperCase();
      return /^[A-Z]{1,16}$/.test(normalized) ? normalized : "UNKNOWN";
    }
    function safeApiPath2(rawPath) {
      try {
    ...
    function classifyApiRequestError2(error) {
    ...
    function formatApiRequestDiagnostic2({
      phase,
      requestId,
      method,
      requestPath,
      ...
      const parts = [
        ...
        `method=${safeApiMethod2(method)}`,
        `path=${safeApiPath2(requestPath)}`,
    ...
    module2.exports = {
      API_PENDING_LOG_MS: API_PENDING_LOG_MS2,
      API_SLOW_LOG_MS: API_SLOW_LOG_MS2,
      classifyApiRequestError: classifyApiRequestError2,
      formatApiRequestDiagnostic: formatApiRequestDiagnostic2,
      safeApiMethod: safeApiMethod2,
      safeApiPath: safeApiPath2,
      safeProfileLabel
    };
  }
});
...
var require_embed_referer = __commonJS2({
...
var require_bootstrap_platform = __commonJS2({
...
var require_bootstrap_runner = __commonJS2({
  "electron/bootstrap-runner.cjs"(exports2, module2) {
    ...
    async function resolveInstallScript({
      installStamp,
      sourceRepoRoot,
      bundledSourceRoot: bundledSourceRoot2,
      hermesHome,
      emit,
      _download = downloadInstallScript
    }) {
      if (bundledSourceRoot2) {
        const bundledScript = path2.join(bundledSourceRoot2, "scripts", installScriptName());
        try {
          fs2.accessSync(bundledScript, fs2.constants.R_OK);
          emit({ type: "log", line: `[bootstrap] using bundled source at ${bundledSourceRoot2}` });
          return { path: bundledScript, source: "bundled", kind: installScriptKind(), bundledSource: bundledSourceRoot2 };
        } catch {
          emit({ type: "log", line: `[bootstrap] bundledSourceRoot set but ${bundledScript} not readable; falling through` });
        }
      }
    ...
    function buildPinArgs(installStamp, bundledSource) {
      const args = [];
      if (bundledSource) {
        args.push("-LocalSource", bundledSource);
        return args;
      }
    ...
    async function fetchManifest({ scriptPath, installerKind, emit, hermesHome, activeRoot, installStamp, bundledSource }) {
      const isPosix = installerKind === "posix";
      const args = isPosix ? ["--manifest", ...buildPosixPinArgs({ installStamp, activeRoot, hermesHome, bundledSource })] : ["-Manifest", ...buildPinArgs(installStamp, bundledSource)];
...
${result.stdout}`
    ...
    async function runStage({ scriptPath, installerKind, stage, emit, hermesHome, activeRoot, abortSignal, installStamp, bundledSource }) {
      ...
      const args = isPosix ? [
        ...
        "--non-interactive",
        "--json",
        ...buildPosixPinArgs({ installStamp, activeRoot, hermesHome, bundledSource })
      ] : ["-Stage", stage.name, "-NonInteractive", "-Json", ...buildPinArgs(installStamp, bundledSource)];
      const result = await (isPosix ? spawnBash : spawnPowerShell)(scriptPath, args, {
        emit,
...
`);
      ...
      try {
        const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, bundledSourceRoot, hermesHome, emit });
        const bundledSource = scriptInfo.bundledSource || null;
...
var require_session_windows = __commonJS2({
...
var require_backend_probes = __commonJS2({
...
var require_link_title_window = __commonJS2({
...
var require_gateway_ws_probe = __commonJS2({
...
var require_dashboard_token = __commonJS2({
...
var require_backend_ready = __commonJS2({
...
var require_oauth_net_request = __commonJS2({
...
var require_vscode_marketplace = __commonJS2({
...
var require_backend_env = __commonJS2({
...
var require_windows_user_env = __commonJS2({
...
var require_wsl_clipboard_image = __commonJS2({
...
var require_titlebar_overlay_width = __commonJS2({
...
var require_hardening = __commonJS2({
...
var require_fs_read_dir = __commonJS2({
...
var require_update_marker = __commonJS2({
...
var require_update_relaunch = __commonJS2({
...
var require_git_root = __commonJS2({
...
var require_git_worktree_ops = __commonJS2({
...
var require_git_review_ops = __commonJS2({
...
var require_git_repo_scan = __commonJS2({
...
var require_update_count = __commonJS2({
...
var require_update_rebuild = __commonJS2({
...
var require_desktop_uninstall = __commonJS2({
...
var require_workspace_cwd = __commonJS2({
...
var require_window_state = __commonJS2({
...
var require_connection_config = __commonJS2({
...
var { buildDisplayMediaStreams } = require_display_media_streams();
...
var {
  ...
  safeApiMethod,
  safeApiPath
} = require_api_request_diagnostics();
var nodePty = null;
var nodePtyDir = null;
try {
...
if (IS_MAC && !IS_PACKAGED) {
  ...
  console.log("[multimodal] macOS dev mode: using legacy screen-share system-audio capture path");
}
...
async function applyUpdatesPosixInApp() {
  ...
  const rebuiltApp = [
    path.join(updateRoot, "apps", "desktop", "release", "mac-arm64", "Argus.app"),
    path.join(updateRoot, "apps", "desktop", "release", "mac", "Argus.app")
      ...
      (res) => {
        ...
        res.on("end", () => {
          ...
          if ((res.statusCode || 500) >= 400) {
            safeReject(new Error(`${res.statusCode}: ${text || res.statusMessage}`));
            return;
          }
          if (!text) {
            safeResolve(null);
          ...
          try {
            ...
            safeReject(new Error(`Invalid JSON from ${url} (status ${res.statusCode}): ${text.slice(0, 200)}`));
          }
        });
      }
    );
...
function getTrayIconPath() {
  ...
  return candidates.find(fileExists) || getAppIconPath();
}
...
function createTray() {
  ...
  try {
    ...
    console.warn("[tray] failed to create:", err?.message || err);
    tray = null;
  }
}
...
function installMediaPermissions() {
  ...
  session.defaultSession.setPermissionCheckHandler((_webContents, permission, _origin, _details) => {
    return permission === "media" || permission === "audioCapture";
  });
}
var pickedSource = null;
function _sourceToPayload(s) {
  let thumb = "";
  try {
  ...
  return {
    ...
    appIconDataUrl: (() => {
      try {
        return s.appIcon && !s.appIcon.isEmpty() ? s.appIcon.toDataURL() : "";
      } catch {
        return "";
      }
    })(),
...
function installMultimodalSourceIpc() {
  ipcMain.handle("hermes:multimodal:listSources", async () => {
    try {
      ...
      return { ok: true, sources: (sources || []).map(_sourceToPayload) };
    } catch (e) {
      console.warn("[multimodal] listSources \u5931\u8D25:", e?.message || e);
      return { ok: false, error: String(e?.message || e), sources: [] };
    }
  });
  ipcMain.handle("hermes:multimodal:setSelectedSource", (_evt, payload) => {
  ...
  ipcMain.handle("hermes:multimodal:getSelectedSource", () => {
    return pickedSource ? { ...pickedSource } : null;
  });
}
      ...
      async (_request, callback) => {
        ...
        try {
          ...
          if (!sources || sources.length === 0) {
            console.warn("[multimodal] getSources \u8FD4\u56DE\u7A7A \u2014\u2014 macOS \u591A\u534A\u662F\u5C4F\u5E55\u5F55\u5236\u6743\u9650\u672A\u6388\u4E88 (Electron)\u3002");
            answer(null);
            return;
          }
          let chosen = null;
          if (pickedSource && pickedSource.id) {
            chosen = sources.find((s) => s.id === pickedSource.id) || null;
            if (!chosen) {
              console.warn("[multimodal] pickedSource id \u5DF2\u5931\u6548, \u56DE\u843D screen[0]:", pickedSource.id);
            }
          }
          if (!chosen) {
            chosen = sources.find((s) => s.id.startsWith("screen:")) || sources[0];
          }
          answer(buildDisplayMediaStreams(
            chosen,
            _request?.audioRequested === true,
            process.platform
          ));
        } catch (e) {
          console.warn(
            "[multimodal] desktopCapturer.getSources \u5931\u8D25:",
            e?.message || e,
            isMac ? "\u2192 macOS \u8BF7\u5728 \u7CFB\u7EDF\u8BBE\u7F6E\u2192\u9690\u79C1\u4E0E\u5B89\u5168\u2192\u5C4F\u5E55\u5F55\u5236 \u7ED9 Electron \u6388\u6743\u540E\u91CD\u542F app\u3002" : ""
          );
          answer(null);
        }
      },
      isMac ? void 0 : { useSystemPicker: true }
    );
  } catch (err) {
    console.warn("[multimodal] setDisplayMediaRequestHandler unavailable:", err?.message || err);
  }
}
...
var hermesApiDebugSequence = 0;
function emitHermesApiDiagnostic(message) {
  console.warn(message);
  rememberLog(message);
}
ipcMain.handle("hermes:api", async (_event, request) => {
  const requestId = `api-${++hermesApiDebugSequence}`;
  const method = safeApiMethod(request?.method);
  const profile = request?.profile;
  const timeoutMs = resolveTimeoutMs(request?.timeoutMs, DEFAULT_FETCH_TIMEOUT_MS);
  const startedAt = Date.now();
  let debugPath = request?.path;
  let debugStage = "routing";
  const diagnosticLine = (phase, extra = {}) => formatApiRequestDiagnostic({
    phase,
    requestId,
    method,
    requestPath: debugPath,
    profile,
    stage: debugStage,
  ...
  const pendingTimer = setTimeout(() => {
    emitHermesApiDiagnostic(diagnosticLine("pending"));
  }, API_PENDING_LOG_MS);
  try {
    const rerouted = await interceptSessionRequestForRemote(request);
    if (rerouted !== void 0) {
      debugStage = "remote_reroute";
      const elapsedMs = Date.now() - startedAt;
      if (elapsedMs >= API_SLOW_LOG_MS) {
        emitHermesApiDiagnostic(diagnosticLine("complete"));
      }
      return rerouted;
    }
    ...
    debugStage = "backend";
    const connection = await ensureBackend(profile);
    debugPath = pathWithGlobalRemoteProfile(request.path, profile, {
      globalRemote: globalRemoteActive(),
      profileRemoteOverride: profileHasRemoteOverride(profile)
    });
    debugStage = "http";
    const url = `${connection.baseUrl}${debugPath}`;
    let result;
    if (connection.authMode === "oauth") {
      result = await fetchJsonViaOauthSession(url, {
        method: request?.method,
        body: request?.body,
        timeoutMs
      });
    } else {
      result = await fetchJson(url, connection.token, {
        method: request?.method,
        body: request?.body,
        timeoutMs
      });
    }
    if (Date.now() - startedAt >= API_SLOW_LOG_MS) {
      emitHermesApiDiagnostic(diagnosticLine("complete"));
    }
    return result;
  } catch (error) {
    emitHermesApiDiagnostic(diagnosticLine("failed", { errorKind: classifyApiRequestError(error) }));
    throw error;
  } finally {
