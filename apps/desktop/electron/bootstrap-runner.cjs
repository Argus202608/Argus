 ...
 *     hermesHome,          // ARGUS_HOME
 *     logRoot,             // ARGUS_HOME/logs
...
// checkout under $ARGUS_HOME/argus. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(hermesHome) {
  if (!hermesHome) return null
  const candidate = path.join(hermesHome, 'argus', 'scripts', installScriptName())
  try {
    fs.accessSync(candidate, fs.constants.R_OK)
    return candidate
  } catch {
...
async function resolveInstallScript({
  ...
  if (bundledSourceRoot) {
    const bundledScript = path.join(bundledSourceRoot, 'scripts', installScriptName())
    try {
      fs.accessSync(bundledScript, fs.constants.R_OK)
      emit({ type: 'log', line: `[bootstrap] using bundled source at ${bundledSourceRoot}` })
      return {
        path: bundledScript,
        source: 'bundled',
        kind: installScriptKind(),
        bundledSource: bundledSourceRoot
      }
    } catch {
      emit({
        type: 'log',
        line: `[bootstrap] bundledSourceRoot set but ${bundledScript} not readable; falling through`
      })
    }
  }
...
function buildPinArgs(installStamp, bundledSource) {
  const args = []
  // Local-source (self-contained app) wins: copy the bundled tree, don't clone
  // from GitHub, so -Commit/-Branch are irrelevant (and the commit likely isn't
  // on GitHub anyway — that's the whole reason for bundling).
  if (bundledSource) {
...
async function fetchManifest({ scriptPath, installerKind, emit, hermesHome, activeRoot, installStamp, bundledSource }) {
  ...
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, hermesHome, bundledSource })]
    : ['-Manifest', ...buildPinArgs(installStamp, bundledSource)]
...
async function runBootstrap(opts) {
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({
      installStamp, sourceRepoRoot, bundledSourceRoot, hermesHome, emit
    })
