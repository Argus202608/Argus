...
test('parseRegQueryValue matches the name case-insensitively', () => {
  const out = 'HKEY_CURRENT_USER\\Environment\r\n    Argus_Home    REG_EXPAND_SZ    %USERPROFILE%\\h\r\n'
  assert.equal(parseRegQueryValue(out, 'ARGUS_HOME'), '%USERPROFILE%\\h')
})
...
test('parseRegQueryValue returns null when the value line is absent', () => {
  const out = 'HKEY_CURRENT_USER\\Environment\r\n    Path    REG_SZ    C:\\x\r\n'
  assert.equal(parseRegQueryValue(out, 'ARGUS_HOME'), null)
  assert.equal(parseRegQueryValue('', 'ARGUS_HOME'), null)
  assert.equal(parseRegQueryValue('garbage', 'ARGUS_HOME'), null)
})
...
test('readWindowsUserEnvVar returns null off Windows without spawning', () => {
  ...
  assert.equal(readWindowsUserEnvVar('ARGUS_HOME', { platform: 'linux', exec }), null)
...
test('readWindowsUserEnvVar returns null when reg exits non-zero (value missing)', () => {
  ...
  assert.equal(readWindowsUserEnvVar('ARGUS_HOME', { platform: 'win32', exec }), null)
})

test('readWindowsUserEnvVar returns null for an empty value', () => {
  const exec = () => '    ARGUS_HOME    REG_SZ    \r\n'
  assert.equal(readWindowsUserEnvVar('ARGUS_HOME', { platform: 'win32', exec }), null)
})
