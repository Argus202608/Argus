...
test('resolveUnpackedRelease is null for AppImage / .deb / .rpm / dev / unresolved paths', () => {
  ...
  assert.equal(resolveUnpackedRelease('/opt/Argus/hermes', ROOT, 'linux'), null)
  // dev electron
  assert.equal(
...
test('collectRelaunchArgs drops Electron internals, keeps user/launcher args', () => {
  const argv = [
    ...
    '--lang=en-US',
    'argus://open/agent/42', // deep link — keep
    '--profile=work', // app flag — keep
    '--remote-debugging-port=9222' // internal — drop
  ]
  assert.deepEqual(collectRelaunchArgs(argv), ['--no-sandbox', 'argus://open/agent/42', '--profile=work'])
...
test('buildRelaunchScript embeds pid/exec/args/env/cwd and is valid bash', () => {
  const script = buildRelaunchScript({
    pid: 4242,
    execPath: '/home/u/.argus/argus/apps/desktop/release/linux-unpacked/Argus',
    args: ['argus://open/agent/42', "--note=it's fine"],
  ...
  assert.match(script, /exec '.*\/linux-unpacked\/Argus' 'argus:\/\/open\/agent\/42' '--note=it'\\''s fine'/)
...
test('buildRelaunchScript with no args/env still lints clean', () => {
  ...
  assert.match(script, /exec '\/opt\/Argus\/Argus'\n/)
})
