 ...
 *
 * Stamps the Argus icon + identity onto the packed Windows Argus.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `argus desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Argus')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Argus'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Argus.exe keeps the stock Electron icon`)
  }
}
