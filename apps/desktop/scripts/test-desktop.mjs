...
// no longer bundles the Argus Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Argus.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Argus'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Argus.exe'),
  ...
  return {
    appPath: unpacked,
      binary: path.join(unpacked, 'Argus'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default Argus home for non-sandboxed runs -- matches main.cjs's resolver.
// The fresh-install sandbox launchFresh() sets its own ARGUS_HOME and never
// touches this directory.
const DEFAULT_ARGUS_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'argus')
  }
  return path.join(os.homedir(), '.argus')
})()
const VENV_ROOT = path.join(DEFAULT_ARGUS_HOME, 'argus', 'venv')
...
function ensurePackagedApp() {
  if (process.env.ARGUS_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Argus-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Argus-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Argus-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Argus-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.ARGUS_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.ARGUS_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const argusHome = path.join(sandbox, 'argus-home')
  ...
  env.ARGUS_DESKTOP_CWD = cwd
  env.ARGUS_DESKTOP_IGNORE_EXISTING = '1'
  env.ARGUS_DESKTOP_TEST_MODE = 'fresh-install'
  env.ARGUS_DESKTOP_USER_DATA_DIR = userDataDir
  env.ARGUS_HOME = argusHome
  delete env.ARGUS_DESKTOP_ARGUS
  delete env.ARGUS_DESKTOP_ARGUS_ROOT
  delete env.ARGUS_DESKTOP_HERMES
  delete env.ARGUS_DESKTOP_HERMES_ROOT
  ...
  console.log(`  ARGUS_HOME: ${argusHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(argusHome, 'argus', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Argus Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  const staleFactoryMarkers = [
    path.join(APP.resourcesPath, 'argus', 'hermes_cli', 'main.py'),
    path.join(APP.resourcesPath, 'hermes-agent', 'hermes_cli', 'main.py')
  ]
  for (const staleFactoryMarker of staleFactoryMarkers) {
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Argus
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + ARGUS_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  ARGUS_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
