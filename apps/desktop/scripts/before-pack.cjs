 ...
 * product name (`Argus`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Argus'
 *
 * This is a hard failure with no obvious cause for the user — `argus desktop`
 ...
 * (the mac-unpacked Argus.app bundle) and Windows (win-unpacked), so we
