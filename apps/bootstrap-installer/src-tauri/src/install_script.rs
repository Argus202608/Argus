...
//! Electron app's APP_ROOT/../.. trick, because Argus-Setup.exe is meant
...
///
/// `pin` is the commit-or-branch from either Argus-Setup's build-time
...
async fn download(kind: ScriptKind, commit_or_ref: &str, dest_path: &Path) -> Result<()> {
    let url = format!(
        "https://raw.githubusercontent.com/MMArgus-Team/Argus/{}/scripts/{}",
        commit_or_ref,
        kind.filename()
    );
    ...
    let response = reqwest::Client::new()
        .get(&url)
        .header("User-Agent", "argus-setup/0.0.1")
        .send()
        .await
