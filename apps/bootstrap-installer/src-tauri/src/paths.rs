...
//!   macOS:   ~/.argus
//!   Linux:   ~/.argus  (override via $ARGUS_HOME)
//!
//! NOTE (macOS): Python's get_hermes_home(), scripts/install.sh, and the
//! Electron desktop's resolveHermesHome() ALL use ~/.argus on macOS — there
...
pub fn hermes_home() -> PathBuf {
    ...
    {
        // %LOCALAPPDATA%\argus — matches scripts/install.ps1's $ArgusHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("argus");
        }
    }

    // macOS + Linux + fallback: ~/.argus (matches Python get_hermes_home(),
    // install.sh, and the Electron desktop's resolveHermesHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".argus");
    }
...
/// hermes-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\argus\argus-setup.exe`; on other
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[argus-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
