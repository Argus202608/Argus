...
export async function fetchJSON<T>(
  ...
  if (res.status === 401) {
    ...
    // (``argus update``, ``argus gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__ARGUS_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__HERMES_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      ...
      try {
        alreadyReloaded =
          sessionStorage.getItem("argus.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("argus.tokenReloadAttempted", "1");
        } catch {
  ...
  if (res.ok) {
    ...
    // current ``window.__ARGUS_SESSION_TOKEN__`` is valid, so the next
    // 401 — if any — should be allowed to trigger its own reload cycle.
    try {
      sessionStorage.removeItem("argus.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode — ignore */
    }
  }
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__ARGUS_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the Argus dashboard server");
}
 ...
 * ``window.__ARGUS_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  getSessions: (
    ...
    // Comma-separated source classes to drop (e.g. "tool,cron"). The gateway's
    // own session.list deny-lists "tool" subagent runs; the REST endpoint does
    // NOT by default, so callers that surface human chats must pass it.
    excludeSources?: string,
  ) =>
    fetchJSON<PaginatedSessions>(
      appendProfileParam(
        `/api/sessions?limit=${limit}&offset=${offset}&order=${order}` +
          (excludeSources ? `&exclude_sources=${encodeURIComponent(excludeSources)}` : ""),
        profile,
      ),
    ),
  ...
  getMultimodalMemoryDebugSessions: (limit = 50) =>
    fetchJSON<MmMemoryDebugSessionsResponse>(
      `/api/multimodal/memory/debug/sessions?limit=${limit}`,
    ),
  getMultimodalMemoryDebugSession: (
    ...
    return fetchJSON<MmMemoryDebugSessionResponse>(
      `/api/multimodal/memory/debug/session/${encodeURIComponent(db)}${suffix}`,
    );
  },
  getMultimodalMemoryDebugFrame: (db: string, frameId: string) =>
    fetchJSON<MmMemoryDebugFrameResponse>(
      `/api/multimodal/memory/debug/session/${encodeURIComponent(db)}/frame/${encodeURIComponent(frameId)}`,
    ),
  searchMultimodalMemoryDebug: (
    query: string,
    params: { scope?: "latest" | "today" | "all"; session?: string; limit?: number } = {},
  ) => {
    const qs = new URLSearchParams();
    qs.set("query", query);
    ...
    if (params.limit) qs.set("limit", String(params.limit));
    return fetchJSON<MmMemoryDebugSearchResponse>(
      `/api/multimodal/memory/debug/search?${qs.toString()}`,
    );
  },
  getMultimodalMemoryDebugTrace: (
    params: { session_id?: string; db?: string; limit?: number } = {},
  ) => {
    const qs = new URLSearchParams();
    if (params.session_id) qs.set("session_id", params.session_id);
    if (params.db) qs.set("db", params.db);
    if (params.limit) qs.set("limit", String(params.limit));
    return fetchJSON<MmMemoryDebugTraceResponse>(
      `/api/multimodal/memory/debug/trace?${qs.toString()}`,
    );
  },
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``argus update``. */
