...
def sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for msg in messages:
        ...
        if role not in _ra().AIAgent._VALID_API_ROLES:
            ...
            continue
        # Gateway-internal marker: QueryWorker completions are inserted into
        # model history as an otherwise ordinary user/assistant Q/A pair. The
        # provider must see only those standard fields so the main agent has no
        # indication that another worker produced its earlier answer.
        if "_mm_query_projection" in msg:
            msg = msg.copy()
            msg.pop("_mm_query_projection", None)
        filtered.append(msg)
    messages = filtered
...
def apply_pending_steer_to_tool_results(agent, messages: list, num_tool_msgs: int) -> None:
    ...
    """
    if getattr(agent, "_suppress_tool_result_steer_drain", False):
        return
