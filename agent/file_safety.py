...
def _hermes_home_path() -> Path:
    ...
    try:
        ...
        return Path(os.path.expanduser("~/.argus"))


def _hermes_root_path() -> Path:
    """Resolve the Argus root dir (always the parent of any profile, never per-profile)."""
    try:
...
def get_safe_write_roots() -> set[str]:
    """Return resolved ARGUS_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("ARGUS_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Argus path.

    Three categories are blocked:

      * Internal Argus cache files under ``HERMES_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under HERMES_HOME and the global Argus
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.argus/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Argus root so credential stores at <root>/auth.json etc. are also
    ...
    for hd in hermes_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Argus cache file "
    ...
    for hd in hermes_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Argus credential store "
    ...
    for hd in hermes_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Argus MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Argus MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# Argus profiles are separate HERMES_HOME dirs under
...
# edited skills under both ``~/.argus/profiles/hermes-security/skills/``
# AND ``~/.argus/skills/`` (the default profile's skills) without realizing
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Argus scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Argus entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Argus profile {info['target_profile']!r}, but the "
...
#
#   <HERMES_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.argus/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.argus/…`` segment and warns
# regardless of which Argus profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.argus/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    ...
    index where the inner Argus-state portion starts. Returns ``None`` for
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Argus state.
    ...
    Detection is path-shape-only — does not require any Argus resolver to
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Argus process never "
...
# full ``…/sandboxes/<backend>/<task>/home/.argus/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.argus/…``.  The root:root
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Argus state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Argus "
