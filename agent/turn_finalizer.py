...
def finalize_turn(
    ...
    *,
    ...
    ephemeral_control = bool(
        ...
        and handoff.get("control") == "handoff"
        and handoff.get("history_policy") == "ephemeral_control"
    )
    deferred_worker_reply = bool(
        isinstance(handoff, dict)
        and handoff.get("control") == "handoff"
        and handoff.get("handoff_mode") == "deferred_reply"
    )
    ephemeral_internal_turn = bool(
        getattr(agent, "_ephemeral_internal_turn", False)
    )
    # These control/internal turns are not ordinary user turns from the main
    # conversation's point of view. Monitor control owns no conversational Q/A,
    # a deferred QueryWorker later projects the real Q/A pair, and a watcher
    # completion hook exposes only its Assistant result. None may transform,
    # sync, or review its hidden scheduling/control prompt as a user message.
    suppress_completion_side_effects = (
        ephemeral_control or deferred_worker_reply or ephemeral_internal_turn
    )
    ...
    try:
        ...
        # Deferred worker turns and pure control-plane turns never commit their
        # working suffix to canonical/provider history.  A deferred worker adds
        # a complete Q/A projection only after it has a real answer; an
        # ephemeral control turn (Monitor CRUD) lives in its registry/event
        # side channel instead.  The gateway already deferred crash-resilience
        # writes, so this is exclusion-before-commit rather than mutating a
        # previously persisted prompt prefix.
        if (ephemeral_internal_turn
                or (
                    isinstance(handoff, dict)
                    and handoff.get("control") == "handoff"
                    and (
                        handoff.get("handoff_mode") == "deferred_reply"
                        or handoff.get("history_policy") == "ephemeral_control"
                    )
                )):
    ...
    if (model_called and not suppress_completion_side_effects
    ...
    if not interrupted and not deferred_worker_reply:
        try:
    ...
    last_reasoning = None
    if not suppress_completion_side_effects:
    ...
    if isinstance(handoff, dict) and handoff.get("control") == "handoff":
        # Gateway surfaces use this to distinguish a deterministic dispatch
        # receipt from a deferred worker-owned answer.  It is response metadata,
        # not a synthetic history message, so it does not perturb prompt caching.
    ...
    if sync_external_memory and not suppress_completion_side_effects:
    ...
    if not deferred_worker_reply and not ephemeral_internal_turn:
        try:
            from hermes_cli.plugins import invoke_hook as _invoke_hook
            _invoke_hook(
                "on_session_end",
