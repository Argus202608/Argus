...
async def _send_rag_request(
    client: httpx.AsyncClient, payload: dict, task_name: str
) -> List[dict]:
    ...
    for attempt in range(3):
        ...
        try:
            response = await client.post(url=RAG_API_URL, headers=HEADERS, json=payload)
            ...
            logging.warning(
                f"[{task_name}] Unexpected response type: {type(resp_json)}"
            )
            return []
...
async def text_search_observation(query: str, text_search_keys=["google", "xhs"]):
    ...
    for i, obs in enumerate(obs_list):
        ...
        txt_list.append(
            f"网页{i + 1}. 相关性: {score:.2f} 标题: {title} 内容: {content}"
        )
