"""Anthropic Messages API adapter for Argus.
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    # Strip any trailing /v1 or /v1/messages — the Anthropic SDK appends
    # /v1/messages. Some internal configs point directly at that leaf endpoint.
    normalized_base_url = _normalize_base_url_text(base_url)
    if normalized_base_url:
        import re as _re
        normalized_base_url = _re.sub(
            r"/v1(?:/messages)?/?$", "", normalized_base_url.rstrip("/"))
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``argus models``) never mutates ``~/.argus/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      ...
      4. Anthropic credential_pool OAuth entry (~/.argus/auth.json)
...
# Stores credentials in ~/.argus/.anthropic_oauth.json (our own file).
...
def read_hermes_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Hermes-managed OAuth credentials from ~/.argus/.anthropic_oauth.json."""
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Argus", "Claude Code")
                text = text.replace("Argus", "Claude Code")
