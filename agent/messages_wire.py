...
for every payload dimension.  Some hybrid gateways use a ``/v1/messages``
leaf, accept OpenAI-style ``image_url`` parts, and require Messages-style
``tool_use``/``tool_result`` history plus Anthropic top-level tool declarations.
Keep this decision shared by the main-agent and multimodal clients so the two
call paths cannot silently diverge.
"""
...
def uses_anthropic_messages_wire(
    ...
    *,
    ...
    ``/v1/messages`` alone is deliberately not treated as proof: compatible
    gateways commonly reuse that path with OpenAI-style content parts. Explicit
    API mode wins; otherwise only well-known Anthropic hosts or an ``/anthropic``
    route select Anthropic content blocks.
    """
...
def uses_anthropic_tools_wire(
    ...
    *,
    ...
    separate protocol dimensions on a hybrid gateway.  The direct
...
def hybrid_messages_from_chat(
    ...
    *,
    ...
    """Build history for a hybrid ``/v1/messages`` gateway.
