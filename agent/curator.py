...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.argus/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `argus update` re-seeds.
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``argus update``. On first observation we seed ``last_run_at`` to "now"
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `argus curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Argus' background skill CURATOR. This is an "
    ...
    "into ~/.argus/skills/.archive/) is the maximum destructive action. "
    ...
    "~/.argus/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.argus/logs/curator/``)
    ...
    authored skill data in ``~/.argus/skills/``.
...
def _build_rename_summary(
    *,
    ...
    lines.append("full report: argus curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: argus curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.argus/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.argus/skills/.archive/` for "
            "safety and can be restored via `argus curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.argus/skills/.archive/`. "
            "Restore any via `argus curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `argus curator restore <name>`")
    lines.append("- All archives live under `~/.argus/skills/.archive/` and are recoverable by `mv`")
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `argus update` only if "
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``argus model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `argus model` → auxiliary
