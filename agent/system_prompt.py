...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Argus never re-renders parts of this string mid-
    ...
    # Pointer to the hermes-agent skill + docs for user questions about Argus itself.
    ...
    # Multimodal live-stream guidance: include only when at least one capability
    # it describes is actually visible. Tool availability is fixed for the
    ...
    # ARGUS_KANBAN_TASK env var). Normal chat sessions never see
    ...
    if tool_guidance:
        # ★ 用 "\n\n" 而非 " " 分隔各 guidance 块: 每块 (MEMORY / SESSION_SEARCH /
        #   MM_LIVE / SKILLS / KANBAN) 是独立优化的完整段落, 单空格拼接会把块边界挤成
        #   一行、彼此糊在一起, 模型分不清"这是另一个主题"。段落级分隔跟 stable_parts
        #   自身的 "\n\n".join 保持一致。
        stable_parts.append("\n\n".join(tool_guidance))
    ...
    # Coding posture (base Argus, any interactive coding surface in a code
    ...
    #
    # Skipped when the session opted out of context files (skip_context_files):
    # source="tool" sessions (multimodal video chat over WS) launch the gateway
    # from inside this dev repo, so without the guard they inherit a Workspace
    # block that tells the vision assistant to "re-check with git before acting"
    # — which makes it run `git status` in response to plain "你好". Same skip
    # signal already used to hide AGENTS.md there.
    if agent.valid_tool_names and not getattr(agent, "skip_context_files", False):
        try:
    ...
    # Active-profile hint — names the Argus profile the agent is running
    # under so it doesn't conflate ~/.argus/skills/ (default profile) with
    # ~/.argus/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Argus profile: default. Other profiles (if any) live "
            "under ~/.argus/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Argus profile: {active_profile}. This session reads "
            f"and writes ~/.argus/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.argus/skills/, ~/.argus/plugins/, "
            f"~/.argus/cron/, ~/.argus/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Argus never rebuilds or reinjects parts of it
