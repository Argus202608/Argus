...
def redact_url_userinfo(text: str) -> str:
    """Public wrapper: strip ``user:password@`` from URLs in *text*.

    Deliberately NOT part of :func:`redact_sensitive_text` — see the NOTE near
    the end of that function: blanket URL redaction breaks magic-link and
    OAuth-callback workflows the agent is meant to follow.

    Callers that render a value into a UI surface (rather than pass it to a
    tool) can opt in per-value. ``agent.display.describe_arg_fields`` does
    exactly this: URL userinfo is the one credential shape the global
    redactor provably lets through, so the display layer plugs it locally
    without changing what tools receive.
    """
