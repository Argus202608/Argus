...
Run standalone for debugging::
    ...
    ARGUS_MEET_URL=https://meet.google.com/abc-defg-hij \\
    ARGUS_MEET_OUT_DIR=/tmp/meet-debug \\
    ARGUS_MEET_HEADED=1 \\
...
# Filenames the bot reads/writes in ``ARGUS_MEET_OUT_DIR``.
...
def run_bot() -> int:  # noqa: C901 — orchestration, explicit branches
    url = os.environ.get("ARGUS_MEET_URL", "").strip()
    out_dir_env = os.environ.get("ARGUS_MEET_OUT_DIR", "").strip()
    headed = os.environ.get("ARGUS_MEET_HEADED", "").lower() in {"1", "true", "yes"}
    auth_state = os.environ.get("ARGUS_MEET_AUTH_STATE", "").strip()
    guest_name = os.environ.get("ARGUS_MEET_GUEST_NAME", "Argus")
    duration_s = _parse_duration(os.environ.get("ARGUS_MEET_DURATION", ""))
    # v2: optional realtime mode. Enabled when ARGUS_MEET_MODE=realtime.
    mode = os.environ.get("ARGUS_MEET_MODE", "transcribe").strip().lower()
    realtime_model = os.environ.get("ARGUS_MEET_REALTIME_MODEL", "gpt-realtime")
    realtime_voice = os.environ.get("ARGUS_MEET_REALTIME_VOICE", "alloy")
    realtime_instructions = os.environ.get("ARGUS_MEET_REALTIME_INSTRUCTIONS", "")
    realtime_api_key = os.environ.get("ARGUS_MEET_REALTIME_KEY") or os.environ.get("OPENAI_API_KEY", "")

    if not url or not _is_safe_meet_url(url):
        sys.stderr.write(
            "google_meet bot: refusing to launch — ARGUS_MEET_URL must be a "
            "meet.google.com URL. got: %r\n" % url
        )
        return 2
    if not out_dir_env:
        sys.stderr.write("google_meet bot: ARGUS_MEET_OUT_DIR is required\n")
        return 2
    ...
    if rt["enabled"]:
        if not realtime_api_key:
            state.set(error="realtime mode requested but no API key in ARGUS_MEET_REALTIME_KEY/OPENAI_API_KEY — falling back to transcribe")
            rt["enabled"] = False
        else:
            try:
    ...
    try:
        with sync_playwright() as pw:
            ...
            lobby_deadline = time.time() + float(
                os.environ.get("ARGUS_MEET_LOBBY_TIMEOUT", "300")
            )
