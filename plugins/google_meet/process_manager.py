...
def start(
    ...
    *,
    ...
    env = os.environ.copy()
    env["ARGUS_MEET_URL"] = url
    env["ARGUS_MEET_OUT_DIR"] = str(out)
    env["ARGUS_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["ARGUS_MEET_HEADED"] = "1"
    if auth_state:
        env["ARGUS_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["ARGUS_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if ARGUS_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["ARGUS_MEET_MODE"] = mode
    if realtime_model:
        env["ARGUS_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["ARGUS_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["ARGUS_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["ARGUS_MEET_REALTIME_KEY"] = realtime_api_key
