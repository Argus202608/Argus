...
Credential storage mirrors every other Argus channel:

    * runtime SDK creds  -> ``~/.argus/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.argus/auth.json`` under
...
# registers Argus as its own client_id.
...
# Default name of the project Argus provisions for the operator.
DEFAULT_PROJECT_NAME = "Argus"
...
def _auth_json_path() -> Path:
    """Resolve ``~/.argus/auth.json`` honouring the active Argus profile."""
    try:
        from hermes_constants import get_hermes_home
        return Path(get_hermes_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.argus")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.argus/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.argus/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.argus/.env`` (canonical runtime store).
