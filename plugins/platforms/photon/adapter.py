"""
Photon Spectrum (iMessage) platform adapter for Argus.
...
if TYPE_CHECKING:
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already a Argus dep
...
def check_requirements() -> bool:
    ...
    if not (_SIDECAR_DIR / "node_modules").exists():
        ...
        # surfaces the missing-deps state in `argus setup` / status.
        return False
    return True
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use Argus defaults).
    ...
    async def _inbound_loop(self) -> None:
        ...
        headers = {"X-Argus-Sidecar-Token": self._sidecar_token}
        backoff = 1.0
    ...
    def _pid_is_sidecar(pid: int) -> bool:
        ...
        try:
            ...
            return False
        # Checkout-agnostic: any Argus checkout's sidecar entry point.
...
async def _standalone_send(
    ...
    *,
    ...
    headers = {"X-Argus-Sidecar-Token": token}
...
def register(ctx) -> None:
    """Called by the Argus plugin loader at startup."""
    ...
    ctx.register_platform(
        ...
        # Surfaces Photon in `argus gateway setup` alongside every other
