...
try:
    ...
    def get_hermes_home() -> Path:
        ...
        return Path(val) if val else Path.home() / ".argus"
...
# ``ls ~/.argus/google_chat_user_tokens/`` trivial.
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Install mm-argus[google_chat]."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("  pip install 'mm-argus[google_chat]'")
        return False
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for Argus (native attachment delivery)"
    )
