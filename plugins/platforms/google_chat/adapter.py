...
# ``model_tools`` import time, so ``argus status``, ``hermes chat``, etc.
...
#     fresh "Argus is thinking…" card during the small window between
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # they don't sit in the chat forever as "Argus is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("ARGUS_HOME", str(_Path.home() / ".argus"))
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Argus is thinking…' marker message.
        ...
        "Argus is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Argus is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Argus is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Argus is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Argus is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``argus setup``.
    ...
    prompt for env vars, persist them to ``~/.argus/.env`` so the next
    ...
    print()
    print_success("Google Chat configuration saved to ~/.argus/.env")
    print_info("Restart the gateway: argus gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    separate process from ``argus gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Argus plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'mm-argus[google_chat]'",
        ...
        # the "Argus is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Argus is thinking…' marker message "
