"""
Email platform adapter for the Argus gateway.

Allows users to interact with Argus by sending emails.
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `argus config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `argus gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
