"""SimpleX Chat platform adapter (Argus plugin).
...
This adapter ships as a Argus platform plugin under
``plugins/platforms/simplex/``. The Argus plugin loader scans the
...
Optional environment variables:
    ...
    ARGUS_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``argus setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Argus — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("ARGUS_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``argus setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.argus/.env`` via
    ``hermes_cli.config``.
    """
    print()
