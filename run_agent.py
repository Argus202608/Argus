...
def _session_source_for_agent(platform: Optional[str]) -> str:
    try:
        ...
        source = get_session_env("ARGUS_SESSION_SOURCE", "")
    except Exception:
        source = os.environ.get("ARGUS_SESSION_SOURCE", "")
...
def _routermint_headers() -> dict:
    ...
    return {
        "User-Agent": f"ArgusAgent/{_HERMES_VERSION}",
    }
...
class AIAgent:
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``ARGUS_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``ARGUS_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return env_float("ARGUS_API_TIMEOUT", 1800.0)
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``ARGUS_API_CALL_STALE_TIMEOUT`` env var
        ...
        env_timeout = os.getenv("ARGUS_API_CALL_STALE_TIMEOUT")
    ...
    def _persist_session(self, messages: List[Dict], conversation_history: List[Dict] = None):
        ...
        """
        # Monitor/Watcher completion hooks run through the ordinary agent loop so
        # they can reuse the configured model and tools, but their implementation
        # prompt is private control context.  The normal finalizer already keeps
        # successful internal turns out of canonical history; error, interrupt,
        # and overflow exits call this lower-level crash-resilience helper
        # directly.  Fail closed here as well, before even mutating ``messages``
        # or ``_session_messages``, so no hidden prompt/tool suffix can enter the
        # cached conversation prefix on an exceptional path.
        if getattr(self, "_ephemeral_internal_turn", False):
            return
        ...
        try:
            ...
            for msg in messages:
                ...
                if msg_id in flushed_ids:
                    continue
                is_mm_query_projection = bool(msg.get("_mm_query_projection"))
                if msg_id in history_ids and not is_mm_query_projection:
                    flushed_ids.add(msg_id)
                    continue
                role = msg.get("role", "unknown")
                content = msg.get("content")
                if is_mm_query_projection:
                    # These messages must be durable and remain in chronological
                    # DB order before the new foreground turn, but they must not
                    # reappear as duplicate chat bubbles on resume. Store a
                    # tagged content envelope; the multimodal gateway restores
                    # it to a plain user/assistant message only for model input.
                    content = {
                        "type": "mm_query_projection",
                        "projection_key": str(
                            msg.get("_mm_query_projection") or ""),
                        "text": str(content or ""),
                    }
                ...
                self._session_db.append_message(
                    ...
                    # Record the model actually used for this turn, per-message.
    ...
    def _hook_payload_max_chars() -> int:
        raw = os.getenv("ARGUS_PLUGIN_PAYLOAD_MAX_CHARS", "50000")
        try:
    ...
    def _redact_message_content(content):
        ...
        Respects ``ARGUS_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.argus/sessions/session_{sid}.json``
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects ARGUS_REDACT_SECRETS via
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.argus/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        True).  ``ARGUS_TURN_COMPLETION_EXPLAINER`` env var overrides
        config.  Exposed as a method so tests can patch a single seam,
        mirroring ``_file_mutation_verifier_enabled``.
        """
        try:
            import os as _os
            env = _os.environ.get("ARGUS_TURN_COMPLETION_EXPLAINER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
            # Read from the persisted config.yaml so gateway and CLI share
            # the same setting.  Import lazily to avoid a startup-time cycle.
    ...
    def _touch_activity(self, desc: str) -> None:
        ...
        process is a dispatcher-spawned worker (ARGUS_KANBAN_TASK set),
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        """
        # Dev test fixture (ARGUS_DEV_CREDITS_FIXTURE): inject a chosen notice state
        ...
        if _fixture is not None:
            ...
            logger.info(
                ...
                "(real headers bypassed — `echo clear` / unset ARGUS_DEV_CREDITS_FIXTURE to restore)",
        ...
        if not headers:
            return
        _dev = is_truthy_value(os.environ.get("ARGUS_DEV_CREDITS"))
        ...
        if _dev:
            # ARGUS_DEV_CREDITS: stream each capture to agent.log — watch live with
        ...
        *,
        ...
        try:
            ...
            creds = resolve_nous_runtime_credentials(
                timeout_seconds=env_float("ARGUS_NOUS_TIMEOUT_SECONDS", 15),
                force_refresh=force,
            )
        except Exception as exc:
