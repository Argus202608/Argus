---
sidebar_position: 7
title: "Use SOUL.md with Argus"
description: "How to use SOUL.md to shape Argus Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Argus

`SOUL.md` is the **primary identity** for your Argus instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Argus to feel like the same assistant every time you talk to it — or if you want to replace the Argus persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Argus should be
- what Argus should avoid stylistically
- how Argus should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Argus is and how Argus speaks
...
Argus now uses only the global SOUL file for the current instance:

```text
~/.argus/SOUL.md
```

If you run Argus with a custom home directory, it becomes:
...
Argus automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Argus does not overwrite it
- if the file exists but is empty, Argus adds nothing from it to the prompt

## How Argus uses it

When Argus starts a session, it reads `SOUL.md` from `ARGUS_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Argus feels.
...
Argus already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Argus should sound.

# Avoid
What Argus should not do.

# Defaults
How Argus should behave when ambiguity appears.
```
...
Then restart Argus or start a new session.
...
### I edited SOUL.md but Argus still sounds the same

Check:
- you edited `~/.argus/SOUL.md` or `$ARGUS_HOME/SOUL.md`
...
### Argus is ignoring parts of my SOUL.md
