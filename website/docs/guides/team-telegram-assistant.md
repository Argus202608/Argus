...
This tutorial walks you through setting up a Telegram bot powered by Argus Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Argus Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.argus/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Argus itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Argus Assistant`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Argus Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Argus Gateway...
...
argus gateway start
argus gateway stop
tail -f ~/.argus/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `argus gateway install` to update the plist.
:::
...
argus pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.argus/.env`:
...
Control how much detail the bot shows when using tools. In `~/.argus/config.yaml`:
...
Customize how the bot communicates by editing `~/.argus/SOUL.md`:

For a full guide, see [Use SOUL.md with Argus](/guides/use-soul-with-argus).
...
```bash
# From the CLI
argus cron list          # View all scheduled jobs
argus cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u hermes-gateway` (Linux) or `~/.argus/logs/gateway.log` (macOS) |
| Cron job output | `~/.argus/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.argus/cron/jobs.json` |
| Pairing data | `~/.argus/pairing/` |
| Session history | `~/.argus/sessions/` |
