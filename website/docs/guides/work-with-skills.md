...
description: "Find, install, use, and create skills — on-demand knowledge that teaches Argus new workflows"
---

# Working with Skills

Skills are on-demand knowledge documents that teach Argus how to handle specific tasks — from generating ASCII art to managing GitHub PRs. This guide walks you through using them day to day.
...
Every Argus installation ships with bundled skills. See what's available:
...
You can also trigger skills through natural conversation — ask Argus to use a specific skill, and it will load it via the `skill_view` tool.
...
Official optional skills ship with Argus but aren't active by default. Install them explicitly:

```bash
# Install an official optional skill
argus skills install official/research/arxiv
...
What happens:
1. The skill directory is copied to `~/.argus/skills/`
...
For how to ship skills in your own plugin, see [Build an Argus Plugin → Bundle skills](/guides/build-an-argus-plugin#bundle-skills).
...
When a skill with config is first loaded, Argus prompts you for the values. They're stored in `config.yaml` under `skills.config.*`.

Manage skill config from the CLI:

```bash
# Interactive config for a specific skill
argus skills config gif-search

# View all skill config
argus config show | grep '^skills\.config'
```
...
```bash
mkdir -p ~/.argus/skills/my-category/my-skill
```

### 2. Write SKILL.md

```markdown title="~/.argus/skills/my-category/my-skill/SKILL.md"
---
name: my-skill
...
```bash
argus chat -q "/my-skill help me with the thing"
```

The skill appears automatically — no registration needed. Drop it in `~/.argus/skills/` and it's live.

:::info
The agent can also create and update skills itself using `skill_manage`. After solving a complex problem, Argus may offer to save the approach as a skill for next time.
:::
...
**Let the agent create skills.** After a complex multi-step task, Argus will often offer to save the approach as a skill. Say yes — these agent-authored skills capture the exact workflow including pitfalls that were discovered along the way.

**Use categories.** Organize skills into subdirectories (`~/.argus/skills/devops/`, `~/.argus/skills/research/`, etc.). This keeps the list manageable and helps the agent find relevant skills faster.

**Update skills when they go stale.** If you use a skill and hit issues not covered by it, tell Argus to update the skill with what you learned. Skills that aren't maintained become liabilities.
