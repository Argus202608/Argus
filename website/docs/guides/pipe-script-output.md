...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `argus send`."
---

# Pipe Script Output to Messaging Platforms

`argus send` is a small, scriptable CLI that pushes a message to any
messaging platform Argus is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | argus send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `argus
...
argus send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
argus send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
argus send --list

# Filter by platform
argus send --list telegram
```
...
Any platform Argus ships adapters for works as a target:
...
`argus send` resolves the message body in this order:

1. **Positional argument** — `argus send --to telegram "hi"`
2. **`--file PATH`** — `argus send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | argus send --to telegram`

When stdin is a TTY (no pipe), Argus does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  argus send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `argus send` reuses your Argus config, the same script works on
any host where Argus is installed — no need to export bot tokens into
...
`argus send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  argus send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | argus send \
...
```bash
./train.py --epochs 200 && \
  argus send --to telegram "training done" || \
  argus send --to telegram "training failed (exit $?)"
```
...
argus send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(argus send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `argus send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `argus send` calls
the platform's REST endpoint directly using credentials from
`~/.argus/.env` and `~/.argus/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `argus gateway start` and retry.
...
# Machine-readable
argus send --list --json
```

The listing is built from `~/.argus/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`argus gateway
...
| Approach | Multi-platform | Reuses Argus creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `argus send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`argus send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `argus send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `argus send` shares with cron delivery.
