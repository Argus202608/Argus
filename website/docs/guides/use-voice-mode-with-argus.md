---
sidebar_position: 8
title: "Use Voice Mode with Argus"
description: "A practical guide to setting up and using Argus voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Argus
...
- you want Argus sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Argus.
...
## Step 1: make sure normal Argus works first
...
Argus supports both local and cloud speech stacks.
...
### If you use `argus setup`

If you choose NeuTTS in the setup wizard, Argus checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Argus transcribes and responds
...
- asking Argus to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Argus loop.

## Tuning CLI behavior

### Silence threshold

If Argus starts/stops too aggressively, tune:
...
Argus stays a normal chat bot, but can speak replies.
...
- you want Argus to function like a portable research or ops assistant
...
Argus joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Argus detects speech boundaries
- transcripts are posted in the associated text channel
- Argus responds in text and audio
...
1. get text Argus working
2. install `mm-argus[voice]`
