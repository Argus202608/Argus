...
Copy-paste blueprints for common automation patterns. Each blueprint uses Argus's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`argus webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `argus webhook subscribe`) |
...
```bash
argus cron create "0 2 * * *" \
  "You are a project manager triaging the MMArgus-Team/Argus GitHub repo.

1. Run: gh issue list --repo MMArgus-Team/Argus --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
argus webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
argus cron create "0 9 * * 1" \
  "Scan the MMArgus-Team/Argus repo for documentation drift.

1. Run: gh pr list --repo MMArgus-Team/Argus --state merged --json number,title,files,mergedAt --limit 30
...
```bash
argus cron create "0 6 * * *" \
  "Run a dependency security audit on the hermes-agent project.

1. cd ~/.argus/argus && source .venv/bin/activate
...
```bash
argus webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
argus webhook subscribe alert-triage \
...
```python title="~/.argus/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Argus-Monitor/1.0"})
...
argus cron create "every 30m" \
  ...
  --script ~/.argus/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
argus cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Argus Agent development." \
...
```bash
argus webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
argus webhook subscribe stripe-payments \
