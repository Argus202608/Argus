...
```bash
argus teams-pipeline token-health
argus teams-pipeline token-health --force-refresh
```
...
```bash
argus teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
argus teams-pipeline maintain-subscriptions
argus teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Argus cron (recommended if you already run the Argus gateway)

Argus ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.argus/scripts/`. First create the script:

```bash
mkdir -p ~/.argus/scripts
cat > ~/.argus/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec argus teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.argus/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
argus cron create "0 */12 * * *" \
...
```ini
[Unit]
Description=Argus Teams pipeline subscription maintenance
...
ExecStart=/usr/local/bin/argus teams-pipeline maintain-subscriptions
```
...
```ini
[Unit]
Description=Run Argus Teams pipeline subscription maintenance every 12 hours
...
```cron
0 */12 * * * /usr/local/bin/argus teams-pipeline maintain-subscriptions >> /var/log/hermes/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.argus/.env` at the top of a wrapper script that crontab calls.
...
```bash
argus teams-pipeline subscriptions   # should show expirationDateTime advanced
argus teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
argus teams-pipeline list
argus teams-pipeline list --status failed
argus teams-pipeline show <job-id>
```

### Replay a stored job

```bash
argus teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
argus teams-pipeline fetch --meeting-id <meeting-id>
argus teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `argus teams-pipeline maintain-subscriptions --dry-run`
- inspect `argus teams-pipeline list --status failed`
...
- run `argus teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `argus teams-pipeline run`
...
- [ ] `argus teams-pipeline validate` returns an OK snapshot
- [ ] `argus teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Argus cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
