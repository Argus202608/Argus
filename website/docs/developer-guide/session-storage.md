...
Argus Agent uses a SQLite database (`~/.argus/state.db`) to persist session
...
Multiple argus processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.argus/state.db
...
Default path: `~/.argus/state.db`

This is derived from `hermes_constants.get_hermes_home()` which resolves to
`~/.argus/` by default, or the value of `ARGUS_HOME` environment variable.
