...
The ACP adapter wraps Argus' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
argus acp / argus-acp / python -m acp_adapter
...
The Zed ACP Registry path launches the same adapter through `uvx --from 'mm-argus[acp]==<version>' argus-acp`, pointed at the `mm-argus` PyPI release.
...
- `allow_once` -> Argus `once`
- `allow_always` -> Argus `always`
- reject options -> Argus `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Argus tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["argus-acp"])
...
Instead it reuses Argus' runtime resolver:

- `acp_adapter/auth.py`
- `hermes_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Argus provider/credentials. It also always advertises a terminal setup auth method (`hermes-setup`, args `--setup`) so first-run registry clients can open Argus' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.argus/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `argus-acp` toolset definition (with a legacy `hermes-acp` alias)
- `hermes_cli/main.py` — `argus acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + canonical `argus-acp` script
