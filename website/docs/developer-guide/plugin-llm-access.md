...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Argus already has applies. The plugin never sees a
...
Argus itself). For the async surface (`acomplete()` /
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Argus provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Argus provider
        # (must be one Argus already knows about — same names as
        # `argus model` and config.yaml model.provider).
        ...
        # plugin sends — Argus does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.argus/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Argus' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Argus subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/MMArgus-Team/Argus/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/MMArgus-Team/Argus/blob/main/tests/agent/test_plugin_llm.py)
