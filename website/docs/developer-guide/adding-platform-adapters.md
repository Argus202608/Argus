...
This guide covers adding a new messaging platform to the Argus gateway. A platform adapter connects Argus to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.

:::tip
There are two ways to add a platform:
- **Plugin** (recommended for community/third-party): Drop a plugin directory into `~/.argus/plugins/` — zero core code changes needed. See [Plugin Path](#plugin-path-recommended) below.
...
The plugin system lets you add a platform adapter without modifying any core Argus code. Your plugin is a directory with two files:

```
~/.argus/plugins/my-platform/
...
Plugin metadata. The `requires_env` and `optional_env` blocks auto-populate `argus config` UI entries (see [Surfacing Env Vars](#surfacing-env-vars-in-argus-config) below).
...
| `argus config` UI entries | `requires_env` / `optional_env` in `plugin.yaml` auto-populate |
...
| `argus status` | Shows plugin platforms with `(plugin)` tag |
| `argus gateway setup` | Plugin platforms appear in setup menu |
| `argus tools` / `argus skills` | Plugin platforms in per-platform config |
...
Most users set up a platform by dropping env vars into `~/.argus/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your plugin pick those env vars up **before** the adapter is constructed, so `argus gateway status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.
...
`cron_deliver_env_var` makes your platform a recognized `deliver=` target. To make the actual send succeed when the cron job runs in a separate process from the gateway (i.e., `argus cron run` separate from `argus gateway`), register a `standalone_sender_fn`:
...
## Surfacing Env Vars in `argus config`
...
version: 1.0.0
description: >
  My Platform gateway adapter for Argus Agent.
author: Your Name
requires_env:
...
:::note
This checklist is for adding a platform directly to the Argus core codebase — typically done by core contributors for officially supported platforms. Community/third-party platforms should use the [Plugin Path](#plugin-path-recommended) above.
:::
