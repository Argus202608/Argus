...
This page covers all commands related to [Argus profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `argus profile` without a subcommand shows help.
...
```bash
argus profile use <name>
```

Sets `<name>` as the active profile. All subsequent `argus` commands (without `-p`) will use this profile.
...
argus profile use default
```

## `argus profile create`

```bash
argus profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `argus profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `argus update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.argus`), use `argus skills opt-out` / `argus skills opt-in`. |
...
argus profile create work2 --clone-from work

# Clone everything from a specific profile
argus profile create work2-backup --clone-from work --clone-all
```

## `argus profile describe`

```bash
argus profile describe [<name>] [options]
```
...
argus profile describe researcher

# Set it explicitly
argus profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
argus profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
argus profile describe --all --auto
```

## `argus profile delete`

```bash
argus profile delete <name> [options]
```
...
```bash
argus profile delete mybot
argus profile delete mybot --yes
```
...
```bash
argus profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Argus home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ argus profile show work
Profile: work
Path:    ~/.argus/profiles/work
...
```bash
argus profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Argus installation.
...
```bash
argus profile alias work
# Creates/updates ~/.local/bin/work

argus profile alias work --name mywork
# Creates ~/.local/bin/mywork

argus profile alias work --remove
# Removes the wrapper script
```

## `argus profile rename`

```bash
argus profile rename <old-name> <new-name>
```
...
```bash
argus profile rename mybot assistant
# ~/.argus/profiles/mybot → ~/.argus/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `argus profile export`

```bash
argus profile export <name> [options]
```
...
```bash
argus profile export work
# Creates work.tar.gz in the current directory

argus profile export work -o ./work-2026-03-29.tar.gz
```

## `argus profile import`

```bash
argus profile import <archive> [options]
```
...
```bash
argus profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

argus profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`argus profile export` / `import` are still the right commands for
...
### `argus profile install`

```bash
argus profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `argus -p telemetry`). |
...
argus profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
argus profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
argus profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
argus profile install ./telemetry/
```

### `argus profile update`

```bash
argus profile update <name> [--force-config] [--yes]
```
...
```bash
argus profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Argus version, author, env var requirements, the source URL/path, and
...
`argus profile list` also shows the distribution name and version in a
`Distribution` column, and `argus profile show <name>` / `delete <name>`
...
argus profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
argus profile install https://github.com/your-org/internal-assistant.git
```
...
Argus version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Argus can clone from.
3. Tell recipients to run `argus profile install <your-repo-url>`.
...
## `argus -p` / `argus --profile`

```bash
argus -p <name> <command> [options]
argus --profile <name> <command> [options]
```

Global flag to run any Argus command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
argus -p work chat -q "Check the server status"
argus --profile dev gateway start
argus -p personal skills list
argus -p work config edit
```

## `argus completion`

```bash
argus completion <shell>
```
...
- `argus profile <TAB>` — subcommands (list, use, create, etc.)
- `argus profile use <TAB>` — profile names
- `argus -p <TAB>` — profile names
