...
A **profile distribution** packages a complete Argus agent — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## What this means

Before distributions, sharing an Argus agent meant sending someone:
...
```bash
argus profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `argus profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
The tradeoff: recipients need git installed. On any machine running Argus in 2026, that's already true.
...
- **You just want to back up a profile on your own machine.** Use [`argus profile export` / `import`](../reference/profile-commands.md#argus-profile-export) — that's what those are for.
...
:::caution
**Argus does not control git.** The file exclusions described on this page are applied by the **installer** when someone runs `argus profile install` or `argus profile update`. They are **not** applied when you run `git add` or `git commit`.
:::
...
```bash
argus profile create research-bot
research-bot setup                    # configure model, API keys
# Edit ~/.argus/profiles/research-bot/SOUL.md
...
Create `~/.argus/profiles/research-bot/distribution.yaml`:
...
Create `~/.argus/profiles/research-bot/.gitignore` with at minimum:
...
```bash
cd ~/.argus/profiles/research-bot
git init
git add .
git commit -m "v1.0.0"
...
Recipients who run `argus profile update research-bot` will pull the latest.
...
```bash
argus profile install github.com/you/research-bot --alias
```
...
5. Copies distribution-owned files into `~/.argus/profiles/research-bot/` (or wherever the manifest's `name` resolves). The [hard-excluded paths](#whats-not-in-a-distribution-ever) are stripped during this copy, even if the author accidentally left them in the repo.
...
```bash
# GitHub shorthand
argus profile install github.com/you/research-bot

# Full HTTPS
argus profile install https://github.com/you/research-bot.git

# SSH
argus profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
argus profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
argus profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
argus profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
argus profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
argus profile install github.com/acme/support-bot --name support-eu --alias
```
...
```
# Environment variables required by this Argus distribution.
...
```bash
cp ~/.argus/profiles/research-bot/.env.EXAMPLE ~/.argus/profiles/research-bot/.env
...
```bash
argus profile info research-bot
```
...
`argus profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
argus profile update research-bot
```
...
```bash
argus profile delete research-bot
```
...
Path:    ~/.argus/profiles/research-bot
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `argus profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
# Each engineer
argus profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `argus profile update pr-reviewer` and everyone's on the new version within minutes.
...
cd ~/.argus/profiles/polymarket-trader
...
# Anyone
argus profile install github.com/you/hermes-polymarket-trader --alias
```
...
You built Argus-on-top — maybe a compliance-monitoring harness, a customer-support stack, a domain-specific research platform. You want to distribute it as a product.
...
# Each on-call
argus profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
argus profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `argus profile info <name>` and hold off on updates until you're ready.
:::
...
argus profile info research-bot | grep Version
...
```yaml
# ~/.argus/profiles/research-bot/local/my-overrides.yaml
...
argus profile delete research-bot --yes
argus profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
argus profile update research-bot --force-config --yes
```
...
argus profile install github.com/yourname/forked-research-bot --alias

# Iterate locally in ~/.argus/profiles/forked-research-bot/
...
argus profile install ~/.argus/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
argus profile delete research-bot-test --yes
argus profile install ~/.argus/profiles/research-bot --name research-bot-test
```
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `argus -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `argus profile` — not a parallel command tree.
...
- Reserved profile names (`argus`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [`argus profile export` / `import`](../reference/profile-commands.md#argus-profile-export) — local backup / restore (not distribution)
- [Using SOUL with Argus](../guides/use-soul-with-argus.md) — authoring personalities
