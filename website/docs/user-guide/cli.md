...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Argus Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Argus Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `argus setup --portal` — and you're ready to `argus chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
Argus also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `argus --tui` — see the [TUI](tui.md) guide.
:::
...
argus chat --model "anthropic/claude-sonnet-4"

# With a specific provider
argus chat --provider nous        # Use Nous Portal
argus chat --provider openrouter  # Force OpenRouter
...
argus chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
argus --continue             # Resume the most recent CLI session (-c)
argus --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
argus chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
argus -w                         # Interactive mode in worktree
argus -w -z "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Argus CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Argus CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `ARGUS_YOLO_MODE` is on (either `argus --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
When resuming a previous session (`argus -c` or `argus --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Argus to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. Argus supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Argus replies |
...
```bash
argus -s hermes-agent-dev,github-auth
argus chat -s github-pr-workflow -s github-auth
```

Argus loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.argus/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.argus/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Argus..."
  # Add your own!
```
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Argus recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Argus — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The very first time you press Enter while Argus is working, Argus prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Argus to the background — just like any terminal process. The shell prints a confirmation:

```
Argus Agent has been suspended. Run `fg` to bring Argus Agent back.
```
...
Resume this session with:
  argus --resume 20260225_143052_a1b2c3
...
```bash
argus --continue                          # Resume the most recent CLI session
argus -c                                  # Short form
argus -c "my project"                     # Resume a named session (latest in lineage)
argus --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
argus --resume "refactoring auth"         # Resume by title
argus -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `argus sessions rename <id> <title>` from the command line. Use `argus sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Argus's SQLite state database under `~/.argus/state.db`. The database keeps:
...
Argus immediately confirms the task and gives you back the prompt:
