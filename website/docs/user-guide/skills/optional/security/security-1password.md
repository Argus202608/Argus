...
| | |
|---|---|
| Source | Optional — install with `argus skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by MMArgus Team |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Argus terminal calls (desktop app flow only)
...
### Service Account (recommended for Argus)
...
## Argus Execution Pattern (desktop app flow)

Argus terminal commands are non-interactive by default and can lose auth context between calls.
