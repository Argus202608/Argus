...
| | |
|---|---|
| Source | Optional — install with `argus skills install official/devops/watchers` |
...
Watch a GitHub repo (set `GITHUB_TOKEN` in `${ARGUS_HOME:-~/.argus}/.env` to avoid the 60 req/hr anonymous rate limit):

```bash
python $ARGUS_HOME/skills/devops/watchers/scripts/watch_github.py \
  --name argus-issues --repo MMArgus-Team/Argus --scope issues
```
