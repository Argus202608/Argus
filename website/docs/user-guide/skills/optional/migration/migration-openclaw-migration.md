---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Argus"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Argus"
---
...
Migrate a user's OpenClaw customization footprint into Argus. Imports Argus-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `argus skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Hermes Agent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Argus`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> Argus Migration

Use this skill when a user wants to move their OpenClaw setup into Argus with minimal manual cleanup.
...
```bash
argus claw migrate              # Full interactive migration
argus claw migrate --dry-run    # Preview what would be migrated
argus claw migrate --preset user-data   # Migrate without secrets
argus claw migrate --overwrite  # Overwrite existing conflicts
argus claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `argus setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_hermes.py` to:

- import `SOUL.md` into the Argus home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Argus memory entries
- merge OpenClaw command approval patterns into Argus `command_allowlist`
- migrate Argus-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Argus working-directory configuration
- copy OpenClaw skills into `~/.argus/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Argus workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.argus/tts/`
- archive non-secret docs that do not have a direct Argus destination
...
- `~/.argus/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py`

Do not guess a shorter path like `~/.argus/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.argus/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Argus-compatible secrets, currently:
...
Argus CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Argus. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Argus workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Argus skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.argus/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py
```
...
```bash
python3 ~/.argus/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.argus/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.argus/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.argus/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Argus unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Argus targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Argus destination. Unsupported auth blobs must still be reported as skipped.
...
- Argus persona state imported
- Argus memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.argus/skills/openclaw-imports/`
