...
description: "Customize Argus Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Argus Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `ARGUS_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Argus is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Argus now seeds a default `SOUL.md` automatically in:

```text
~/.argus/SOUL.md
```

More precisely, it uses the current instance's `ARGUS_HOME`, so if you run Argus with a custom home directory, it will use:
...
- Argus creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Argus loads `SOUL.md` only from `ARGUS_HOME`
- Argus does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Argus falls back to a built-in default identity
...
If Argus loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `ARGUS_HOME`, the personality belongs to the Argus instance itself.

That also makes it easier to teach users:
- "Edit `~/.argus/SOUL.md` to change Argus' default personality."
...
- how Argus should handle uncertainty, disagreement, or ambiguity
...
## What Argus injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Argus falls back to a built-in default identity ("You are Argus Agent, an intelligent AI assistant created by MMArgus Team..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Argus ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Argus, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Argus its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.argus/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.argus/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-argus)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Argus speaks
- `display.skin` and `/skin` affect how Argus looks in the terminal
