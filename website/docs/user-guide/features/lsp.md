...
Argus runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Argus
...
1. Argus captures a baseline of current diagnostics for the file.
...
…). Argus auto-detects the binary on PATH or in
`<ARGUS_HOME>/lsp/bin/`.
...
`node_modules` tree — Argus installs both packages together when you
run `argus lsp install typescript` or auto-install fires on first
use.

## CLI

```
argus lsp status          # service state + per-server install status
argus lsp list            # registry, optionally --installed-only
argus lsp install <id>    # eagerly install one server
argus lsp install-all     # try every server with a known recipe
argus lsp restart         # tear down running clients
argus lsp which <id>      # print resolved binary path
```

`argus lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <ARGUS_HOME>/lsp/bin
...
When `install_strategy: auto`, Argus installs binaries into
`<ARGUS_HOME>/lsp/bin/`. NPM packages land in
`<ARGUS_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Argus-owned and is
...
Servers are kept alive for the life of the Argus process. There's
...
**`argus lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<ARGUS_HOME>/lsp/bin/`. Run
`argus lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `argus lsp status`**
...
When `argus lsp status` shows a `Backend warnings` section, install
...
`~/.argus/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.argus/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `argus lsp restart` to clear the set;
