...
Argus Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
argus memory setup      # interactive picker + configuration
argus memory status     # check what's active
argus memory off        # disable external provider
```

You can also select the active memory provider via `argus plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.argus/config.yaml`:
...
When a memory provider is active, Argus automatically:
...
**Setup Wizard:**
```bash
argus memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `argus honcho setup` command still works (it now redirects to `argus memory setup`), but is only registered after Honcho is selected as the active memory provider.

**Config:** `$ARGUS_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$ARGUS_HOME/honcho.json` > `~/.argus/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/MMArgus-Team/Argus/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/hermes).
...
:::tip Migrating from `argus honcho`
If you previously used `argus honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Argus profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Argus profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Argus profile. Host key `argus` → default; `hermes.<profile>` for others. |
...
Scans every Argus profile, creates host blocks for any profile without one, inherits settings from the default `argus` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `argus memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/MMArgus-Team/Argus/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/hermes).
...
argus memory setup    # select "openviking"
# Or manually:
argus config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.argus/.env
# Authenticated servers should use a user/admin API key:
echo "OPENVIKING_API_KEY=..." >> ~/.argus/.env
```
...
`OPENVIKING_AGENT` is Argus' peer ID in OpenViking for peer-scoped memories.
...
**Setup (Platform):**
```bash
argus memory setup    # select "mem0" → "Platform"
# Or manually:
argus config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.argus/.env
```

**Setup (OSS):**
```bash
argus memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
argus memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
argus memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Config:** `$ARGUS_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.argus/.env`.

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `platform` | `platform` (Mem0 Cloud) or `oss` (self-hosted) |
| `user_id` | `hermes-user` | User identifier |
| `agent_id` | `argus` | Agent identifier |
...
**Switching modes:** Re-run `argus memory setup mem0 --mode <platform|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
argus memory setup    # select "hindsight"
# Or manually:
argus config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.argus/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p argus ui start`

**Config:** `$ARGUS_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `argus` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Argus Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/MMArgus-Team/Argus/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
argus memory setup    # select "holographic"
# Or manually:
argus config set memory.provider holographic
```
...
| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$ARGUS_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
argus memory setup    # select "retaindb"
# Or manually:
argus config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.argus/.env
```
...
# Then configure Argus
argus memory setup    # select "byterover"
# Or manually:
argus config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$ARGUS_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
argus memory setup    # select "supermemory"
# Or manually:
argus config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.argus/.env
```

**Config:** `$ARGUS_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `argus` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `hermes-{identity}` → `hermes-coder`) to isolate memories per Argus profile
...
argus config set memory.provider memori
argus memory setup
```
...
- **Local storage providers** (Holographic, ByteRover) use `$ARGUS_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$ARGUS_HOME/` so each profile has its own credentials
