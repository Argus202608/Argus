...
Argus Agent can drive your desktop — clicking, typing, scrolling,
...
on macOS/Linux, `install.ps1` on Windows. Use `argus computer-use
status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `argus tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or Argus app). `argus computer-use doctor` will tell you which permission is missing. |
...
```
argus -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.argus/config.yaml`.

## `argus computer-use doctor` — your first triage stop

`argus computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ argus computer-use doctor
...
agent is acting here." Each Argus run declares its own cua-driver
...
Argus intentionally keeps its skill (`skills/computer-use/SKILL.md`)
focused on the Argus-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the Argus skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Argus
...
Argus applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.argus/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Argus applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Argus agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Argus agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Argus warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Argus from inside the RDP / console
...
upstream. **Argus disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Argus sets
...
When it's on, `argus computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Argus at a binary you built from source instead
of the published release. Argus resolves the driver with
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Argus testing (see notes).
...
### Option B — point Argus straight at the built binary (fastest loop)
...
### Confirm Argus is using your build

- `argus computer-use status` prints the resolved binary path and
  version.
- `argus computer-use doctor` confirms the binary is reachable and
...
- **Argus spawns its own `cua-driver mcp` child over stdio** — it does
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Argus
  change needed either way.
- **Local builds skip the version check.** Argus warns when the
...
**First action when anything's off: run `argus computer-use doctor`.**
...
Run `argus computer-use install` to fetch the cua-driver binary, or
run `argus tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `argus computer-use doctor` will
...
- **Argus-side skill** — `skills/computer-use/SKILL.md` — teaches the
  Argus `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects Argus (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Argus among them)
