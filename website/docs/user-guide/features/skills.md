...
All skills live in **`~/.argus/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point Argus at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
By default every profile is seeded with the bundled skill catalog, and each `argus update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:

**At install time** (applies to the default `~/.argus` profile):

```bash
curl -fsSL https://raw.githubusercontent.com/MMArgus-Team/Argus/main/scripts/install.sh | bash -s -- --no-skills
```

**At profile-create time** (named profiles):

```bash
argus profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
argus skills opt-out            # stop future seeding — nothing on disk is touched
argus skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
argus skills opt-in --sync      # undo: remove the marker and re-seed now
```

All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `argus update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `argus skills opt-in`) to re-enable.

:::note Safe by default
`argus skills opt-out` only stops *future* seeding — it never deletes anything already on disk. The optional `--remove` flag deletes bundled skills **only** when they are unmodified (byte-identical to the version Argus installed). Skills you have edited, skills installed from the hub, and skills you wrote yourself are always kept.
:::
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Argus to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.argus/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
argus chat --toolsets skills -q "What skills do you have?"
argus chat --toolsets skills -q "Show me the axolotl skill"
```
...
description, the standard section order, Argus-tool framing, no invented
commands).
...
/home/user/.argus/cache/chart-q4-2025.png
...
When a missing value is encountered, Argus asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `argus setup` or `~/.argus/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `argus config migrate` prompts for unconfigured settings, and `argus config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.argus/skills/                  # Single source of truth
...
If you maintain skills outside of Argus — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Argus to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.argus/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.argus/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the Argus process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Argus ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.argus/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Bundles live in **`~/.argus/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `argus bundles list`.
...
argus bundles create research

# Overwrite an existing bundle
argus bundles create backend-dev --skill ... --force

# Delete a bundle
argus bundles delete backend-dev

# Re-scan ~/.argus/skill-bundles/ and report changes
argus bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.argus/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.argus/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
Staged writes survive restarts under `~/.argus/pending/skills/` and are
...
```bash
argus skills browse                              # Browse all hub skills (official first)
argus skills browse --source official            # Browse only official optional skills
argus skills search kubernetes                   # Search all sources
argus skills search react --source skills-sh     # Search the skills.sh directory
argus skills search https://mintlify.com/docs --source well-known
argus skills inspect openai/skills/k8s           # Preview before installing
argus skills install openai/skills/k8s           # Install with security scan
argus skills install official/security/1password
argus skills install skills-sh/vercel-labs/json-render/json-render-react --force
argus skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
argus skills list --source hub                   # List hub-installed skills
argus skills check                               # Check installed hub skills for upstream updates
argus skills update                              # Reinstall hub skills with upstream changes when needed
argus skills audit                               # Re-scan all hub skills for security
argus skills uninstall k8s                       # Remove a hub skill
argus skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
argus skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
argus skills publish skills/my-skill --to github --repo owner/repo
argus skills snapshot export setup.json          # Export skill config
argus skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Argus. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `argus skills search <query> --source skills-sh`. Argus resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Argus currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Argus repository itself and install with built-in trust.
...
This is Vercel's public skills directory. Argus can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
argus skills search https://mintlify.com/docs --source well-known
argus skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
argus skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Argus can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
argus skills install openai/skills/k8s
argus skills tap add myorg/skills-repo
```
...
[Skills Hub](https://mmargus-team.github.io/Argus/docs) page — instead of a
tag-derived guess. This is generic: any tap that ships the file gets real
categorization, no Argus-side changes required.
...
- Argus source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

Argus supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

Argus source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

Argus can search and convert agent entries from LobeHub's public catalog into installable Argus skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Argus source id: `lobehub`

#### 8. browse.sh (`browse-sh`)

Argus integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with Argus' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- Argus source id: `browse-sh`
- Trust level: `community`

```bash
argus skills search airbnb --source browse-sh
argus skills inspect browse-sh/airbnb.com/search-listings-ddgioa
argus skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Argus fetches the URL, parses the YAML frontmatter, security-scans it, and installs.
...
```bash
argus skills install https://sharethis.chat/SKILL.md
argus skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
argus skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `argus skills update` re-fetches from the same URL automatically when you want to refresh.
...
`argus skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Argus | Always trusted |
...
```bash
argus skills check          # Report which installed hub skills changed upstream
argus skills update         # Reinstall only the skills with updates available
argus skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Argus users add with `argus skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
Argus discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.
...
After pushing that to GitHub, any Argus user can subscribe and install:

```bash
argus skills tap add my-org/hermes-skills
argus skills search deploy
argus skills install my-org/hermes-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.argus/.hub/taps.json`:
...
The `argus skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `argus skills tap list` shows the effective path per tap.
...
```bash
argus skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.argus/.hub/taps.json` (created on demand).

## Bundled skill updates (`argus skills reset`)

Argus ships with a set of bundled skills in `skills/` inside the repo. On install and on every `argus update`, a sync pass copies those into `~/.argus/skills/` and records a manifest at `~/.argus/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Argus recomputes the hash of your local copy and compares it to the origin hash:
...
argus skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `ARGUS_HOME`, so `argus -p coder skills reset <name>` only affects that profile.
:::
