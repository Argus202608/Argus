...
When Argus Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.argus/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.argus/config.yaml` (switchable per session
via `/personality`).
...
If you use Argus' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.argus/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Argus Agent's deliverable
