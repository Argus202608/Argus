...
**Tool Search** is Argus' opt-in progressive-disclosure layer for that
...
:::info Built-in Argus tools never defer
The tools that make up Argus' core capability set (`terminal`,
...
When the model invokes `tool_call`, Argus **unwraps the bridge** and
...
- **No JS sandbox.** Argus uses the simpler "structured tools" mode
