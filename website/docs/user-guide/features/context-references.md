...
Type `@` followed by a reference to inject content directly into your message. Argus expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Argus env: `$ARGUS_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$ARGUS_HOME/skills/.hub/`
