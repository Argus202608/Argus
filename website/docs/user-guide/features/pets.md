...
Argus can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<ARGUS_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Argus renders the sprite at full fidelity when your
...
argus pets install boba --select
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `argus pets list [query] [--limit N]` |
| List installed pets | `argus pets list --installed` |
| Install a pet | `argus pets install <slug> [--select] [--force]` |
...
| Preview/animate | `argus pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `argus pets off` |
| Remove an installed pet | `argus pets remove <slug>` |
| Diagnose setup | `argus pets doctor` |

`argus pets show` flags:
...
always-on-top desktop window. Out there it stays visible while Argus is
...
Run `argus pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Argus uses its own
  profile-scoped `<ARGUS_HOME>/pets/` instead — install through `argus pets`.
