...
On a brand-new install (or the first time a pre-curator install ticks after `argus update`), the curator **does not run immediately**. The first observation seeds `last_run_at` to "now" and defers the first real pass by one full `interval_hours`. This gives you a full interval to review your skill library, pin anything important, or opt out entirely before the curator ever touches it.

If you want to see what the curator *would* do before it runs for real, run `argus curator run --dry-run` — it produces the same review report without mutating the library.
:::

A run has two phases:

1. **Automatic transitions** (deterministic, no LLM). Skills unused for `stale_after_days` (30) become `stale`; skills unused for `archive_after_days` (90) are moved to `~/.argus/skills/.archive/`. This is the always-on pruning behavior — it runs whenever the curator is enabled, with no aux-model cost.
...
```bash
argus model                   # → "Auxiliary models — side-task routing"
...
:::note Legacy config
Earlier releases used a one-off `curator.auxiliary.{provider,model}` block. That path still works but emits a deprecation log line — please migrate to `auxiliary.curator` above so the curator shares the same plumbing (`argus model`, dashboard Models tab, `base_url`, `api_key`, `timeout`, `extra_body`) as every other aux task.
:::

## CLI

```bash
argus curator status         # last run, counts, pinned list, LRU top 5
argus curator run            # trigger a run now (blocks until done). Prune-only unless curator.consolidate: true
argus curator run --consolidate # force the LLM consolidation pass on for this run, overriding the config default
argus curator run --background  # fire-and-forget: start the run in a background thread
argus curator run --dry-run  # preview only — report without any mutations
argus curator backup         # take a manual snapshot of ~/.argus/skills/
argus curator rollback       # restore from the newest snapshot
argus curator rollback --list     # list available snapshots
argus curator rollback --id <ts>  # restore a specific snapshot
argus curator rollback -y         # skip the confirmation prompt
argus curator pause          # stop runs until resumed
argus curator resume
argus curator pin <skill>    # never auto-transition this skill
argus curator unpin <skill>
argus curator restore <skill>  # move an archived skill back to active
argus curator list-archived    # list skills currently in ~/.argus/skills/.archive/
argus curator archive <skill>  # manually archive a single skill now
argus curator prune [--days N] # bulk-archive agent-created skills idle >= N days (default 90)
```

## Backups and rollback

Before every real curator pass, Argus takes a tar.gz snapshot of `~/.argus/skills/` at `~/.argus/skills/.curator_backups/<utc-iso>/skills.tar.gz`. If a pass archives or consolidates something you didn't want touched, you can undo the whole run with one command:

```bash
argus curator rollback        # restore newest snapshot (with confirmation)
argus curator rollback -y     # skip the prompt
argus curator rollback --list # see all snapshots with reason + size
```

The rollback itself is reversible: before replacing the skills tree, Argus takes another snapshot tagged `pre-rollback to <target-id>`, so a mistaken rollback can be undone by rolling forward to that one with `--id`.

You can also take manual snapshots at any time with `argus curator backup --reason "before-refactor"`. The `--reason` string lands in the snapshot's `manifest.json` and is shown in `--list`.
...
Set `curator.backup.enabled: false` to disable automatic snapshotting. The manual `argus curator backup` command still works when backups are disabled only if you set `enabled: true` first — the flag gates both paths symmetrically so there's no way to accidentally skip the pre-run snapshot on mutating runs.

`argus curator status` also lists the five least-recently-used skills — a quick way to see what's likely to become stale next.
...
`~/.argus/skills/.usage.json`. A skill qualifies when ALL of the following
are true:

1. Its name is **not** in `~/.argus/skills/.bundled_manifest` (bundled skills shipped with the repo).
2. Its name is **not** in `~/.argus/skills/.hub/lock.json` (hub-installed skills).
...
If you manually created a `SKILL.md` or pointed Argus at an external skill
...
**To see which skills the curator actually manages**, run `argus curator status`.
...
- Archives are recoverable via `argus curator restore <name>`

If you want to protect a specific skill from ever being touched — for example a
hand-authored skill you rely on — use `argus curator pin <name>`. See the next
section.
...
- The **agent's `skill_manage` tool** refuses `delete` on it, pointing the user at `argus curator unpin <name>`. Patches and edits still go through, so the agent can improve a pinned skill's content as pitfalls come up without a pin/unpin/re-pin dance.

Pin and unpin with:

```bash
argus curator pin <skill>
argus curator unpin <skill>
```

The flag is stored as `"pinned": true` on the skill's entry in `~/.argus/skills/.usage.json`, so it survives across sessions.

Only **agent-created** skills can be pinned — `argus curator pin` refuses on bundled and hub-installed skills with an explanatory message if you try. Hub-installed skills are never subject to curator mutation. Bundled built-in skills are only touched when `curator.prune_builtins: true` (the default), and even then only archived after `archive_after_days` of non-use — never patched, consolidated, or deleted. Set `curator.prune_builtins: false` to exempt bundled skills entirely.
...
If you want a stronger guarantee than "no deletion" — for instance, freezing a skill's content entirely while the agent still reads it — edit `~/.argus/skills/<name>/SKILL.md` directly with your editor. The pin guards tool-driven deletion, not your own filesystem access.

## Usage telemetry

The curator maintains a sidecar at `~/.argus/skills/.usage.json` with one entry per skill:
...
Every curator run writes a timestamped directory under `~/.argus/logs/curator/`:
...
```bash
argus curator restore <skill-name>
```

This moves the skill back from `~/.argus/skills/.archive/` to the active tree and resets its state to `active`. The restore refuses if a bundled or hub-installed skill has since been installed under the same name (would shadow upstream).

## Disabling per environment

The curator is on by default. To turn it off:

- **For one profile only:** edit `~/.argus/config.yaml` (or the active profile's config) and set `curator.enabled: false`.
- **For just one run:** `argus curator pause` — the pause persists across sessions; use `resume` to re-enable.
