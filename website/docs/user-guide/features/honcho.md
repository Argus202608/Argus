...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Argus's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Argus instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
argus memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `argus` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Argus syncs them back at session init.
...
The `argus honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `argus memory setup honcho` (or run `argus memory setup` and pick it from the list); the `argus honcho` subcommand then appears on the next invocation.

```bash
argus memory setup honcho    # Configure Honcho directly (works before activation)
argus honcho status          # Connection status, config, and key settings
argus honcho setup           # Redirects to `argus memory setup` (post-activation alias)
argus honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
argus honcho peer            # Show or update peer names + dialectic reasoning level
argus honcho mode            # Show or set recall mode (hybrid/context/tools)
argus honcho tokens          # Show or set token budget for context and dialectic
argus honcho identity        # Seed or show the AI peer's Honcho identity
argus honcho sync            # Sync Honcho config to all existing profiles
argus honcho peers           # Show peer identities across all profiles
argus honcho sessions        # List known Honcho session mappings
argus honcho map             # Map current directory to a Honcho session name
argus honcho enable          # Enable Honcho for the active profile
argus honcho disable         # Disable Honcho for the active profile
argus honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `argus honcho`

If you previously used the standalone `argus honcho setup`:
...
No re-login or re-setup needed. Run `argus memory setup` and select "honcho" — the wizard detects your existing config.
