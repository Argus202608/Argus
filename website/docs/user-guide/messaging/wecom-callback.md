...
Connect Argus to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Argus supports two WeCom integration modes:
...
> Run `argus gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Argus decrypts the message, queues it for the agent
...
(Use `argus gateway start` only after `argus gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Argus and the token the
...
in `~/.argus/.env` values around `=` will also break signature checks. After
fixing, restart `argus gateway run`.
...
Check `argus gateway run` logs for the bound host/port. If the adapter bound to
