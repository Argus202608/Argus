...
description: "Chat with Argus from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Argus from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Argus](/guides/use-voice-mode-with-argus).
...
```mermaid
flowchart TB
    subgraph Gateway["Argus Gateway"]
...
For group chats, hooks, and automation flows, Argus supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Argus keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Argus does not hide failures just because the text resembles a silence token.
...
```bash
argus gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
argus gateway              # Run in foreground
argus gateway setup        # Configure messaging platforms interactively
argus gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo argus gateway install --system   # Linux only: install a boot-time system service
argus gateway start        # Start the default service
argus gateway stop         # Stop the default service
argus gateway status       # Check default service status
argus gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Argus Agent to the latest version |
...
Configure per-platform overrides in `~/.argus/gateway.json`:
...
argus pairing approve telegram XKGH5N7P

# Other pairing commands:
argus pairing list          # View pending + approved users
argus pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Argus appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.argus/config.yaml`:
...
Off by default. When enabled, Argus prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.argus/config.yaml`:
...
```bash
argus gateway install               # Install as user service
argus gateway start                 # Start the service
argus gateway stop                  # Stop the service
...
sudo argus gateway install --system
sudo argus gateway start --system
sudo argus gateway status --system
...
The unit Argus installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit hermes-gateway` (or `sudo systemctl edit hermes-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `argus update`. When `argus update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart hermes-gateway` command (it never blocks on an interactive password prompt).
...
```bash
argus gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `argus update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo argus update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/hermes-gateway`:

```
argus ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed hermes-gateway*, /usr/bin/systemctl --no-ask-password start hermes-gateway*, /usr/bin/systemctl --no-ask-password restart hermes-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Argus will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Argus installations on the same machine (with different `ARGUS_HOME` directories), each gets its own systemd service name. The default `~/.argus` uses `hermes-gateway`; other installations use `hermes-gateway-<hash>`. The `argus gateway` commands automatically target the correct service for your current `ARGUS_HOME`.
:::

### macOS (launchd)

```bash
argus gateway install               # Install as launchd agent
argus gateway start                 # Start the service
argus gateway stop                  # Stop the service
argus gateway status                # Check status
tail -f ~/.argus/logs/gateway.log   # View logs
```
...
- **ARGUS_HOME** — scopes the gateway to your Argus installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `argus gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `ARGUS_HOME` directory gets its own launchd label. The default `~/.argus` uses `ai.hermes.gateway`; other installations use `ai.hermes.gateway-<suffix>`.
:::
...
1. **Gateway log** (`~/.argus/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
