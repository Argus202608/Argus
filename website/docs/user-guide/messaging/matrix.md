...
sidebar_position: 9
title: "Matrix"
description: "Set up Argus Agent as a Matrix bot"
---

# Matrix Setup

Argus Agent integrates with Matrix, the open, federated messaging protocol. Matrix lets you run your own homeserver or use a public one like matrix.org — either way, you keep control of your communications. The bot connects via the `mautrix` Python SDK, processes messages through the Argus Agent pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, file attachments, images, audio, video, and optional end-to-end encryption (E2EE).

Argus works with any Matrix homeserver — Synapse, Conduit, Dendrite, or matrix.org.

Before setup, here's the part most people want to know: how Argus behaves once it's connected.

## How Argus Behaves

| Context | Behavior |
|---------|----------|
| **DMs** | Argus responds to every message. No `@mention` needed. Each DM has its own session. Set `MATRIX_DM_MENTION_THREADS=true` to start a thread when the bot is `@mentioned` in a DM. |
| **Rooms** | By default, Argus requires an `@mention` to respond. Set `MATRIX_REQUIRE_MENTION=false` or add room IDs to `MATRIX_FREE_RESPONSE_ROOMS` for free-response rooms. Room invites are auto-accepted. |
| **Threads** | Argus supports Matrix threads (MSC3440). If you reply in a thread, Argus keeps the thread context isolated from the main room timeline. Threads where the bot has already participated do not require a mention. |
| **Auto-threading** | By default, Argus auto-creates a thread for each message it responds to in a room. This keeps conversations isolated. Set `MATRIX_AUTO_THREAD=false` to disable. Set `MATRIX_DM_AUTO_THREAD=true` (default false) to also auto-create threads for DM messages — this is distinct from `MATRIX_DM_MENTION_THREADS`, which only starts a thread when the bot is `@mentioned` in a DM. |
| **Commands** | Argus accepts normal `/commands` when your Matrix client sends them. If your client reserves `/` for local commands, use `!commands` instead; Argus normalizes known `!command` aliases to `/command`. |
...
| **Shared rooms with multiple users** | By default, Argus isolates session history per user inside the room. Two people talking in the same room do not share one transcript unless you explicitly disable that. |
...
Argus sends structured Matrix user mentions for explicit Matrix IDs such as `@alice:example.org`. Room-wide `@room` notifications are disabled by default; set `MATRIX_ALLOW_ROOM_MENTIONS=true` only in rooms where the bot is allowed to notify everyone.
:::
...
Argus now includes the current Matrix room name, room ID, topic, message ID,
...
2. Choose a username like `argus` — the full user ID will be `@hermes:your-server.org`.
...
You can also run Argus as your own user. This means the bot posts as you — useful for personal assistants.

## Step 2: Get an Access Token

Argus needs an access token to authenticate with the homeserver. You have two options:
...
Instead of providing an access token, you can give Argus the bot's user ID and password. Argus will log in automatically on startup. This is simpler but means the password is stored in your `.env` file.
...
Argus Agent uses your Matrix User ID to control who can interact with the bot. Matrix User IDs follow the format `@username:server`.
...
Add the following to your `~/.argus/.env` file:
...
Bridge and appservice deployments need extra loop protection. Argus always
...
Optional behavior settings in `~/.argus/config.yaml`:
...
:::tip
You can run `argus gateway` in the background or as a systemd service for persistent operation. See the deployment docs for details.
:::

## End-to-End Encryption (E2EE)

Argus supports Matrix end-to-end encryption, so you can chat with your bot in encrypted rooms.
...
# Or install with argus extras
cd ~/.argus/argus && uv pip install -e ".[matrix]"
```
...
When E2EE is enabled, Argus:

- Stores encryption keys in `~/.argus/platforms/matrix/store/` (legacy installs: `~/.argus/matrix/store/`)
...
In Matrix conversations, Argus exposes Matrix-specific tools to the agent:
...
Set `MATRIX_APPROVAL_REQUIRE_SENDER=false` if you intentionally want any authorized Matrix user in the room to operate an approval/model picker prompt. The default is requester-bound when Argus knows who requested the action.

### Media Limits

Argus uploads and downloads Matrix images, files, audio, and video through Matrix media APIs. Multiple generated images are sent as one ordered logical batch, preserving captions and thread context across the batch.
...
Inbound media must use Matrix `mxc://` content URIs. Argus rejects arbitrary
...
Argus includes an opt-in Synapse harness for local validation:
...
On each startup, if `MATRIX_RECOVERY_KEY` is set, Argus imports cross-signing keys from the homeserver's secure secret storage and signs the current device. This is idempotent and safe to leave enabled permanently.

If Argus bootstraps a new Matrix recovery key, it never logs the raw key. Set
...
If you delete `~/.argus/platforms/matrix/store/crypto.db`, the bot loses its encryption identity. Simply restarting with the same device ID will **not** fully recover — the homeserver still holds one-time keys signed with the old identity key, and peers cannot establish new Olm sessions.

Argus detects this condition on startup and refuses to enable E2EE, logging: `device XXXX has stale one-time keys on the server signed with a previous identity key`.
...
2. Delete the local crypto store and restart Argus:
   ```bash
   rm -f ~/.argus/platforms/matrix/store/crypto.db*
   # restart hermes
   ```
...
Argus supports the same gateway commands in Matrix that it supports on other
...
Argus only normalizes `!command` when the command is known to the gateway, a
...
If you previously used Argus with `MATRIX_ENCRYPTION=true` and are upgrading to
...
1. **Generate a new access token** to get a fresh device ID. The simplest way:
   ...
   curl -X POST https://your-server/_matrix/client/v3/login \
     ...
     -d '{
       ...
       "password": "***",
       "initial_device_display_name": "Argus Agent"
     }'
   ```

   Copy the new `access_token` and update `MATRIX_ACCESS_TOKEN` in `~/.argus/.env`.

2. **Delete old encryption state**:

   ```bash
   rm -f ~/.argus/platforms/matrix/store/crypto.db
   rm -f ~/.argus/platforms/matrix/store/crypto_store.*
   ```

3. **Set your recovery key** (if you use cross-signing — most Element users do). Add to `~/.argus/.env`:
...
a working E2EE setup with a previous version of Argus and are upgrading.
...
services:
  hermes-matrix:
    ...
    volumes:
      - ./matrix-store:/root/.argus/platforms/matrix/store
```
...
RUN cd ~/.argus/argus && uv pip install -e ".[matrix]"
...
| Setting | Description |
|---------|-------------|
| `GATEWAY_PROXY_URL` | URL of the remote Argus API server (e.g., `http://192.168.1.100:8642`) |
...
:::tip
Session continuity is maintained via the `X-Argus-Session-Id` header. The host's API server tracks sessions by this ID, so conversations persist across messages just like they would with a local agent.
:::
...
**Fix**: Argus uses an explicit sync loop that calls `client.handle_sync()` on
both the initial sync and every incremental sync response. This matches the
diagnosis in upstream issue #7914 and closed PR #37807, but keeps Argus's own
...
**Fix**: The sync loop automatically retries every 5 seconds on error. Check the Argus logs for sync-related warnings. If the bot consistently falls behind, ensure your homeserver has adequate resources.

### Bot is offline

**Cause**: The Argus gateway isn't running, or it failed to connect.

**Fix**: Check that `argus gateway` is running. Look at the terminal output for error messages. Common issues: wrong homeserver URL, expired access token, homeserver unreachable.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `MATRIX_ALLOWED_USERS`.

**Fix**: Add your User ID to `MATRIX_ALLOWED_USERS` in `~/.argus/.env` and restart the gateway. Use the full `@user:server` format.
...
For more information on securing your Argus Agent deployment, see the [Security Guide](../security.md).
...
- **Media support**: Argus can send and receive images, audio, video, and file attachments. Media is uploaded to your homeserver using the Matrix content repository API.
