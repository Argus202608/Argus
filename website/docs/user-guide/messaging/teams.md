...
description: "Set up Argus Agent as a Microsoft Teams bot"
---

# Microsoft Teams Setup

Connect Argus Agent to Microsoft Teams as a bot. Unlike Slack's Socket Mode, Teams delivers messages by calling a **public HTTPS webhook**, so your instance needs a publicly reachable endpoint — either a dev tunnel (local dev) or a real domain (production).
...
> Run `argus gateway setup` and pick **Microsoft Teams** for a guided walk-through.
...
Teams delivers @mentions as regular messages with `<at>BotName</at>` tags, which Argus strips automatically before processing.
...
Alternatively, configure via `~/.argus/config.yaml`:
...
| `No inference provider configured` | Check that `ANTHROPIC_API_KEY` (or another provider key) is set in `~/.argus/.env` |
...
| Teams shows "This bot is not responding" | The webhook returned an error. Check `docker logs argus` for tracebacks |
...
- Store credentials in `~/.argus/.env` with permissions `600` (`chmod 600 ~/.argus/.env`)
