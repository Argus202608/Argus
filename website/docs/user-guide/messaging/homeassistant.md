...
description: Control your smart home with Argus Agent via Home Assistant integration.
...
Argus Agent integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Argus Agent"
5. Copy the token
...
Argus Agent registers four tools for smart home control:
...
Configure which events the agent sees in `~/.argus/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Argus Agent".
...
The adapter reads credentials from `~/.argus/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Argus profile
...
running Argus — `curl -H "Authorization: Bearer <token>" <url>/api/` should
