...
Connect Argus to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Argus — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Argus runs it in a
...
argus photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Argus Agent` project on your account.
...
Runtime credentials are written to `~/.argus/.env`
...
(device token, dashboard project id) lives in `~/.argus/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Argus
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Argus replies with a pairing code. Approve it with:

```bash
argus pairing approve photon <CODE>
```

Use `argus pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.argus/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.argus/.env`):
...
By default Argus responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Argus` and
`@Argus agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Argus will reply.
...
assigned iMessage line Argus uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run argus photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `argus photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `argus photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `argus photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Argus sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Argus wake words  | JSON list / comma / newline regex patterns for group mentions |
