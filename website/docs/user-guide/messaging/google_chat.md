...
sidebar_position: 12
title: "Google Chat"
description: "Set up Argus Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Argus Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Argus
...
> Run `argus gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Argus can read (e.g.,
`~/.argus/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a argus restart)
...
Also grant `Pub/Sub Viewer` on the same subscription — Argus calls
...
- **App name**: whatever you want users to see ("Argus" is reasonable).
...
`ADDED_TO_SPACE` event that Argus uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Argus

Add the Google Chat section to `~/.argus/.env`:
...
Install the dependencies the Google Chat adapter needs (no Argus extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Argus is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Argus detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Argus session.
...
3. Download the JSON. Move it onto the host that runs Argus.
4. Register the client with Argus (run under the profile you want it scoped to):
...
argus -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Argus home (e.g.
`~/.argus/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.argus/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.argus/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Argus isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `argus gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Argus will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.argus/google_chat_user_tokens/<sanitized_email>.json` (filesystem
