...
title: "SMS (Twilio)"
description: "Set up Argus Agent as an SMS chatbot via Twilio"
---

# SMS Setup (Twilio)

Argus connects to SMS through the [Twilio](https://www.twilio.com/) API. People text your Twilio phone number and get AI responses back — same conversational experience as Telegram or Discord, but over standard text messages.
...
- **aiohttp** — `cd ~/.argus/argus && uv pip install -e ".[sms]"`
...
If you're running Argus locally, use a tunnel to expose the webhook:
...
Text your Twilio number — Argus will respond via SMS.
...
Argus validates that inbound webhooks genuinely originate from Twilio by verifying the `X-Twilio-Signature` header (HMAC-SHA1). This prevents attackers from injecting forged messages.
...
3. Check Argus gateway logs for Twilio API errors
