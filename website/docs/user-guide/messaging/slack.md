...
sidebar_position: 4
title: "Slack"
description: "Set up Argus Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Argus Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Argus instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Argus uses the modern
...
The fastest path is to paste a manifest Argus generates for you. It
...
### Option A: From an Argus-generated manifest (recommended)

1. Generate the manifest:
   ```bash
   argus slack manifest --write
   ```
   This writes `~/.argus/slack-manifest.json` and prints paste-in
   instructions.
...
4. Enter an app name (e.g., "Argus Agent") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, Argus can chat but **cannot reliably read user-uploaded attachments**.
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not an Argus configuration issue.
:::
...
Argus uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
argus gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
argus gateway              # Foreground
argus gateway install      # Install as a user service
sudo argus gateway install --system   # Linux only: boot-time system service
```
...
Every Argus command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
Argus command with its description.

Under the hood: Argus ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/MMArgus-Team/Argus/blob/main/hermes_cli/commands.py)
...
When Argus adds new commands (e.g. after `argus update`), regenerate
...
3. Paste the new contents of `~/.argus/slack-manifest.json`
4. **Save**. Slack will prompt to reinstall the app if scopes or slash
   commands changed.

### Legacy `/argus <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/argus btw run the tests` — Argus routes it the same way as `/btw
run the tests`. Free-form questions also work: `/argus what's the
...
Slack never delivers them to Argus.

As a workaround, Argus recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
Argus treats it identically to the slash form and replies in the same
thread.
...
Argus falls back to a text prompt, the prompt instructs you to reply
...
```bash
argus slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how Argus behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Argus Agent what time is it?`). In channels, Argus replies in a thread attached to that message. |
| **Threads** | If you @mention Argus inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.argus/config.yaml`.
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Argus in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before Argus will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Argus will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Argus Agent`).

---

## Multi-Workspace Support

Argus can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Argus also loads tokens from an **OAuth token file** at:

```
~/.argus/slack_tokens.json
```
...
- When a message arrives, Argus uses the correct workspace-specific client to respond.
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Argus Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Argus Agent` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Argus now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @Argus Agent`)
...
- Tokens should be stored in `~/.argus/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your Argus config directory
