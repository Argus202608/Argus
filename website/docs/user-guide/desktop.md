...
sidebar_position: 3
title: "Desktop App"
description: "The native Argus desktop app — a polished experience for chatting with Argus, with streaming tool output, side-by-side previews, a file browser, voice, cron, profiles, skills, and settings. macOS, Windows, and Linux."
---

# Desktop App

The Argus desktop app is a native app built around the **same** agent you get from the CLI and the gateway — same config, same API keys, same sessions, same skills, same memory. It is not a separate product or a lightweight clone; it uses the same Argus Agent core and settings, and drives it through a modern & thoughtfully designed UI. If you have used `argus` in a terminal, everything you set up there is already here, and anything you do here shows up there.

It runs on **macOS, Windows, and Linux**.

:::tip Which interface is which?
Argus has several front ends that all talk to the same agent:

- **Desktop App** (this page) — a native application with a purpose-built UI for chat, configuration, and management.
- **CLI** (`argus`) and **[TUI](./tui.md)** (`argus --tui`) — terminal interfaces.
- **[Web Dashboard](./features/web-dashboard.md)** (`argus dashboard`) — a browser admin panel; its optional **Chat** tab embeds the TUI through a pseudo-terminal.
...
Follow the [installation instructions for Argus Desktop](../getting-started/installation.md).

If you already have Argus installed, simply run
...
- **The same conversation history** as every other Argus surface — sessions started here resume in the CLI/TUI and vice versa.
...
Explore and preview the working directory without leaving the app — useful for following along as the agent reads, writes, and edits files. Set the initial project directory with `argus desktop --cwd <path>` (or the `ARGUS_DESKTOP_CWD` environment variable).

### Voice

Talk to Argus and hear it back, the same [voice mode](./features/voice-mode.md) available elsewhere. On macOS the OS will prompt once for microphone access.
...
- **Every provider and model in the menus** — the GUI surfaces the full provider list and every model that `argus model` knows about, so you pick from the same catalog the CLI sees rather than a curated subset.
...
The app also surfaces the broader Argus management surface so you don't have to drop to a terminal:

- **Skills** — browse, install, and manage [skills](./features/skills.md).
- **Cron** — view and manage [scheduled jobs](../reference/cli-commands.md#argus-cron).
- **Profiles** — switch between [Argus profiles](./profiles.md) (isolated config/skills/sessions).
...
The [manual update process](https://mmargus-team.github.io/Argus/docs/getting-started/updating) also works with the GUI.

## Uninstalling

Open **Settings → About → Danger zone** and pick how much to remove:

- **Uninstall Chat GUI only** — removes the desktop app and its data; the Argus agent, your config, and your chats stay. (Same as `argus uninstall --gui`.)
- **Uninstall GUI + agent, keep my data** — removes the app and the agent but keeps config, chats, and secrets for a future reinstall. (Same as `argus uninstall`.)
- **Uninstall everything** — removes the app, the agent, and all user data. (Same as `argus uninstall --full`.)
...
You can do the same from the terminal — `argus uninstall --gui` for the GUI alone, or `argus uninstall` / `argus uninstall --full` for the agent too.

:::note
Running `argus uninstall --gui` from a **source checkout** (a `argus desktop` dev build) also removes the workspace `node_modules` and `apps/desktop/{dist,release}` build output, since those are GUI build artifacts. They're recoverable with `argus desktop` (or `npm install` + a rebuild) — but if you're actively hacking on the desktop app, expect to reinstall dependencies afterward.
:::

## CLI reference: `argus desktop`

To launch via the CLI, simply run `argus desktop`. By default it installs workspace Node dependencies, builds the current OS's unpacked Electron app, then launches that packaged artifact.
...
| `--build-only`       | Build the desktop app but do not launch it (used by `argus update`)                      |
| `--source`           | Launch via `electron .` against `apps/desktop/dist` instead of the packaged app           |
| `--cwd PATH`         | Initial project directory for desktop chat sessions (sets `ARGUS_DESKTOP_CWD`)           |
| `--argus-root PATH`  | Override the Argus source root the app uses (sets `ARGUS_DESKTOP_ARGUS_ROOT`)          |
| `--ignore-existing`  | Force the app to ignore any `argus` CLI already on `PATH` during backend resolution      |
| `--fake-boot`        | Enable deterministic boot delays for validating the startup UI                            |

## How it works

The packaged app ships the Electron shell and a native React chat surface. On first launch it can install the Argus Agent runtime into `ARGUS_HOME` (`~/.argus`, or `%LOCALAPPDATA%\argus` on Windows) — **the same layout a CLI install uses**, which is why the two are interchangeable. Backend resolution first honours `ARGUS_DESKTOP_ARGUS_ROOT`, then a completed managed install, then a probed `argus` on `PATH` (unless `--ignore-existing` / `ARGUS_DESKTOP_IGNORE_EXISTING=1` is set), and finally an explicit `ARGUS_DESKTOP_ARGUS` command override for packagers such as Nix. The React renderer talks to a `argus dashboard` backend over the `tui_gateway`/dashboard APIs and reuses the agent runtime rather than embedding `argus --tui`. Install, backend-resolution, and self-update logic live in the Electron main process.

## Connecting to a remote backend

By default the app starts and manages its own **local** backend. You can instead point it at an Argus backend running on another machine — a VPS, a home server, or a Mini behind Tailscale.

:::info The remote backend is a running `argus dashboard` process
"Remote backend" means a **`argus dashboard`** server running on the remote machine — that is the process the desktop app connects to. Nothing in this section works unless that dashboard is actually up and reachable. The desktop app does not start it for you; you (or a `systemd` service) keep `argus dashboard` running on the remote host, and the app attaches to it. If you also use messaging channels (Telegram, Discord, etc.), the **gateway** is a *separate* long-running process you start independently — see the note after the setup steps.
:::
...
Set a username and password, then start the dashboard bound to a reachable address. The credentials live in `~/.argus/.env` (the secrets file, mode 0600):
...
Keep that `argus dashboard` process running for as long as you want the desktop app to be able to connect — if it stops, the app can no longer reach the backend. Run it under `systemd`, `tmux`, or your process manager of choice so it survives logout and reboots.
...
Prefer not to keep a plaintext password at rest? Set `ARGUS_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` to a scrypt hash instead — compute it with `python -c "from plugins.dashboard_auth.basic import hash_password; print(hash_password('PW'))"`. Full configuration surface (config.yaml keys, every env var, the rate limiter): [Web Dashboard → Username/password provider](./features/web-dashboard.md#usernamepassword-provider-no-oauth-idp).

Running the dashboard as a systemd service? Give the unit `EnvironmentFile=%h/.argus/.env` so the credentials are in the environment at boot.
...
1. **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/argus` work if you front it with a reverse proxy)
...
3. **Save and reconnect** — switches the desktop shell onto the remote backend. The session refreshes automatically; you stay signed in across restarts when `ARGUS_DASHBOARD_BASIC_AUTH_SECRET` is set.
...
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `ARGUS_DASHBOARD_BASIC_AUTH_USERNAME` / `ARGUS_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for an unknown user and a wrong password (no enumeration oracle), so double-check both. Confirm the gate is on with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
- **No "Sign in" button — it asks for a session token instead** — the backend's username/password provider isn't active. `/api/status` won't list `"basic"` in `auth_providers`. Make sure both the username and a password (or password hash) are set in `~/.argus/.env` and that the dashboard process actually loaded them.
- **Signed out on every restart** — set `ARGUS_DASHBOARD_BASIC_AUTH_SECRET` to a stable value. Without it the token-signing key is regenerated per boot, invalidating all sessions.
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting Argus Desktop to a remote backend](./features/web-dashboard.md#connecting-argus-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & Argus Desktop](../reference/environment-variables.md#web-dashboard--argus-desktop).

## Troubleshooting

Boot logs land in `ARGUS_HOME/logs/desktop.log` (it includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure. You can also tail it from the CLI:
...
ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/ \
  bash -c 'cd "$HOME/.argus/argus/apps/desktop" && CSC_IDENTITY_AUTO_DISCOVERY=false npm run pack'
```
...
```bash
ARGUS_DESKTOP_ARGUS_ROOT=/path/to/clone npm run dev
ARGUS_HOME=/tmp/throwaway npm run dev
...
- [TUI](./tui.md) — the modern terminal UI used by `argus --tui` and the dashboard chat tab
