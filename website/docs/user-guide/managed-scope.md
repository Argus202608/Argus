...
`~/.argus/config.yaml`, `~/.argus/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/argus`:

```text
/etc/hermes/
├── config.yaml     # managed config layer (wins over ~/.argus/config.yaml)
└── .env            # managed env layer (wins over ~/.argus/.env + shell)
```
...
The location can be relocated with the `ARGUS_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `ARGUS_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Argus.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export ARGUS_MANAGED_DIR=/opt/org/hermes-policy
```

:::warning
A user who can set `ARGUS_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `argus doctor` reports the *resolved* managed directory so
...
| 2 | `~/.argus/config.yaml` (user) | `~/.argus/.env` (user) |
...
```bash
argus config        # shows a header naming the managed source + the pinned keys
argus doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Argus refuses and names the source:

```bash
$ argus config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/hermes/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `argus config set` / setup will not write
...
Changes take effect on the next Argus start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`argus doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Argus as `root`), managed scope is advisory.
