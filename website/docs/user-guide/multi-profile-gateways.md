...
If you only run one Argus agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Argus agents that should all
...
argus profile create coder
argus profile create personal-bot
argus profile create research
...
```bash
argus config set gateway.multiplex_profiles true
argus gateway restart
```

Equivalently, in the default profile's `~/.argus/config.yaml`:
...
You do **not** run `argus gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `argus gateway start` / `run` is a
...
home). `argus status` reports the multiplexer and the profiles it serves;
`argus status -p <name>` slices to one profile. Each profile still writes its
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    argus gateway "$action"
  else
    argus -p "$profile" gateway "$action"
  fi
}
...
hermes-gateways list       # delegates to `argus gateway list`
```

:::tip
The `default` profile is targeted with `argus gateway <action>` (no `-p`),
not `argus -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `argus -p coder gateway <action>` — useful if a
...
tail -f ~/.argus/logs/gateway.error.log

# Named profile
tail -f ~/.argus/profiles/<name>/logs/gateway.log
tail -f ~/.argus/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.argus/logs/gateway.log ~/.argus/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
argus logs -f                  # follow default profile
argus -p coder logs -f         # follow one profile
argus logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
argus profile list             # profiles + model + gateway state
hermes-gateways status          # full status across every profile
launchctl list | grep argus    # macOS — PIDs and labels
...
The default profile uses `~/.argus/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
argus config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.argus/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=argus --why="gateways running" \
  sleep infinity &
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.argus/.env ~/.argus/profiles/*/.env
```

## Updating the code

`argus update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `argus gateway start` after a previous `argus gateway stop`. The
...
cat ~/.argus/profiles/<profile>/gateway.pid
...
```bash
argus doctor                  # default profile
argus -p <profile> doctor     # one profile
```
