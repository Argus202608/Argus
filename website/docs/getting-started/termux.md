...
description: "Run Argus Agent directly on an Android phone with Termux"
---

# Argus on Android with Termux
...
Argus Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Argus from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Argus now ships a Termux-aware installer path:
...
- links `argus` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `argus` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/argus" "$PREFIX/bin/argus"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `argus` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.argus/.env`.
...
### `argus doctor` says ripgrep or Node is missing
