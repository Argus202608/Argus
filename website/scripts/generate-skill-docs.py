...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/MMArgus-Team/Argus/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    title = f"{name}"
    # Public display names may differ from stable skill identifiers. This lets
    # renamed products keep accepting an established `/skill <name>` alias.
    display_name = fm.get("display_name") or name.replace("-", " ").replace("_", " ").title()
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `argus skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Argus"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Argus ships with a large built-in skill library copied into `~/.argus/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Argus also syncs bundled skills on `argus update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.argus/skills/` tree, it is still shipped with Argus; restore it with `argus skills reset <name> --restore`.",
        "",
    ...
    for category in sorted(by_cat):
        ...
        for meta, parsed in by_cat[category]:
            ...
            name = fm.get("display_name") or fm.get("name", meta["slug"])
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with Argus — install via argus skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with Argus under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
        "argus skills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "argus skills install official/blockchain/solana",
        "argus skills install official/mlops/flash-attention",
        "```",
        "",
        ...
        "",
        "```bash",
        "argus skills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):
