...
sidebar_position: 17
title: "扩展 Dashboard"
description: "为 Argus Web Dashboard 构建主题和插件——调色板、字体排版、布局、自定义标签页、shell 插槽、页面级插槽以及后端 API 路由"
---

# 扩展 Dashboard

Argus Web Dashboard（`argus dashboard`）在设计上支持换肤和扩展，无需 fork 代码库。对外暴露三个层次：

1. **主题（Themes）** — YAML 文件，用于重绘 dashboard 的调色板、字体排版、布局以及各组件的外观。将文件放入 `~/.argus/dashboard-themes/`，即可在主题切换器中看到它。
...
主题是存储在 `~/.argus/dashboard-themes/` 中的 YAML 文件。文件名无关紧要（系统使用主题的 `name:` 字段），但惯例是 `<name>.yaml`。所有字段均为可选——缺失的键会回退到内置的 `default` 主题，因此一个主题可以只包含一个颜色。
...
| 主题 | 调色板 | 字体排版 | 布局 |
|-------|---------|------------|--------|
| **Argus Teal**（`default`） | 深青色 + 奶油色 | 系统字体栈，15px | 0.5rem 圆角，comfortable |
| **Argus Teal (Large)**（`default-large`） | 同 default | 系统字体栈，18px，行高 1.65 | 0.5rem 圆角，spacious |
...
引用 Google Fonts 的主题（除 Argus Teal 外均如此）会按需加载样式表——首次切换时会向 `<head>` 注入一个 `<link>` 标签。
...
Dashboard 插件是一个包含 `manifest.json`、预构建 JS bundle，以及可选的 CSS 文件和带 FastAPI 路由的 Python 文件的目录。插件与其他 Argus 插件一起存放在 `~/.argus/plugins/<name>/`——dashboard 扩展是该插件目录内的 `dashboard/` 子文件夹，因此一个插件可以从单次安装中同时扩展 CLI/gateway 和 dashboard。
...
| 优先级 | 目录 | 来源标签 |
|----------|-----------|--------------|
| 1（冲突时优先） | `~/.argus/plugins/<name>/dashboard/` | `user` |
| 2 | `<repo>/plugins/memory/<name>/dashboard/` | `bundled` |
| 2 | `<repo>/plugins/<name>/dashboard/` | `bundled` |
| 3 | `./.argus/plugins/<name>/dashboard/` | `project`——仅在设置 `HERMES_ENABLE_PROJECT_PLUGINS` 时生效 |
...
打开 dashboard，从主题切换器中选择 **Strike Freedom**。驾驶舱侧边栏出现，徽标显示在顶栏，标语替换底栏。切换回 **Argus Teal**，插件仍然安装但不可见（`sidebar` 插槽仅在 `cockpit` 布局变体下渲染）。
...
**我的主题没有出现在选择器中。**
检查文件是否在 `~/.argus/dashboard-themes/` 中且以 `.yaml` 或 `.yml` 结尾。刷新页面。运行 `curl http://127.0.0.1:9119/api/dashboard/themes`——你的主题应出现在响应中。如果 YAML 有解析错误，dashboard 会记录到 `~/.argus/logs/` 下的 `errors.log`。

**我的插件标签页没有显示。**
1. 检查 manifest 是否在 `~/.argus/plugins/<name>/dashboard/manifest.json`（注意 `dashboard/` 子目录）。
...
2. 重启 `argus dashboard`——插件 API 路由在启动时挂载一次，**不会**在重新扫描时挂载。
3. 检查 `plugin_api.py` 是否导出了模块级的 `router = APIRouter()`。其他导出名称不会被识别。
4. 查看 `~/.argus/logs/errors.log` 中的 `Failed to load plugin <name> API routes`——导入错误会记录在那里。
...
**我想在 PyPI 上发布插件。**
Dashboard 插件通过目录结构安装，而非 pip 入口点。目前最简洁的分发方式是用户克隆到 `~/.argus/plugins/` 的 git 仓库。基于 pip 的 dashboard 插件安装器目前尚未实现。