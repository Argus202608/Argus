...
| | |
|---|---|
| 来源 | 可选 — 通过 `argus skills install official/devops/watchers` 安装 |
...
安装 skill 后，三个脚本均位于 `$ARGUS_HOME/skills/devops/watchers/scripts/`。每个脚本读取 `WATCHER_STATE_DIR`（默认为 `$ARGUS_HOME/watcher-state/`）作为状态文件目录，以 `--name` 参数作为键名。
...
```bash
python $ARGUS_HOME/skills/devops/watchers/scripts/watch_rss.py \
...
```bash
python $ARGUS_HOME/skills/devops/watchers/scripts/watch_github.py \
  --name hermes-issues --repo MMArgus-Team/Argus --scope issues
```

轮询任意 JSON API：

```bash
python $ARGUS_HOME/skills/devops/watchers/scripts/watch_http_json.py \
...
每个 watcher 将状态写入 `$ARGUS_HOME/watcher-state/<name>.json`。查看状态：

```bash
cat $ARGUS_HOME/watcher-state/hn.json
```

强制重放（下次运行视为首次轮询）：

```bash
rm $ARGUS_HOME/watcher-state/hn.json
```
...
4. **状态目录位于 agent 沙箱无法写入的位置。** `$ARGUS_HOME/watcher-state/` 始终可写。Docker/Modal 后端可能无法访问任意宿主机路径。