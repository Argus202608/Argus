...
title: "Git Worktrees"
description: "使用 git worktrees 和隔离检出在同一仓库中安全运行多个 Argus agent"
---

# Git Worktrees

Argus Agent 常用于大型、长期维护的仓库。当你需要：
...
本页介绍如何将 worktrees 与 Argus 结合使用，使每个会话拥有干净、隔离的工作目录。

## 为什么在 Argus 中使用 Worktrees？

Argus 将**当前工作目录**视为项目根目录：

- CLI：运行 `argus` 或 `argus chat` 时所在的目录
...
现在可以 `cd` 进入新 worktree 并在其中运行 Argus：

```bash
cd ../repo-feature

# 在 worktree 中启动 Argus
argus
```
...
- `~/.argus/checkpoints/` 下的 Argus checkpoint 数据在移除 worktree 时不会自动清理，但通常体积很小。

## 最佳实践

- **每个 Argus 实验对应一个 worktree**
...
- **使用 worktrees 时避免从裸仓库根目录运行 Argus**
  - 优先使用 worktree 目录，使每个 agent 拥有明确的作用范围。

## 使用 `argus -w`（自动 Worktree 模式）

Argus 内置 `-w` 标志，可**自动创建一个一次性 git worktree** 及其独立分支。无需手动配置 worktree——只需 `cd` 进入仓库并运行：
...
如需并行运行多个 agent，在多个终端中分别运行 `argus -w`——每次调用都会自动获得独立的 worktree 和分支。

## 综合运用

- 使用 **git worktrees** 为每个 Argus 会话提供独立的干净检出。
- 使用**分支**记录实验的高层级历史。
