...
ACP 适配器将 Argus 的同步 `AIAgent` 封装为异步 JSON-RPC stdio 服务器。
...
Zed ACP Registry 路径通过 `uvx --from 'mm-argus[acp]==<version>' argus-acp` 启动同一适配器，指向 `mm-argus` PyPI 发布包。
...
`acp_adapter/tools.py` 将 Argus 工具映射到 ACP 工具类型，并构建面向编辑器的内容。
...
因此 ACP 通告并使用当前配置的 Argus provider/凭据。它还始终通告一个终端 setup 认证方法（`hermes-setup`，参数 `--setup`），以便首次运行的 registry 客户端在启动正常 ACP 会话前可以打开 Argus 的交互式模型/provider 配置。
...
- ACP 会话持久化至共享的 `~/.argus/state.db`（SessionDB），在进程重启后透明恢复；它们会出现在 `session_search` 中
...
- `tests/acp/` — ACP 测试套件
- `toolsets.py` — `argus-acp` toolset 定义（并保留旧的 `hermes-acp` 别名）
- `hermes_cli/main.py` — `argus acp` CLI 子命令
- `pyproject.toml` — `[acp]` 可选依赖 + 标准 `argus-acp` 脚本
