...
每个 Argus `task_id` 对应一个在后台守护线程中运行的 `asyncio.Task`。持有一个到后端 CDP 端点的持久 WebSocket 连接。维护：
...
- `toolsets.py` — 在 `browser`、`argus-acp`、`argus-api-server`、核心工具集中注册 `browser_dialog`（通过 CDP 可达性门控）
