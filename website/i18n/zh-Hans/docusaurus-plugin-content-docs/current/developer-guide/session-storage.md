...
Argus Agent 使用 SQLite 数据库（`~/.argus/state.db`）跨 CLI 和 gateway 会话持久化会话元数据、完整消息历史及模型配置。这替代了早期的逐会话 JSONL 文件方案。
...
```
~/.argus/state.db (SQLite, WAL mode)
...
多个 argus 进程（gateway + CLI 会话 + worktree agent）共享同一个 `state.db`。`SessionDB` 类通过以下方式处理写入竞争：
...
db = SessionDB()                           # 默认：~/.argus/state.db
...
该路径由 `hermes_constants.get_hermes_home()` 推导，默认解析为 `~/.argus/`，或 `ARGUS_HOME` 环境变量的值。
