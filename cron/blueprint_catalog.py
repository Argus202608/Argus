...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + an ``argus://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the canonical ``argus://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"argus://blueprint/{quote(blueprint.key)}{qs}"
