...
Uses a file-based lock (~/.argus/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `argus update` reloads
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``argus tools`` config for the ``cron`` platform.
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("ARGUS_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid ARGUS_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    os.environ["ARGUS_CRON_SESSION"] = "1"
    ...
    # gateway message. Do not seed ARGUS_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads ARGUS_SESSION_PLATFORM
    #     and ARGUS_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via ARGUS_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     ARGUS_SESSION_PLATFORM == "telegram".
    ...
    #     ARGUS_SESSION_PLATFORM.
    #   - tools/send_message_tool.py: mirror source labelling and the
    #     send_message gate read ARGUS_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the ARGUS_CRON_AUTO_DELIVER_* vars set
    # below, so clearing ARGUS_SESSION_* here does not affect delivery.
    ...
    _cron_delivery_vars = (
        "ARGUS_CRON_AUTO_DELIVER_PLATFORM",
        "ARGUS_CRON_AUTO_DELIVER_CHAT_ID",
        "ARGUS_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    try:
        ...
        if delivery_target:
            _VAR_MAP["ARGUS_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["ARGUS_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["ARGUS_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
                if delivery_target.get("thread_id") is None
                else str(delivery_target["thread_id"])
            )

        # Model resolution precedence: per-job override > ARGUS_MODEL env >
        ...
        model = job.get("model") or os.getenv("ARGUS_MODEL") or ""
        ...
        prefill_file = (
            os.getenv("ARGUS_PREFILL_MESSAGES_FILE", "")
        ...
        try:
            # Do not inject ARGUS_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        # override via ARGUS_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("ARGUS_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid ARGUS_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
def tick(verbose: bool = True, adapters=None, loop=None, sync: bool = True) -> int:
    ...
    try:
        ...
        # Set ARGUS_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("ARGUS_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid ARGUS_CRON_MAX_PARALLEL value; defaulting to unbounded")
