...
argus kanban watch --tenant <project-slug>

# Snapshot of the board
argus kanban list --tenant <project-slug>
argus kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
argus kanban stats --tenant <project-slug>

# Visual dashboard (browser)
argus dashboard

# Inspect a specific task (includes comments + events)
argus kanban show <task-id>

# Follow a single task's event stream
argus kanban tail <task-id>
```

Verify available subcommands with `argus kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `argus kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `argus kanban block <id>` then `argus kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `argus kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `argus kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `argus profile list` (profile exists?), `argus status` (gateway/dispatcher up?), and verify tenant. |
...
argus kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
argus kanban create "Scene 3 — re-render with feedback" \
...
argus kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
argus kanban block <task-id>
argus kanban archive <task-id>

# Diagnose what happened
argus kanban show <task-id>      # task body, comments, recent events
argus kanban tail <task-id>      # follow the live event stream
argus kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `argus kanban create` for the director
...
while true; do
    clear
    argus kanban list --tenant <slug>
    echo "---"
    argus kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `argus kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `argus profile list` and start
