...
2. **Create profiles** — `argus profile create <name> --clone`
...
6. **Fire the initial kanban task** — `argus kanban create` assigned to the director
...
```bash
argus profile create director --clone 2>/dev/null || true
```
...
```bash
argus kanban create "Direct production of <video title>" \
    --assignee director \
