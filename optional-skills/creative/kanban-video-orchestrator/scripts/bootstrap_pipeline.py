...
and the `argus kanban create --workspace dir:` initial-task pattern are
...
def render_setup_sh(plan: dict, brief_md: str, team_md: str) -> str:
    ...
    for t in plan["team"]:
        profile_creates.append(
            f'argus profile create {t["profile"]} --clone 2>/dev/null || true'
        )
    ...
    for t in plan["team"]:
        soul_writes.append(
            f'cat > "$HOME/.argus/profiles/{t["profile"]}/SOUL.md" <<\'SOUL_EOF\'\n'
