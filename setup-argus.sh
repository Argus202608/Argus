...
# 5. Symlinks the canonical 'argus' CLI command (plus legacy 'hermes' alias)
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/argus-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — expose Argus plus the legacy Hermes alias
# ============================================================================

echo -e "${CYAN}→${NC} Setting up argus command..."

ARGUS_BIN="$SCRIPT_DIR/venv/bin/argus"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$ARGUS_BIN" "$COMMAND_LINK_DIR/argus"
ln -sf "$ARGUS_BIN" "$COMMAND_LINK_DIR/hermes"
echo -e "${GREEN}✓${NC} Symlinked argus → $COMMAND_LINK_DISPLAY_DIR/argus"
echo -e "${GREEN}✓${NC} Kept legacy hermes alias → $COMMAND_LINK_DISPLAY_DIR/hermes"
...
# ============================================================================
# Seed bundled skills into ~/.argus/skills/
# ============================================================================

ARGUS_SKILLS_DIR="${ARGUS_HOME:-$HOME/.argus}/skills"
mkdir -p "$ARGUS_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.argus/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$ARGUS_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  argus status        # Check configuration"
if is_termux; then
    echo "  argus gateway       # Run gateway in foreground"
else
    echo "  argus gateway install # Install gateway service (messaging + cron)"
fi
echo "  argus cron list      # View scheduled jobs"
echo "  argus doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m argus_cli.main setup
fi
