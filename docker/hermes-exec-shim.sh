...
# $ARGUS_HOME — auth.json, .env, config.yaml — ends up root-owned and
...
# $ARGUS_HOME is uid-aligned with the supervised processes. When invoked
...
case "${ARGUS_DOCKER_EXEC_AS_ROOT:-}" in
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "hermes-shim: re-run with --user hermes or set ARGUS_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi
