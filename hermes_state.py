...
-- Multimodal sidechannel: monitor alerts + watcher round reports + watcher
-- streaming segments + watcher final reports. These are stored SEPARATELY from
-- the main-agent `messages` table because they are proactive UI events the
-- main agent should never re-read as conversation history. On session reopen
-- the frontend loads them via dedicated RPCs and renders them in the right
-- multimodal panel (never as chat bubbles).
CREATE TABLE IF NOT EXISTS mm_monitor_alerts (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    session_id TEXT NOT NULL REFERENCES sessions(id),
...
-- Streaming per-segment payloads that make up the DeepWindow view. One row per
-- watcher.report_append / multimodal.bg segment update.
CREATE TABLE IF NOT EXISTS mm_watcher_segments (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    session_id TEXT NOT NULL REFERENCES sessions(id),
    watcher_id TEXT NOT NULL,
...
-- Final report per watcher_id (one row per watcher). Upserted on watcher.final.
CREATE TABLE IF NOT EXISTS mm_watcher_finals (
    session_id TEXT NOT NULL,
    watcher_id TEXT NOT NULL,
    text TEXT NOT NULL,
    wall_ts REAL NOT NULL,
    PRIMARY KEY (session_id, watcher_id),
    FOREIGN KEY (session_id) REFERENCES sessions(id)
);

CREATE INDEX IF NOT EXISTS idx_mm_monitor_alerts_session ON mm_monitor_alerts(session_id, wall_ts);
CREATE INDEX IF NOT EXISTS idx_mm_watcher_reports_session ON mm_watcher_reports(session_id, wall_ts);
CREATE INDEX IF NOT EXISTS idx_mm_watcher_segments_session ON mm_watcher_segments(session_id, watcher_id, seg_idx);
"""
...
class SessionDB:
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `argus update` to rebuild the venv with a "
    ...
    # ── Multimodal sidechannel (monitor alerts + watcher content) ────────────
    # These live in dedicated tables so they never mingle with the main-agent
    # LLM conversation. On session reopen the frontend hydrates the right
    # multimodal panel via the RPCs backed by list_mm_* below.
    ...
    def insert_mm_watcher_report(
        ...
        round_idx: int = 0, label: str | None = None,
        wall_ts: float | None = None,
    ) -> None:
        ts = float(wall_ts) if wall_ts is not None else time.time()
        def _do(conn):
            conn.execute(
                "INSERT INTO mm_watcher_reports "
                "(session_id, watcher_id, round_idx, text, label, wall_ts) "
                "VALUES (?, ?, ?, ?, ?, ?)",
                (session_id, watcher_id, int(round_idx), text, label, ts),
            )
        self._execute_write(_do)

    def list_mm_watcher_reports(self, session_id: str) -> list[dict]:
        with self._lock:
            cur = self._conn.execute(
                "SELECT watcher_id, round_idx, text, label, wall_ts "
                "FROM mm_watcher_reports WHERE session_id = ? "
                "ORDER BY wall_ts ASC, id ASC",
                (session_id,),
            )
            return [{"watcher_id": r[0], "round_idx": r[1], "text": r[2],
                     "label": r[3], "wall_ts": r[4]} for r in cur.fetchall()]

    def insert_mm_watcher_segment(
        self, session_id: str, watcher_id: str, seg_idx: int,
        text: str | None = None, payload: str | None = None,
        wall_ts: float | None = None,
    ) -> None:
        ts = float(wall_ts) if wall_ts is not None else time.time()
        def _do(conn):
            conn.execute(
                "INSERT INTO mm_watcher_segments "
                "(session_id, watcher_id, seg_idx, text, payload, wall_ts) "
                "VALUES (?, ?, ?, ?, ?, ?)",
                (session_id, watcher_id, int(seg_idx), text, payload, ts),
            )
        self._execute_write(_do)

    def list_mm_watcher_segments(self, session_id: str) -> list[dict]:
        with self._lock:
            cur = self._conn.execute(
                "SELECT watcher_id, seg_idx, text, payload, wall_ts "
                "FROM mm_watcher_segments WHERE session_id = ? "
                "ORDER BY watcher_id ASC, seg_idx ASC, wall_ts ASC",
                (session_id,),
            )
            return [{"watcher_id": r[0], "seg_idx": r[1], "text": r[2],
                     "payload": r[3], "wall_ts": r[4]} for r in cur.fetchall()]

    def upsert_mm_watcher_final(
        self, session_id: str, watcher_id: str, text: str,
        wall_ts: float | None = None,
    ) -> None:
        ts = float(wall_ts) if wall_ts is not None else time.time()
        def _do(conn):
            conn.execute(
                "INSERT INTO mm_watcher_finals (session_id, watcher_id, text, wall_ts) "
                "VALUES (?, ?, ?, ?) "
                "ON CONFLICT(session_id, watcher_id) DO UPDATE SET "
                "text = excluded.text, wall_ts = excluded.wall_ts",
                (session_id, watcher_id, text, ts),
            )
        self._execute_write(_do)

    def list_mm_watcher_finals(self, session_id: str) -> list[dict]:
        with self._lock:
            cur = self._conn.execute(
                "SELECT watcher_id, text, wall_ts FROM mm_watcher_finals "
                "WHERE session_id = ? ORDER BY wall_ts ASC",
                (session_id,),
            )
            return [{"watcher_id": r[0], "text": r[1], "wall_ts": r[2]}
                    for r in cur.fetchall()]

    def clear_mm_sidechannel(self, session_id: str) -> None:
        """Wipe all monitor/watcher sidechannel rows for a session (rewind reset)."""
        def _do(conn):
            for t in ("mm_monitor_alerts", "mm_watcher_reports",
                      "mm_watcher_segments", "mm_watcher_finals"):
                conn.execute(f"DELETE FROM {t} WHERE session_id = ?",
                             (session_id,))
        self._execute_write(_do)
        ...
        def _do(conn):
            cursor = conn.execute(
                """INSERT INTO messages (session_id, role, content, tool_call_id,
                   ...
                   codex_message_items, platform_message_id, observed, model)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (
                    session_id,
                    role,
    ...
    def get_messages_as_conversation(
        ...
        with self._lock:
            ...
            rows = self._conn.execute(
                ...
                "codex_reasoning_items, codex_message_items, platform_message_id, observed, timestamp, model "
        ...
        for row in rows:
            ...
            if row["role"] == "assistant":
                # Per-message model: the model actually used for this turn.
                # Kept faithful to history — older rows / non-assistant turns
                # have model=NULL and simply omit the field.
                if row["model"]:
                    msg["model"] = row["model"]
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``ARGUS_DISABLE_FTS_TRIGRAM`` or not yet created), so
