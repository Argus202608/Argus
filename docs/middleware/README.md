...
Argus middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Argus'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Argus calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Argus retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Argus guardrails,
...
Argus stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Argus runs
them as a nested chain in registration order. Middleware failures are fail-open:
Argus logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Argus applies middleware in this order:
...
For each tool call, Argus applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Argus pre-execution path against the effective arguments:
...
```bash
argus plugins enable <plugin-name>
```
...
```bash
export ARGUS_HOME=/tmp/argus-middleware-test
mkdir -p "$ARGUS_HOME"
argus plugins enable <plugin-name>
argus chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run argus plugins enable <plugin-name>
uv run argus chat --query 'Reply exactly ok'
```
...
Return the same response shape Argus expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Argus treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Argus preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Argus does not convert downstream
