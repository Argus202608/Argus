...
This document is the formal interface between the **Argus gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `argus gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`argus gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `argus` CLI.
