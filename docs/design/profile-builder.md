...
Supersedes: PR #31781 (prompt_toolkit `argus profile wizard`)

## Why this, not the CLI wizard

PR #31781 added a keyboard-driven `argus profile wizard` in the terminal.
...
A profile = a full `~/.argus/profiles/<name>/` directory with its own:
...
5. spawn `argus -p <profile> skills install <id>` per hub skill (async, returns PIDs)
