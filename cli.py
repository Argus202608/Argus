#!/usr/bin/env python3
"""
Argus CLI - Interactive Terminal Interface

A beautiful command-line interface for the Argus, inspired by Claude Code.
...
os.environ["ARGUS_QUIET"] = "1"  # Our own modules
...
def _resolve_prefill_messages_file(config: Dict[str, Any]) -> str:
    ...
    """
    env_path = os.getenv("ARGUS_PREFILL_MESSAGES_FILE", "").strip()
    if env_path:
        return env_path
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.argus/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If ARGUS_IGNORE_USER_CONFIG=1 is set (via ``hermes chat --ignore-user-config``),
    the user config at ``~/.argus/config.yaml`` is skipped entirely and only the
    ...
    ignore_user_config = os.environ.get("ARGUS_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain Argus, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me Argus - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
    ...
    # `argus config`/`doctor`/guards (which use load_config) honor it. The
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["ARGUS_REDACT_SECRETS"] = str(redact).lower()
...
# Initialize centralized logging early — agent.log + errors.log in ~/.argus/logs/.
...
def _prepare_deferred_agent_startup() -> None:
    ...
    if _deferred_agent_startup_done:
        return
    if os.environ.get("ARGUS_DEFER_AGENT_STARTUP") != "1":
        return
    _deferred_agent_startup_done = True
    _accept_hooks = os.environ.get("ARGUS_ACCEPT_HOOKS", "").lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _resolve_worktree_base(repo_root: str) -> tuple:
    ...
    (the ``~/.argus/hermes-agent`` clone is updated only by ``argus update``,
...
def _setup_worktree(repo_root: str = None, sync_base: bool = True) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run argus -w")
        return None
...
#
# Detection priority:
#   1. ARGUS_LIGHT / ARGUS_TUI_LIGHT env (true/false) — explicit override
#   2. ARGUS_TUI_THEME=light|dark — explicit theme
#   3. ARGUS_TUI_BACKGROUND=#RRGGBB — explicit bg hint
#   4. COLORFGBG env (set by xterm/Konsole/urxvt) — bg slot 7/15 = light
#   5. OSC 11 query (\x1b]11;?\x1b\\) — ask the terminal directly
#   6. Default: assume dark (matches the legacy Argus assumption)
#
...
def _detect_light_mode() -> bool:
    ...
    try:
        ...
        for var in ("ARGUS_LIGHT", "ARGUS_TUI_LIGHT"):
            ...
            if _FALSE_RE.match(v):
                ...
                return result
        # 2. Theme hint
        theme = (os.environ.get("ARGUS_TUI_THEME") or "").strip().lower()
        if theme == "light":
            result = True
            _LIGHT_MODE_CACHE = result
            return result
        if theme == "dark":
            ...
            return result
        # 3. Explicit bg hex
        bg_hint = os.environ.get("ARGUS_TUI_BACKGROUND") or ""
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in Argus output it's common to display cron schedules verbatim.
    #
...
# ASCII Art - Argus Caduceus (compact, fits in left panel)
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "⚕ ARGUS - AI Agent Framework"
        tiny_line = "⚕ ARGUS"
    else:
        agent_name = _skin.get_branding("agent_name", "Argus") if _skin else "Argus"
        line1 = f"{agent_name} - AI Agent Framework"
        tiny_line = agent_name

    if os.environ.get("ARGUS_FAST_STARTUP_BANNER") == "1":
        from hermes_cli import __release_date__ as _release_date
        from hermes_cli import __version__ as _version

        version_line = f"Argus v{_version} ({_release_date})"
    else:
        version_line = format_banner_version_label()

    w = min(shutil.get_terminal_size().columns - 2, 88)
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]- MMArgus Team[/]\n"
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.argus/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.argus/config.yaml on first use)
...
class HermesCLI(CLIAgentSetupMixin, CLICommandsMixin):
    ...
    def __init__(
        ...
        # Read max_tokens from config (env var override: ARGUS_MAX_TOKENS)
        ...
        self.requested_provider = (
            ...
            or os.getenv("ARGUS_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("ARGUS_MAX_ITERATIONS"):
            try:
                self.max_turns = int(os.getenv("ARGUS_MAX_ITERATIONS", ""))
            except (TypeError, ValueError):
                self.max_turns = 90
        else:
            self.max_turns = 90
        ...
        self.ignore_rules = ignore_rules or os.environ.get("ARGUS_IGNORE_RULES") == "1"
        
        # Ephemeral system prompt: env var takes precedence, then config
        self.system_prompt = (
            os.getenv("ARGUS_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        try:
            ...
            try:
                ...
                Console(stderr=True).print(
                    ...
                    f"  Reason: {e}\n"
                    "  Fix the state.db store (e.g. `argus update` to rebuild the venv) to restore persistence."
                )
            except Exception:
        ...
        # it's shared across all Argus processes for this HERMES_HOME.
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            return f"⚕ {self.model if getattr(self, 'model', None) else 'Argus'}"
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from hermes_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "⚕ Argus")
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``argus doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``argus doctor`` so the banner stays
        small.
        """
        try:
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Argus needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.argus/images/ and appends the path to
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'argus setup' to configure[/]")
        except Exception:
    ...
    def show_help(self):
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Argus!{_RST}")
    ...
    def show_history(self):
        ...
        for msg in self.conversation_history:
            ...
            print(f"\n  [Argus #{visible_index}]{_ts_suffix(msg)}")
            ...
            if self._session_db:
                try:
                    ...
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("ARGUS_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.argus/sessions/saved/.
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: argus --resume {self.session_id}")
        except Exception as e:
    ...
    def _confirm_expensive_model_switch(self, result) -> bool:
        ...
        choices = [
            ("once", "Switch anyway", "Use this model for the current Argus session."),
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `argus model` and gateway pickers).
    ...
    def _handle_codex_runtime(self, cmd_original: str) -> None:
        ...
        Usage:
            ...
            /codex-runtime auto                  — Argus default (chat_completions)
        ...
        if canonical in {"quit", "exit"}:
            ...
            try:
                ...
                if not user_entries:
                    ...
                    print("  Install one: argus plugins install owner/repo")
                    print(f"  Or drop a plugin directory into {display_hermes_home()}/plugins/")
                    if bundled_count:
                        print(f"  ({bundled_count} bundled plugins available — see: argus plugins list)")
                else:
                    ...
                    if bundled_count:
                        print(f"  (+{bundled_count} bundled — see: argus plugins list)")
                    print("  Enable/disable: argus plugins enable/disable <name>")
            except Exception as e:
    ...
    def _is_session_yolo_active(self) -> bool:
        ...
        ``ARGUS_YOLO_MODE`` into ``_YOLO_MODE_FROZEN`` before tool imports
        happen.
        """
        try:
            from tools.approval import (
    ...
    def _toggle_yolo(self):
        ...
        ``ARGUS_YOLO_MODE`` here — that env var is read once at module import
    ...
    def _print_nous_credits_block(self) -> bool:
        ...
        wall-clock-bounded inside the helper; also honors ARGUS_DEV_CREDITS_FIXTURE
    ...
    def _show_credits(self):
        ...
        if not view.logged_in:
            ...
            print("  Run `argus portal` to log in, then /credits.")
            return
    ...
    def _show_billing(self, command: str = "/billing"):
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                _cprint(f"  💳 {_d('Not logged into Nous Portal.')}")
                print("  Run `argus portal` to log in, then /billing.")
            return
    ...
    def _billing_handle_scope_required(self, state):
        ...
        if not getattr(self, "_app", None):
            print("  Run `argus portal` and approve terminal billing, then retry.")
            return
        confirm_choices = [
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.argus/skills/ and queue a note for the
        next user turn.
    ...
    def chat(self, message, images: list = None) -> Optional[str]:
        ...
        if images:
            # Native image attachment. If the active model doesn't support
            # vision the provider will error — configure a vision-capable
            # model. The old describe-then-text fallback is gone.
            from agent.image_routing import build_native_content_parts
            _text_for_parts = message if isinstance(message, str) else ""
            _img_str_paths = [str(p) for p in images]
            _parts, _skipped = build_native_content_parts(
                _text_for_parts, _img_str_paths,
            )
            if _skipped:
                _cprint(
            ...
            if any(p.get("type") == "image_url" for p in _parts):
                ...
                _cprint(
                    f"  {_DIM}📎 attaching {len(images)} image(s): {_img_names}{_RST}"
                )
                message = _parts
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            print(f"  argus --resume {self.session_id}{profile_flag}")
            if session_title:
                print(f"  argus -c \"{session_title}\"{profile_flag}")
            print()
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Argus! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Argus! Type your message or /help for commands."
        ...
        try:
            ...
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    ...
                    f"(ARGUS_REDACT_SECRETS={_redact_raw}). "
        ...
        # after an OpenClaw→Argus migration (especially migrations done by
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Argus binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from hermes_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Argus")
        ...
        # Dynamic prompt: shows Argus symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Argus needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Argus needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Argus needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``ARGUS_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("ARGUS_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: argus setup"
            )
            _run_cleanup()
...
def _run_kanban_goal_loop_q(cli: "HermesCLI", first_response: str) -> None:
    ...
    only when ``ARGUS_KANBAN_GOAL_MODE`` is set (dispatcher-spawned
    ...
    task_id = (_os.environ.get("ARGUS_KANBAN_TASK") or "").strip()
    if not task_id:
        return
...
def main(
    ...
    """
    Argus CLI - Interactive AI Assistant
    ...
    if gateway:
        ...
        print("Starting Argus Gateway (messaging platforms)...")
    ...
    if toolsets:
        ...
        # Coding posture (base Argus): with no explicit --toolsets, collapse
    ...
    # so main unwinds normally.  ARGUS_SIGTERM_GRACE overrides the 1.5 s
    ...
    def _signal_handler_q(signum, frame):
        ...
        if os.environ.get("ARGUS_KANBAN_TASK"):
            try:
    ...
    if query or image:
        ...
        try:
            ...
            _kanban_task_id = os.environ.get("ARGUS_KANBAN_TASK", "").strip()
            if _kanban_task_id:
                try:
                    from hermes_cli import kanban_db as _kb
            ...
            if quiet:
                ...
                if cli._ensure_runtime_credentials():
                    ...
                    if single_query_images or single_query_image_urls:
                        # Native image attachment only. If the active model
                        # isn't vision-capable the provider will error — set
                        # `model.supports_vision: true` in config.yaml or pick
                        # a vision model.
                        from agent.image_routing import build_native_content_parts
                        _parts, _skipped = build_native_content_parts(
                    ...
                    if cli._init_agent(
                        ...
                        # machine-readable (no styled "Argus" box, no tool-gen
                        ...
                        if os.environ.get("ARGUS_KANBAN_GOAL_MODE") == "1":
                            try:
                        ...
                        if isinstance(result, dict) and result.get("failed"):
                            _exit_code = 1
                            if os.environ.get("ARGUS_KANBAN_TASK") and result.get(
                                "failure_reason"
