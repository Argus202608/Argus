...
def tool_handoff(
    ...
    *,
    reply_owner: str,
    handoff_mode: str = "receipt",
    history_policy: str = "persist",
    task_id: str = "",
    parent_user_message_id: str = "",
    ack: str = "",
) -> str:
    """Decorate an existing tool result with terminal reply-ownership metadata.

    ``control=handoff`` is consumed by the conversation loop after the tool
    result has been appended normally.  The loop closes the tool-call sequence
    with a deterministic assistant receipt and ends the main-agent turn without
    making another model request.  ``handoff_mode=deferred_reply`` additionally
    tells interactive gateways that the named worker will complete the original
    answer slot later; ``receipt`` means the deterministic receipt completes the
    foreground turn while future worker notifications use their own channel.
    ``history_policy=ephemeral_control`` marks a pure control-plane turn (for
    example Monitor CRUD): the gateway may render its side-channel UI, but the
    user/tool/receipt suffix must not be committed to provider-facing history.

    The helper intentionally accepts the JSON string returned by
    :func:`tool_result` / :func:`tool_error`, so tool implementations retain one
    authoritative success/error payload and add control metadata only at their
    ...
    owner = str(reply_owner or "worker").strip() or "worker"
    mode = str(handoff_mode or "receipt").strip() or "receipt"
    if mode not in {"receipt", "deferred_reply"}:
        mode = "receipt"
    policy = str(history_policy or "persist").strip() or "persist"
    if policy not in {"persist", "ephemeral_control"}:
        policy = "persist"
    resolved_ack = str(
        ack
        or data.get("note")
    ...
    data.update({
        ...
        "task_id": str(task_id or data.get("task_id") or "").strip(),
        "parent_user_message_id": str(
            parent_user_message_id
            or data.get("parent_user_message_id")
            or ""
        ).strip(),
        "ack": resolved_ack,
    })
