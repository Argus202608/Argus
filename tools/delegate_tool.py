...
DELEGATE_BLOCKED_TOOLS = frozenset(
    [
        ...
        "set_live_watcher",  # needs per-session WatcherAgent + session_id
        "set_monitor",  # needs per-session MonitorEngine + session_id
        "query_multimodal",  # needs per-session multimodal engine + session_id
    ]
)
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.argus/logs/subagent-<sid>-<ts>.log``
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'argus auth'."
        )
        ...
        "properties": {
            ...
            "acp_command": {
                ...
                "description": (
                    ...
                    "Leave empty to use the parent's default transport (Argus subagents)."
                ),
            },
            "acp_args": {
                "type": "array",
