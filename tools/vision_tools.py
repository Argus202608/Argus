...
def _resolve_download_timeout() -> float:
    env_val = os.getenv("ARGUS_VISION_DOWNLOAD_TIMEOUT", "").strip()
    if env_val:
        try:
...
def _should_use_native_vision_fast_path() -> bool:
    ...
    inside a tool result instead of routing through the auxiliary vision LLM.

    True when the provider is known to accept images inside tool results, or
    the user explicitly declared the main model vision-capable via the
    ``model.supports_vision`` config override.
    """
    try:
...
async def vision_analyze_tool(
    ...
    """
    Analyze an image via an auxiliary vision LLM (aux describes → returns text).

    Still live: called by Computer Use (tools/computer_use/tool.py) to describe a
    captured screenshot region. The `vision_analyze` TOOL no longer uses this for
    non-vision main models (that text fallback was removed → native routing only),
    but this standalone helper remains the aux-vision entry point for callers that
    genuinely need image→text.
...
VISION_ANALYZE_SCHEMA = {
    "name": "vision_analyze",
    "description": (
        "Load a STORED image (a URL, local file path, or data URL) into the "
        "conversation so you can see it. Use this ONLY when there is an actual "
        "image file/URL to open — a filepath in the user's message, a URL in tool "
        "output, a saved screenshot, etc. When your model has native vision the "
        "image is attached to your context and you read it next turn; otherwise it "
...
def _handle_vision_analyze(args: Dict[str, Any], **kw: Any) -> Awaitable[str]:
    ...
    # Native image routing: the active main model sees the image pixels directly
    # (provider accepts images in tool results, or the user set the
    # model.supports_vision override) — no aux call, no information loss, no
    # extra latency. This is the ONLY supported path now; the old aux-LLM
    # "describe the image to text" fallback (for non-vision main models) was
    # removed. If the main model isn't vision-capable, return a clear error
    # rather than silently degrading the image into a lossy text description.
    if _should_use_native_vision_fast_path():
        logger.info("vision_analyze: native image routing")
        return _vision_analyze_native(image_url, question)

    async def _unsupported() -> str:
        return tool_error(
            "vision_analyze requires a vision-capable main model. The current "
            "model isn't recognized as vision-capable — set "
            "`model.supports_vision: true` in config.yaml if it is, or switch to "
            "a vision model. (The legacy aux-LLM image→text fallback was removed.)"
        )
    return _unsupported()
...
VIDEO_ANALYZE_SCHEMA = {
    "name": "video_analyze",
    "description": (
        "Analyze a STORED video file from a URL or local file path using a multimodal AI model. "
