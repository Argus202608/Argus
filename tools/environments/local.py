...
# Argus-internal env vars that should NOT leak into terminal subprocesses.
_HERMES_PROVIDER_ENV_FORCE_PREFIX = "_HERMES_FORCE_"

# Argus-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Argus inference secret exactly
...
# project's dependencies into the Argus venv path instead of the project's own
# ``.venv`` — silently clobbering the Argus environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# Argus venv stays reachable via PATH (its bin dir is first), so stripping
# these markers is safe and only prevents the cross-project clobber (#23473).
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX")


def _inject_context_hermes_home(env: dict) -> None:
    """Bridge the context-local Argus home override into subprocess env."""
    try:
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Argus-managed secrets from a subprocess environment."""
    try:
...
# legitimate child Argus spawns needs them, and they are the highest-value
...
def hermes_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child Argus spawns legitimately needs them.
...
def _find_bash() -> str:
    ...
    raise RuntimeError(
        "Git Bash not found. Argus requires Git for Windows on Windows.\n"
...
def _resolve_hermes_bin_dir() -> str | None:
    """Return the directory holding the ``argus`` console-script, or None.
    ...
    Argus install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``argus``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``argus`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``argus`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("argus")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``argus`` executable (covers nix-store / venv wrappers).
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base in {"argus", "hermes"} or base.startswith(("argus.", "hermes.")))
    ...
    if candidate is None:
        ...
        if exe_dir:
            shim = "argus.exe" if _IS_WINDOWS else "argus"
            legacy_shim = "hermes.exe" if _IS_WINDOWS else "hermes"
            if os.path.isfile(os.path.join(exe_dir, shim)) or os.path.isfile(os.path.join(exe_dir, legacy_shim)):
                candidate = exe_dir
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Argus trusts them.
    """
