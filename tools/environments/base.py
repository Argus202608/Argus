"""Base class for all Argus execution environment backends.
...
# ARGUS_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
# every is_interrupted() state change from _wait_for_process.  Off by default
# to avoid flooding production gateway logs.
_DEBUG_INTERRUPT = bool(os.getenv("ARGUS_DEBUG_INTERRUPT"))
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all Argus backends.
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"ARGUS_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(self, proc: ProcessHandle, timeout: int = 120) -> dict:
        ...
        # --- Debug tracing (opt-in via ARGUS_DEBUG_INTERRUPT=1) -------------
