...
def _load_hermes_env_vars() -> dict[str, str]:
    """Load ~/.argus/.env values without failing Docker command execution."""
    try:
...
def _get_active_profile_name() -> str:
    """Return the active Argus profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    ...
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Argus process whose reuse path will pick them
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Argus
    process leaves its container behind permanently because there's no
    subsequent Argus process scheduled to reuse that exact (task, profile)
    pair.
    """
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``ARGUS_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("ARGUS_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using ARGUS_DOCKER_BINARY override: %s", override)
        return override
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.argus/sandboxes/). Non-persistent
        ...
        # _sanitize_label_value(); the active Argus profile is captured at
        ...
        # container shared across sessions").  If a prior Argus process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Argus secret blocklist. Only implicit passthrough
        # keys are filtered.
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        # container) but can happen if a previous Argus process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Argus exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Argus process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Argus startup. That covers the
        ...
        # outlives Argus processes, processes inside it stay alive, and
