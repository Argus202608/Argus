...
def _get_disabled_skill_names() -> Set[str]:
    ...
    NOTE: WatcherAgent-owned skills (under the reserved ``_watcher/`` subdir) are
    hidden from the MAIN agent by FOLDER, not by name: skills_list / skill_view
    scan via iter_skill_index_files(), which excludes ``_watcher/`` by default, so
    those skills never appear here and no name-list fold-in is needed.
    """
