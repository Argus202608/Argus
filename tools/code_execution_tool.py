...
Lets the LLM write a Python script that calls Argus tools via RPC,
...
# available on every platform Argus itself runs on.
...
# match a safe prefix, the operational ARGUS_ allowlist, or (on Windows) an
# OS-essential name.
#
# NB: the broad "ARGUS_" prefix was deliberately removed (#27303) — it leaked
# ARGUS_*-named config that lacks a secret substring (e.g. ARGUS_BASE_URL,
# ARGUS_KANBAN_DB, ARGUS_*_WEBHOOK).  The child only needs the few
# location/profile vars in _HERMES_CHILD_ALLOWED below; ARGUS_RPC_SOCKET /
# ARGUS_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational ARGUS_* vars the child legitimately needs by exact name — these
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational ARGUS_* vars (_HERMES_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret ARGUS_* vars dropped by the tightened allowlist (#27303). The
    # broad "ARGUS_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like ARGUS_KANBAN_DB / ARGUS_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("ARGUS_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped ARGUS_* var.
            _dropped_hermes.append(k)
    if _dropped_hermes:
        logger.debug(
            "execute_code: dropped %d non-allowlisted ARGUS_* var(s) from the "
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Argus-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Argus-profile soft guard."""',
...
"""Auto-generated Argus tools RPC stubs."""
...
def _connect():
    ...
    ARGUS_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["ARGUS_RPC_SOCKET"]
...
"""Auto-generated Argus tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("ARGUS_RPC_DIR") or os.path.join(tempfile.gettempdir(), "hermes_rpc")
_seq = 0
...
def _execute_remote(
    ...
    try:
        ...
        env_prefix = (
            f"ARGUS_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("ARGUS_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    # selector from ARGUS_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    try:
        ...
        #   connect.  ARGUS_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["ARGUS_RPC_SOCKET"] = rpc_endpoint
        ...
        # ARGUS_TIMEZONE is an internal Argus setting and must not leak
        # into child processes.
        _tz_name = os.getenv("ARGUS_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("ARGUS_TIMEZONE", None)
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.argus/.env')).
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``argus tools`` (e.g. web is turned off),
    ...
    if mode == "strict":
        cwd_note = (
            ...
            "(os.path.expanduser('~/.argus/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Argus tools programmatically. "
