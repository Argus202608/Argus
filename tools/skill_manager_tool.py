...
~/.argus/skills/. Existing skills (bundled, hub-installed, or user-created)
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `argus config set skills.guard_agent_created true`.
    """
    try:
...
# All skills live in ~/.argus/skills/ (single source of truth)
ARGUS_HOME = get_hermes_home()
SKILLS_DIR = ARGUS_HOME / "skills"
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`argus curator unpin {name}` if they want to delete it. "
        ...
        if skill_usage.get_record(name).get("pinned"):
            return {
                ...
                "error": (
                    ...
                    f"`argus curator unpin {name}` if they want it changed."
                ),
            }
    except Exception:
...
def _validate_owner(owner: Optional[str]) -> Optional[str]:
    """Validate the create-time skill owner. 'main' (default) or 'watcher'."""
    if owner is None:
        return None
    if not isinstance(owner, str):
        return "owner must be a string ('main' or 'watcher')."
    if owner.strip().lower() not in ("", "main", "watcher"):
        return (f"Invalid owner '{owner}'. Use 'main' (default, the main agent) "
                "or 'watcher' (the multimodal WatcherAgent).")
    return None
...
def _resolve_skill_dir(name: str, category: str = None,
                       owner: str = "main") -> Path:
    """Build the directory path for a new skill.

    owner="watcher" installs UNDER the reserved ``_watcher/`` subdir so the skill
    is owned by the multimodal WatcherAgent (folder-based isolation): the main
    agent won't index/view/load it, and the WatcherAgent loads only those. See
    agent.skill_utils.WATCHER_SKILLS_SUBDIR. owner="main" (default) installs at
    the normal location.
    """
    from agent.skill_utils import WATCHER_SKILLS_SUBDIR
    base = SKILLS_DIR
    if (owner or "main").strip().lower() == "watcher":
        base = base / WATCHER_SKILLS_SUBDIR
    if category:
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.argus/skills/) first, then any
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER Argus profiles.
    ...
    # Default profile (~/.argus/skills) — only consider when active is non-default.
    ...
    # All named profiles (~/.argus/profiles/*/skills)
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"another profile, switch profiles (`argus -p "
            ...
            base += (
                ...
                f"Switch profiles (`argus -p <name>`) to edit there, or "
...
def _create_skill(name: str, content: str, category: str = None,
                  owner: str = "main") -> Dict[str, Any]:
    """Create a new user skill with SKILL.md content.

    owner: "main" (default) → normal skills dir; "watcher" → under the reserved
    _watcher/ subdir (owned by the multimodal WatcherAgent).
    """
    # Validate name
    ...
    skill_dir = _resolve_skill_dir(name, category, owner)
...
def _delete_skill(name: str, absorbed_into: Optional[str] = None) -> Dict[str, Any]:
    ...
    # RECOVERABLE: archival into ~/.argus/skills/.archive/ is documented as
...
def skill_manage(
    ...
    if action == "create":
        ...
        result = _create_skill(name, content, category, owner or "main")
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        "★ On create, ASK the user (or infer from context) which agent owns the "
        "skill and pass `owner`: 'main' (default — the main agent) or 'watcher' "
        "(the multimodal WatcherAgent, i.e. set_live_watcher's deep-research "
        "sub-agent). owner='watcher' installs it under skills/_watcher/ so ONLY "
        "the WatcherAgent loads it — the main agent won't see or use it. Choose "
        "'watcher' only for skills about analysing the live video stream; "
        ...
        "will refuse with a message pointing the user to `argus curator unpin <name>`. "
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "owner": {
                ...
                "enum": ["main", "watcher"],
                "description": (
                    "Which agent owns this new skill (create only). 'main' "
                    "(default) = the main agent. 'watcher' = the multimodal "
                    "WatcherAgent (set_live_watcher's deep-research sub-agent); it "
                    "installs under skills/_watcher/ so ONLY the WatcherAgent loads "
                    "it and the main agent can't see/use it. Ask the user which "
                    "agent the skill is for; use 'watcher' only for live-video "
                    "stream analysis skills."
                )
            },
            "file_path": {
                "type": "string",
