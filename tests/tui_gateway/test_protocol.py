...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `ARGUS_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("ARGUS_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("ARGUS_TUI_GATEWAY_NO_FLUSH", raising=False)
...
# NOTE: the dispatch()/pool-routing tests live in test_protocol_dispatch.py —
# they drive the module-level RPC ThreadPoolExecutor with Event-gated handlers
# and are kept in a separate per-file-isolated process.
