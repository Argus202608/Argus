...
class TestIsBotMentioned:
    ...
    def test_matrix_pill_in_formatted_body(self):
        html = '<a href="https://matrix.to/#/@hermes:example.org">Argus</a> help'
        assert self.adapter._is_bot_mentioned("Argus help", html)
...
class TestStripMention:
    ...
    def test_does_not_strip_bare_localpart_word(self):
        # Regression: plain words like "Argus" should not be mutated.
        result = self.adapter._strip_mention("Argus")
        assert result == "Argus"
...
async def test_require_mention_html_pill(monkeypatch):
    ...
    adapter = _make_adapter()
    formatted = '<a href="https://matrix.to/#/@hermes:example.org">Argus</a> help'
    event = _make_event("Argus help", formatted_body=formatted)
