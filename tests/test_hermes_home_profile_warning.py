...
profile is active, get_hermes_home() should:
  1. STILL return ~/.argus (raising would brick 30+ module-level callers)
...
class TestGetHermesHomeProfileWarning:
    def test_classic_mode_no_active_profile_no_warning(
        ...
        """Classic mode: no active_profile file → silent, returns ~/.argus."""
    ...
    def test_default_active_profile_no_warning(
        ...
        """active_profile=default → still no warning, returns ~/.argus."""
    ...
    def test_named_profile_unset_home_warns_once(
        ...
        """active_profile=coder + ARGUS_HOME unset → warn loudly, still return fallback."""
        hermes_dir = tmp_path / ".argus"
        hermes_dir.mkdir()
        ...
        assert err.count("ARGUS_HOME fallback") == 1
        assert "'coder'" in err
        assert "#18594" in err
        ...
        assert "ARGUS_HOME fallback" not in err2
    ...
    def test_hermes_home_set_suppresses_warning(
        ...
        (tmp_path / ".argus" / "active_profile").write_text("coder\n")
