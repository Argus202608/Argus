...
class TestEntryPointsImportBootstrap:
    ...
    def test_entry_point_imports_bootstrap(self, path):
        ...
        half-finished ``argus update`` (git-reset landed new code but
...
class TestHardenImportPath:
    ...
    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("ARGUS_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["ARGUS_PYTHON_SRC_ROOT"] = env
            elif "ARGUS_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["ARGUS_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/hermes")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("ARGUS_PYTHON_SRC_ROOT", None)
            else:
                os.environ["ARGUS_PYTHON_SRC_ROOT"] = original_env
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["ARGUS_PYTHON_SRC_ROOT"] = "/env/hermes"
