...
def test_session_create_rejects_at_active_session_limit(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert second["error"]["message"] == (
            "Argus is at the active session limit (1/1). "
...
def test_tui_verbose_tool_details_fail_closed_when_redaction_fails(monkeypatch):
    ...
    assert server._redact_tui_display_text("api_key=secret") == ""
    ...
    assert server._history_tool_result(
        "terminal", json.dumps({"error": "token=secret"})
    ) == ("", "")
    assert server._tool_summary(
        "web_search", json.dumps({"fallback_warning": "token=secret"}), 0.1
    ) is None
    assert server._preview_tool_result_preview(
        "terminal", json.dumps({"output": "token=secret"})
    ) == ""


def test_tui_display_redaction_masks_url_userinfo_and_secret_shapes(monkeypatch):
    from agent import redact

    # This is a client-display safety boundary, so it stays active even when
    # runtime/logging redaction is disabled.  Ordinary URL paths remain useful;
    # only the credential-bearing userinfo is masked.
    monkeypatch.setattr(redact, "_REDACT_ENABLED", False)
    text = (
        "failed https://alice:p4ssw0rd@internal.example/api?code=opaque-callback "
        "sk-ant-api03-AbCdEfGh1234567890"
    )

    safe = server._redact_tui_display_text(text)

    assert "p4ssw0rd" not in safe
    assert "sk-ant-api03-AbCdEfGh1234567890" not in safe
    assert (
        "https://alice:***@internal.example/api?code=opaque-callback" in safe
    )
...
def _capture_tool_events(monkeypatch, sid, mode, name, args, result="ok"):
    """Run one tool.start/tool.complete pair and return both payloads."""
    events: list[tuple[str, str, dict]] = []
    monkeypatch.setattr(
        server, "_emit", lambda event_type, s, payload: events.append((event_type, s, payload))
    )
    monkeypatch.setitem(
        server._sessions, sid, {"tool_progress_mode": mode, "tool_started_at": {}}
    )
    server._on_tool_start(sid, "tool-1", name, args)
    server._on_tool_complete(sid, "tool-1", name, args, result)
    start = next(p for t, _, p in events if t == "tool.start")
    complete = next(p for t, _, p in events if t == "tool.complete")
    return start, complete


def test_tool_result_text_ships_outside_verbose_mode(monkeypatch):
    # `result_text` used to be verbose-only, which left the web UI's tool row
    # with no detail to expand — a finished tool showed a name and nothing else.
    # It must ship in the default "all" mode too.
    _start, complete = _capture_tool_events(
        monkeypatch, "res-default", "all", "terminal", {"command": "pwd"}, "/home/me"
    )

    assert complete["result_text"] == "/home/me"


def test_tool_result_text_still_capped_when_default_on(monkeypatch):
    # Shipping by default must not ship unbounded output: a single read_file can
    # be 150KB+ and every client holds it in component state.
    _start, complete = _capture_tool_events(
        monkeypatch, "res-cap", "all", "terminal", {"command": "cat big"}, "x" * 50_000
    )

    assert 0 < len(complete["result_text"]) < 2_000


def test_live_tool_result_and_summary_redact_display_credentials(monkeypatch):
    result = json.dumps(
        {
            "fallback_warning": (
                "failed https://alice:p4ssw0rd@internal.example/api "
                "sk-ant-api03-AbCdEfGh1234567890"
            )
        }
    )
    ...
    _start, complete = _capture_tool_events(
        monkeypatch, "result-redaction", "all", "web_search", {}, result
    )

    projected = json.dumps(
        {"summary": complete["summary"], "result_text": complete["result_text"]},
        ensure_ascii=False,
    )
    assert "p4ssw0rd" not in projected
    assert "sk-ant-api03-AbCdEfGh1234567890" not in projected
    assert "https://alice:***@internal.example/api" in complete["summary"]
    assert "https://alice:***@internal.example/api" in complete["result_text"]


def test_preview_restart_tool_result_redacts_display_credentials():
    credential_text = (
        "failed https://alice:p4ssw0rd@internal.example/api "
        "sk-ant-api03-AbCdEfGh1234567890"
    )
    ...
    previews = [
        server._preview_tool_result_preview(
            "terminal", json.dumps({"output": credential_text})
        ),
        server._preview_tool_result_preview(
            "web_extract", json.dumps({"error": credential_text})
        ),
    ]

    for preview in previews:
        assert "p4ssw0rd" not in preview
        assert "sk-ant-api03-AbCdEfGh1234567890" not in preview
        assert "https://alice:***@internal.example/api" in preview


def test_tool_start_ships_structured_args_by_default(monkeypatch):
    # The 💭 line and the expandable row both need to say what the call DID.
    # `args_fields` is the structured form and is not gated on verbose;
    # `args_text` (raw JSON dump) still is.
    start, _complete = _capture_tool_events(
        monkeypatch,
        "args-default",
        "all",
        "computer_use",
    ...
    assert start["context"] == "capture som · Google Chrome"
    assert {"key": "action", "kind": "literal", "value": "capture"} in start["args_fields"]
    assert {"key": "app", "kind": "literal", "value": "Google Chrome"} in start["args_fields"]
    assert "args_text" not in start


def test_tool_start_structured_args_elide_payload_values(monkeypatch):
    # Privacy boundary: a DM body is user content, not an identifier. It must
    # reach the UI as a length only — never as text — while the routing fields
    # around it stay visible so the user can still tell what happened.
    start, _complete = _capture_tool_events(
        monkeypatch,
        "args-privacy",
        "all",
        "yb_send_dm",
        {"group_code": "G9", "message": "my password is hunter2"},
    )

    fields = {f["key"]: f for f in start["args_fields"]}
    assert fields["group_code"]["value"] == "G9"
    assert fields["message"]["kind"] == "freeform"
    assert fields["message"]["chars"] == len("my password is hunter2")
    assert "hunter2" not in json.dumps(start, ensure_ascii=False)
...
def test_voice_toggle_returns_configured_record_key(monkeypatch):
    ...
    # ``voice.toggle`` action=on mutates ``os.environ["ARGUS_VOICE"]``
    ...
    # review on #19835).
    monkeypatch.setenv("ARGUS_VOICE", "0")
...
def test_voice_record_start_handles_non_dict_voice_cfg(monkeypatch):
    ...
    monkeypatch.setenv("ARGUS_VOICE", "1")
...
def test_voice_toggle_tts_branch_also_carries_record_key(monkeypatch):
    ...
    monkeypatch.setenv("ARGUS_VOICE", "1")
    monkeypatch.delenv("ARGUS_VOICE_TTS", raising=False)
...
def test_load_enabled_toolsets_prefers_tui_env(monkeypatch):
    monkeypatch.setenv("ARGUS_TUI_TOOLSETS", "web, terminal, ,memory")
...
def test_load_enabled_toolsets_filters_invalid_tui_env(monkeypatch, capsys):
    monkeypatch.setenv("ARGUS_TUI_TOOLSETS", "web, nope")
    monkeypatch.setitem(
        sys.modules,
        "hermes_cli.plugins",
...
def test_load_enabled_toolsets_accepts_plugin_env_after_discovery(monkeypatch):
    monkeypatch.setenv("ARGUS_TUI_TOOLSETS", "plugin_demo")
...
def test_load_enabled_toolsets_folds_project_into_focus_posture(monkeypatch):
    ...
    monkeypatch.delenv("ARGUS_TUI_TOOLSETS", raising=False)
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    monkeypatch.setenv("ARGUS_TUI_TOOLSETS", "mcp-off")
    monkeypatch.setitem(
        sys.modules,
        "hermes_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
...
def test_load_enabled_toolsets_falls_back_when_tui_env_invalid(monkeypatch, capsys):
    monkeypatch.setenv("ARGUS_TUI_TOOLSETS", "nope")
    monkeypatch.setitem(
        sys.modules,
        "hermes_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
...
def test_load_enabled_toolsets_honors_builtin_env_if_config_fails(monkeypatch):
    monkeypatch.setenv("ARGUS_TUI_TOOLSETS", "web")
...
def test_load_enabled_toolsets_all_env_means_all(monkeypatch):
    monkeypatch.setenv("ARGUS_TUI_TOOLSETS", "all")
...
def test_load_enabled_toolsets_all_env_warns_about_ignored_extra_entries(
    ...
    monkeypatch.setenv("ARGUS_TUI_TOOLSETS", "all,nope")
...
def test_load_enabled_toolsets_reports_disabled_mcp_separately(monkeypatch, capsys):
    monkeypatch.setenv("ARGUS_TUI_TOOLSETS", "web,mcp-off,nope")
    monkeypatch.setitem(
        sys.modules,
        "hermes_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    assert server._load_enabled_toolsets() == ["web"]
    err = capsys.readouterr().err
    assert "ignoring unknown ARGUS_TUI_TOOLSETS entries: nope" in err
    assert "ignoring disabled MCP servers" in err
    assert "mcp-off" in err
...
def test_history_to_messages_preserves_tool_calls_for_resume_display():
    ...
    # `tool_call_id` rides along so clients can bind a result to the exact call
    # it answers (matching by tool name alone mismatches when one turn calls the
    # same tool twice). An empty `{}` result contributes no `content` key — the
    # card then shows a single summary line instead of a disclosure onto "{}".
    #
    # `args_fields` mirrors what the live tool.start carries, so a reopened
    # session's tool row expands to the same detail as the stream that produced
    # it (previously history was strictly poorer: preview text only).
    assert server._history_to_messages(history) == [
        {"role": "user", "text": "first prompt"},
        {
            ...
            "name": "search_files",
            "role": "tool",
            "args_fields": [{"key": "pattern", "kind": "literal", "value": "resume"}],
            "tool_call_id": "call_1",
        },
...
def test_history_to_messages_projects_tool_output_not_arg_preview():
    # Regression: the tool's own output (messages.content) was dropped from the
    # resume projection, so a reopened session showed "✓ terminal" whose only
    # expandable content was the 80-char command echoed back from `context`.
    # Output must survive (capped), separately from the call-side preview.
    history = [
        {
            "role": "assistant",
            "content": "",
        ...
        },
        {
            "role": "tool",
            "tool_call_id": "call_t",
            "content": json.dumps({"output": "hi\n", "exit_code": 0}),
        },
    ]

    (row,) = server._history_to_messages(history)
    assert row["name"] == "terminal"
    assert row["context"] == "echo hi"      # call side: the command
    assert row["content"] == "hi\n"         # result side: the real output
    assert "summary" not in row             # exit 0 needs no annotation


def test_history_to_messages_summarises_failed_tool_and_caps_output():
    # A non-zero exit is surfaced as a summary so the collapsed line is useful,
    # and a huge output is capped rather than shipped verbatim (a long session
    # would otherwise push megabytes through resume into client state).
    big = "x" * 50_000
    history = [
        {
            "role": "tool",
            "tool_name": "terminal",
            "content": json.dumps({"output": big, "exit_code": 2}),
        },
    ]

    (row,) = server._history_to_messages(history)
    assert row["summary"] == "exit 2"
    assert 0 < len(row["content"]) < 2_000


def test_history_to_messages_redacts_error_body_and_summary_credentials():
    error = (
        "failed https://alice:p4ssw0rd@internal.example/api "
        "sk-ant-api03-AbCdEfGh1234567890"
    )
    ...
    (row,) = server._history_to_messages(history)
    projected = json.dumps(row, ensure_ascii=False)
    assert "p4ssw0rd" not in projected
    assert "sk-ant-api03-AbCdEfGh1234567890" not in projected
    assert "https://alice:***@internal.example/api" in row["content"]
    assert "https://alice:***@internal.example/api" in row["summary"]
...
def test_resolve_model_uses_inference_model_env(monkeypatch):
    ...
    monkeypatch.setenv("ARGUS_INFERENCE_MODEL", " anthropic/claude-sonnet-4.6\n")
...
def test_resolve_model_strips_config_model(monkeypatch):
    monkeypatch.delenv("ARGUS_MODEL", raising=False)
    monkeypatch.delenv("ARGUS_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(
...
def test_config_sync_config_wins_over_env_seed(monkeypatch):
    # Hosted instances set ARGUS_INFERENCE_MODEL as a provision-time seed;
    # the per-turn sync must follow config.yaml edits, not stay pinned to it.
    monkeypatch.setenv("ARGUS_INFERENCE_MODEL", "seed/model")
    monkeypatch.delenv("ARGUS_MODEL", raising=False)
...
def test_startup_runtime_uses_tui_provider_env(monkeypatch):
    monkeypatch.setenv("ARGUS_MODEL", "nous/hermes-test")
    monkeypatch.setenv("ARGUS_TUI_PROVIDER", "nous")
    monkeypatch.delenv("ARGUS_INFERENCE_PROVIDER", raising=False)
...
def test_startup_runtime_does_not_treat_inference_provider_as_explicit(monkeypatch):
    monkeypatch.setenv("ARGUS_MODEL", "nous/hermes-test")
    monkeypatch.delenv("ARGUS_TUI_PROVIDER", raising=False)
    monkeypatch.setenv("ARGUS_INFERENCE_PROVIDER", "nous")
    monkeypatch.setattr(
...
def test_startup_runtime_detects_provider_for_model_env(monkeypatch):
    monkeypatch.setenv("ARGUS_MODEL", "sonnet")
    monkeypatch.delenv("ARGUS_TUI_PROVIDER", raising=False)
    monkeypatch.delenv("ARGUS_INFERENCE_PROVIDER", raising=False)
...
def test_config_set_yolo_process_scope_treats_false_like_env_as_disabled(monkeypatch):
    monkeypatch.setenv("ARGUS_YOLO_MODE", "false")
    ...
    assert resp["result"]["value"] == "1"
    assert os.environ.get("ARGUS_YOLO_MODE") == "1"
...
def test_enable_gateway_prompts_sets_gateway_env(monkeypatch):
    monkeypatch.delenv("ARGUS_EXEC_ASK", raising=False)
    monkeypatch.delenv("ARGUS_GATEWAY_SESSION", raising=False)
    monkeypatch.delenv("ARGUS_INTERACTIVE", raising=False)

    server._enable_gateway_prompts()

    assert server.os.environ["ARGUS_GATEWAY_SESSION"] == "1"
    assert server.os.environ["ARGUS_EXEC_ASK"] == "1"
    assert server.os.environ["ARGUS_INTERACTIVE"] == "1"
...
def test_config_set_model_does_not_leak_inference_provider_env(monkeypatch):
    ...
    process; writing ARGUS_INFERENCE_PROVIDER on a switch leaked the new
    ...
    server._sessions["sid"] = session
    monkeypatch.setenv("ARGUS_INFERENCE_PROVIDER", "openrouter")
    monkeypatch.setattr(
        "hermes_cli.model_switch.switch_model", lambda **_kwargs: result
    )
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    ...
    try:
        ...
        assert os.environ["ARGUS_INFERENCE_PROVIDER"] == "openrouter"
...
def test_config_set_model_records_per_session_override_not_env(monkeypatch):
    ...
    try:
        ...
        assert "ARGUS_TUI_PROVIDER" not in os.environ
        assert "ARGUS_INFERENCE_PROVIDER" not in os.environ
...
def test_config_set_model_switches_agent_without_touching_env(monkeypatch):
    ...
    a per-session override; it does NOT write ARGUS_MODEL / ARGUS_TUI_PROVIDER
    ...
    session = _session(agent=agent)
    server._sessions["sid"] = session
    monkeypatch.setenv("ARGUS_TUI_PROVIDER", "openai-codex")
    monkeypatch.delenv("ARGUS_MODEL", raising=False)
    monkeypatch.delenv("ARGUS_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    ...
    try:
        ...
        assert os.environ["ARGUS_TUI_PROVIDER"] == "openai-codex"
        assert "ARGUS_MODEL" not in os.environ
        assert "ARGUS_INFERENCE_MODEL" not in os.environ
    finally:
        server._sessions.clear()
...
def test_file_attach_uploads_remote_file_into_session_workspace(monkeypatch, tmp_path):
    ...
    try:
        ...
        stored = workspace / ".argus" / "desktop-attachments" / "report.txt"
        assert resp["result"]["attached"] is True
        assert resp["result"]["uploaded"] is True
        assert resp["result"]["path"] == str(stored)
        assert resp["result"]["ref_text"] == "@file:.argus/desktop-attachments/report.txt"
...
def test_file_attach_copies_gateway_visible_file_outside_workspace(monkeypatch, tmp_path):
    ...
    try:
        ...
        stored = workspace / ".argus" / "desktop-attachments" / "outside.txt"
        assert resp["result"]["attached"] is True
        assert resp["result"]["uploaded"] is True
        assert resp["result"]["ref_text"] == "@file:.argus/desktop-attachments/outside.txt"
...
def test_file_attach_uses_in_workspace_file_without_copying(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert not (workspace / ".argus" / "desktop-attachments").exists()
    finally:
        server._sessions.pop("sid", None)
...
def test_file_attach_quotes_ref_with_spaces(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert resp["result"]["attached"] is True
        assert resp["result"]["ref_text"] == "@file:`.argus/desktop-attachments/my exam schedule.csv`"
    finally:
        server._sessions.pop("sid", None)
...
def test_reload_env_rpc_calls_hermes_cli_reload_env(monkeypatch):
    """reload.env mirrors classic CLI's `/reload` — re-reads ~/.argus/.env
