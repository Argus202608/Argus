...
def test_models_dev_snapshot_ships_in_both_wheel_and_sdist():
    """The cold-start model picker must retain its offline fallback.

    ``agent/models_dev_api.json`` is package data rather than Python code, so
    wheels and sdists need independent declarations. Missing either channel
    makes a sealed install synchronously depend on models.dev before it has a
    disk cache.
    """
    snapshot = REPO_ROOT / "agent" / "models_dev_api.json"
    assert snapshot.is_file(), "expected the bundled models.dev snapshot"

    data = tomllib.loads((REPO_ROOT / "pyproject.toml").read_text(encoding="utf-8"))
    agent_package_data = data["tool"]["setuptools"]["package-data"].get("agent", [])
    assert "models_dev_api.json" in agent_package_data, (
        "pyproject package-data 'agent' must ship models_dev_api.json (wheel)"
    )

    manifest = (REPO_ROOT / "MANIFEST.in").read_text(encoding="utf-8")
    assert "include agent/models_dev_api.json" in manifest, (
        "MANIFEST.in must include agent/models_dev_api.json (sdist)"
    )
