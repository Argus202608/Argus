...
class TestSaveSessionLogRedactsSecrets:
    ...
    def _ensure_redaction_enabled(self, monkeypatch):
        """Force redaction on regardless of host ARGUS_REDACT_SECRETS state.
        ...
        monkeypatch.delenv("ARGUS_REDACT_SECRETS", raising=False)
...
class TestBuildApiKwargs:
    ...
    def test_qwen_thinking_visual_query_is_not_forced_or_soft_overridden(self):
        with (
            patch(
                "run_agent.get_tool_definitions",
                return_value=_make_tool_defs("query_multimodal"),
            ),
            patch("run_agent.check_toolset_requirements", return_value={}),
            patch("run_agent.OpenAI"),
        ...
        kwargs = agent._build_api_kwargs([
            {"role": "user", "content": "论文的table1中支持Omni的benchmark有哪些"}
        ])
        ...
        assert not any(
            ...
            and "SYSTEM OVERRIDE" in str(m.get("content"))
            for m in kwargs["messages"]
        )

    def test_non_thinking_visual_query_is_not_hard_forced(self):
        with (
            patch(
                "run_agent.get_tool_definitions",
                return_value=_make_tool_defs("query_multimodal"),
        ...
        agent._extra_body_additions = {"enable_thinking": False}
...
class TestRunConversation:
    ...
    def test_ollama_small_runtime_context_fails_before_api_call(self, agent, caplog):
        ...
        assert "Ollama runtime context too small for Argus tool use" in caplog.text
    ...
    def test_kanban_block_called_on_iteration_exhaustion(self, agent, monkeypatch):
        ...
        monkeypatch.setenv("ARGUS_KANBAN_TASK", "t_test_task_123")
    ...
    def test_no_kanban_block_when_not_in_kanban_mode(self, agent, monkeypatch):
        """The exhaustion bridge must NOT fire when ARGUS_KANBAN_TASK
        is unset (non-kanban runs are unaffected by #29747 gap 2)."""
        self._setup_agent(agent)
        agent.max_iterations = 2

        monkeypatch.delenv("ARGUS_KANBAN_TASK", raising=False)
...
class TestSystemPromptStability:
    ...
    def test_fresh_build_when_no_history(self, agent):
        ...
        assert "You are Argus" in agent._cached_system_prompt
