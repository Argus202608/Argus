...
# ── WatcherAgent folder-based skill isolation (skills/_watcher/) ──────────────


def test_is_watcher_skill_path():
    from pathlib import Path
    assert WATCHER_SKILLS_SUBDIR == "_watcher"
    assert is_watcher_skill_path(Path("/x/skills/_watcher/foo/SKILL.md")) is True
    assert is_watcher_skill_path("/x/skills/_watcher/foo/SKILL.md") is True
    assert is_watcher_skill_path(Path("/x/skills/normal/SKILL.md")) is False
    # A skill literally named "_watcher" as a nested dir still counts as watcher-
    # owned (any _watcher component) — acceptable: the reserved name is reserved.
    assert is_watcher_skill_path(Path("/x/skills/normal/_watcher/SKILL.md")) is True


def _mk_skill(root, *parts, name):
...
def test_iter_skill_index_files_excludes_watcher_by_default(tmp_path):
    """Main-agent scan (default): _watcher/ skills are NOT yielded."""
    normal = _mk_skill(tmp_path, "normal", name="normal")
    _mk_skill(tmp_path, "_watcher", "wskill", name="wskill")

    found = list(iter_skill_index_files(tmp_path, "SKILL.md"))
    assert found == [normal]  # watcher skill excluded


def test_iter_skill_index_files_include_watcher_yields_only_watcher(tmp_path):
    """WatcherAgent scan (include_watcher=True): ONLY _watcher/ skills."""
    _mk_skill(tmp_path, "normal", name="normal")
    w1 = _mk_skill(tmp_path, "_watcher", "wskill", name="wskill")

    found = list(iter_skill_index_files(tmp_path, "SKILL.md", include_watcher=True))
    assert found == [w1]  # only the watcher skill
