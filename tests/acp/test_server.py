...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.name == "argus"
        ...
        assert payloads == [
            {
                "args": ["--setup"],
                "description": (
                    "Open Argus' interactive model/provider setup in a terminal. "
                    "Use this when Argus has not been configured on this machine yet."
                ),
                "id": TERMINAL_SETUP_AUTH_METHOD_ID,
                "name": "Configure Argus provider",
                "type": "terminal",
            }
        ]
...
class TestPrompt:
    ...
    async def test_prompt_propagates_hermes_session_id_env(self, agent, monkeypatch):
        ...
        via ``ARGUS_SESSION_ID`` so tools that want to stamp side-effects
        ...
        monkeypatch.delenv("ARGUS_SESSION_ID", raising=False)
        ...
        def mock_run(user_message, conversation_history=None, task_id=None, **kwargs):
            ...
            captured["env"] = os.environ.get("ARGUS_SESSION_ID")
        ...
        assert captured["env"] == new_resp.session_id, (
            "ARGUS_SESSION_ID must be set to the originating ACP session id "
        ...
        assert os.environ.get("ARGUS_SESSION_ID") is None, (
            "ARGUS_SESSION_ID must be restored after the agent call so "
    ...
    async def test_prompt_restores_prior_hermes_session_id(self, agent, monkeypatch):
        ...
        def mock_run(*args, **kwargs):
            captured["inner"] = os.environ.get("ARGUS_SESSION_ID")
            return {"final_response": "ok", "messages": []}
        ...
        assert os.environ.get("ARGUS_SESSION_ID") == "outer-sess"
