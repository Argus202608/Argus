...
def _route_calls(backend, responses: Dict[str, Any]) -> None:
    """Route backend._session.call_tool by TOOL NAME (first positional arg),
    so tests are robust to extra calls (e.g. the inserted set_config scope
    switch) that would desync an ordered side_effect list. Unlisted tools get
    an empty-but-well-formed result."""
    default = {"data": "", "images": [], "isError": False, "structuredContent": None}

    def _router(name, *args, **kwargs):
        return responses.get(name, default)

    backend._session.call_tool.side_effect = _router
...
class TestCaptureAppFilterNoMatch:
    ...
    def test_app_filter_match_still_works(self):
        ...
        backend = _make_cua_backend_with_windows(windows)
        # An app-targeted capture switches to window scope (set_config), lists
        # windows, then reads the matched window. Route by tool name so the
        # inserted set_config call doesn't desync an ordered side_effect list.
        _route_calls(backend, {
            "set_config": {"structuredContent": {"capture_scope": "window"}},
            "list_windows": {"data": "", "images": [], "isError": False,
                             "structuredContent": {"windows": windows}},
            "get_window_state": {"data": '✅ 計算機 — 0 elements\n', "images": [],
    ...
    def test_no_app_uses_desktop_capture(self):
        """When no app= is given, capture takes the full-display desktop path
        (get_desktop_state) instead of resolving a window — the old window
        heuristic grabbed cua-driver's own hidden window and returned a blank
        frame on Windows. Desktop scope is window-less, so no pid is set."""
        backend = _make_cua_backend_with_windows([])
        _route_calls(backend, {
            "set_config": {"structuredContent": {"capture_scope": "desktop"}},
            "get_desktop_state": {
                "data": "", "images": [], "isError": False,
                "structuredContent": {
                    "screenshot_width": 3840, "screenshot_height": 2160,
                    "screen_width": 3840, "screen_height": 2160,
                    "screenshot_png_b64": _TINY_PNG_B64,
        ...
        # Full-screen path: no window resolved, app labelled "screen".
        assert backend._active_pid is None
        assert backend._active_window_id is None
        assert cap.app == "screen"
        assert cap.elements == []
        # get_desktop_state was the capture tool used (not get_window_state).
        called = [c.args[0] for c in backend._session.call_tool.call_args_list]
        assert "get_desktop_state" in called
...
class TestStructuredElementsConsumption:
    ...
    def test_capture_prefers_structured_over_markdown_when_both_present(self):
        ...
        cap = backend.capture(mode="ax", app="Demo")
    ...
    def test_capture_falls_back_to_markdown_when_structured_absent(self):
        ...
        backend._session.call_tool.side_effect = fake_call_tool
        cap = backend.capture(mode="ax", app="Old")
        assert len(cap.elements) == 1
    ...
    def test_vision_capture_falls_back_to_get_window_state_when_screenshot_dropped(self):
        ...
        backend._session.call_tool.side_effect = fake_call_tool
        cap = backend.capture(mode="vision", app="Demo")

        tool_names = [call.args[0] for call in backend._session.call_tool.call_args_list]
        # An app-targeted capture first switches to window scope (set_config),
        # then lists windows and reads the window state.
    ...
    def test_capture_app_screen_uses_desktop_state(self):
        """capture(app='screen'|'desktop'|...) takes the full-display desktop
        path (get_desktop_state) — a true whole-screen PNG — NOT a per-window
        resolution. This is the fix for the blank-screenshot bug: the old code
        resolved a shell/desktop window (or, with no app, cua-driver's own
        hidden window) and returned a transparent frame on Windows."""
        from tools.computer_use.cua_backend import CuaDriverBackend
        ...
        def fake_call_tool(name, args):
            if name == "set_config":
                return {"data": "", "images": [],
                        "structuredContent": {"capture_scope": "desktop"},
                        "isError": False}
            ...
            if name in ("list_windows", "get_window_state"):
                raise AssertionError(f"{name} must NOT be called for a screen capture")
            return {"data": "", "images": [], "structuredContent": None, "isError": False}

        backend._session.call_tool.side_effect = fake_call_tool
        cap = backend.capture(mode="som", app="screen")

        # Full-display path: no window resolved, screen-labelled, real frame.
        assert backend._active_pid is None
        assert backend._active_window_id is None
        assert cap.app == "screen"
        assert cap.png_b64 == _TINY_PNG_B64
        tool_names = [c.args[0] for c in backend._session.call_tool.call_args_list]
        assert "get_desktop_state" in tool_names
        assert "get_window_state" not in tool_names

    def test_desktop_scope_click_is_window_less_screen_coords(self):
        """After a desktop capture, click(x,y) routes as TRUE SCREEN pixels with
        no pid/window_id (cua-driver's window-less desktop-scope click)."""
        from tools.computer_use.cua_backend import CuaDriverBackend
        ...
        def fake_call_tool(name, args):
            if name == "set_config":
                return {"data": "", "images": [],
                        "structuredContent": {"capture_scope": "desktop"}, "isError": False}
            if name == "get_desktop_state":
                return {"data": "", "images": [], "isError": False,
                        "structuredContent": {
                            ...
                            "screenshot_png_b64": _TINY_PNG_B64,
                            "screenshot_mime_type": "image/png"}}
            return {"data": "clicked", "images": [], "structuredContent": None, "isError": False}

        backend._session.call_tool.side_effect = fake_call_tool
        backend.capture(mode="som")  # no app → desktop scope
        res = backend.click(x=100, y=200)

        assert res.ok
        click_call = next(c for c in backend._session.call_tool.call_args_list
                          if c.args[0] == "click")
        click_args = click_call.args[1]
        assert click_args.get("x") == 100 and click_args.get("y") == 200
        assert "pid" not in click_args        # window-less: screen-absolute
        assert "window_id" not in click_args
...
class TestElementTokenAttachment:
    ...
    def test_capture_refreshes_snapshot_tokens(self):
        ...
        backend._session.call_tool.side_effect = fake_call_tool
        backend.capture(mode="ax", app="Demo")
...
class TestSessionLifecycle:
    ...
    def test_capture_list_windows_carries_session(self):
        backend = self._backend_with_mock_session()
        # An app-targeted capture goes the window path (list_windows). It returns
        # no windows so capture short-circuits — but every call must carry the
        # session id, including list_windows.
        backend._session.call_tool.return_value = {
            "data": "", "images": [], "image_mime_types": [],
            "structuredContent": {"windows": []}, "isError": False,
        }
        backend.capture(mode="ax", app="SomeApp")
        names_args = [c.args for c in backend._session.call_tool.call_args_list]
        list_calls = [a for a in names_args if a[0] == "list_windows"]
        assert list_calls, "app-targeted capture must call list_windows"
        assert list_calls[0][1]["session"] == backend._session_id

    def test_capture_desktop_carries_session(self):
        backend = self._backend_with_mock_session()
        backend._session.call_tool.return_value = {
            "data": "", "images": [], "isError": False,
            "structuredContent": {"screenshot_width": 100, "screenshot_height": 100},
        }
        backend.capture(mode="som")  # no app → desktop path
        names_args = [c.args for c in backend._session.call_tool.call_args_list]
        gds = [a for a in names_args if a[0] == "get_desktop_state"]
        assert gds, "no-app capture must call get_desktop_state"
        assert gds[0][1]["session"] == backend._session_id
