...
class TestCreateSkill:
    ...
    def test_create_owner_watcher_installs_under_reserved_dir(self, tmp_path):
        # owner='watcher' → under skills/_watcher/ (folder-based isolation:
        # only the multimodal WatcherAgent loads it).
        with _skill_dir(tmp_path):
            result = _create_skill("scene-splitter", VALID_SKILL_CONTENT,
                                   owner="watcher")
        assert result["success"] is True
        assert (tmp_path / "_watcher" / "scene-splitter" / "SKILL.md").exists()
        # NOT at the normal (main-agent) location.
        assert not (tmp_path / "scene-splitter" / "SKILL.md").exists()

    def test_create_owner_main_is_default_location(self, tmp_path):
        with _skill_dir(tmp_path):
            r_default = _create_skill("a-skill", VALID_SKILL_CONTENT)
            r_explicit = _create_skill(
                "b-skill",
                VALID_SKILL_CONTENT.replace("a-skill", "b-skill"),
                owner="main")
        assert r_default["success"] and r_explicit["success"]
        assert (tmp_path / "a-skill" / "SKILL.md").exists()
        assert (tmp_path / "b-skill" / "SKILL.md").exists()
        assert not (tmp_path / "_watcher").exists()

    def test_create_owner_invalid_rejected(self, tmp_path):
        with _skill_dir(tmp_path):
            result = _create_skill("c-skill", VALID_SKILL_CONTENT, owner="bogus")
