...
"""
    ...
    assert "Tyler Williams > Active Projects: Argus" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `argus cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    #                      tell the user to *review* ~/.argus/config.yaml;
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-A ``Argus``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Argus prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Argus to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Argus config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Argus uses tools well"
    # All-lowercase matches → lowercase ``hermes``; this preserves the
    # real filesystem path ``~/.argus`` (Argus home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "argus should always respond concisely"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    # Same case-preservation rule as above.
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Argus remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "argus prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Argus was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "argus uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Argus said to ask Argus about Argus settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.argus`` (the real Argus home), not the broken
    ``.Argus``.
    ...
    rewritten to ``~/.Argus/config.yaml`` — a path that doesn't exist —
    and the agent kept trying to read it.
    """
    mod = load_module()
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.argus/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .argus directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.argus'"
    # Sentence with both lowercase path and capitalized prose.
    assert mod.rebrand_text("openclaw config path: ~/.openclaw/") == \
        "argus config path: ~/.argus/"
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are Argus" in result
