...
class TestProfileScopedModel:
    ...
    def test_model_options_does_not_take_skills_profile_lock(
        self, client, isolated_profiles, monkeypatch
    ):
        """Model inventory is config-only and must not serialize skill I/O.

        Live provider/model discovery can be slow. Routing this endpoint
        through ``_profile_scope`` held the process-wide skills lock for the
        whole discovery, blocking unrelated profile and skill requests.
        """
        import hermes_cli.inventory as inventory
        import hermes_cli.web_server as web_server

        class _ForbiddenSkillsLock:
            def __enter__(self):
                raise AssertionError("model/options acquired the skills profile lock")

            def __exit__(self, exc_type, exc, tb):
                return False

        def fake_build(ctx, **_kwargs):
            return {
                "providers": [],
        ...
        (isolated_profiles["worker_beta"] / "config.yaml").write_text(
            "model:\n  provider: openrouter\n  default: worker/lock-free\n",
            encoding="utf-8",
        )
        monkeypatch.setattr(web_server, "_SKILLS_PROFILE_LOCK", _ForbiddenSkillsLock())
        monkeypatch.setattr(inventory, "build_models_payload", fake_build)

        resp = client.get("/api/model/options", params={"profile": "worker_beta"})

        assert resp.status_code == 200
        assert resp.json()["model"] == "worker/lock-free"
