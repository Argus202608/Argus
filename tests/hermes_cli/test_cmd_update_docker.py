"""Tests for ``argus update`` / ``--check`` inside the Docker container.
...
fix, ``argus update`` would fall through to ``"✗ Not a git repository.
...
def test_cmd_update_in_docker_prints_guidance_and_exits(
    ...
    """``argus update`` inside Docker → friendly message + exit 1, no git calls."""
    ...
    assert "docker pull mmargus/argus:latest" in out
...
def test_cmd_update_check_in_docker_prints_guidance_and_exits(
    ...
    """``argus update --check`` inside Docker → same message + exit 1, no fetch."""
    with pytest.raises(SystemExit) as excinfo:
...
def test_format_docker_update_message_contents():
    ...
    assert "docker pull mmargus/argus:latest" in msg
    ...
    assert "ARGUS_HOME" in msg or "/opt/data" in msg, (
