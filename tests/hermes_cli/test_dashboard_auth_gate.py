...
@pytest.mark.parametrize(
    "header_name",
    ["X-Argus-Session-Token", "X-Hermes-Session-Token"],
)
def test_loopback_protected_route_accepts_session_token(client_loopback, header_name):
    """The canonical token header and legacy alias unlock protected routes."""
...
def test_loopback_index_injects_session_token(client_loopback):
    ...
    assert "__ARGUS_SESSION_TOKEN__" in r.text
...
def test_start_server_surfaces_nous_skip_reason_when_unconfigured(monkeypatch):
    ...
    'set ARGUS_DASHBOARD_OAUTH_CLIENT_ID', not 'install a plugin'."""
    from hermes_cli.dashboard_auth import clear_providers
    ...
    clear_providers()
    _stub_uvicorn_run(monkeypatch)
    monkeypatch.delenv("ARGUS_DASHBOARD_OAUTH_CLIENT_ID", raising=False)
    monkeypatch.delenv("ARGUS_DASHBOARD_PORTAL_URL", raising=False)
    from unittest.mock import MagicMock
    nous_plugin.register(MagicMock())  # populates LAST_SKIP_REASON
    assert "ARGUS_DASHBOARD_OAUTH_CLIENT_ID" in nous_plugin.LAST_SKIP_REASON
    ...
    assert "ARGUS_DASHBOARD_OAUTH_CLIENT_ID" in msg
    assert "nous:" in msg
