...
def test_seed_supervise_skeleton_creates_expected_layout(tmp_path) -> None:
    ...
    assert stat.S_IMODE(event.stat().st_mode) in {0o1730, 0o3730}, (
...
def test_seed_supervise_skeleton_handles_log_subservice(tmp_path) -> None:
    ...
    # An unprivileged POSIX caller cannot retain setgid when the temp
    # directory's group is not one of its supplementary groups (macOS
    # /private/tmp is commonly root:wheel). The sticky/group access bits are
    # still required; the root-run container integration test verifies 03730.
    assert stat.S_IMODE(log_event.stat().st_mode) in {0o1730, 0o3730}
    assert log_supervise.is_dir()
    assert log_supervise_event.is_dir()
    assert stat.S_IMODE(log_supervise_event.stat().st_mode) in {0o1730, 0o3730}
...
def test_lifecycle_raises_gateway_not_registered_for_missing_slot(
    ...
    assert "'typo'" in msg
    assert "argus profile create typo" in msg
