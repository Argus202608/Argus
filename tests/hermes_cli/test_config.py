...
class TestBareCustomEndpointNormalization:
    """A hand-written ``model: {provider: custom, base_url: ...}`` must be given
    a durable ``custom_providers`` row at startup.

    Regression (the "one-way door"): that shape has no registry row — the picker
    fabricates one from ``model.base_url``
    (``model_switch._bare_custom_provider_def``). Switching the main model to
    another provider correctly clears the stale URL, which destroyed the only
    record of the endpoint, so its picker row dropped to zero models and the
    user could never switch back. Normalizing at startup means no switch path
    has to know about this.
    """

    def test_registers_bare_custom_endpoint(self):
        from hermes_cli.config import normalize_bare_custom_endpoint

        cfg = {
            "model": {
                "provider": "custom",
                "default": "GPT-5.6 Luna",
                "base_url": "https://doc.example.com/u/abc/v1/messages",
                "api_key": "whatever",
            }
        }

        name = normalize_bare_custom_endpoint(cfg)

        assert name  # a display label is always produced
        rows = cfg["custom_providers"]
        assert len(rows) == 1
        assert rows[0]["base_url"] == "https://doc.example.com/u/abc/v1/messages"
        assert rows[0]["model"] == "GPT-5.6 Luna"
        assert rows[0]["api_key"] == "whatever"

    def test_is_idempotent(self):
        """Must be safe to run on every startup — including right after
        sync_project_config re-copies a project config.yaml carrying this shape.
        """
        from hermes_cli.config import normalize_bare_custom_endpoint
        ...
        assert normalize_bare_custom_endpoint(cfg) is not None
        assert normalize_bare_custom_endpoint(cfg) is None
        assert len(cfg["custom_providers"]) == 1

        # Dedup ignores a trailing slash (matching _save_custom_provider).
        cfg["model"]["base_url"] = "https://x.example.com/v1/"
        assert normalize_bare_custom_endpoint(cfg) is None
        assert len(cfg["custom_providers"]) == 1

    def test_preserves_existing_rows_and_no_duplicate(self):
        from hermes_cli.config import normalize_bare_custom_endpoint

        cfg = {
            "model": {"provider": "custom", "base_url": "https://new.example.com/v1"},
            "custom_providers": [
                {"name": "Existing", "base_url": "https://old.example.com/v1"}
            ],
        }

        assert normalize_bare_custom_endpoint(cfg) is not None
        urls = [r["base_url"] for r in cfg["custom_providers"]]
        assert urls == ["https://old.example.com/v1", "https://new.example.com/v1"]

    def test_local_provider_also_normalized(self):
        from hermes_cli.config import normalize_bare_custom_endpoint

        cfg = {"model": {"provider": "local", "base_url": "http://127.0.0.1:8000/v1"}}
        assert normalize_bare_custom_endpoint(cfg) is not None
        assert cfg["custom_providers"][0]["base_url"] == "http://127.0.0.1:8000/v1"
        # Localhost gets the friendly label the CLI flow uses.
        assert "Local" in cfg["custom_providers"][0]["name"]

    @pytest.mark.parametrize(
        "cfg",
        [
            # Non-custom provider: has a real registry row already.
            {"model": {"provider": "openrouter", "base_url": "https://x/v1"}},
            # Custom with no base_url: nothing to register.
            {"model": {"provider": "custom", "default": "m"}},
            {"model": {"provider": "custom", "base_url": "   "}},
            # Malformed / absent model config must never raise.
            {},
            {"model": "a-bare-string"},
            {"model": None},
        ],
    )
    def test_no_op_cases_never_write_or_raise(self, cfg):
        from hermes_cli.config import normalize_bare_custom_endpoint

        assert normalize_bare_custom_endpoint(cfg) is None
        assert "custom_providers" not in cfg

    def test_non_list_custom_providers_is_replaced(self):
        from hermes_cli.config import normalize_bare_custom_endpoint

        cfg = {
            "model": {"provider": "custom", "base_url": "https://x/v1", "default": "m"},
            "custom_providers": "garbage",
        }

        assert normalize_bare_custom_endpoint(cfg) is not None
        assert isinstance(cfg["custom_providers"], list)
        assert len(cfg["custom_providers"]) == 1

    def test_normalize_config_shapes_persists_and_is_write_free_when_clean(
        self, tmp_path
    ):
        """The startup wrapper saves exactly once on a config that needs the
        repair, and does not touch disk on an already-normalized one."""
        ...
        with patch.dict(os.environ, {"HERMES_HOME": str(tmp_path)}):
            ...
            raw = yaml.safe_load(config_path.read_text(encoding="utf-8"))
            rows = raw["custom_providers"]
            assert [r["base_url"] for r in rows] == [
                "https://doc.example.com/u/abc/v1/messages"
            ]
            # The endpoint stays selectable AND the main slot is untouched.
            assert raw["model"]["default"] == "GPT-5.6 Luna"
            assert raw["model"]["base_url"] == (
                "https://doc.example.com/u/abc/v1/messages"
            )

            # Second run: already normalized → no write.
