...
def test_global_llm_timeout_used_as_fallback(monkeypatch, tmp_path):
    ...
    # No provider/model override anywhere → universal fallback wins.
    assert get_provider_request_timeout("anthropic", "claude-opus-4.6") == 60.0
    # Unknown provider (e.g. a submodule's custom endpoint) → still inherits it.
    assert get_provider_request_timeout("custom", "Kimi-K2.6") == 60.0
    # Even an empty provider inherits the shared knob.
    assert get_provider_request_timeout("") == 60.0
...
def test_provider_override_beats_global_llm_timeout(monkeypatch, tmp_path):
    ...
    assert get_provider_request_timeout("anthropic", "claude-opus-4.6") == 120.0
    # Provider-wide override wins over global for other models.
    assert get_provider_request_timeout("anthropic", "claude-sonnet-4.5") == 30.0
    # A provider with no override falls back to the universal key.
    assert get_provider_request_timeout("openai") == 60.0
