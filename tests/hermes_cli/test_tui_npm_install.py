...
def test_workspace_check_ignores_uninstalled_sibling_workspaces(
    tmp_path: Path, main_mod
) -> None:
    tui = tmp_path / "ui-tui"
    tui.mkdir()
    (tui / "package.json").write_text('{"name":"hermes-tui"}')
    _touch_ink(tmp_path)
    wanted = {
        "packages": {
            "ui-tui": {"name": "hermes-tui", "dependencies": {"ink": "1"}},
            "node_modules/ink": {"version": "1"},
            "apps/desktop": {"name": "desktop", "dependencies": {"electron": "1"}},
            "node_modules/electron": {"version": "1"},
        }
    }
    installed = {
        "packages": {
            "ui-tui": wanted["packages"]["ui-tui"],
            "node_modules/ink": wanted["packages"]["node_modules/ink"],
        }
    }
    (tmp_path / "package-lock.json").write_text(json.dumps(wanted))
    (tmp_path / "node_modules" / ".package-lock.json").write_text(
...
def test_workspace_check_detects_missing_tui_dependency(tmp_path: Path, main_mod) -> None:
    tui = tmp_path / "ui-tui"
    tui.mkdir()
    (tui / "package.json").write_text('{"name":"hermes-tui"}')
    _touch_ink(tmp_path)
    ...
    installed = {"packages": {"ui-tui": wanted["packages"]["ui-tui"]}}
    (tmp_path / "package-lock.json").write_text(json.dumps(wanted))
    (tmp_path / "node_modules" / ".package-lock.json").write_text(
        json.dumps(installed))
