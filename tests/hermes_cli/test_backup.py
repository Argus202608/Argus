...
def _make_hermes_tree(root: Path) -> None:
    """Create a realistic ~/.argus directory structure for testing."""
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".argus"
        src_home.mkdir(parents=True)
        _make_hermes_tree(src_home)

        monkeypatch.setenv("ARGUS_HOME", str(src_home))
        ...
        dst_home = tmp_path / "dest" / ".argus"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("ARGUS_HOME", str(dst_home))
...
class TestBackupEdgeCases:
    def test_nonexistent_hermes_home(self, tmp_path, monkeypatch):
        ...
        fake_home = tmp_path / "nonexistent" / ".argus"
        monkeypatch.setenv("ARGUS_HOME", str(fake_home))
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert "argus -p coder" in coder_wrapper
...
# ---------------------------------------------------------------------------
# Pre-update backup (argus update safety net)
# ---------------------------------------------------------------------------
...
class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``argus update``
...
class TestRunPreUpdateBackup:
    ...
    def test_default_disabled_is_silent(self, hermes_home, capsys):
        """With the default (``pre_update_backup: false``), ``argus update``
...
class TestPreMigrationBackup:
    ...
    ``hermes claw migrate`` runs before mutating ~/.argus/."""
    ...
    def test_restorable_with_hermes_import(self, hermes_home, tmp_path):
        ...
        backup — `argus import` should accept it."""
    ...
    def test_missing_hermes_home_returns_none(self, tmp_path):
        """Fresh install with no ~/.argus yet — nothing to back up."""
        from hermes_cli.backup import create_pre_migration_backup
...
class TestRestoreCronJobsIfEmptied:
    """`argus update` config migration can leave cron/jobs.json valid-but-empty,
