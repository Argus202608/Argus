...
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    ``argus update``'s ``git stash push --include-untracked`` does not sweep it
    ...
    marker = tmp_path / ".argus-bootstrap-complete"
    marker.write_text("")

    # Exact flags used by argus update (hermes_cli/main.py).
    git("stash", "push", "--include-untracked", "-m", "argus-update-autostash")

    assert marker.exists(), (
        ".argus-bootstrap-complete was swept into the update autostash — it must "
    ...
    assert ".argus-bootstrap-complete" not in status
