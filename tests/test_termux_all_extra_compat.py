...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"mm-argus[termux]"' in text
    assert '"mm-argus[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"mm-argus[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
